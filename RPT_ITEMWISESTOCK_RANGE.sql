ALTER PROCEDURE RPT_ITEMWISESTOCK_RANGE    
(     
@DBNAME VARCHAR(100)    
,@FRMDATE VARCHAR(12)    
,@TODATE VARCHAR(12)       
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' STNPCS INT,STNWT DECIMAL(15,3),ITEMSIZEID INT NULL)'    
PRINT @SQLQRY    
EXECUTE SP_EXECUTESQL @SQLQRY    
...
SELECT @SQLQRY = @SQLQRY +' AND ISNULL(T.OLDTAGNO,'''')='''''
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,EXTRAWT,VALUE,SALVALUE,SNO,RECISS,STONE,STYLENO,RATE,DIAPCS,DIAWT,STNPCS,STNWT,ITEMSIZEID)'    
...
END    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,SIZEID'    
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN ITEMMAST AS IT ON T.ITEMID=IT.ITEMID AND ISNULL(ACTIVE,'''') = ''Y'' AND ISNULL(STOCKREPORT,'''') = ''Y'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN CTRANSFER  AS I ON I.TAGSNO = T.SNO AND I.ITEMID = T.ITEMID AND I.TAGNO = T.TAGNO AND ISNULL(I.COSTID,'''')=ISNULL(T.COSTID,'''') AND (I.ISSDATE IS NULL OR I.ISSDATE >= '''+@FRMDATE + ''') AND I.RECDATE < '''+@FRMDATE + ''''    
...
/* APPROVAL OPENING*/    
SELECT @SQLQRY =  CHAR(13) + 'INSERT INTO TEMPTABLEDB..TEMP'+@SYSTEMID+'ITEMSTOCK1 (SEP,COSTID,DESIGNERID,ITEMTYPEID,ITEMCTRID,ITEMGROUP,TITEMID,ITEMID,SUBITEMID,TAGS,PCS,GRSWT,NETWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,EXTRAWT,VALUE,SALVALUE,T.SNO,RECISS,STONE,STYLENO,RATE,DIAPCS,DIAWT,STNPCS,STNWT,ITEMSIZEID)'    
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' WHERE ' + @RECDATE + ' < '''+@FRMDATE + ''' AND IT.STOCKTYPE=''T'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' AND (ISSDATE >= '''+@FRMDATE+''' OR ISSDATE IS NULL) '    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' AND (TAGNO IN (SELECT TAGNO FROM '+ @DBNAME + '..ISSUE WHERE TRANDATE < '''+@FRMDATE+''' AND TRANTYPE = ''AI''AND (REFDATE IS NULL OR REFDATE>='''+ @FRMDATE + ''') AND ISNULL(CANCEL,'''') = '''' AND ISNULL(COSTID,'''')=ISNULL(T.COSTID,'''') AND ISNULL(ITEMID,'''')=ISNULL(T.ITEMID,''''))'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' OR TAGNO IN (SELECT TAGNO FROM '+ @DBNAME + '..APPISSUE WHERE TRANDATE < '''+@FRMDATE+''' AND TRANTYPE = ''AI''  AND (REFDATE IS NULL OR REFDATE>='''+ @FRMDATE + ''') AND ISNULL(CANCEL,'''') = '''' AND ISNULL(COSTID,'''')=ISNULL(T.COSTID,'''') AND ISNULL(ITEMID,'''')=ISNULL(T.ITEMID,'''')))'    
...
SELECT @SQLQRY =  CHAR(13) + 'INSERT INTO TEMPTABLEDB..TEMP'+@SYSTEMID+'ITEMSTOCK1 (SEP,COSTID,DESIGNERID,ITEMTYPEID,ITEMCTRID,ITEMGROUP,TITEMID,ITEMID,SUBITEMID,TAGS,PCS,GRSWT,NETWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,EXTRAWT,VALUE,SALVALUE,SNO,RECISS,STONE,STYLENO,RATE,DIAPCS,DIAWT,STNPCS,STNWT,ITEMSIZEID)'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' SELECT CASE WHEN TOFLAG=''MI'' THEN ''MIS'' ELSE ''ISS'' END SEP,T.COSTID,T.DESIGNERID,T.ITEMTYPEID,T.ITEMCTRID,IT.ITEMGROUP,T.ITEMID TITEMID,T.ITEMID,T.SUBITEMID,case when isnull(t.pcs,0) > 0 then T.PCS/T.PCS else 0 end,T.PCS,T.GRSWT,T.NETWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,T.EXTRAWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,(SELECT SUM(ISNULL(STNAMT,0)) FROM ITEMTAGSTONE WHERE TAGSNO = T.SNO) AS VALUE,SALVALUE'    
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN ITEMMAST AS IT ON T.ITEMID=IT.ITEMID AND ISNULL(ACTIVE,'''') = ''Y'' AND ISNULL(STOCKREPORT,'''') = ''Y'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' WHERE ' + @RECDATE + ' <= '''+@TODATE + ''' AND ISSDATE BETWEEN '''+@FRMDATE + ''' AND '''+@TODATE + ''' AND IT.STOCKTYPE=''T'''    
PRINT @SQLQRY    
EXECUTE SP_EXECUTESQL @SQLQRY    
SELECT @SQLQRY =  CHAR(13) + 'INSERT INTO TEMPTABLEDB..TEMP'+@SYSTEMID+'ITEMSTOCK1 (SEP,COSTID,DESIGNERID,ITEMTYPEID,ITEMCTRID,ITEMGROUP,TITEMID,ITEMID,SUBITEMID,TAGS,PCS,GRSWT,NETWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,EXTRAWT,VALUE,SALVALUE,SNO,RECISS,STONE,STYLENO,RATE,DIAPCS,DIAWT,STNPCS,STNWT,ITEMSIZEID)'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' SELECT ''CTRANISS'' SEP,T.COSTID,T.DESIGNERID,T.ITEMTYPEID,I.ITEMCTRID,IT.ITEMGROUP,T.ITEMID TITEMID,T.ITEMID,T.SUBITEMID,case when isnull(t.pcs,0) > 0 then T.PCS/T.PCS else 0 end,T.PCS,T.GRSWT,T.NETWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,T.EXTRAWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,(SELECT SUM(ISNULL(STNAMT,0)) FROM ITEMTAGSTONE WHERE TAGSNO = T.SNO) AS VALUE,SALVALUE'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,T.SNO,''''RECISS,0 STONE,T.STYLENO,CASE WHEN RATE <> 0 THEN RATE ELSE BOARDRATE END AS RATE'    
IF @WITHDIA='Y'    
...
/*    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN ITEMMAST AS IT ON T.ITEMID=IT.ITEMID AND ISNULL(ACTIVE,'''') = ''Y'' AND ISNULL(STOCKREPORT,'''') = ''Y'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN CTRANSFER  AS I ON'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' I.ISSDATE  BETWEEN '''+@FRMDATE + ''' AND '''+@TODATE + ''' AND IT.STOCKTYPE=''T'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' AND I.ITEMID = T.ITEMID AND I.TAGNO = T.TAGNO AND ISNULL(I.COSTID,'''')=ISNULL(T.COSTID,'''')'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' AND (T.ISSDATE IS NULL OR T.ISSDATE >= I.ISSDATE) '    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' WHERE '*/    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN ITEMMAST AS IT ON T.ITEMID=IT.ITEMID AND ISNULL(ACTIVE,'''') = ''Y'' AND ISNULL(STOCKREPORT,'''') = ''Y'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN CTRANSFER  AS I ON I.TAGSNO = T.SNO AND I.ITEMID = T.ITEMID AND I.TAGNO = T.TAGNO AND ISNULL(I.COSTID,'''')=ISNULL(T.COSTID,'''') AND I.ISSDATE  BETWEEN '''+@FRMDATE + ''' AND '''+@TODATE + ''' '    
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' FROM TEMPTABLEDB..TEMP'+@SYSTEMID+'TEMPITEMSTOCK T'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN ITEMMAST AS IT ON T.ITEMID=IT.ITEMID AND ISNULL(ACTIVE,'''') = ''Y'' AND ISNULL(STOCKREPORT,'''') = ''Y'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' WHERE ' + @RECDATE + ' BETWEEN '''+@FRMDATE + ''' AND '''+@TODATE + ''' AND IT.STOCKTYPE=''T'''    
PRINT @SQLQRY    
EXECUTE SP_EXECUTESQL @SQLQRY    
SELECT @SQLQRY =  CHAR(13) + 'INSERT INTO TEMPTABLEDB..TEMP'+@SYSTEMID+'ITEMSTOCK1 (SEP,COSTID,DESIGNERID,ITEMTYPEID,ITEMCTRID,ITEMGROUP,TITEMID,ITEMID,SUBITEMID,TAGS,PCS,GRSWT,NETWT'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,EXTRAWT,VALUE,SALVALUE,SNO,RECISS,STONE,STYLENO,RATE,DIAPCS,DIAWT,STNPCS,STNWT,ITEMSIZEID)'    
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN CTRANSFER  AS I ON'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' I.RECDATE  BETWEEN '''+@FRMDATE + ''' AND '''+@TODATE + ''' AND IT.STOCKTYPE=''T'''    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' AND (T.ISSDATE IS NULL OR T.ISSDATE >= I.ISSDATE) '    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' AND I.ITEMID = T.TEMID AND I.TAGNO = T.TAGNO AND ISNULL(I.COSTID,'''')=ISNULL(T.COSTID,'''')'    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' WHERE '    
*/    
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' INNER JOIN ITEMMAST AS IT ON T.ITEMID=IT.ITEMID AND ISNULL(ACTIVE,'''') = ''Y'' AND ISNULL(STOCKREPORT,'''') = ''Y'''    
...
SELECT @QRY = @QRY + CHAR(13) + ' ,T.VALUE,T.SALVALUE,0 STONE,T.DIAPCS,T.DIAWT,T.STNPCS,T.STNWT,T.STYLENO,T.RATE,ITEMSIZEID FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTOCK1 T'    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CASE WHEN RECISS = ''R'' THEN STNWT ELSE -1*STNWT END) AS STNWT,STYLENO,RATE,ITEMSIZEID'    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMNONTAGSTOCK AS X WHERE SEP<>''APPOPE'''    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY SEP,ITEMID,TITEMID,STONE,ITEMCTRID,ITEMGROUP,DESIGNERID,ITEMTYPEID,COSTID,SUBITEMID,STYLENO,RATE,VALUE,SALVALUE,ITEMSIZEID'    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CASE WHEN RECISS = ''I'' THEN STNWT ELSE -1*STNWT END) AS STNWT,STYLENO,RATE,ITEMSIZEID'    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMNONTAGSTOCK AS X WHERE SEP=''APPOPE'''     
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY SEP,ITEMID,TITEMID,STONE,ITEMCTRID,ITEMGROUP,DESIGNERID,ITEMTYPEID,COSTID,SUBITEMID,STYLENO,RATE,VALUE,SALVALUE,ITEMSIZEID'    
END    
...
SELECT @QRY = @QRY + CHAR(13) + ' WHERE A.GRSWT BETWEEN B.FROMWEIGHT AND B.TOWEIGHT AND A.COSTID=B.COSTID AND A.ITEMID=B.ITEMID'    
SELECT @QRY = @QRY + CHAR(13) + ' AND A.SUBITEMID=B.SUBITEMID  AND A.ITEMID NOT IN (SELECT ITEMID FROM ITEMMAST WHERE METALID = ''S'')'    
...
SELECT @QRY = @QRY + CHAR(13) + ' AND A.ITEMID IN (SELECT ITEMID FROM ITEMMAST WHERE METALID = ''S'')'    
SELECT @QRY = @QRY + CHAR(13)  
SELECT @QRY = @QRY + CHAR(13) + ' UPDATE A SET A.RANGE = A.RANGE + ''-'' + B.SIZENAME FROM TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKVIEW A '      SELECT @QRY = @QRY + CHAR(13) + ' INNER JOIN ITEMSIZE AS B ON A.ITEMID=B.ITEMID AND A.ITEMSIZEID = B.SIZEID '     SELECT @QRY = @QRY + CHAR(13) + ' WHERE ISNULL(A.ITEMSIZEID,0) <> 0 '  
PRINT 'HHHHHHH'    
PRINT @QRY    
EXEC (@QRY)     
IF @WITHRANGEOUT='Y'    
...
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' STYLENO VARCHAR(20),RATE DECIMAL(15,2),PARTICULAR VARCHAR(40),RANGE VARCHAR(40),RANGEWT NUMERIC(15,2)'    
...
SELECT @QRY = @QRY + 'RATE,RANGE,RANGEWT,RESULT,COLHEAD,STONE,DIASTONE,METALID,ITEMID,PARTICULAR'       
SELECT @QRY = @QRY + CHAR(13) + ' ) '     
...
IF @RPTBASED='I'    
BEGIN    
SELECT @QRY = @QRY + ',ITEM AS PARTICULAR'    
END    
ELSE    
BEGIN    
SELECT @QRY = @QRY + ',CASE WHEN ISNULL(SUBITEM,'''') <> '''' THEN SUBITEM ELSE ITEM END AS PARTICULAR'    
END    
...
SELECT @QRY = @QRY + CHAR(13) + ' GROUP BY  GROUPNAME,TITEM,ITEM,STONE,RATE,RANGE,RANGEWT,ITEMID,X.ITEMCTRID,C.ITEMCTRNAME,X.DESIGNERID,D.DESIGNERNAME,X.COSTID,CC.COSTNAME,SUBITEM,STYLENO,ITEMID'    
IF @RPTBASED<>'I'    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,RANGE,RANGEWT,SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ITAGS)) ITAGS,SUM(CONVERT(INT,IPCS)) IPCS,SUM(IGRSWT)IGRSWT,SUM(INETWT) INETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,AIITAGS)) AIITAGS,SUM(CONVERT(INT,AIIPCS)) AIIPCS,SUM(AIIGRSWT)AIIGRSWT,SUM(AIINETWT)AIINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CIITAGS)) CIITAGS,SUM(CONVERT(INT,CIIPCS)) CIIPCS,SUM(CIIGRSWT)CIIGRSWT,SUM(CIINETWT)CIINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,ITEM'    
IF @RPTBASED<>'I'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUBITEM'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE'    
SELECT @QRY = @QRY + CHAR(13) + '  ,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) AS METAL '    
SELECT @QRY = @QRY + CHAR(13) + '  ,COUNTER,5 AS RESULT '    
...
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY PARTICULAR,ITEM'    
IF @RPTBASED<>'I'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUBITEM'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,COUNTER,RANGE,RANGEWT,METALID'    
PRINT @QRY    
EXEC (@QRY)    
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,COUNTER,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT DISTINCT COSTCENTRE AS PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,NULL COUNTER,0 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + ' FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T WHERE ISNULL(COSTCENTRE,'''')<>'''''    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE'    
PRINT @QRY    
EXEC (@QRY)    
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT DISTINCT (SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) AS PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) METAL,NULL COUNTER,1 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + ' FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID'    
PRINT @QRY    
/*EXEC (@QRY)*/    
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT DISTINCT COUNTER AS PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) METAL,COUNTER COUNTER,2 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID,COUNTER'    
PRINT @QRY    
EXEC (@QRY)        
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,ITEM,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT DISTINCT ITEM AS PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) METAL,COUNTER,ITEM,3 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID,COUNTER,ITEM'    
PRINT @QRY    
EXEC (@QRY)    
IF @RPTBASED<>'I'    
BEGIN    
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,ITEM,SUBITEM,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT DISTINCT SUBITEM AS PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) METAL,COUNTER,ITEM,SUBITEM,4 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID,COUNTER,ITEM,SUBITEM'    
PRINT @QRY    
EXEC (@QRY)         
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'    
SELECT @QRY = @QRY + CHAR(13) + '  ,OTAGS,OPCS,OGRSWT,ONETWT,RTAGS,RPCS,RGRSWT,RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,ARRTAGS,ARRPCS,ARRGRSWT,ARRNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,CRRTAGS,CRRPCS,CRRGRSWT,CRRNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,ITAGS,IPCS,IGRSWT,INETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,AIITAGS,AIIPCS,AIIGRSWT,AIINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,CIITAGS,CIIPCS,CIIGRSWT,CIINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,MITAGS,MIPCS,MIGRSWT,MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,CTAGS,CPCS,CGRSWT,CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,ITEM,SUBITEM,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT SUBITEM AS PARTICULAR,'    
SELECT @QRY = @QRY + CHAR(13) + '  SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ITAGS)) ITAGS,SUM(CONVERT(INT,IPCS)) IPCS,SUM(IGRSWT)IGRSWT,SUM(INETWT) INETWT'    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID),COUNTER,ITEM,SUBITEM,6 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID,COUNTER,ITEM,SUBITEM'    
PRINT @QRY    
EXEC (@QRY)    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,MITAGS,MIPCS,MIGRSWT,MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,CTAGS,CPCS,CGRSWT,CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,ITEM'    
IF @RPTBASED<>'I'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUBITEM'    
SELECT @QRY = @QRY + CHAR(13) + '  ,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT ''      TOTAL'' AS PARTICULAR,'    
SELECT @QRY = @QRY + CHAR(13) + '  SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'        
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID),COUNTER,ITEM'    
IF @RPTBASED<>'I'    
SELECT @QRY = @QRY + CHAR(13) + '  ,''ZZZZZ'' AS SUBITEM'        
SELECT @QRY = @QRY + CHAR(13) + '  ,7 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID,COUNTER,ITEM'    
PRINT @QRY    
EXEC (@QRY)    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,CTAGS,CPCS,CGRSWT,CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,ITEM,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT ''COUNTER TOTAL'' AS PARTICULAR,'    
SELECT @QRY = @QRY + CHAR(13) + '  SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'     
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID),COUNTER,''ZZZZZ'' ITEM,8 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID,COUNTER'    
PRINT @QRY    
EXEC (@QRY)    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,CTAGS,CPCS,CGRSWT,CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT (SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID) + '' TOTAL'' AS PARTICULAR,'    
SELECT @QRY = @QRY + CHAR(13) + '  SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'     
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,(SELECT TOP 1 METALNAME FROM METALMAST WHERE METALID = T.METALID),''ZZZZZ'' COUNTER,9 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + '  FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE,METALID'    
PRINT @QRY    
/*EXEC (@QRY)*/    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,CTAGS,CPCS,CGRSWT,CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,METAL,COUNTER,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT ISNULL(COSTCENTRE,'''') + '' TOTAL'' AS PARTICULAR,SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'     
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ITAGS)) ITAGS,SUM(CONVERT(INT,IPCS)) IPCS,SUM(IGRSWT)IGRSWT,SUM(INETWT) INETWT'    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,''ZZZZZ'',''ZZZZZ'' COUNTER,10 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + ' FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T WHERE ISNULL(COSTCENTRE,'''')<>'''''    
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE'    
PRINT @QRY    
/*EXEC (@QRY)*/    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,CTAGS,CPCS,CGRSWT,CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,COSTCENTRE,COUNTER,RESULT)'    
SELECT @QRY = @QRY + CHAR(13) + '  SELECT ''GRAND TOTAL'' AS PARTICULAR,SUM(CONVERT(INT,OTAGS)) OTAGS,SUM(CONVERT(INT,OPCS)) OPCS,SUM(OGRSWT) OGRSWT,SUM(ONETWT) ONETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,RTAGS)) RTAGS,SUM(CONVERT(INT,RPCS)) RPCS,SUM(RGRSWT) RGRSWT,SUM(RNETWT) RNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ARRTAGS)) ARRTAGS,SUM(CONVERT(INT,ARRPCS)) ARRPCS,SUM(ARRGRSWT) ARRGRSWT,SUM(ARRNETWT) ARRNETWT'     
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CRRTAGS)) CRRTAGS,SUM(CONVERT(INT, CRRPCS)) CRRPCS,SUM(CRRGRSWT) CRRGRSWT,SUM(CRRNETWT) CRRNETWT'        
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,ITAGS)) ITAGS,SUM(CONVERT(INT,IPCS)) IPCS,SUM(IGRSWT)IGRSWT,SUM(INETWT) INETWT'    
...
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,MITAGS)) MITAGS,SUM(CONVERT(INT,MIPCS)) MIPCS,SUM(MIGRSWT)MIGRSWT,SUM(MINETWT)MINETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,SUM(CONVERT(INT,CTAGS)) CTAGS,SUM(CONVERT(INT,CPCS)) CPCS,SUM(CGRSWT) CGRSWT,SUM(CNETWT) CNETWT'    
SELECT @QRY = @QRY + CHAR(13) + '  ,''ZZZZZ'' COSTCENTRE,''ZZZZZ'' COUNTER,11 AS RESULT '    
SELECT @QRY = @QRY + CHAR(13) + ' FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T'    
PRINT @QRY    
/*EXEC (@QRY)*/    
END