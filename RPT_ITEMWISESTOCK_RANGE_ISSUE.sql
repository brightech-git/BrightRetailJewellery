ALTER PROCEDURE [dbo].[RPT_ITEMWISESTOCK_RANGE_ISSUE]      
(       
@DBNAME VARCHAR(100)      
,@FRMDATE VARCHAR(12)      
,@TODATE VARCHAR(12)         
...
SELECT @SQLQRY =  CHAR(13) + 'INSERT INTO TEMPTABLEDB..TEMP'+@SYSTEMID+'ITEMSTOCK1 (SEP,COSTID,DESIGNERID,ITEMTYPEID,ITEMCTRID,ITEMGROUP,TITEMID,ITEMID,SUBITEMID,TAGS,PCS,GRSWT,NETWT'      
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,EXTRAWT,VALUE,SALVALUE,SNO,RECISS,STONE,STYLENO,RATE,DIAPCS,DIAWT,STNPCS,STNWT,ITEMSIZEID)'      
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' SELECT CASE WHEN TOFLAG=''MI'' THEN ''ISS'' ELSE ''ISS'' END SEP,T.COSTID,T.DESIGNERID,T.ITEMTYPEID,T.ITEMCTRID,IT.ITEMGROUP,T.ITEMID TITEMID,T.ITEMID,T.SUBITEMID,case when isnull(t.pcs,0) > 0 then T.PCS/T.PCS else 0 end,T.PCS,T.GRSWT,T.NETWT'      
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,T.EXTRAWT'      
SELECT @SQLQRY = @SQLQRY + CHAR(13) + ' ,(SELECT SUM(ISNULL(STNAMT,0)) FROM ITEMTAGSTONE WHERE TAGSNO = T.SNO) AS VALUE,SALVALUE'      
...
--IF @ISSUEONLY='Y'      
--BEGIN       
--SELECT @QRY = ' DELETE FROM TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN  WHERE ISNULL(IPCS,'''')<>'''' '      
--PRINT @QRY      
--EXEC (@QRY)      
--END    
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'      
...
SELECT @QRY = @QRY + CHAR(13) + ' FROM TEMPTABLEDB..TEMP' + @SYSTEMID + 'ITEMSTK AS T WHERE ISNULL(COSTCENTRE,'''')<>''''' 
SELECT @QRY = @QRY + CHAR(13) + '  AND ISNULL(T.IPCS,'''')<>'''''
SELECT @QRY = @QRY + CHAR(13) + '  GROUP BY COSTCENTRE'      
PRINT @QRY      
/*EXEC (@QRY)*/      
SELECT @QRY =' INSERT INTO TEMPTABLEDB..TEMP'+ @SYSTEMID +'ITEMSTKFIN (PARTICULAR'      
