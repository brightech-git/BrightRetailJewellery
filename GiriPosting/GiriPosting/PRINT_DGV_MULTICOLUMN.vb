...
Public Class PRINT_DGV_MULTICOLUMN
    ...
    Private ShrinkContent As Boolean
    Private PrintHeadBold As Boolean
    ...
    , Optional ByVal ShrinkContent As Boolean = True _
    , Optional ByVal PrintHeadTotalBold As Boolean = False _
   , Optional ByVal PrintUser As String = ""
        )
        ...
        Try
            ...
            Me.ShrinkContent = ShrinkContent
            Me.PrintHeadBold = PrintHeadTotalBold
            Me.PrintDateTime = PrintDateTime
            Me.PrintUser = PrintUser
            Me.BottomFooter = False
    ...
    Private Sub PrintDoc_BeginPrint(ByVal sender As Object,
        ...
        Try
            ...
            If pageLandscape = False Then
                ...
                PrintDoc.DefaultPageSettings.Margins.Right = 50
            Else
                PrintDoc.DefaultPageSettings.Margins.Left = 35 ''50
                PrintDoc.DefaultPageSettings.Margins.Right = 45 ''50
            End If
    ...
    Private Sub PrintDoc_PrintPage(ByVal sender As Object,
        ...
        'Header split for max length header name by vasanthan(2013-12-16)
        Dim strColl As String = Nothing
        If strTemp IsNot Nothing Then PrintTitle = strTemp
        If strTemp Is Nothing Then strTemp = PrintTitle
        For Each str As String In PrintTitle
            'strColl = Nothing
            Dim tmplen As Integer = 0
nextsplit:
            If str.Length > Math.Ceiling(e.MarginBounds.Width / 8) + tmplen Then
                For k As Integer = Math.Ceiling(e.MarginBounds.Width / 8) + tmplen To 0 Step -1
                    If str(k) = "," Or str(k) = " " Then str = str.Insert(k + 1, Environment.NewLine) : tmplen = k + 1 : Exit For
                Next
                'str = str.Insert(Math.Ceiling(e.MarginBounds.Width / 8), Environment.NewLine)
            End If
            If str.Length > Math.Ceiling(e.MarginBounds.Width / 8) + tmplen Then
                If tmplen = 0 Then tmplen = Math.Ceiling(e.MarginBounds.Width / 8)
                GoTo nextsplit
            End If
            strColl += str
        Next
            ...
            If PageNo = 1 Then
                    ...
                    For NoCnt As Integer = 1 To NoOfLables
                        For Each GridCol As DataGridViewColumn In dgvHeader.Columns
                            ...
                            If FitToPageWidth Then
                                tmpWidth = CType(Math.Ceiling(leng / TotalWidth *
                                           TotalWidth * (e.MarginBounds.Width / TotalWidth)), Int16)
                                gFont = New Font(GridCol.InheritedStyle.Font.Name, 6, FontStyle.Regular)
                                If (leng / 2) > tmpWidth Then GridCol.InheritedStyle.Font = gFont
                ...
                For NoCnt As Integer = 1 To NoOfLables
                    For Each GridCol As DataGridViewColumn In dgv.Columns
                        ...
                        ' Detemining whether the columns are fitted to page or not.
                        If FitToPageWidth Then
                            tmpWidth = CType(Math.Ceiling(GridCol.Width / TotalWidth *
                                       TotalWidth * (e.MarginBounds.Width / TotalWidth)), Int16)
                            gFont = New Font(GridCol.InheritedStyle.Font.Name, 6, FontStyle.Regular)
                            If (GridCol.Width / 2) > tmpWidth Then GridCol.InheritedStyle.Font = gFont
                        Else
...
flag:
                        ...
                        If Not (dgvHeader Is Nothing) Then
                            For Each GridCol As DataGridViewColumn In dgvHeader.Columns
                                ...
                                e.Graphics.DrawRectangle(Me.PenColor, New Rectangle(headColumnLefts(i),
                                        tmpTop, headColumnWidths(i), HeaderHeight))
                                StrFormat.Alignment = StringAlignment.Center
                                Dim drawHeadFont As Font = drawFont
                                drawHeadFont = New Font("verdana", 8, FontStyle.Bold)
                                e.Graphics.DrawString(" " + GridCol.HeaderText, drawHeadFont,
                                        New SolidBrush(GridCol.InheritedStyle.ForeColor),
                                        New RectangleF(headColumnLefts(i), tmpTop, headColumnWidths(i),
                        ...
                        For Each GridCol As DataGridViewColumn In dgv.Columns
                            ...
                            e.Graphics.DrawRectangle(Me.PenColor, New Rectangle(ColumnLefts(i),
                                    tmpTop, ColumnWidths(i), HeaderHeight))

                            e.Graphics.DrawString(" " + GridCol.HeaderText, drawFont,
                                    New SolidBrush(GridCol.InheritedStyle.ForeColor),
                           ...
                           ColumnTypes(i) Is GetType(DataGridViewLinkColumn) Then
                            ...
                            If PrnValue.Contains(Chr(13)) Then
                                ...
                                For Each s As String In PrnValue.Split(Chr(13))
                                    ...
                                    e.Graphics.DrawString(s, drawFont,
                                           New SolidBrush(Cel.InheritedStyle.ForeColor),
                                           New RectangleF(ColumnLefts(i), tp, ColumnWidths(i),
                                           hit), StrFormat)
                                    tp += hit
                                Next
                            Else
                                If dgv.Columns.Contains("COLHEAD") = True And PrintHeadBold Then
                                    If ShrinkContent = True And GridRow.Cells("COLHEAD").Value.ToString <> "T" And GridRow.Cells("COLHEAD").Value.ToString <> "G" Then
                                        drawFont = New Font(Cel.InheritedStyle.Font.Name, 7, FontStyle.Regular)
                                        CellHeight = GridRow.Height
                                    End If
                                    If ShrinkContent = True And (GridRow.Cells("COLHEAD").Value.ToString = "T" Or GridRow.Cells("COLHEAD").Value.ToString = "G") Then
                                        drawFont = New Font(Cel.InheritedStyle.Font.Name, 7, FontStyle.Bold)
                                        CellHeight = GridRow.Height
                                    End If
                                Else
                                    If ShrinkContent = True Then
                                        drawFont = New Font(Cel.InheritedStyle.Font.Name, 7, FontStyle.Regular)
                                        CellHeight = GridRow.Height
                                    End If
                                End If
                                e.Graphics.DrawString(Cel.FormattedValue.ToString, drawFont,
                                       New SolidBrush(Cel.InheritedStyle.ForeColor),
                                       New RectangleF(ColumnLefts(i), tmpTop, ColumnWidths(i),
                        ...
                        ElseIf ColumnTypes(i) Is GetType(DataGridViewImageColumn) Then
                           ...
                           tmpTop, ColumnWidths(i), CellHeight)
                            ...
                            e.Graphics.DrawImage(bm_dest, New Rectangle(ColumnLefts(i) _
                                    ...
                                    Int32), CType(bm_dest, Image).Width, CType(bm_dest,
                                    Image).Height))
    ...
    Private Sub DrawFooter(ByVal Top As Integer, ByVal e As System.Drawing.Printing.PrintPageEventArgs, ByVal RowsPerPage As Int32)
        ...
        If BottomFooter Then
            e.Graphics.DrawString(PageNum, drawFont, Brushes.Black,
                        e.MarginBounds.Left + (e.MarginBounds.Width - e.Graphics.MeasureString(PageNum, dgv.Font, e.MarginBounds.Width).Width) / 2, e.MarginBounds.Top + e.MarginBounds.Height)
        Else
            e.Graphics.DrawString(PageNum, drawFont, Brushes.Black,
            e.MarginBounds.Left + (e.MarginBounds.Width -
            e.Graphics.MeasureString(PageNum, dgv.Font, e.MarginBounds.Width).Width) / 2, e.MarginBounds.Top + Top)
        End If
        ...
        If PrintDateTime Then
            ...
            Else
                e.Graphics.DrawString(s, New Font(dgv.Font, FontStyle.Bold), Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top + Top)
            End If
        End If

        If PrintUser.ToString <> "" Then
            Dim s As String = "User : " & PrintUser.ToString
            If BottomFooter Then
                e.Graphics.DrawString(s, New Font(dgv.Font, FontStyle.Bold), Brushes.Black,
                                 e.MarginBounds.Right - (e.MarginBounds.Width - e.Graphics.MeasureString(PageNum, dgv.Font, e.MarginBounds.Width).Width) / 3,
                                      e.MarginBounds.Top + e.MarginBounds.Height)
            Else
                e.Graphics.DrawString(s, New Font(dgv.Font, FontStyle.Bold), Brushes.Black,
                                      e.MarginBounds.Right - (e.MarginBounds.Width - e.Graphics.MeasureString(PageNum, dgv.Font, e.MarginBounds.Width).Width) / 3,
                                      e.MarginBounds.Top + Top)
            End If
        End If
