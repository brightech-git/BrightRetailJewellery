...
Public Class frmSaleReturnParty
    ...
    Private Sub btnView_Search_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        ...
        strSql = $";with cte as ("
        strSql += vbCrLf + $"Select TAGDESIGNER,TRFNO,CATCODE,COSTID,sum(PCS) SRPCS,sum(GRSWT) SRGRSWT from {cnStockDb}..RECEIPT where TRANTYPE='SR'"
        strSql += vbCrLf + $"and trandate between '{dtpFrom.Value}' and '{dtpTo.Value}'"
        strSql += vbCrLf + "group by TAGDESIGNER,TRFNO, CATCODE, COSTID, TRFNO"
        strSql += vbCrLf + ")"
        strSql += vbCrLf + ",cte2 as("
        strSql += vbCrLf + $"SELECT TAGDESIGNER,TRFNO,CATCODE,SUM(PCS) AS RPCS,SUM(GRSWT) AS RGRSWT,MIN(SNO) AS RECSNO from {cnStockDb}..receipt"
        strSql += vbCrLf + "where TRANTYPE ='RIN' "
        strSql += vbCrLf + "GROUP BY TAGDESIGNER, TRFNO, CATCODE"
        strSql += vbCrLf + ")"
        strSql += vbCrLf + ",cte3 as("
        strSql += vbCrLf + "SELECT cte.*,cte2.RPCS,cte2.RGRSWT,cte2.RECSNO,li.PCS AS LIPCS,li.GRSWT AS LIGRSWT,li.LOTSNO FROM cte"
        strSql += vbCrLf + "Left Join cte2  ON cte.TRFNO = cte2.TRFNO And cte.CATCODE = cte2.CATCODE  And cte.TAGDESIGNER = cte2.TAGDESIGNER"
        strSql += vbCrLf + $"Left Join {cnStockDb}..LOTISSUE li ON cte2.RECSNO = li.RECSNO"
        strSql += vbCrLf + ")"
        strSql += vbCrLf + ",cte4 as("
        strSql += vbCrLf + "SELECT cte3.*,il.PCS AS LPCS,il.GRSWT AS LGRSWT FROM cte3"
        strSql += vbCrLf + $"LEFT JOIN {cnAdminDb}..ITEMLOT il  ON il.SNO = cte3.LOTSNO"
        strSql += vbCrLf + "),"
        strSql += vbCrLf + "ITEMTAG_AGG AS (SELECT LOTSNO,DESIGNERID,SUM(PCS) AS TPCS,SUM(GRSWT) AS TGRSWT FROM BMGADMINDB..ITEMTAG"
        strSql += vbCrLf + "GROUP BY LOTSNO, DESIGNERID )"
        strSql += vbCrLf + "SELECT cte4.*,d_sr.DESIGNERNAME AS SR_DESIGNERNAME,d.DESIGNERID,d.DESIGNERNAME,it.TPCS,it.TGRSWT FROM cte4"
        strSql += vbCrLf + $" Left Join {cnAdminDb}..DESIGNER d_sr ON cte4.TAGDESIGNER = d_sr.DESIGNERID"
        strSql += vbCrLf + "Left Join ITEMTAG_AGG it ON it.LOTSNO = cte4.LOTSNO"
        strSql += vbCrLf + $"Left Join {cnAdminDb}..DESIGNER d ON it.DESIGNERID = d.DESIGNERID  ORDER BY d_sr.DESIGNERNAME;"
