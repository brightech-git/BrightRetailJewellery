...
Imports System.Data
Public Class frmStoneGroupReport
    Dim strSql As String = Nothing
    Private Sub frmStoneGroupReport_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        NewLoad()
    End Sub
    Private Sub NewLoad()
        ...
        Dt.Rows.Add("Item")
        BrighttechPack.FillCombo(chkCmbGropBy, Dt, "NAME", , "StoneGroup")
        ...
        BrighttechPack.FillCombo(chkCmbStoneGroup, GetSqlTable(strSql, cn), "NAME", , "ALL")
    ...
    Private Sub CompanyLoad()
        ...
        BrighttechPack.FillCombo(chkCmbCompany, GetSqlTable(strSql, cn), "NAME", , "ALL")
        chkCmbCompany.DisplayMember = "NAME"
        chkCmbCompany.ValueMember = "ID"
        'chkCmbCompany.Enabled = chkCmbCompany.Items.Count < 1
    End Sub
    Private Sub CostCentreLoad(Optional ByVal Value As String = Nothing)
        strSql = ""
        strSql += vbCrLf + "SELECT '0' ID,'ALL' NAME "
        strSql += vbCrLf + "UNION "
        ...
        End If
        strSql += vbCrLf + "ORDER BY ID,NAME "
        BrighttechPack.FillCombo(chkCmbCostCentre, GetSqlTable(strSql, cn), "NAME", , "ALL")
    ...
    Private Sub MetalLoad()
        ...
        strSql += vbCrLf + "WHERE ISNULL(ACTIVE,'Y') <> 'N' "
        strSql += vbCrLf + "ORDER BY ID,NAME "
        BrighttechPack.FillCombo(chkCmbMetel, GetSqlTable(strSql, cn), "NAME", , "ALL")
        chkCmbMetel.DisplayMember = "NAME"
        chkCmbMetel.ValueMember = "ID"
        'chkCmbMetel.Enabled = chkCmbMetel.Items.Count < 1
    End Sub
    Private Sub CategoryLoad(Optional ByVal Value As String = Nothing)
        strSql = ""
        strSql += vbCrLf + "SELECT '0' ID,'ALL' NAME "
        strSql += vbCrLf + "UNION "
        ...
        End If
        strSql += vbCrLf + "ORDER BY ID,NAME "
        BrighttechPack.FillCombo(chkCmbCategory, GetSqlTable(strSql, cn), "NAME", , "ALL")
        chkCmbCategory.DisplayMember = "NAME"
        chkCmbCategory.ValueMember = "ID"
        'chkCmbCategory.Enabled = chkCmbCategory.Items.Count < 1
    End Sub
    Private Sub ItemLoad(Optional ByVal Value As String = Nothing)
        strSql = ""
        strSql += vbCrLf + "SELECT '0' ID,'ALL' NAME "
        strSql += vbCrLf + "UNION "
        ...
        End If
        strSql += vbCrLf + "ORDER BY ID,NAME "
        BrighttechPack.FillCombo(chkCmbItem, GetSqlTable(strSql, cn), "NAME", , "ALL")
        chkCmbItem.DisplayMember = "NAME"
        chkCmbItem.ValueMember = "ID"
        'chkCmbItem.Enabled = chkCmbItem.Items.Count < 1
    End Sub
    Private Sub SubItemLoad(Optional ByVal Value As String = Nothing)
        strSql = ""
        strSql += vbCrLf + "SELECT '0' ID,'ALL' NAME "
        strSql += vbCrLf + "UNION "
        ...
        End If
        strSql += vbCrLf + "ORDER BY ID,NAME "
        BrighttechPack.FillCombo(chkCmbSubItem, GetSqlTable(strSql, cn), "NAME", , "ALL")
        chkCmbSubItem.DisplayMember = "NAME"
        chkCmbSubItem.ValueMember = "ID"
        'chkCmbSubItem.Enabled = chkCmbSubItem.Items.Count < 1
    End Sub
    Private Sub chkCmbCompany_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkCmbCompany.TextChanged
        CostCentreLoad(chkCmbCompany.Text)
    End Sub

    Private Sub chkCmbMetel_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkCmbMetel.TextChanged
        CategoryLoad(chkCmbMetel.Text)
    End Sub

    Private Sub chkCmbCategory_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkCmbCategory.TextChanged
        ItemLoad(chkCmbCategory.Text)
    End Sub

    Private Sub chkCmbItem_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkCmbItem.TextChanged
    ...
    Private Sub btnView_Search_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnView_Search.Click
        strSql = Nothing
        ...
        strSql += vbCrLf + ", @STONEGROUP = '" + chkCmbStoneGroup.Text.Trim + "'"
    ...
    Private Sub frmStoneGroupReport_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
