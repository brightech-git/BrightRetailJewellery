...
Imports System.Linq
Public Class frmStockCheckLoading
    ...
    Function funcStuddedDetails(ByVal itemId As String, ByVal tagNo As String, ByVal picPath As String) As Integer
    ...
    Private Sub frmStockCheckLoading_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            ...
            If chkCheckingByScan.Checked And txtTagNo.Focused Then Exit Sub
            SendKeys.Send("{TAB}")
        End If
    End Sub
    ...
    Private Sub txtTagNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtTagNo.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            ...
            If txtItemCode.Text = "" Then
                MsgBox("Item Code Should Not Empty", MsgBoxStyle.Exclamation)
                txtItemCode.Focus()
                Exit Sub
            End If
            ...
            With dtFullView
                For cnt As Integer = 0 To .Rows.Count - 1
                    If Val(.Rows(cnt).Item("itemId").ToString) = Val(txtItemCode.Text) And .Rows(cnt).Item("tagNo").ToString = txtTagNo.Text Then
                        rwIndex = cnt
                        Exit For
                    End If
                Next
                If rwIndex <> -1 Then
                    ...
                    strSql = " IF OBJECT_ID('" & cnAdminDb & "..TEMPTRGNAME') IS NOT NULL DROP TABLE " & cnAdminDb & "..TEMPTRGNAME"
                    cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                    cmd.ExecuteNonQuery()
                    ...
                    strSql += vbCrLf + " where itemid = " & Val(txtItemCode.Text) & ""
                    strSql += vbCrLf + " and TagNo = '" & txtTagNo.Text & "'"
                    If cmbCostCentre_MAN.Text <> "" And cmbCostCentre_MAN.Text <> "ALL" Then
                        strSql += vbCrLf + " AND COSTID = (SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME = '" & cmbCostCentre_MAN.Text & "')"
                    End If
                    'strSql += VBCRLF + " AND COMPANYID = '" & GetStockCompId() & "'"
                    strSql += " EXEC " & cnAdminDb & "..DISABLEENABLE_AUDITTRG 'ITEMTAG','ENABLE','UPD'"
                    cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                    cmd.ExecuteNonQuery()
                    ...
                    ''MarkVal += Val(gridFullView.Rows(rwIndex).Cells("PCS").Value.ToString())
                    'gridFullView.Rows(gridFullView.Rows.Count - 3).Cells("PCS").Value = MarkVal
                    'Dim UnMarkVal As Double
                    'UnMarkVal = Val(CType(gridFullView.DataSource, DataTable).Compute("SUM(PCS)", "CHKTRAY = '' AND RESULT =1"))
                    ''UnMarkVal -= Val(gridFullView.Rows(rwIndex).Cells("PCS").Value.ToString())
                    'gridFullView.Rows(gridFullView.Rows.Count - 2).Cells("PCS").Value = UnMarkVal

                    'MarkVal = Val(CType(gridFullView.DataSource, DataTable).Compute("SUM(GRSWT)", "CHKTRAY <> '' AND RESULT =1"))
                    ''MarkVal += Val(gridFullView.Rows(rwIndex).Cells("GRSWT").Value.ToString())
                    'gridFullView.Rows(gridFullView.Rows.Count - 3).Cells("GRSWT").Value = MarkVal

                    'UnMarkVal = Val(CType(gridFullView.DataSource, DataTable).Compute("SUM(GRSWT)", "CHKTRAY = '' AND RESULT =1"))
                    ''UnMarkVal -= Val(gridFullView.Rows(rwIndex).Cells("GRSWT").Value.ToString())
                    'gridFullView.Rows(gridFullView.Rows.Count - 2).Cells("GRSWT").Value = UnMarkVal

                    'MarkVal = Val(CType(gridFullView.DataSource, DataTable).Compute("SUM(NETWT)", "CHKTRAY <> '' AND RESULT =1"))
                    ''MarkVal += Val(gridFullView.Rows(rwIndex).Cells("NETWT").Value.ToString())
                    'gridFullView.Rows(gridFullView.Rows.Count - 3).Cells("NETWT").Value = MarkVal

                    'UnMarkVal = Val(CType(gridFullView.DataSource, DataTable).Compute("SUM(NETWT)", "CHKTRAY = '' AND RESULT =1"))
                    ''UnMarkVal -= Val(gridFullView.Rows(rwIndex).Cells("NETWT").Value.ToString())
                    'gridFullView.Rows(gridFullView.Rows.Count - 2).Cells("NETWT").Value = UnMarkVal
    ...
    Private Sub txtTagNo_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtTagNo.LostFocus
        If chkCheckingByScan.Checked = False Then
    ...
    Private Sub txtItemCode_NUM_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtItemCode.KeyDown
        If textBoxInputValidation(e) = True And chkCheckingByScan.Checked = True And STKCHKDEVICELOCK <> "" Then
            e.Handled = True
        End If
    ...
    Private Function textBoxInputValidation(ByVal e As System.Windows.Forms.KeyEventArgs) As Boolean
        ...
        Else
            Return False
        End If
        ''    Or UCase(e.KeyCode) = Keys.D0 _
        ''    Or UCase(e.KeyCode) = Keys.D1 _
        ''    Or UCase(e.KeyCode) = Keys.D2 _
        ''    Or UCase(e.KeyCode) = Keys.D3 _
        ''    Or UCase(e.KeyCode) = Keys.D4 _
        ''    Or UCase(e.KeyCode) = Keys.D5 _
        ''    Or UCase(e.KeyCode) = Keys.D6 _
        ''    Or UCase(e.KeyCode) = Keys.D7 _
        ''    Or UCase(e.KeyCode) = Keys.D8 _
        ''    Or UCase(e.KeyCode) = Keys.D9 _
    End Function
    ...
    Private Function textBoxInputValidation_keypress(ByVal e As System.Windows.Forms.KeyPressEventArgs) As Boolean
        ...
        Else
            Return False
        End If
        ''    Or UCase(e.KeyChar) = Chr(Keys.D0) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D1) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D2) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D3) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D4) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D5) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D6) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D7) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D8) _
        ''    Or UCase(e.KeyChar) = Chr(Keys.D9) _
    End Function
    ...
    Private Sub txtItemCode_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtItemCode.KeyPress
        If textBoxInputValidation_keypress(e) = True And chkCheckingByScan.Checked = True And STKCHKDEVICELOCK <> "" Then
            e.Handled = True
        End If
        If e.KeyChar = Chr(Keys.Enter) Then
            If txtTrayNo.Text.Trim = "" Then
...
nextloop:
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        If Not BrighttechPack.Methods.GetRights(_DtUserRights, Me.Name, BrighttechPack.Methods.RightMode.Print) Then Exit Sub
        If WithMarkedTotal.Checked Then
            ismarked_detail_print()
        Else
            If gridFullView.Rows.Count > 0 Then
                BrightPosting.GExport.Post(Me.Name, strCompanyName, "Stock Checking", gridFullView, BrightPosting.GExport.GExportType.Print)
            End If
    ...
    Public Function ismarked_detail_print() As Boolean
        If MessageBox.Show("Do you wish to Print only Marked Total?", "Print Alert", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
            Dim dtorggridCopy As DataTable
            dtorggridCopy = New DataTable
            With dtorggridCopy.Columns
                .Add("PARTICULAR", GetType(String))
                .Add("PCS", GetType(String))
                .Add("GRSWT", GetType(Double))
                .Add("NETWT", GetType(Double))
            End With
            Dim dr As DataRow
            dtorggridCopy.Rows.Clear()
            For cnt As Integer = 0 To lstTotal.Items.Count - 1
                dr = dtorggridCopy.NewRow()
                dr!PARTICULAR = lstTotal.Items(cnt).SubItems(0).Text
                dr!PCS = Val(lstTotal.Items(cnt).SubItems(1).Text)
                dr!GRSWT = Val(lstTotal.Items(cnt).SubItems(3).Text)
                dr!NETWT = Val(lstTotal.Items(cnt).SubItems(4).Text)
                dtorggridCopy.Rows.Add(dr)
            Next
            gridFullViewprint.DataSource = Nothing
            gridFullViewprint.DataSource = dtorggridCopy
            If gridFullViewprint.Rows.Count > 0 Then
                BrightPosting.GExport.Post(Me.Name, strCompanyName, "Stock Checking", gridFullViewprint, BrightPosting.GExport.GExportType.Print)
            End If
        Else
            Dim dtorggrid As DataTable
            Dim dtorggridCopy As DataTable
            dtorggrid = New DataTable
            dtorggridCopy = New DataTable
            dtorggrid = gridFullView.DataSource
            dtorggridCopy = dtorggrid.Copy
            Dim dr As DataRow
            dtorggridCopy.Rows.Add()
            dtorggridCopy.Rows.Add()
            For cnt As Integer = 0 To lstTotal.Items.Count - 1
                dr = dtorggridCopy.NewRow()
                dr!tagno = lstTotal.Items(cnt).SubItems(0).Text
                dr!PCS = Val(lstTotal.Items(cnt).SubItems(1).Text)
                dr!grswt = Val(lstTotal.Items(cnt).SubItems(3).Text)
                dr!netwt = Val(lstTotal.Items(cnt).SubItems(4).Text)
                dtorggridCopy.Rows.Add(dr)
            ...
            gridFullViewprint.DataSource = Nothing
            gridFullViewprint.DataSource = dtorggridCopy
            funcLastViewStyle(gridFullViewprint)
            If gridFullViewprint.Rows.Count > 0 Then
                BrightPosting.GExport.Post(Me.Name, strCompanyName, "Stock Checking", gridFullViewprint, BrightPosting.GExport.GExportType.Print)
            End If
        End If
    ...
    Private Sub chkCheckingByScan_CheckedChanged(sender As Object, e As EventArgs) Handles chkCheckingByScan.CheckedChanged
        If chkCheckingByScan.Checked = True Then
