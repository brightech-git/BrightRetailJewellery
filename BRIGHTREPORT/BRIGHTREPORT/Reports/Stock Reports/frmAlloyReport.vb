...
Public Class frmAlloyReport
    ...
    Private Sub GridStyle()
        ...
        With gridView
            ...
            .Columns("WEIGHT").HeaderText = "ALLOY-WEIGHT"
    ...
    Private Sub Report()
        ...
        strSql = " Exec " & cnStockDb & "..Sp_RPT_ALLOYDETAILS "
        strSql += vbCrLf + " @FROMDATE ='" & dtpFrom.Value.ToString("yyyy-MM-dd") & "',"
        strSql += vbCrLf + " @TODATE ='" & dtpTo.Value.ToString("yyyy-MM-dd") & "',"
        strSql += vbCrLf + " @BMETAL ='" & chkCmbMetal.Text & "', "
        ...
        dtSource.Columns("KEYNO").AutoIncrementSeed = 0
        dtSource.Columns("KEYNO").AutoIncrementStep = 1
        strSql = "SELECT * FROM " + cnStockDb + "..TEMPALLOYDETAILS ORDER BY RESULT,TRANDATE,TRANNO"
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dtSource)
        ...
        ObjGrouper.pColumns_Sum.Add("WEIGHT")
        ObjGrouper.pColumns_Sum.Add("GRSWT")
        'If rbtCategorywise.Checked = True Then
        ObjGrouper.pColName_Particular = "METAL"
        ObjGrouper.pColName_ReplaceWithParticular = "METAL"
        ...
        If rbtTranDate.Checked = True Then
            title += " TRANDATE WISE"
        Else
            title += " CATEGORY WISE"
        End If
        title += " ALLOY " + IIf(rbtReceipt.Checked = True, "RECEIPT", "ISSUE") + " REPORT BASED ON "
        title += chkCmbMetal.Text + " BASE METAL AND "
        title += chkCmbALLOY.Text + " ALLOY METAL"
        title += " FROM " + dtpFrom.Text + " TO " + dtpTo.Text
        lblTitle.Font = New Font("VERDANA", 8, FontStyle.Bold)
        lblTitle.Text = title & IIf(chkCmbCostCentre.Text <> "" And chkCmbCostCentre.Text <> "ALL", " :" & chkCmbCostCentre.Text, "")
        gridView.Columns("METAL").Visible = True
        Prop_Sets()
    End Sub
...
Public Class frmALLOYREPORT_Properties
