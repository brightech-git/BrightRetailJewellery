...
Public Class frmCategoryStock
    ...
    Dim CATSTOCK_NETWT_DISABLE As Boolean = IIf(GetAdmindbSoftValue("CATSTOCK_NETWT_DISABLE", "N") = "Y", True, False)
    Dim CATSTOCK_PCS_VALUE As Boolean = IIf(GetAdmindbSoftValue("CATSTOCK_PCS_VALUE", "N") = "Y", True, False)
    Dim CATSTOCK_OTHERISS_RATE As Boolean = IIf(GetAdmindbSoftValue("CATSTOCK_OTHERISS_RATE", "N") = "Y", True, False)
    Dim SPECIFICFORMAT As String = GetAdmindbSoftValue("SPECIFICFORMAT", "")
    Dim RPT_ALLOYDETAIL As Boolean = IIf(GetAdmindbSoftValue("RPT_ALLOYDETAIL", "N") = "Y", True, False)
    Dim RPT_CATSTOCK_GRSNET As Boolean = IIf(GetAdmindbSoftValue("RPT_CATSTOCK_GRSNET", "N") = "Y", True, False)
    ...
    Function funcTranNoWise() As Integer
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " ,SUM(OPENING)OPENING,SUM(OPENING_NETWT)OPENING_NETWT,SUM(RECEIPT)RECEIPT,SUM(RECEIPT_NETWT)RECEIPT_NETWT"
            strSql += vbCrLf + " ,SUM(ISSUE)ISSUE,SUM(ISSUE_NETWT)ISSUE_NETWT,SUM(CLOSING)CLOSING,SUM(CLOSING_NETWT)CLOSING_NETWT"
        Else
            strSql += vbCrLf + " ,SUM(OPENING)OPENING,SUM(RECEIPT)RECEIPT,SUM(ISSUE)ISSUE,SUM(CLOSING)CLOSING"
        End If
        strSql += vbCrLf + " ,RATE,VALUE,CATCODE,RESULT"
        strSql += vbCrLf + " ,FRM"
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " DECLARE @OPENING_NETWT NUMERIC(12,3)"
            strSql += vbCrLf + " DECLARE @ISSUE_NETWT NUMERIC(12,3)"
            strSql += vbCrLf + " DECLARE @RECEIPT_NETWT NUMERIC(12,3)"
            strSql += vbCrLf + " DECLARE @CLOSING_NETWT NUMERIC(12,3)"
        End If
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "         FOR SELECT TRANNO,OPENING,ISSUE,RECEIPT,CLOSING,OPENING_NETWT,ISSUE_NETWT,RECEIPT_NETWT,CLOSING_NETWT "
            strSql += vbCrLf + "         FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANNO WHERE CATNAME = @CATNAME"
        Else
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " 		UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANNO SET CLOSING_NETWT = OPENING_NETWT+RECEIPT_NETWT-ISSUE_NETWT WHERE TRANNO = @TRANNO And CATNAME = @CATNAME"
            strSql += vbCrLf + "  		FETCH NEXT FROM CURMONTH INTO @TRANNO,@OPENING,@ISSUE,@RECEIPT,@CLOSING,@OPENING_NETWT,@ISSUE_NETWT,@RECEIPT_NETWT,@CLOSING_NETWT"
        Else
            strSql += vbCrLf + "  		FETCH NEXT FROM CURMONTH INTO @TRANNO,@OPENING,@ISSUE,@RECEIPT,@CLOSING"
        End If
        strSql += vbCrLf + " 		IF @PRETRANNO <> @TRANNO"
        strSql += vbCrLf + "                    BEGIN"
        strSql += vbCrLf + "                    UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANNO SET OPENING = (SELECT SUM(CLOSING)CLOSING FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANNO WHERE TRANNO = @PRETRANNO And CATNAME = @CATNAME)"
        strSql += vbCrLf + "                    WHERE TRANNO = @TRANNO And CATNAME = @CATNAME"
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "                    UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANNO SET OPENING_NETWT = (SELECT SUM(CLOSING_NETWT)CLOSING_NETWT FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANNO WHERE TRANNO = @PRETRANNO And CATNAME = @CATNAME)"
            strSql += vbCrLf + "                    WHERE TRANNO = @TRANNO And CATNAME = @CATNAME"
        End If
        strSql += vbCrLf + "                    END"
    ...
    Function funcMonthWise() As Integer
        ...
        strSql += vbCrLf + "         DECLARE CURMONTH CURSOR"
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "         FOR SELECT MMONTHID,OPENING,ISSUE,RECEIPT,CLOSING,OPENING_NETWT,ISSUE_NETWT,RECEIPT_NETWT,CLOSING_NETWT "
            strSql += vbCrLf + "         FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKMONTH WHERE CATNAME = @CATNAME"
        Else
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " 		UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKMONTH SET CLOSING_NETWT = OPENING_NETWT+RECEIPT_NETWT-ISSUE_NETWT WHERE MMONTHID = @MMONTHID And CATNAME = @CATNAME"
            strSql += vbCrLf + "  		FETCH NEXT FROM CURMONTH INTO @MMONTHID,@OPENING,@ISSUE,@RECEIPT,@CLOSING,@OPENING_NETWT,@ISSUE_NETWT,@RECEIPT_NETWT,@CLOSING_NETWT"
        Else
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "                    UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKMONTH SET OPENING_NETWT = (SELECT CLOSING_NETWT FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKMONTH WHERE MMONTHID = @PREMMONTHID And CATNAME = @CATNAME)"
            strSql += vbCrLf + "                    WHERE MMONTHID = @MMONTHID And CATNAME = @CATNAME"
        End If
        strSql += vbCrLf + "                    END"
    ...
    Function funcSummaryWise() As Integer
        ...
        strSql += vbCrLf + " SUM(RECEIPT)RECEIPT,SUM(ISSUE)ISSUE,SUM(CLOSING)CLOSING"
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " ,SUM(OPENING_NETWT)OPENING_NETWT,SUM(RECEIPT_NETWT)RECEIPT_NETWT,SUM(ISSUE_NETWT)ISSUE_NETWT,SUM(CLOSING_NETWT)CLOSING_NETWT"
        End If
    ...
    Function funcDateWise() As Integer
        ...
        strSql += vbCrLf + " ,TRANDATE,' 'TRANNO, ' 'COLHEAD "
        strSql += vbCrLf + " ,ROW_NUMBER() OVER ( ORDER BY CATNAME,TRANDATE) ROWID "
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "         FOR SELECT TRANDATE,OPENING,ISSUE,RECEIPT,CLOSING,OPENING_NETWT,ISSUE_NETWT,RECEIPT_NETWT,CLOSING_NETWT "
            strSql += vbCrLf + "         FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE WHERE CATNAME = @CATNAME"
        Else
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " 		UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE SET CLOSING_NETWT = OPENING_NETWT+RECEIPT_NETWT-ISSUE_NETWT WHERE TRANDATE = @TRANDATE And CATNAME = @CATNAME"
            strSql += vbCrLf + "  		FETCH NEXT FROM CURTRANDATE INTO @TRANDATE,@OPENING,@ISSUE,@RECEIPT,@CLOSING,@OPENING_NETWT,@ISSUE_NETWT,@RECEIPT_NETWT,@CLOSING_NETWT"
        Else
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "                    UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE SET OPENING_NETWT = (SELECT CLOSING_NETWT FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE WHERE TRANDATE = @PREDATE And CATNAME = @CATNAME)"
            strSql += vbCrLf + "                    WHERE TRANDATE = @TRANDATE And CATNAME = @CATNAME"
        End If
        strSql += vbCrLf + "                    END"
        ...
        cmd.ExecuteNonQuery()
        Try
            strSql = "ALTER TABLE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE DROP COLUMN ROWID"
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
    ...
    Function funcMetalDateWise() As Integer
        ...
        strSql = " IF (SELECT 1 FROM " & Temptable & "..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "CATSTOCKTRANDATE')>0"
        strSql += vbCrLf + " DROP TABLE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE"
        strSql += vbCrLf + " SELECT CONVERT(VARCHAR(50),CONVERT(VARCHAR,TRANDATE,103)) PARTICULAR,METALNAME"
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " ,SUM(OPENING)OPENING,SUM(OPENING_NETWT)OPENING_NETWT,SUM(RECEIPT)RECEIPT,SUM(RECEIPT_NETWT)RECEIPT_NETWT"
            strSql += vbCrLf + " ,SUM(ISSUE)ISSUE,SUM(ISSUE_NETWT)ISSUE_NETWT,SUM(CLOSING)CLOSING,SUM(CLOSING_NETWT)CLOSING_NETWT"
        Else
            strSql += vbCrLf + " ,SUM(OPENING)OPENING,SUM(RECEIPT)RECEIPT,SUM(ISSUE)ISSUE,SUM(CLOSING)CLOSING"
        End If
        strSql += vbCrLf + " ,RATE,VALUE,RESULT"
        strSql += vbCrLf + " ,FRM"
        strSql += vbCrLf + " ,' ' YYEAR,' 'MMONTHID,' 'MMONTH,CONVERT(VARCHAR,TRANDATE,103)DDATE"
        strSql += vbCrLf + " ,TRANDATE,' 'TRANNO, ' 'COLHEAD "
        strSql += vbCrLf + " INTO " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE "
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "         FOR SELECT TRANDATE,OPENING,ISSUE,RECEIPT,CLOSING,OPENING_NETWT,ISSUE_NETWT,RECEIPT_NETWT,CLOSING_NETWT "
            strSql += vbCrLf + "         FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE WHERE METALNAME = @METALNAME"
        Else
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " 		UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE SET CLOSING_NETWT = OPENING_NETWT+RECEIPT_NETWT-ISSUE_NETWT WHERE TRANDATE = @TRANDATE And METALNAME = @METALNAME"
            strSql += vbCrLf + "  		FETCH NEXT FROM CURTRANDATE INTO @TRANDATE,@OPENING,@ISSUE,@RECEIPT,@CLOSING,@OPENING_NETWT,@ISSUE_NETWT,@RECEIPT_NETWT,@CLOSING_NETWT"
        Else
            strSql += vbCrLf + "  		FETCH NEXT FROM CURTRANDATE INTO @TRANDATE,@OPENING,@ISSUE,@RECEIPT,@CLOSING"
        End If
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + "                    UPDATE " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE SET OPENING_NETWT = (SELECT SUM(CLOSING_NETWT) FROM " & Temptable & "..TEMP" & systemId & "CATSTOCKTRANDATE WHERE TRANDATE = @PREDATE And METALNAME = @METALNAME)"
            strSql += vbCrLf + "                    WHERE TRANDATE = @TRANDATE And METALNAME = @METALNAME"
        End If
        strSql += vbCrLf + "                    END"
        strSql += vbCrLf + " 		SELECT @PREDATE = @TRANDATE"
    ...
    Private Sub btnView_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        ...
        If chkalloy.Checked = True And RPT_ALLOYDETAIL = False Then
            ...
            ElseIf rbtGrsNetwt.Checked = True Then
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP = 'REC_OPENING' THEN (GRSWT+ALLOY) WHEN SEP = 'ISS_OPENING' THEN -1*(GRSWT+ALLOY) END),0)AS OPENING"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP = 'REC_OPENING' THEN (NETWT+ALLOY) WHEN SEP = 'ISS_OPENING' THEN -1*(NETWT+ALLOY) END),0)AS OPENING_NETWT"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP = 'ISSUE' THEN (GRSWT+ALLOY) END),0)AS ISSUE"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP = 'ISSUE' THEN (NETWT+ALLOY) END),0)AS ISSUE_NETWT"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP = 'RECEIPT' THEN (GRSWT+ALLOY) END),0)AS RECEIPT"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP = 'RECEIPT' THEN (NETWT+ALLOY) END),0)AS RECEIPT_NETWT"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP IN ('REC_OPENING','RECEIPT') THEN (GRSWT+ALLOY) ELSE -1*(GRSWT+ALLOY) END),0)AS CLOSING"
                strSql += vbCrLf + " ,ISNULL(SUM(CASE WHEN SEP IN ('REC_OPENING','RECEIPT') THEN (NETWT+ALLOY) ELSE -1*(NETWT+ALLOY) END),0)AS CLOSING_NETWT"
            Else 'NETWT
        ...
        strSql += vbCrLf + " AND I.SNO NOT IN( SELECT ISSSNO FROM " & cnStockDb & "..ISSMETAL WHERE TRANDATE <  @FRMDATE AND I.TRANTYPE IN ('SA','OD','RD'))"
        ...
        End If
        strSql += vbCrLf + " GROUP BY I.CATCODE,TRANDATE,TRANNO,TRANSTATUS,M.STUDDED"
        If rbtStkType.Checked Then
        ...
        If rbtGeneral.Checked = True Then
            ...
            strSql += vbCrLf + " ,SUM(CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END)NETWT"
            strSql += vbCrLf + " ,SUM(CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END)PUREWT"
        ElseIf rbtCarat.Checked = True Then
            strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID = 'T' THEN I.GRSWT * 5 ELSE GRSWT END)GRSWT"
            strSql += vbCrLf + " ,0 ALLOY"
            strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'T' THEN CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END * 5 ELSE CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END END)NETWT"
            strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'T' THEN CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END * 5 ELSE CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END END)PUREWT"
        Else
            strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID = 'D' THEN GRSWT / 5 ELSE GRSWT END)GRSWT"
            strSql += vbCrLf + " ,0 ALLOY"
            strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'D' THEN CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END/ 5 ELSE CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END END)NETWT"
            strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'D' THEN CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END/ 5 ELSE CASE WHEN ISNULL(I.NETWT,0)<>0 THEN I.NETWT ELSE I.GRSWT END END)PUREWT"
        End If
        strSql += vbCrLf + " ,@FRMDATE TRANDATE,-1 TRANNO,''TRANSTATUS"
        strSql += vbCrLf + " ,DATEPART(YEAR,TRANDATE)YYEAR,DATEPART(MONTH,TRANDATE)MMONTHID,"
        strSql += vbCrLf + " LEFT(DATENAME(MONTH,TRANDATE),3)MMONTH"
        ...
        strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') IN  ('SA','OD','RD') "
        ...
        strSql += vbCrLf + " SELECT "
        strSql += vbCrLf + " CASE WHEN ISNULL(M.STUDDED,'')='S' THEN '" & IIf(_SepStud = False, "", "2STUD") & "' ELSE '" & IIf(_SepStud = False, "", "1LOOSE") & "' END FRM,'REC_OPENING'SEP,R.CATCODE"
        If rbtGeneral.Checked = True Then
        ...
        strSql += vbCrLf + " WHERE TRANDATE < @frmDATE AND ISNULL(CANCEL,'') = ''"
        strSql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('RRC','WOT') "
        strSql += vbCrLf + " AND R.SNO NOT IN( SELECT ISSSNO FROM " & cnStockDb & "..RECEIPTMETAL WHERE TRANDATE <  @FRMDATE)"
        If chkWithApproval.Checked = False Then strSql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('AR') "
        If ftrReceipt.ToString <> "" And Not ftrReceipt Is Nothing Then
        ...
        End If
        'If Not cnCentStock Then strSql += vbcrlf + " AND COMPANYID = '" & GetStockCompId() & "'"
        strSql += vbCrLf + " GROUP BY R.CATCODE,TRANDATE,TRANNO,TRANSTATUS,M.STUDDED"
        If rbtStkType.Checked Then
            strSql += vbCrLf + ",R.STKTYPE"
        End If

        ''RECEIPT METAL AS PER MAGESH SIR INSTRUCTION 21-04-2022
        strSql += vbCrLf + " UNION ALL"
        ...
        If rbtGeneral.Checked = True Then
            strSql += vbCrLf + " ,SUM(R.GRSWT)GRSWT"
            strSql += vbCrLf + " ,0 ALLOY"
            strSql += vbCrLf + " ,SUM(CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END)NETWT"
            strSql += vbCrLf + " ,SUM(CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END)PUREWT"
        ElseIf rbtCarat.Checked = True Then
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'G' AND C.METALID IN ('T','D') THEN R.GRSWT * 5 ELSE R.GRSWT END)GRSWT"
            strSql += vbCrLf + " ,0 ALLOY"
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'G' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END * 5 ELSE CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END END)NETWT"
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'G' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END * 5 ELSE CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END END)PUREWT"
        Else
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN R.GRSWT / 5 ELSE R.GRSWT END)GRSWT"
            strSql += vbCrLf + " ,0 ALLOY"
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END / 5 ELSE CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END END)NETWT"
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END / 5 ELSE CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END END)PUREWT"
        End If
        strSql += vbCrLf + " ,@FRMDATE TRANDATE,-1 TRANNO,'' TRANSTATUS"
        strSql += vbCrLf + " ,DATEPART(YEAR,R.TRANDATE)YYEAR,DATEPART(MONTH,R.TRANDATE)MMONTHID,"
        strSql += vbCrLf + " LEFT(DATENAME(MONTH,R.TRANDATE),3)MMONTH"
        If rbtStkType.Checked Then
        ...
        End If
        strSql += vbCrLf + " FROM " & cnStockDb & "..RECEIPTMETAL AS R"
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY C ON R.CATCODE = C.CATCODE"
        strSql += vbCrLf + " INNER JOIN " & cnStockDb & "..RECEIPT REC ON REC.SNO = R.ISSSNO "
        strSql += vbCrLf + " WHERE R.TRANDATE < @frmDATE "
        strSql += vbCrLf + " AND ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT "
        strSql += vbCrLf + " WHERE ISNULL(CANCEL,'') = '' "
        strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') NOT IN  ('RRC','WOT') "
        If chkWithApproval.Checked = False Then strSql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('AR') "
        If ftrReceipt.ToString <> "" And Not ftrReceipt Is Nothing Then
            strSql += vbCrLf + Replace(Replace(ftrReceipt, "R.METALID", "METALID"), "R.CATCODE", "CATCODE")
        End If
        CompanyFilt1 = Replace(CompanyFilt, "AND R.COMPANYID", "AND COMPANYID")
        strSql += vbCrLf + CompanyFilt1 & ")"
        If rbtMfg.Checked Then
            strSql += vbCrLf + " AND ISNULL(REC.STKTYPE,'')='M'"
        ElseIf rbtTrading.Checked Then
            strSql += vbCrLf + " AND ISNULL(REC.STKTYPE,'')NOT IN('M','E')"
        ElseIf rbtExem.Checked Then
            strSql += vbCrLf + " AND ISNULL(REC.STKTYPE,'')='E'"
        End If
        'If Not cnCentStock Then strSql += vbcrlf + " AND COMPANYID = '" & GetStockCompId() & "'"
        strSql += vbCrLf + " GROUP BY R.CATCODE,R.TRANDATE/*,R.TRANNO*/"
        If rbtStkType.Checked Then
        ...
        If rbtMfg.Checked Or rbtAll.Checked Then
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT '" & IIf(_SepStud = False, "", "1LOOSE") & "'FRM,'REC_OPENING'SEP,CATCODE ,GRSWT,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
            ...
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT '" & IIf(_SepStud = False, "", "1LOOSE") & "'FRM,'ISS_OPENING'SEP,CATCODE ,GRSWT ,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
            strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
            If rbtStkType.Checked Then
                strSql += vbCrLf + " ,'MANUFACTURING' AS STOCKTYPE"
            End If
        ...
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        If ftrReceipt.ToString <> "" And Not ftrReceipt Is Nothing Then
            strSql += vbCrLf + ftrReceipt.Replace("R.METALID", "METALID").ToString
        End If
        ...
        strSql += vbCrLf + " SELECT CASE WHEN FRM='' THEN '" & IIf(_SepStud = False, "", "1LOOSE") & "' ELSE " & IIf(_SepStud = False, "''", "FRM") & " END AS FRM,'ISSUE'SEP,CATCODE ,GRSWT,ALLOY ,NETWT,PUREWT,TRANDATE,TRANNO,TRANSTATUS "
        strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
        If rbtStkType.Checked Then
            strSql += vbCrLf + " ,STOCKTYPE"
        End If
        ...
        strSql += vbCrLf + " AND I.SNO NOT IN( SELECT ISSSNO FROM " & cnStockDb & "..ISSMETAL WHERE TRANDATE BETWEEN  @FRMDATE AND @TODATE AND I.TRANTYPE IN ('SA','OD','RD'))"
        strSql += vbCrLf + " AND ISNULL(I.TRANTYPE,'') NOT IN  ('IRC') "
        ...
        strSql += vbCrLf + " AND ISNULL(I.TRANTYPE,'') IN ('SA','OD','RD') "
        strSql += vbCrLf + " AND ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..ISSUE "
        strSql += vbCrLf + " WHERE ISNULL(CANCEL,'') = '' "
        ...
        If chkalloy.Checked And RPT_ALLOYDETAIL Then
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT FRM,CATCODE ,GRSWT,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
            strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
            If rbtStkType.Checked Then
                strSql += vbCrLf + " ,STOCKTYPE"
            End If
            strSql += vbCrLf + " FROM ("
            strSql += vbCrLf + " SELECT '' FRM,ISNULL((SELECT TOP 1 CATCODE FROM " & cnAdminDb & "..CATEGORY WHERE METALID=A.ALLOYID),'') CATCODE"
            If rbtGeneral.Checked = True Then
                strSql += vbCrLf + " ,SUM(WEIGHT)GRSWT"
                strSql += vbCrLf + " ,0 AS ALLOY"
                strSql += vbCrLf + " ,SUM(WEIGHT) NETWT,SUM(WEIGHT) PUREWT"
            ElseIf rbtCarat.Checked = True Then
                strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'T' THEN WEIGHT * 5 ELSE WEIGHT END)GRSWT"
                strSql += vbCrLf + " ,0 AS ALLOY"
                strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID = 'T' THEN WEIGHT * 5 ELSE WEIGHT END) NETWT"
                strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'T' THEN WEIGHT * 5 ELSE WEIGHT END) PUREWT"
            Else
                strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'D' THEN WEIGHT / 5 ELSE WEIGHT END)GRSWT"
                strSql += vbCrLf + " ,0 AS ALLOY"
                strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'D' THEN WEIGHT / 5 ELSE WEIGHT END)NETWT"
                strSql += vbCrLf + " ,SUM(CASE WHEN I.METALID= 'D' THEN WEIGHT / 5 ELSE WEIGHT END)PUREWT"
            End If
            strSql += vbCrLf + " ,A.TRANDATE,A.TRANNO,''TRANSTATUS,I.TRANTYPE"
            strSql += vbCrLf + " ,DATEPART(YEAR,A.TRANDATE)YYEAR,DATEPART(MONTH,A.TRANDATE)MMONTHID,"
            strSql += vbCrLf + " LEFT(DATENAME(MONTH,A.TRANDATE),3)MMONTH"
            If rbtStkType.Checked Then
                strSql += vbCrLf + " ,(SELECT CASE WHEN ISNULL(STKTYPE,'')='M' THEN 'MANUFACTURING' WHEN ISNULL(STKTYPE,'')='E' THEN 'EXEMPTED' ELSE 'TRADING' END FROM " & cnStockDb & "..ISSUE WHERE SNO=A.ISSSNO) AS STOCKTYPE"
            End If
            strSql += vbCrLf + " FROM " & cnStockDb & "..ALLOYDETAILS AS A "
            strSql += vbCrLf + " LEFT JOIN " & cnStockDb & "..ISSUE AS I ON A.ISSSNO=I.SNO"
            strSql += vbCrLf + " WHERE A.TRANDATE BETWEEN @frmDATE AND @toDATE "
            strSql += vbCrLf + " AND ISNULL(A.TRANTYPE,'') IN ('IIS') "
            strSql += vbCrLf + " AND ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..ISSUE "
            strSql += vbCrLf + " WHERE ISNULL(CANCEL,'') = '' "
            strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') NOT IN  ('IRC') "
            If chkWithApproval.Checked = False Then strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') NOT IN  ('AI') "
            strSql += vbCrLf + " AND (TRANDATE BETWEEN @frmDATE AND @toDATE )"
            'strSql += vbCrLf + Replace(ftrIssue, "I.METALID", "METALID")
            CompanyFilt1 = CompanyFilt.Replace("AND COMPANYID", "AND I.COMPANYID").Replace("AND COSTID", "AND I.COSTID")
            strSql += vbCrLf + CompanyFilt
            If rbtMfg.Checked Then
                strSql += vbCrLf + " AND ISNULL(STKTYPE,'')='M'"
            ElseIf rbtTrading.Checked Then
            ...
            CompanyFilt1 = CompanyFilt.Replace("AND COMPANYID", "AND I.COMPANYID").Replace("AND COSTID", "AND I.COSTID")
            strSql += vbCrLf + CompanyFilt1
            strSql += vbCrLf + " GROUP BY I.CATCODE,A.TRANDATE,A.TRANNO,ALLOYID,I.TRANTYPE"
            If rbtStkType.Checked Then
                strSql += vbCrLf + " ,A.ISSSNO"
            End If
            strSql += vbCrLf + " )X"
            strSql += vbCrLf + " WHERE 1=1"
            strSql += vbCrLf + ftrIssue.Replace("I.TRANTYPE", "X.TRANTYPE").Replace("I.CATCODE", "X.CATCODE")
        End If
        ...
        strSql += vbCrLf + " ) X"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT CASE WHEN FRM='' THEN '" & IIf(_SepStud = False, "", "1LOOSE") & "' ELSE " & IIf(_SepStud = False, "''", "FRM") & " END AS FRM,'RECEIPT'SEP,CATCODE ,GRSWT,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
        strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
        If rbtStkType.Checked Then
            strSql += vbCrLf + " ,STOCKTYPE"
        End If
        ...
        strSql += vbCrLf + " WHERE TRANDATE BETWEEN @frmDATE AND @toDATE AND ISNULL(CANCEL,'') = ''"
        strSql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('RRC','WOT') "
        strSql += vbCrLf + " AND R.SNO NOT IN( SELECT ISSSNO FROM " & cnStockDb & "..RECEIPTMETAL WHERE TRANDATE BETWEEN @frmDATE AND @toDATE)"
        If chkWithApproval.Checked = False Then strSql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('AR') "
        ''strSql += vbcrlf + " AND ISNULL(TRANTYPE,'') <> 'RPU' "
        If ftrReceipt.ToString <> "" And Not ftrReceipt Is Nothing Then
            strSql += vbCrLf + Replace(ftrReceipt, "AND CATCODE", "AND R.CATCODE").ToString
        End If
        'If Not cnCentStock Then strSql += vbcrlf + " AND COMPANYID = '" & GetStockCompId() & "'"
        CompanyFilt1 = Replace(CompanyFilt, "AND COMPANYID", "AND R.COMPANYID")
        strSql += vbCrLf + CompanyFilt1
        ...
        strSql += vbCrLf + " SELECT '' FRM,R.CATCODE"
        If rbtGeneral.Checked = True Then
            strSql += vbCrLf + " ,SUM(R.GRSWT)GRSWT"
            strSql += vbCrLf + " ,0 ALLOY"
            strSql += vbCrLf + " ,SUM(CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END)NETWT"
        ...
        Else
            ...
            strSql += vbCrLf + " ,SUM(CASE WHEN REC.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END / 5 ELSE CASE WHEN ISNULL(R.NETWT,0)<>0 THEN R.NETWT ELSE R.GRSWT END END)PUREWT"
        End If
        strSql += vbCrLf + " ,R.TRANDATE,R.TRANNO,'' TRANSTATUS"
        strSql += vbCrLf + " ,DATEPART(YEAR,R.TRANDATE)YYEAR,DATEPART(MONTH,R.TRANDATE)MMONTHID,"
        strSql += vbCrLf + " LEFT(DATENAME(MONTH,R.TRANDATE),3)MMONTH"
        If rbtStkType.Checked Then
            strSql += vbCrLf + " ,(SELECT CASE WHEN ISNULL(STKTYPE,'')='M' THEN 'MANUFACTURING' WHEN ISNULL(STKTYPE,'')='E' THEN 'EXEMPTED' ELSE 'TRADING' END FROM " & cnStockDb & "..RECEIPT WHERE SNO=R.ISSSNO) AS STOCKTYPE"
        ...
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY C ON R.CATCODE = C.CATCODE"
        strSql += vbCrLf + " INNER JOIN " & cnStockDb & "..RECEIPT REC ON REC.SNO = R.ISSSNO "
        strSql += vbCrLf + " WHERE R.TRANDATE BETWEEN @frmDATE AND @toDATE "
        strSql += vbCrLf + " AND ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT "
        strSql += vbCrLf + " WHERE ISNULL(CANCEL,'') = '' "
        strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') NOT IN  ('RRC','WOT') "
        If chkWithApproval.Checked = False Then strSql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('AR') "
        ...
        If chkalloy.Checked And RPT_ALLOYDETAIL Then
            ...
            End If
            strSql += vbCrLf + " FROM " & cnStockDb & "..ALLOYDETAILS AS A "
            strSql += vbCrLf + " LEFT JOIN " & cnStockDb & "..RECEIPT AS I ON A.ISSSNO=I.SNO"
            strSql += vbCrLf + " WHERE A.TRANDATE BETWEEN @frmDATE AND @toDATE "
            strSql += vbCrLf + " AND ISNULL(A.TRANTYPE,'') IN ('RRE') "
            strSql += vbCrLf + " AND ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT "
            strSql += vbCrLf + " WHERE ISNULL(CANCEL,'') = '' "
            strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') NOT IN  ('RRC','WOT') "
            If chkWithApproval.Checked = False Then strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') NOT IN  ('AI') "
        ...
        If chkWithStuddedStone.Checked = True Then
            ...
            CompanyFilt1 = Replace(CompanyFilt, "AND COMPANYID", "AND R.COMPANYID")
            strSql += vbCrLf + CompanyFilt1
            ''strSql += vbCrLf + Replace(Replace(ftrReceipt, "AND CATCODE", "AND R.CATCODE").ToString, "R.METALID", "METALID").ToString
            If ftrReceipt.ToString <> "" And Not ftrReceipt Is Nothing Then
                strSql += vbCrLf + Replace(ftrReceipt.Replace("AND CATCODE", "AND R.CATCODE").ToString, "R.METALID", "METALID").ToString
            End If
        ...
        If rbtGrsNetwt.Checked Then
            ...
            strSql += vbCrLf + " OPENING_NETWT NUMERIC(12,3),"
            strSql += vbCrLf + " RECEIPT NUMERIC(12,3),"
            strSql += vbCrLf + " RECEIPT_NETWT NUMERIC(12,3),"
            strSql += vbCrLf + " ISSUE NUMERIC(12,3),"
            strSql += vbCrLf + " ISSUE_NETWT NUMERIC(12,3),"
            strSql += vbCrLf + " CLOSING NUMERIC(12,3),"
            strSql += vbCrLf + " CLOSING_NETWT NUMERIC(12,3),"
        Else
            strSql += vbCrLf + " OPENING NUMERIC(12,3),"
            strSql += vbCrLf + " RECEIPT NUMERIC(12,3),"
            strSql += vbCrLf + " ISSUE NUMERIC(12,3),"
        ...
        If rbtStkType.Checked = True Then
            ...
            strSql += vbCrLf + " SELECT CONVERT(VARCHAR(250),PARTICULAR)AS PARTICULAR,METALNAME,CATNAME"
            If rbtGrsNetwt.Checked Then
                strSql += vbCrLf + " ,OPENING,OPENING_NETWT,RECEIPT,RECEIPT_NETWT"
                strSql += vbCrLf + " ,ISSUE,ISSUE_NETWT,CLOSING,CLOSING_NETWT"
            Else
            ...
            strSql += vbCrLf + " GROUP BY METALNAME,STOCKTYPE"
            strSql += vbCrLf + " ) X"
            ...
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + " INSERT INTO " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP(METALNAME,CATNAME,STOCKTYPE,RESULT,RATE,"
            strSql += vbCrLf + " VALUE,FRM,YYEAR,MMONTHID,MMONTH,DDATE,TRANDATE,TRANNO,COLHEAD,PARTICULAR )"
            strSql += vbCrLf + " SELECT DISTINCT METALNAME,METALNAME CATNAME,STOCKTYPE, 0,' ',' ',' ','','','','','','','T','' "
            strSql += vbCrLf + " FROM " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP WHERE RESULT =1"
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT DISTINCT NULL,NULL,STOCKTYPE, -1,' ',' ',' ','','','','','','','T','' "
            strSql += vbCrLf + " FROM " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP WHERE RESULT =1"
            strSql += vbCrLf + " END "
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
        ...
        ElseIf rbtMetalDateWise.Checked Then
            ...
            strSql += vbCrLf + " NAME = 'TEMP" & systemId & "CTRSTOCKTEMP')>0 "
            strSql += vbCrLf + " DROP TABLE " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP"
            strSql += vbCrLf + " SELECT PARTICULAR,METALNAME,OPENING,RECEIPT,ISSUE,CLOSING,RATE,VALUE,"
            ...
            strSql += vbCrLf + " SELECT"
            If rbtSummary.Checked = True Then
                strSql += vbCrLf + " METALNAME + ' TOTAL' PARTICULAR,METALNAME,'SUB TOTAL'CATNAME"
            Else
                strSql += vbCrLf + " METALNAME + ' TOTAL' PARTICULAR,METALNAME"
            End If
            strSql += vbCrLf + " ,SUM(0.000)OPENING,SUM(0.000)RECEIPT,SUM(0.000)ISSUE,SUM(0.000)CLOSING"
            strSql += vbCrLf + " ,' ' RATE,' ' VALUE,2 RESULT"
            strSql += vbCrLf + " ,' 'FRM"
            strSql += vbCrLf + " ,' 'YYEAR,' 'MMONTHID,' 'MMONTH,' 'DDATE,' 'TRANDATE,' 'TRANNO, 'S'COLHEAD"
            strSql += vbCrLf + " FROM " & Temptable & "..TEMP" & systemId & "CATSTOCK AS T"
            If rbtSummary.Checked = True Then
            ...
            strSql = " IF (SELECT COUNT(*) FROM " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP)>0 "
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + " INSERT INTO " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP(PARTICULAR,METALNAME,RESULT,RATE,"
            strSql += vbCrLf + " VALUE,FRM,YYEAR,MMONTHID,MMONTH,DDATE,TRANDATE,TRANNO,COLHEAD )"
            If rbtSummary.Checked = True Then
                strSql += vbCrLf + " SELECT DISTINCT METALNAME,METALNAME,0,' ',' ',' ','','','','','','','T' "
            ElseIf rbtDateWise.Checked = True Then
                strSql += vbCrLf + " SELECT DISTINCT METALNAME,METALNAME,0,' ',' ',' ','','','','','','','T' "
            ElseIf rbtMonthWise.Checked = True Then
                strSql += vbCrLf + " SELECT DISTINCT METALNAME,METALNAME,0,' ',' ',' ','','','','','','','T' "
            ...
            strSql = " IF (SELECT COUNT(*) FROM " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP)>0 "
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + " INSERT INTO " & Temptable & "..TEMP" & systemId & "CATSUMM(METALNAME,OPENING,RECEIPT,ISSUE,"
            strSql += vbCrLf + " Closing, Rate, VALUE,RESULT, FRM, YYEAR, MMONTHID, MMONTH, DDATE, "
            strSql += vbCrLf + " TRANDATE, TRANNO, COLHEAD) "
            strSql += vbCrLf + " SELECT METALNAME,OPENING,RECEIPT,ISSUE,"
            strSql += vbCrLf + " Closing, Rate, VALUE, RESULT, FRM, YYEAR, MMONTHID, MMONTH, CONVERT(SMALLDATETIME,DDATE,103)DDATE, "
        ...
        Else
            ...
            strSql = " IF (SELECT COUNT(*) FROM " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP)>0 "
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + " INSERT INTO " & Temptable & "..TEMP" & systemId & "CATSUMM(METALNAME,CATNAME,OPENING,RECEIPT,ISSUE,Closing"
            If rbtGrsNetwt.Checked Then
                strSql += vbCrLf + " ,OPENING_NETWT,RECEIPT_NETWT,ISSUE_NETWT,CLOSING_NETWT"
            End If
            strSql += vbCrLf + " , Rate, VALUE, CATCODE, RESULT, FRM, YYEAR, MMONTHID, MMONTH, DDATE, "
            strSql += vbCrLf + " TRANDATE, TRANNO, COLHEAD) "
            strSql += vbCrLf + " SELECT METALNAME,PARTICULAR,OPENING,RECEIPT,ISSUE,Closing"
            If rbtGrsNetwt.Checked Then
                strSql += vbCrLf + " ,OPENING_NETWT,RECEIPT_NETWT,ISSUE_NETWT,CLOSING_NETWT"
            End If
            strSql += vbCrLf + " , Rate, VALUE, CATCODE, RESULT, FRM, YYEAR, MMONTHID, MMONTH, CONVERT(SMALLDATETIME,DDATE,103)DDATE, "
        ...
        If rbtGrsNetwt.Checked Then
            strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CATSUMM SET OPENING_NETWT=NULL WHERE OPENING_NETWT = 0"
            strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CATSUMM SET RECEIPT_NETWT=NULL WHERE RECEIPT_NETWT = 0"
            strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CATSUMM SET ISSUE_NETWT=NULL WHERE ISSUE_NETWT = 0"
            strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CATSUMM SET CLOSING_NETWT=NULL WHERE CLOSING_NETWT= 0"
        End If
        ...
        If rbtMetalDateWise.Checked Then
            ...
            If rbtGrsNetwt.Checked Then
                strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP SET OPENING_NETWT=NULL WHERE OPENING_NETWT = 0"
                strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP SET RECEIPT_NETWT=NULL WHERE RECEIPT_NETWT = 0"
                strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP SET ISSUE_NETWT=NULL WHERE ISSUE_NETWT = 0"
                strSql += vbCrLf + " UPDATE " & Temptable & "..TEMP" & systemId & "CTRSTOCKTEMP SET CLOSING_NETWT=NULL WHERE CLOSING_NETWT= 0"
            End If
        ...
        With gridView
            ...
            With .Columns("OPENING")
                ...
                .ReadOnly = True
                .SortMode = DataGridViewColumnSortMode.NotSortable
                If rbtGrsNetwt.Checked Then .HeaderText = "OPENING GRSWT"
            End With
            With .Columns("ISSUE")
                .Width = 80
                .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .ReadOnly = True
                .SortMode = DataGridViewColumnSortMode.NotSortable
                If rbtGrsNetwt.Checked Then .HeaderText = "ISSUE GRSWT"
            End With
            With .Columns("RECEIPT")
                ...
                .ReadOnly = True
                .SortMode = DataGridViewColumnSortMode.NotSortable
                If rbtGrsNetwt.Checked Then .HeaderText = "RECEIPT GRSWT"
            End With
            With .Columns("CLOSING")
                ...
                .ReadOnly = True
                .SortMode = DataGridViewColumnSortMode.NotSortable
                If rbtGrsNetwt.Checked Then .HeaderText = "CLOSING GRSWT"
            End With
            If rbtGrsNetwt.Checked Then
                With .Columns("OPENING_NETWT")
                    .Width = 80
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .ReadOnly = True
                    .SortMode = DataGridViewColumnSortMode.NotSortable
                    .HeaderText = "OPENING NETWT"
                End With
                With .Columns("ISSUE_NETWT")
                    .Width = 80
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .ReadOnly = True
                    .SortMode = DataGridViewColumnSortMode.NotSortable
                    .HeaderText = "ISSUE NETWT"
                End With
                With .Columns("RECEIPT_NETWT")
                    .Width = 80
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .ReadOnly = True
                    .SortMode = DataGridViewColumnSortMode.NotSortable
                    .HeaderText = "RECEIPT NETWT"
                End With
                With .Columns("CLOSING_NETWT")
                    .Width = 80
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .ReadOnly = True
                    .SortMode = DataGridViewColumnSortMode.NotSortable
                    .HeaderText = "CLOSING NETWT"
                End With
            End If
            ...
            ''For CNT As Integer = 8 To gridView.ColumnCount - 1
            ''    .Columns(CNT).Visible = False
            ''Next
            If rbtGrsNetwt.Checked Then
                For CNT As Integer = 12 To gridView.ColumnCount - 1
                    .Columns(CNT).Visible = False
                Next
            Else
                For CNT As Integer = 8 To gridView.ColumnCount - 1
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        Prop_Gets()
        If CATSTOCK_NETWT_DISABLE Then
            rbtGrsWt.Checked = True
            rbtNetWt.Enabled = False
            CATSTOCK_NETWT_BASED = False
        End If
    End Sub
    ...
    Private Sub gridView_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridView.CellEndEdit
        With gridView.Rows(e.RowIndex)
            ...
            If (Val(.Cells("OPENING").Value.ToString) = 0 And Val(.Cells("ISSUE").Value.ToString) = 0 And Val(.Cells("RECEIPT").Value.ToString) = 0 And Val(.Cells("CLOSING").Value.ToString) = 0) Then
    ...
    Private Sub gridView_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles gridView.KeyPress
        If gridView.RowCount > 0 Then
            ...
            If UCase(e.KeyChar) = "D" Then
                ...
                End If
                If cmbMetal.Text <> "ALL" Then
                    strsql += vbCrLf + " And R.CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY  WHERE METALID = (SELECT METALID FROM " & cnAdminDb & "..METALMAST  WHERE METALNAME = '" & cmbMetal.Text & "'))"
                End If
                ...
                End If
                If chkWithApproval.Checked = False Then strsql += vbCrLf + " AND ISNULL(R.TRANTYPE,'') NOT IN  ('AR') "
                strsql += vbCrLf + " AND R.SNO NOT IN( SELECT ISSSNO FROM " & cnStockDb & "..RECEIPTMETAL WHERE TRANDATE BETWEEN  '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "')"
                strsql += vbCrLf + " GROUP BY R.CATCODE,TRANDATE,TRANNO,TRANSTATUS,M.STUDDED,R.TRANTYPE"


                strsql += vbCrLf + " UNION ALL "
                strsql += vbCrLf + " SELECT ISM.CATCODE"
                strsql += vbCrLf + " ,(SELECT CATNAME FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE=ISM.CATCODE) CATNAME"
                If rbtGrsWt.Checked = True Then
                    strsql += vbCrLf + " ,SUM(CASE WHEN I.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN ISM.GRSWT / 5 ELSE ISM.GRSWT END)GRSWT"
                End If
                If rbtNetWt.Checked = True Then
                    strsql += vbCrLf + " ,SUM(CASE WHEN I.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(ISM.NETWT,0)<>0 THEN ISM.NETWT ELSE ISM.GRSWT END / 5 ELSE CASE WHEN ISNULL(ISM.NETWT,0)<>0 THEN ISM.NETWT ELSE ISM.GRSWT END END)NETWT"
                End If
                If rbtPurewt.Checked = True Then
                    strsql += vbCrLf + " ,SUM(CASE WHEN I.STONEUNIT = 'C' AND C.METALID IN ('T','D') THEN CASE WHEN ISNULL(ISM.NETWT,0)<>0 THEN ISM.NETWT ELSE ISM.GRSWT END / 5 ELSE CASE WHEN ISNULL(ISM.NETWT,0)<>0 THEN ISM.NETWT ELSE ISM.GRSWT END END)PUREWT"
                End If
                strsql += vbCrLf + " ,CONVERT(VARCHAR(15),I.TRANDATE,105) TRANDATE,I.TRANNO"
                strsql += vbCrLf + " ,CASE WHEN ISM.TRANTYPE = 'PU' THEN 'PURCHASE' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'SR' THEN 'SALES RETURN' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'RIN' THEN 'INTERNAL TRANSFER' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'AD' THEN 'AD' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'RPU' THEN 'PURCHASE' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'RAP' THEN 'APPROVAL RECEIPT' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'ROT' THEN 'OTHER RECEIPT' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'RPA' THEN 'PACKING' "
                strsql += vbCrLf + "  ELSE CASE WHEN ISM.TRANTYPE = 'AD' THEN 'AD' "
                strsql += vbCrLf + " ELSE ISM.TRANTYPE"
                strsql += vbCrLf + " END END END END END END END END END"
                strsql += vbCrLf + " ,'D' COLHEAD, 4 RESULT "
                strsql += vbCrLf + " FROM " & cnStockDb & "..RECEIPTMETAL AS ISM"
                strsql += vbCrLf + " LEFT JOIN " & cnStockDb & "..RECEIPT I ON I.SNO=ISM.ISSSNO "
                strsql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..CATEGORY C ON ISM.CATCODE = C.CATCODE"
                strsql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..ITEMMAST M ON M.ITEMID = I.ITEMID"
                strsql += vbCrLf + " WHERE I.TRANDATE BETWEEN '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "' AND ISNULL(I.CANCEL,'') = ''"
                strsql += vbCrLf + " AND I.SNO IN( SELECT ISSSNO FROM " & cnStockDb & "..RECEIPTMETAL WHERE TRANDATE BETWEEN  '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "')"
                strsql += vbCrLf + " AND ISNULL(ISM.TRANTYPE,'') NOT IN  ('IRC') "
                strsql += vbCrLf + " AND ISM.TRANTYPE <> 'WOT' "
                If rbtMfg.Checked Then
                    strsql += vbCrLf + " AND ISNULL(I.STKTYPE,'')='M'"
                ElseIf rbtTrading.Checked Then
                    strsql += vbCrLf + " AND ISNULL(I.STKTYPE,'')NOT IN('M','E')"
                ...
                End If
                If cmbCatName.Text <> "ALL" Then
                    strsql += vbCrLf + " And ISM.CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY  WHERE CATNAME IN ('" & cmbCatName.Text & "'))"
                End If
                If cmbMetal.Text <> "ALL" Then
                    strsql += vbCrLf + " And ISM.CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY  WHERE METALID = (SELECT METALID FROM " & cnAdminDb & "..METALMAST  WHERE METALNAME = '" & cmbMetal.Text & "'))"
                End If
                If chkCmbCompany.Text <> "ALL" Then
                    strsql += vbCrLf + " AND ISM.COMPANYID IN (" & SelectedCompany & ") "
                End If
                If chkCmbCostCentre.Text <> "ALL" Then
                    strsql += vbCrLf + " And I.COSTID IN(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & SelectedCostCentre & "))"
                End If
                If chkWithApproval.Checked = False Then strsql += vbCrLf + " AND ISNULL(ISM.TRANTYPE,'') NOT IN  ('AR') "
                strsql += vbCrLf + " GROUP BY ISM.CATCODE,I.TRANDATE,I.TRANNO,TRANSTATUS,M.STUDDED,ISM.TRANTYPE"
                ...
                strsql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..ITEMMAST M ON M.ITEMID = I.ITEMID"
                strsql += vbCrLf + " WHERE TRANDATE BETWEEN '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "' AND ISNULL(CANCEL,'') = ''"
                strsql += vbCrLf + " AND I.SNO NOT IN( SELECT ISSSNO FROM " & cnStockDb & "..ISSMETAL WHERE TRANDATE BETWEEN  '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "' AND I.TRANTYPE IN ('SA','OD','RD'))"
                strsql += vbCrLf + " AND ISNULL(I.TRANTYPE,'') NOT IN  ('IRC') "
                ...
                End If
                If cmbMetal.Text <> "ALL" Then
                    strsql += vbCrLf + " And I.CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY  WHERE METALID = (SELECT METALID FROM " & cnAdminDb & "..METALMAST  WHERE METALNAME = '" & cmbMetal.Text & "'))"
                End If
                If chkCmbCompany.Text <> "ALL" Then
                ...
                End If
                strsql += vbCrLf + " ,CONVERT(VARCHAR(15),I.TRANDATE,105) TRANDATE,I.TRANNO"
                strsql += vbCrLf + " ,CASE WHEN ISM.TRANTYPE = 'MI' THEN 'MISC ISSUE'"
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'IIN' THEN 'INTERNAL TRANSFER' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'IIS' THEN 'MATERIAL ISSUE' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'IPU' THEN 'PURCHASE RETRUN' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'IAP' THEN 'APPROVAL ISSUE' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'IOT' THEN 'OTHER ISSUE' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'IPA' THEN 'PACKING' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'OD' THEN 'ORDER DELIVERY' "
                strsql += vbCrLf + " ELSE CASE WHEN ISM.TRANTYPE = 'SA' THEN 'SALES' "
                strsql += vbCrLf + " ELSE ISM.TRANTYPE "
                strsql += vbCrLf + " END END END END END END END END END"
                strsql += vbCrLf + " ,'D' COLHEAD, 4 RESULT "
                strsql += vbCrLf + " FROM " & cnStockDb & "..ISSMETAL AS ISM"
                strsql += vbCrLf + " LEFT JOIN " & cnStockDb & "..ISSUE I ON I.SNO=ISM.ISSSNO "
                strsql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..CATEGORY C ON ISM.CATCODE = C.CATCODE"
                strsql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..ITEMMAST M ON M.ITEMID = I.ITEMID"
                strsql += vbCrLf + " WHERE I.TRANDATE BETWEEN '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "' AND ISNULL(I.CANCEL,'') = ''"
                strsql += vbCrLf + " AND I.SNO IN( SELECT ISSSNO FROM " & cnStockDb & "..ISSMETAL WHERE TRANDATE BETWEEN  '" & Format(dtpFrom.Value.Date, "yyyy-MM-dd") & "' AND '" & Format(dtpTo.Value.Date, "yyyy-MM-dd") & "' AND I.TRANTYPE IN ('SA','OD','RD'))"
                strsql += vbCrLf + " AND ISNULL(ISM.TRANTYPE,'') NOT IN  ('IRC') "
                strsql += vbCrLf + " AND ISM.TRANTYPE <> 'WOT' "
                If rbtMfg.Checked Then
                    strsql += vbCrLf + " AND ISNULL(I.STKTYPE,'')='M'"
                ...
                If chkCmbCostCentre.Text <> "ALL" Then
                    strsql += vbCrLf + " And I.COSTID IN(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & SelectedCostCentre & "))"
                End If
                If chkWithApproval.Checked = False Then strsql += vbCrLf + " AND ISNULL(ISM.TRANTYPE,'') NOT IN  ('AI') "
                If chkWithOtherIssue.Checked = False Then
                    strsql += " AND ISM.TRANTYPE <> 'MI'"
                End If
                strsql += vbCrLf + " GROUP BY ISM.CATCODE,I.TRANDATE,I.TRANNO,TRANSTATUS,M.STUDDED,ISM.TRANTYPE"
    ...
    Private Sub frmCategoryStock_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dtCompany)
        BrighttechPack.GlobalMethods.FillCombo(chkCmbCompany, dtCompany, "COMPANYNAME", , IIf(strCompanyName.ToString <> "" And SPECIFICFORMAT = 1, strCompanyName.ToString, "ALL"))
        ...
        btnNew_Click(Me, e)
        If RPT_CATSTOCK_GRSNET Then
            rbtGrsNetwt.Checked = False
            rbtGrsNetwt.Enabled = True
            rbtGrsNetwt.Visible = True
            rbtGrsWt.Checked = True
        Else
            rbtGrsNetwt.Checked = False
            rbtGrsNetwt.Enabled = False
            rbtGrsNetwt.Visible = False
            rbtGrsWt.Checked = True
        End If
    End Sub
    ...
    Private Sub funcRateValUpdation()
        ...
        If chkWithOtherIssue.Checked = True And CATSTOCK_OTHERISS_RATE = False Then
            Return
        End If
    ...
    Private Sub rbtGrsNetwt_CheckedChanged(sender As Object, e As EventArgs) Handles rbtGrsNetwt.CheckedChanged
        chkWithValue.Enabled = Not rbtGrsNetwt.Checked
        chkWithValue.Checked = False
        btnSave.Enabled = Not rbtGrsNetwt.Checked
    End Sub
End Class

Public Class frmCategoryStock_Properties
