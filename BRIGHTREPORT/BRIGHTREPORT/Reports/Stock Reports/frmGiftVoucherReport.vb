...
Public Class frmGiftVoucherReport
    ...
    Function funcTranNoWise() As Integer
        ...
        strSql += vbcrlf + "         FOR SELECT TRANNO,OPENING,ISSUE,RECEIPT,CLOSING FROM TEMP" & systemId & "CATSTOCKTRANNO WHERE CATNAME = @CATNAME"
    ...
    Private Sub btnView_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        ...
        strSql = " IF (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE XTYPE = 'U' AND NAME = 'TEMP" & systemId & "GIFTVOUCHER')>0"
        strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        If rbtPend.Checked Then
            strSql = " IF (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE XTYPE = 'U' AND NAME = 'TEMP_OUTSTANDING')>0"
            strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMP_OUTSTANDING"
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()
            strSql = vbCrLf + "SELECT RUNNO,TRANNO,CONVERT(VARCHAR,TRANDATE,105)TRANDATE1,CONVERT(VARCHAR,DUEDATE,105)DUEDATE"
            strSql += vbCrLf + ",TRANDATE,ISSUE,RECEIPT,BALANCE ,NAME,MOBILE,ADDRESS1,ADDRESS2,AREA,CITY ,COSTNAME"
            strSql += vbCrLf + ",COSTID,COMPANYID,RESULT,COLHEAD"
            strSql += vbCrLf + ",CONVERT(VARCHAR,(SELECT TOP 1 TRANDATE FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = X.RUNNO AND COSTID = X.COSTID AND COMPANYID = X.COMPANYID "
            strSql += vbCrLf + "ORDER BY TRANDATE DESC),103)AS LASTTRANDATE "
            strSql += vbCrLf + ",REMARK1,REMARK2"
            strSql += vbCrLf + ",IDENTITY(INT,1,1)KNO"
            strSql += vbCrLf + "INTO TEMPTABLEDB..TEMP_OUTSTANDING "
            strSql += vbCrLf + "FROM"
            strSql += vbCrLf + "("
            strSql += vbCrLf + "SELECT CONVERT(VARCHAR(200),NULL)PARTICULAR,O.RUNNO,CONVERT(VARCHAR(50),O.TRANTYPE)TRANTYPE"
            strSql += vbCrLf + ",(SELECT TOP 1 TRANNO FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = O.RUNNO AND COSTID = O.COSTID AND COMPANYID = O.COMPANYID ORDER BY TRANDATE,TRANNO) TRANNO"
            strSql += vbCrLf + ",(SELECT TOP 1 TRANDATE FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = O.RUNNO AND COSTID = O.COSTID AND COMPANYID = O.COMPANYID ORDER BY TRANDATE,TRANNO) TRANDATE"
            strSql += vbCrLf + ",(SELECT TOP 1 DUEDATE FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = O.RUNNO AND COSTID = O.COSTID AND COMPANYID = O.COMPANYID ORDER BY TRANDATE,TRANNO) DUEDATE"
            strSql += vbCrLf + ",SUM(CASE WHEN RECPAY = 'P' THEN AMOUNT ELSE 0 END) AS RECEIPT"
            strSql += vbCrLf + ",SUM(CASE WHEN RECPAY = 'R' THEN AMOUNT ELSE 0 END) AS ISSUE"
            strSql += vbCrLf + ",SUM(CASE WHEN RECPAY = 'R' THEN AMOUNT ELSE -1*AMOUNT END)AS BALANCE"
            strSql += vbCrLf + ",P.PNAME NAME"
            strSql += vbCrLf + ",CASE WHEN ISNULL(P.MOBILE,'')='' THEN P.PHONERES ELSE P.MOBILE END AS MOBILE"
            strSql += vbCrLf + ",P.AREA,P.ADDRESS1,P.ADDRESS2,P.CITY"
            strSql += vbCrLf + ",(SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTID = O.COSTID)AS COSTNAME"
            strSql += vbCrLf + ",CONVERT(INT,2)RESULT,CONVERT(VARCHAR,NULL)COLHEAD,CONVERT(INT,1)AREAORD"
            strSql += vbCrLf + ",O.COSTID,O.COMPANYID"
            strSql += vbCrLf + ",(SELECT TOP 1 REMARK1 FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = O.RUNNO AND COSTID = O.COSTID AND COMPANYID = O.COMPANYID ORDER BY TRANDATE,TRANNO) REMARK1"
            strSql += vbCrLf + ",(SELECT TOP 1 REMARK2 FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = O.RUNNO AND COSTID = O.COSTID AND COMPANYID = O.COMPANYID ORDER BY TRANDATE,TRANNO) REMARK2"
            strSql += vbCrLf + "FROM " & cnAdminDb & "..OUTSTANDING AS O"
            strSql += vbCrLf + "INNER JOIN " & cnAdminDb & "..CUSTOMERINFO AS CU ON CU.BATCHNO = "
            strSql += vbCrLf + "(SELECT TOP 1 BATCHNO FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO = O.RUNNO AND COSTID = O.COSTID AND COMPANYID = O.COMPANYID ORDER BY TRANDATE DESC)"
            strSql += vbCrLf + "INNER JOIN " & cnAdminDb & "..PERSONALINFO AS P ON P.SNO = CU.PSNO"
            strSql += vbCrLf + "WHERE O.TRANDATE BETWEEN '" & dtpFrom.Value.Date.ToString("yyyy-MM-dd") & "'  AND '" & dtpTo.Value.Date.ToString("yyyy-MM-dd") & "' "
            ...
            End If
            strSql += vbCrLf + "AND TRANTYPE='GV'"
            strSql += vbCrLf + "GROUP BY O.RUNNO,O.TRANTYPE,P.PNAME,P.MOBILE,P.PHONERES"
            strSql += vbCrLf + ",O.COSTID,O.COMPANYID,P.AREA,P.ADDRESS1,P.ADDRESS2,P.CITY"
            strSql += vbCrLf + ")X"
            strSql += vbCrLf + "WHERE BALANCE<>0"
            strSql += vbCrLf + "ORDER BY TRANDATE,TRANNO"
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()

            strSql = " SELECT * FROM TEMPTABLEDB..TEMP_OUTSTANDING ORDER BY KNO"
        Else
            strSql = " DECLARE @FRMDATE SMALLDATETIME "
            strSql += vbCrLf + " DECLARE @TODATE SMALLDATETIME "

            strSql += vbCrLf + " SELECT @frmDATE = '" & dtpFrom.Value.Date.ToString("yyyy-MM-dd") & "'"
            strSql += vbCrLf + " SELECT @toDATE = '" & dtpTo.Value.Date.ToString("yyyy-MM-dd") & "'"

            strSql += vbCrLf + " SELECT * INTO TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER FROM ( "
            strSql += vbCrLf + " SELECT G.SNO, G.TRANDATE, G.RUNNO, G.QTY, G.AMOUNT , G.BATCHNO "
            strSql += vbCrLf + " ,(SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTID = G.COSTID)COSTNAME"
            strSql += vbCrLf + " ,1 RESULT, 'Z' COLHEAD"
            strSql += vbCrLf + " FROM " & cnStockDb & "..GVTRAN G LEFT JOIN " & cnAdminDb & "..OUTSTANDING O "
            strSql += vbCrLf + " ON O.RUNNO = G.RUNNO AND O.SNO = G.SNO"
            strSql += vbCrLf + " WHERE G.TRANDATE BETWEEN '" & dtpFrom.Value.Date.ToString("yyyy-MM-dd") & "'  AND '" & dtpTo.Value.Date.ToString("yyyy-MM-dd") & "' "
            'strSql += vbCrLf + CompanyFilt

            If chkCmbCompany.Text <> "ALL" And chkCmbCompany.Text <> "" Then
                strSql += vbCrLf + " AND G.COMPANYID IN (SELECT COMPANYID FROM " & cnAdminDb & "..COMPANY WHERE COMPANYNAME IN (" & GetQryString(chkCmbCompany.Text) & "))"
            Else
                strSql += vbCrLf + " AND G.COMPANYID IN (SELECT COMPANYID FROM " & cnAdminDb & "..COMPANY WHERE ISNULL(ACTIVE,'')<>'N')"
            End If
            If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
                strSql += vbCrLf + " AND G.COSTID IN"
                strSql += vbCrLf + " (SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
            End If
            If rbtIssue.Checked Then
                strSql += vbCrLf + " AND EXISTS (SELECT 1 FROM " & cnAdminDb & "..OUTSTANDING WHERE RUNNO=G.RUNNO)"
            End If
            strSql += vbCrLf + " )X"
            'cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 100
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()

            strSql = " ALTER TABLE TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER ALTER COLUMN SNO VARCHAR(50)"
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()

            Dim dtCount As New DataTable
            strSql = " SELECT * FROM TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER "
            da = New OleDbDataAdapter(strSql, cn)
            da.Fill(dtCount)

            If dtCount.Rows.Count > 0 Then

                strSql = " INSERT INTO TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER (SNO, COSTNAME, RESULT, COLHEAD)"
                strSql += vbCrLf + " SELECT DISTINCT COSTNAME, COSTNAME, 0, 'T'  FROM TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER WHERE ISNULL(COSTNAME,'') <> ''"
                'cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 100
                cmd = New OleDbCommand(strSql, cn)
                cmd.ExecuteNonQuery()


                strSql = " INSERT INTO TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER (SNO, QTY, AMOUNT, COSTNAME, RESULT, COLHEAD)"
                strSql += vbCrLf + " SELECT 'SUB TOTAL', SUM(QTY) QTY, SUM (AMOUNT) AMOUNT, COSTNAME, 2, 'S' FROM TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER WHERE ISNULL(COSTNAME,'') <> '' GROUP BY COSTNAME"
                'cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                cmd = New OleDbCommand(strSql, cn)
                cmd.ExecuteNonQuery()

                strSql = " INSERT INTO TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER (SNO, QTY, AMOUNT, COSTNAME, RESULT, COLHEAD)"
                strSql += vbCrLf + " SELECT 'GRAND TOTAL', SUM(QTY) QTY, SUM (AMOUNT) AMOUNT, 'ZZZ', 3, 'G' FROM TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER WHERE ISNULL(COSTNAME,'') <> '' "
                strSql += vbCrLf + " AND COLHEAD <> 'S'"
                'cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                cmd = New OleDbCommand(strSql, cn)
                cmd.ExecuteNonQuery()
            End If
            strSql = " SELECT * FROM TEMPTABLEDB..TEMP" & systemId & "GIFTVOUCHER  ORDER BY COSTNAME, RESULT, COLHEAD"
        End If
        Prop_Sets()
        ...
        With gridView
            ...
            If rbtPend.Checked = False Then .Columns("COSTNAME").Visible = False
            If .Columns.Contains("KNO") Then .Columns("KNO").Visible = False
            If .Columns.Contains("REMARK1") Then .Columns("REMARK1").Visible = False
            If .Columns.Contains("REMARK2") Then .Columns("REMARK2").Visible = False
            ...
            If .Columns.Contains("LASTTRANDATE") Then .Columns("LASTTRANDATE").Visible = False
            If .Columns.Contains("TRANDATE1") Then .Columns("TRANDATE1").HeaderText = "TRANDATE"
            If .Columns.Contains("ISSUE") Then .Columns("ISSUE").HeaderText = "AMOUNT"
            ...
            If .Columns.Contains("QTY") Then
                ...
                End With
            End If
            If .Columns.Contains("ISSUE") Then
                With .Columns("ISSUE")
                    .Width = 80
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .ReadOnly = True
                ...
                End With
            End If
            If .Columns.Contains("RECEIPT") Then
                ...
                End With
            End If
            If .Columns.Contains("BALANCE") Then
            ...
            If .Columns.Contains("AMOUNT") Then
                ...
                End With
            End If
            If .Columns.Contains("BATCHNO") Then
        ...
        For Each dgvRow As DataGridViewRow In gridView.Rows
            With dgvRow
                Select Case .Cells("COLHEAD").Value.ToString
                    Case " "
                        .Cells("SNO").Style.BackColor = Nothing
                    Case "S"
                        .DefaultCellStyle.ForeColor = reportSubTotalStyle1.ForeColor
                        .DefaultCellStyle.Font = reportSubTotalStyle.Font
                    Case "T"
                        .Cells("SNO").Style.BackColor = reportHeadStyle.BackColor
                        .Cells("SNO").Style.Font = reportHeadStyle.Font
                    Case "G"
    ...
    Private Sub gridView_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles gridView.KeyPress
        If gridView.RowCount > 0 Then
            ...
            End If
            If UCase(e.KeyChar) = "D" And rbtAll.Checked Then
                'gift voucher qrcode
                Dim batchno As String = ""

                Dim GV_QRCODE As Boolean = IIf(GetAdmindbSoftValue("GV_QRCODE", "N") = "Y", True, False)
                batchno = gridView.CurrentRow.Cells("BATCHNO").Value.ToString

                If batchno Is Nothing Or batchno = "" Then Exit Sub
                ...
                Dim strDate As Date
                strDate = gridView.CurrentRow.Cells("TRANDATE").Value
                ...
                If IO.File.Exists(Application.StartupPath & "\BillPrint.exe") Then
                    ...
                    write.WriteLine(LSet("TRANDATE", 15) & ":" & Format(strDate, "yyyy-MM-dd").ToString)
                    ...
                    Else
                        ...
                        LSet("TRANDATE", 15) & ":" & Format(strDate, "yyyy-MM-dd").ToString & ";" &
                        LSet("DUPLICATE", 15) & ":Y")
                    End If
                Else
...
Public Class frmGiftVoucherReport_Properties
