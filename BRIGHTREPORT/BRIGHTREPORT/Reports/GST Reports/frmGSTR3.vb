...
Imports System.IO
Public Class frmGSTR3
    Dim cmd As OleDbCommand
    Dim strSql As String
    Dim dsGridView As New DataSet
    Dim dtCostCentre As New DataTable
    Dim GSTR3B_NEW_FORMAT As String = GetAdmindbSoftValue("GSTR3B_NEW", "N")
    ...
    Private Sub SalesAbs()
        Try
            ...
            strSql = "SELECT * FROM TEMPTABLEDB..TEMPGSTRPT3_1"
            'Else
            'MsgBox("Please Select any one Option...", MsgBoxStyle.Information)
            'Exit Sub
            ' End If
            Dim Sql As String = ""
            If GSTR3B_NEW_FORMAT.ToString = "Y" Then
                Sql = " EXEC " & cnAdminDb & "..SP_GSTR3B_NEW"
                Sql += vbCrLf + " @DBNAME = '" & cnStockDb & "'"
                Sql += vbCrLf + " ,@FRMDATE = '" & dtpFrom.Value.ToString("yyyy-MM-dd") & "'"
                Sql += vbCrLf + " ,@TODATE = '" & dtpTo.Value.ToString("yyyy-MM-dd") & "'"
                Sql += vbCrLf + " ,@COMPANYID = '" & strCompanyId & "'"
                Sql += vbCrLf + " ,@METALNAME = '" & cmbMetal.Text & "'"
                Sql += vbCrLf + " ,@COSTNAME = '" & GetQryString(chkCmbCostCentre.Text).Replace("'", "") & "'"
                Sql += vbCrLf + " ,@CHITDBID = '" & ChitDb & "'"
                cmd = New OleDbCommand(Sql, cn)
                cmd.CommandTimeout = 1000
                cmd.ExecuteNonQuery()
            Else
                Sql = " EXEC " & cnAdminDb & "..PROC_GSTR3"
                Sql += vbCrLf + " @DBNAME = '" & cnStockDb & "'"
                Sql += vbCrLf + " ,@FROMDATE = '" & dtpFrom.Value.ToString("yyyy-MM-dd") & "'"
                Sql += vbCrLf + " ,@TODATE = '" & dtpTo.Value.ToString("yyyy-MM-dd") & "'"
                Sql += vbCrLf + " ,@COMPANYID = '" & strCompanyId & "'"
                Sql += vbCrLf + " ,@METALNAME = '" & cmbMetal.Text & "'"
                Sql += vbCrLf + " ,@COSTNAME = '" & GetQryString(chkCmbCostCentre.Text).Replace("'", "") & "'"
                ...
                cmd.CommandTimeout = 1000
                cmd.ExecuteNonQuery()
                strSql = "SELECT * FROM TEMPTABLEDB..TEMPGSTRPT3_1 ORDER BY RESULT"
            End If
            ...
            With gridView
                ...
                If .Columns.Contains("NAME") Then .Columns("NAME").HeaderText = "Nature of Supplies"
                If .Columns.Contains("TAXABLE") Then .Columns("TAXABLE").HeaderText = "Total Taxable Value"
                If .Columns.Contains("IGST") Then .Columns("IGST").HeaderText = "Integrated Tax"
                If .Columns.Contains("CGST") Then .Columns("CGST").HeaderText = "Central Tax"
                If .Columns.Contains("SGST") Then .Columns("SGST").HeaderText = "State/UT Tax"
                If .Columns.Contains("CESS") Then .Columns("CESS").HeaderText = "Cess"
                If .Columns.Contains("CGST") Then .Columns("CGST").Width = 100
                If .Columns.Contains("SGST") Then .Columns("SGST").Width = 100
                If .Columns.Contains("IGST") Then .Columns("IGST").Width = 100
            End With
    ...
    '                .Columns.Add("TRANNO~TRANDATE~VALUE~NATURE~HSN~TAXABLE", GetType(String))
    '                .Columns.Add("IGSTRATE~IGSTTAX", GetType(String))
    '                .Columns.Add("CGSTRATE~CGSTTAX", GetType(String))
    '                .Columns.Add("SGSTRATE~SGSTTAX", GetType(String))
    ...
    '                .Columns.Add("ITCIGST~ITCCGST~ITCSGST", GetType(String))
    '                .Columns.Add("SCROLL", GetType(String))
    '                .Columns("NAMEOFSUPPLIER~GSTNO").Caption = "GSTIN/Name of unregistered supplier"
    '                .Columns("TRANNO~TRANDATE~VALUE~NATURE~HSN~TAXABLE").Caption = "Invoice"
    ...
    '                .Columns("NAMEOFSUPPLIER~GSTNO").HeaderText = "GSTIN/Name of unregistered supplier"
    '                .Columns("TRANNO~TRANDATE~VALUE~NATURE~HSN~TAXABLE").HeaderText = "Invoice"
    '                .Columns("IGSTRATE~IGSTTAX").HeaderText = "IGST"
    '                .Columns("CGSTRATE~CGSTTAX").HeaderText = "CGST"
    '                .Columns("SGSTRATE~SGSTTAX").HeaderText = "SGST"
    ...
    '            .Columns("TRANNO~TRANDATE~VALUE~NATURE~HSN~TAXABLE").Width = gridView.Columns("TRANNO").Width _
...
Public Class frmGSTR3_Properties
