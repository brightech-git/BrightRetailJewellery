...
Public Class PurchaseVatReportNew
    ...
    Private Sub PurchaseVatReportNew_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            SendKeys.Send("{TAB}")
        End If
    End Sub

    Private Sub PurchaseVatReportNew_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'dtTranType.Columns.Add("TRANTYPE", GetType(String))
        'Dim Ro As DataRow = dtTranType.NewRow
        'Ro.Item("TRANTYPE") = "ALL"
        'dtTranType.Rows.Add(Ro)
        'Ro = dtTranType.NewRow
        'Ro.Item("TRANTYPE") = "PURCHASE"
        'dtTranType.Rows.Add(Ro)
        'Ro = dtTranType.NewRow
        'Ro.Item("TRANTYPE") = "RECEIPT"
        'dtTranType.Rows.Add(Ro)
        'Ro = dtTranType.NewRow
        'Ro.Item("TRANTYPE") = "INTERNAL TRANSFER"
        'dtTranType.Rows.Add(Ro)
        'Ro = dtTranType.NewRow
        'Ro.Item("TRANTYPE") = "APPROVAL RECEIPT"
        'dtTranType.Rows.Add(Ro)
        'Ro = dtTranType.NewRow
        'Ro.Item("TRANTYPE") = "OTHER RECEIPT"
        'dtTranType.Rows.Add(Ro)
        'BrighttechPack.GlobalMethods.FillCombo(chkcmbTranType, dtTranType, "TRANTYPE", , "ALL")
    ...
    Private Function GetTable(ByVal rptType As String, ByVal fromDate As Date, ByVal ToDate As Date, Optional ByVal CatNames As String = "ALL", Optional ByVal MonthName As String = "ALL") As DataTable
        ...
        If chkED.Checked = False Then
            strSql = " EXEC " & cnStockDb & "..SP_RPT_PURCHASEVAT_NEW"
            ...
            strSql += " ,@BILLPREFIX='" & IIf(chkBillPrefix.Checked, "Y", "N") & "'"
        Else
            strSql = " EXEC " & cnStockDb & "..SP_RPT_PURCHASEVAT_ED"
            strSql += " @FROMDATE = '" & fromDate.ToString("yyyy-MM-dd") & "'"
            strSql += " ,@TODATE = '" & ToDate.ToString("yyyy-MM-dd") & "'"
            strSql += " ,@RPTTYPE = '" & rptType & "'"
            strSql += " ,@COSTNAME = '" & IIf(chkCmbCostCentre.Text <> "", chkCmbCostCentre.Text, "ALL") & "'"
    ...
    Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        AddHandler objGridShower.TxtBox.TextChanged, AddressOf GridShower_TxtControl_TextChanged
        ...
        Prop_Sets()
    End Sub
    Private Sub GridShower_TxtControl_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        'Dim f As frmGridDispDia
        'f = objGPack.GetParentControl(CType(sender, TextBox))
        ' ''0 CATVIEW
        ' ''1 MONTH VIEW
        ' ''2 DATE VIEW
        ' ''3 BILLNO VIEW
        'Select Case f.TxtBox.Text
        '    Case "C", "M", "D"
        '        f.lblStatus.Text = " <Press [D]Detail View"
        'End Select
    ...
    Private Sub DataGridView_Formatting_Tranno(ByVal Dgv As DataGridView)
        With Dgv
            ...
            If .Columns.Contains("ADDRESS") Then .Columns("ADDRESS").Visible = True
            ...
            If .Columns.Contains("TINNO") Then .Columns("TINNO").Visible = True
            If .Columns.Contains("CENTRALTAXNO") Then .Columns("CENTRALTAXNO").Visible = True
            ...
            .Columns("REFDATE").DefaultCellStyle.Format = "dd/MM/yyyy"
            If chkBillPrefix.Checked Then
                .Columns("TRANNO").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            End If
    ...
    Private Sub DataGridView_Formatting_Category(ByVal Dgv As DataGridView, ByVal groupby As String)
        With Dgv
            ...
            Next
            .Columns("PARTICULAR").Visible = True
            If .Columns.Contains("ADDRESS") Then .Columns("ADDRESS").Visible = False
            If .Columns.Contains("ADDRESS") And GetReportType() = "C" Then .Columns("ADDRESS").Visible = True
            .Columns("PCS").Visible = True
            .Columns("GRSWT").Visible = True
            .Columns("NETWT").Visible = True
            .Columns("AMOUNT").Visible = True
        ...
        If e.KeyCode = Keys.Escape Then
            ...
            Select Case f.dsGrid.Tables(f.dsGrid.Tables.Count - 1).TableName
                ...
                Case "T"
                    ...
                    NextType = "D"
                Case "A"
                    If f.dsGrid.Tables.Contains("T") = False Then Exit Sub
                    NextType = "T"
            End Select
            f.FormReSize = False
    ...
    Private Sub Prop_Gets()
        ...
        chkBillPrefix.Checked = obj.p_chkPrefix
    End Sub
    ...
    Private Sub Prop_Sets()
        ...
        obj.p_chkPrefix = chkBillPrefix.Checked
        SetSettingsObj(obj, Me.Name, GetType(PurchaseVatReportNew_Properties))
    End Sub
End Class
Public Class PurchaseVatReportNew_Properties
    ...
    Private chkPrefix As Boolean = False
    Public Property p_chkPrefix() As Boolean
        Get
            Return chkPrefix
        End Get
    ...
    Public Property p_rbtTranDate() As Boolean
        ...
        End Set
    End Property
    Private rbtAcname As Boolean = False
