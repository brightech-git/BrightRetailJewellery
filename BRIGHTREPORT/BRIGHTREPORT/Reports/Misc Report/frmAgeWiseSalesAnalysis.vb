...
Public Class frmAgeWiseSalesAnalysis
    ...
    Public Sub New()
        ...
        strSql += " SELECT GROUPNAME,2 RESULT FROM " & cnAdminDb & "..ITEMGROUPMAST  WHERE GROUPTYPE='I' ORDER BY RESULT,GROUPNAME"
        dtItem = New DataTable
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dtItem)
    ...
    Function funcWithStone() As Integer
        ...
        strSql += vbCrLf + " 	drop table temp" & systemId & "AgeStone"
        strSql += vbCrLf + " select  '1'result,tt.itemId,T.tagno,"
        strSql += vbCrLf + " (select '['+CAST (ITEMID AS VARCHAR)+'] '+ITEMNAME  from " & cnAdminDb & "..itemMast where itemid = tt.Itemid)as itemName, "
        strSql += vbCrLf + " isnull((select subItemName from " & cnAdminDb & "..subItemMast where subItemId = tt.SubItemid),0)as subItemName, "
        If chkCounterWisegrp.Checked Then
            strSql += vbCrLf + " (select DISPLAYORDER from " & cnAdminDb & "..ITEMCOUNTER where ITEMCTRID = (select top 1 ITEMCTRID from " & cnAdminDb & "..ITEMTAG where TAGNO=t.TAGNO)) AS DISPORDER,"
        ElseIf ChkItemDesignGrp.Checked Or chkitemgrp.Checked Then
            strSql += vbCrLf + vbCrLf + " ISNULL((SELECT G.DISPORDER FROM " & cnAdminDb & "..ITEMGROUPMAST G WHERE GROUPID = (SELECT TOP 1 ITEMGROUP FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMID=T.ITEMID)),0)AS DISPORDER, "
        Else
            strSql += vbCrLf + " 0 AS DISPORDER,"
        End If
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + vbCrLf + " isnull((select ITEMCTRNAME from " & cnAdminDb & "..ITEMCOUNTER where ITEMCTRID = (select top 1 ITEMCTRID from " & cnAdminDb & "..ITEMTAG where TAGNO=t.TAGNO)),'')as ITEMCTRNAME, "
        strSql += vbCrLf + vbCrLf + " ISNULL((SELECT GROUPNAME FROM " & cnAdminDb & "..ITEMGROUPMAST WHERE GROUPID = (SELECT TOP 1 ITEMGROUP FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMID=T.ITEMID)),'')AS ITEMGROUPNAME, "
        If Chktablegrpwise.Checked Then strSql += vbCrLf + vbCrLf + "ISNULL(TT.TABLECODE,'') TABLECODE, "
        strSql += vbCrLf + vbCrLf + " ISNULL((SELECT DESIGNERNAME FROM " & cnAdminDb & "..DESIGNER WHERE DESIGNERID = (SELECT TOP 1 DESIGNERID FROM " & cnAdminDb & "..DESIGNER WHERE DESIGNERID=TT.DESIGNERID)),'')AS DESIGNER, "
        If chkCatGroup.Checked Then
            strSql += vbCrLf + vbCrLf + " (SELECT CGROUPNAME FROM " & cnAdminDb & "..CATEGORYGROUP WHERE CGROUPID IN "
            strSql += vbCrLf + vbCrLf + " (SELECT CGROUPID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE IN "
            strSql += vbCrLf + vbCrLf + " (SELECT CATCODE FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMID=TT.ITEMID)))CATEGORYGROUP, "
        End If
        '596
        strSql += vbCrLf + " datediff(d," & IIf(chkActualDate.Checked, "TT.ACTUALRECDATE", "TT.RecDate") & ",T.ISSDATE)+1 as Days "
        '596
        strSql += vbCrLf + " ,CASE WHEN IM.DIASTONE ='D' THEN T.STNPCS END AS  DIAPCS"
        strSql += vbCrLf + " ,CASE WHEN IM.DIASTONE ='D' THEN T.STNWT END  AS  DIAWT"
        strSql += vbCrLf + " ,CASE WHEN IM.DIASTONE ='P' THEN T.STNPCS END AS  PREPCS"
        strSql += vbCrLf + " ,CASE WHEN IM.DIASTONE ='P' THEN T.STNWT END  AS  PREWT"
        strSql += vbCrLf + " ,CASE WHEN IM.DIASTONE ='S' THEN T.STNPCS END AS  STNPCS"
        strSql += vbCrLf + " ,CASE WHEN IM.DIASTONE ='S' THEN T.STNWT END  AS  STNWT"
        strSql += vbCrLf + " ,T.STONEUNIT,'1'STONE"
        strSql += vbCrLf + " INTO TEMP" & systemId & "AGESTONE "
        strSql += vbCrLf + " FROM " & cnAdminDb & "..ITEMTAGSTONE AS T "
        strSql += vbCrLf + " INNER JOIN TEMP" & systemId & "AGE TT ON T.TAGSNO=TT.SNO"
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..ITEMMAST IM ON IM.ITEMID=T.STNITEMID"
        strSql += vbCrLf + " WHERE T.TAGNO IN (SELECT TAGNO FROM TEMP" & systemId & "AGE)"
        strSql += vbCrLf + " AND T.ITEMID IN(SELECT ITEMID FROM TEMP" & systemId & "AGE)"
        ...
        If chkason.Checked Then
            strSql += vbCrLf + " AND T.ISSDATE <= '" & dtpAsOnDate.Value.Date.ToString("yyyy-MM-dd") & "'"
        Else
            strSql += vbCrLf + " AND T.ISSDATE BETWEEN '" & dtpAsOnDate.Value.Date.ToString("yyyy-MM-dd") & "'AND'" & dtpTo.Value.Date.ToString("yyyy-MM-dd") & "'"
        End If
    ...
    Function funcSearch() As Integer
        ...
        If ChkItemDesignGrp.Checked Then strSql += vbCrLf + " ,ITEMGROUPNAME VARCHAR(50),DESIGNER VARCHAR(50)"
        If ChkItemDesign.Checked Then strSql += vbCrLf + " ,DESIGNER VARCHAR(50)"
        If chkSubItem.Checked Then strSql += vbCrLf + " ,SUBITEMNAME VARCHAR(50)"
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + " ,ITEMCTRNAME VARCHAR(50)"
        ...
        If chkCatGroup.Checked Then strSql += vbCrLf + " ,CATEGORYGROUP VARCHAR(50)"
        strSql += vbCrLf + " ,DISPORDER INT"
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                ...
                strSql += vbCrLf + ",GRSWT" & i & " NUMERIC(20,3)"
            Next
        End If
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                strSql += vbCrLf + ",DIAPCS" & i & " INT"
                strSql += vbCrLf + ",DIAWT" & i & " NUMERIC(20,3)"
                strSql += vbCrLf + ",PREPCS" & i & " INT"
                strSql += vbCrLf + ",PREWT" & i & " NUMERIC(20,3)"
                strSql += vbCrLf + ",STNPCS" & i & " INT"
                strSql += vbCrLf + ",STNWT" & i & " NUMERIC(20,3)"
            Next
        End If
        ...
        strSql += vbCrLf + " SELECT SNO,RESULT,ITEMID,TAGNO,'['+CAST (ITEMID AS VARCHAR)+'] '+ITEMNAME AS ITEMNAME,SUBITEMID,DESIGNERID,RECDATE,ACTUALRECDATE,COSTID"
        If chkitemgrp.Checked Then
            strSql += vbCrLf + " ,(SELECT TOP 1 DISPORDER FROM " & cnAdminDb & "..ITEMGROUPMAST WHERE GROUPNAME=X.ITEMGROUPNAME)DISPORDER"
        ElseIf chkCounterWisegrp.Checked Then
            strSql += vbCrLf + " ,(SELECT TOP 1 DISPLAYORDER FROM " & cnAdminDb & "..ITEMCOUNTER WHERE ITEMCTRNAME=X.ITEMCTRNAME)DISPORDER"
        ElseIf ChkItemDesignGrp.Checked Then
            strSql += vbCrLf + " ,(SELECT TOP 1 DISPORDER FROM " & cnAdminDb & "..ITEMGROUPMAST WHERE GROUPNAME=X.ITEMGROUPNAME)DISPORDER"
            'ElseIf ChkItemDesign.Checked Then
            '    strSql += vbCrLf + " ,(SELECT TOP 1 DISPORDER FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMNAME=X.ITEMNAME)DISPORDER"
        Else
            strSql += vbCrLf + " ,0 AS DISPORDER"
        End If
        If chkSubItem.Checked Then strSql += vbCrLf + " ,SUBITEMNAME"
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + " ,ITEMCTRNAME"
        strSql += vbCrLf + " ,ITEMGROUPNAME"
        If Chktablegrpwise.Checked Then strSql += vbCrLf + " ,TABLECODE"
        strSql += vbCrLf + " ,DESIGNER"
        If chkCatGroup.Checked Then strSql += vbCrLf + " ,CATEGORYGROUP"
        strSql += vbCrLf + " ,DAYS,SUM(PCS)PCS,SUM(ISSWT)GRSWT"
        strSql += vbCrLf + " ,'1'STONE,CONVERT(VARCHAR(1),'')COLHEAD "
        strSql += vbCrLf + " INTO TEMP" & systemId & "AGE"
        strSql += vbCrLf + " FROM ("
        strSql += vbCrLf + " SELECT  SNO,'1'RESULT,T.ITEMID,T.TAGNO,SUBITEMID,DESIGNERID,RECDATE,ACTUALRECDATE,COSTID,"
        ...
        strSql += vbCrLf + " ISNULL((SELECT GROUPNAME FROM " & cnAdminDb & "..ITEMGROUPMAST WHERE GROUPID =(SELECT ITEMGROUP FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMID=T.ITEMID)),'')AS ITEMGROUPNAME, "
        ...
        If chkCatGroup.Checked Then
            strSql += vbCrLf + " (SELECT CGROUPNAME FROM " & cnAdminDb & "..CATEGORYGROUP WHERE CGROUPID IN "
            strSql += vbCrLf + " (SELECT CGROUPID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE IN "
            strSql += vbCrLf + " (SELECT CATCODE FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMID=T.ITEMID)))CATEGORYGROUP, "
        End If
        strSql += vbCrLf + " DATEDIFF(D," & IIf(chkActualDate.Checked, "ACTUALRECDATE", "RECDATE") & ",ISSDATE) +1 AS DAYS, "
        strSql += vbCrLf + " ISSPCS AS PCS,ISSWT,'1'STONE,CONVERT(VARCHAR(1),'')COLHEAD "
        strSql += vbCrLf + " FROM " & cnAdminDb & "..ITEMTAG AS T "
        strSql += ftrStr
        If chkason.Checked Then
            strSql += vbCrLf + " AND ISSDATE <= '" & dtpAsOnDate.Value.Date.ToString("yyyy-MM-dd") & "'"
        Else
            strSql += vbCrLf + " AND ISSDATE BETWEEN '" & dtpAsOnDate.Value.Date.ToString("yyyy-MM-dd") & "'AND'" & dtpTo.Value.Date.ToString("yyyy-MM-dd") & "'"
        End If
        strSql += vbCrLf + " )X GROUP BY ITEMID,RESULT,TAGNO,ITEMNAME,DAYS,SNO,SUBITEMID,DESIGNERID,RECDATE,ACTUALRECDATE,COSTID"
        If chkSubItem.Checked Then strSql += vbCrLf + " ,SUBITEMNAME"
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + " ,ITEMCTRNAME"
        strSql += vbCrLf + " ,ITEMGROUPNAME"
        If Chktablegrpwise.Checked Then strSql += vbCrLf + " ,TABLECODE"
        ...
        If chkWithStone.Checked Or chkWithDia.Checked Or chkWithPre.Checked Then
            funcWithStone()
        End If
        If rbtDetail.Checked Then STRQRYSTRING = "ITEMNAME" Else STRQRYSTRING = ""
        If chkSubItem.Checked And rbtDetail.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "SUBITEMNAME", ",SUBITEMNAME")
        If chkCounterWisegrp.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "ITEMCTRNAME", ",ITEMCTRNAME")
        If chkitemgrp.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "ITEMGROUPNAME", ",ITEMGROUPNAME")
        If Chktablegrpwise.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "TABLECODE", ",TABLECODE")
        If ChkDesignerGrp.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "DESIGNER", ",DESIGNER")



        If chkCatGroup.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "CATEGORYGROUP", ",CATEGORYGROUP")
        If ChkItemDesignGrp.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "ITEMGROUPNAME,DESIGNER", ",ITEMGROUPNAME,DESIGNER")
        If ChkItemDesign.Checked Then STRQRYSTRING += IIf(STRQRYSTRING = "", "ITEMNAME,DESIGNER", ",ITEMNAME,DESIGNER")
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                ...
                cnthead += 1
                If chkWithStone.Checked Or chkWithDia.Checked Or chkWithPre.Checked Then
                    STRQRYSTRING += " , DIAPCS" & i & ", DIAWT" & i & ", PREPCS" & i & ", PREWT" & i & ", STNPCS" & i & ", STNWT" & i & ""
                    STRQRYSTRING += " , DIAPCS" & i & ", DIAWT" & i & ", PREPCS" & i & ", PREWT" & i & ", STNPCS" & i & ", STNWT" & i & ""
                    DIAPCSQRY += " +CONVERT(INT,ISNULL(DIAPCS" & i & ",0))"
                    DIAWTQRY += " +CONVERT(NUMERIC(15,3),ISNULL(DIAWT" & i & ",0))"
                    PREPCSQRY += " +CONVERT(INT,ISNULL(PREPCS" & i & ",0))"
                    PREWTQRY += " +CONVERT(NUMERIC(15,3),ISNULL(PREWT" & i & ",0))"
                    STNPCSQRY += " +CONVERT(INT,ISNULL(STNPCS" & i & ",0))"
                    STNWTQRY += " +CONVERT(NUMERIC(15,3),ISNULL(STNWT" & i & ",0))"
                    ...
                    If i <> dtrange.Rows.Count - 1 Then
                        'CRTPCSQRY += "+"  CRTWTQRY += "+" : GRMPCSQRY += "+" : GRMWTQRY += "+"
                        DIAPCSQRY += "+"
                        DIAWTQRY += "+"
                        PREPCSQRY += "+"
                        PREWTQRY += "+"
                        ...
                        STNWTQRY += "+"
                    End If
                    STRQRYSTRINGHEAD += " , '' 'PCS" & i & "~GRSWT" & i & "~DIAPCS" & i & "~DIAWT" & i & "~PREPCS" & i & "~PREWT" & i & "~STNPCS" & i & "~STNWT" & i & "'"
                Else
        ...
        If rbtDetail.Checked Then STRQRYSTRING += " , ITEMID"
        STRQRYSTRING += " , RESULT, STONE, COLHEAD,DISPORDER"
        ...
        If rbtDetail.Checked Then strSql += vbCrLf + " itemName," + IIf(chkSubItem.Checked = True, "subItemName,", "")
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + " ITEMCTRNAME,"
        If chkitemgrp.Checked Then strSql += vbCrLf + " ITEMGROUPNAME,"
        If Chktablegrpwise.Checked Then strSql += vbCrLf + " CONVERT(VARCHAR(20),TABLECODE)TABLECODE,"
        If ChkDesignerGrp.Checked Then strSql += vbCrLf + " DESIGNER,"



        If chkCatGroup.Checked Then strSql += vbCrLf + " CATEGORYGROUP,"
        If ChkItemDesignGrp.Checked Then strSql += vbCrLf + " ITEMGROUPNAME,DESIGNER,"
        If ChkItemDesign.Checked Then strSql += vbCrLf + " ITEMNAME,CONVERT(VARCHAR(100),DESIGNER)DESIGNER,"
        If Not chkrangefrmmaster.Checked Then
            If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                strSql += vbCrLf + " convert(INT,sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then pcs else 0 end ))Pcs" & i & " ,"
                strSql += vbCrLf + " convert(numeric(20,3),sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then GrsWt else 0 end ))GrsWt" & i & "  ,"
                If chkWithStone.Checked Or chkWithDia.Checked Or chkWithPre.Checked Then
                    strSql += vbCrLf + " convert(INT,sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then DIAPCS else 0 end ))DIAPCS" & i & " ,"
                    strSql += vbCrLf + " convert(numeric(20,3),sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then DIAWT else 0 end ))DIAWT" & i & "  ,"
                    strSql += vbCrLf + " convert(INT,sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then PREPCS else 0 end ))PREPCS" & i & "  ,"
                    strSql += vbCrLf + " convert(numeric(20,3),sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then PREWT else 0 end ))PREWT" & i & "  ,"
                    strSql += vbCrLf + " convert(INT,sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then STNPCS else 0 end ))STNPCS" & i & "  ,"
                    strSql += vbCrLf + " convert(numeric(20,3),sum(case when Days between " & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & " and " & Val(dtrange.Rows(i).Item("TODAY").ToString) & " then STNWT else 0 end ))STNWT" & i & "  ,"
                End If
                max = Val(dtrange.Rows(i).Item("TODAY").ToString)
            Next
        End If
        ...
        End If
        If rbtDetail.Checked Then strSql += vbCrLf + " itemid,"
        strSql += vbCrLf + " RESULT,Stone,COLHEAD,DISPORDER"
        strSql += vbCrLf + " from"
        strSql += vbCrLf + " ("
        strSql += vbCrLf + " select "
        If rbtDetail.Checked Then strSql += vbCrLf + " itemName," + IIf(chkSubItem.Checked = True, "subItemName,", "")
        ...
        If chkCatGroup.Checked Then
            strSql += vbCrLf + " (SELECT CGROUPNAME FROM " & cnAdminDb & "..CATEGORYGROUP WHERE CGROUPID IN "
            strSql += vbCrLf + " (SELECT CGROUPID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE IN "
            strSql += vbCrLf + " (SELECT CATCODE FROM " & cnAdminDb & "..ITEMMAST AS II WHERE II.ITEMID = TEM.ITEMID))) AS CATEGORYGROUP ,"
        End If
        If ChkItemDesignGrp.Checked Then strSql += vbCrLf + " ITEMGROUPNAME,DESIGNER,"
        If ChkItemDesign.Checked Then strSql += vbCrLf + " ITEMNAME,DESIGNER,"
        ...
        If rbtDetail.Checked Then strSql += vbCrLf + " ,itemId"
        strSql += vbCrLf + " ,days,Stone,COLHEAD,DISPORDER"
        strSql += vbCrLf + " from temp" & systemId & "Age AS TEM "
        If chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked Then strSql += vbCrLf + " where colhead<>'G'"
        strSql += vbCrLf + " group by result,itemid"
        If rbtDetail.Checked Then strSql += vbCrLf + " ,itemName" + IIf(chkSubItem.Checked = True, ",subItemName", "") + ""
        strSql += vbCrLf + " ,days,Stone,COLHEAD,DISPORDER"
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + ",ITEMCTRNAME"
        If chkitemgrp.Checked Then strSql += vbCrLf + ",ITEMGROUPNAME"
        ...
        If ChkItemDesign.Checked Then strSql += vbCrLf + " ,ITEMNAME,DESIGNER"
        If chkWithStone.Checked Or chkWithDia.Checked Or chkWithPre.Checked Then
            Dim CATEGORYGROUPTemp As String = ""
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT"
            CATEGORYGROUPTemp = "(SELECT CGROUPNAME FROM " & cnAdminDb & "..CATEGORYGROUP WHERE CGROUPID IN "
            CATEGORYGROUPTemp += "(SELECT CGROUPID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE IN "
            CATEGORYGROUPTemp += "(SELECT CATCODE FROM " & cnAdminDb & "..ITEMMAST AS II WHERE II.ITEMID = TEM.ITEMID))) AS CATEGORYGROUP"
            If rbtDetail.Checked Then strSql += vbCrLf + " ITEMNAME AS ITEMNAME," + IIf(chkSubItem.Checked = True, "CASE WHEN SUBITEMNAME = '' THEN ''ELSE SUBITEMNAME END AS SUBITEMNAME,", "")
            strSql += vbCrLf + IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", "") + IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", "") + IIf(Chktablegrpwise.Checked, "TABLECODE,", "") + IIf(ChkDesignerGrp.Checked, "DESIGNER,", "") + IIf(chkCatGroup.Checked, "" & CATEGORYGROUPTemp & ",", "") + ""
            If ChkItemDesignGrp.Checked Then strSql += vbCrLf + " ITEMGROUPNAME,DESIGNER,"
            If ChkItemDesign.Checked Then strSql += vbCrLf + " ITEMNAME,DESIGNER,"
            ...
            strSql += vbCrLf + " GROUP BY RESULT,DAYS,STONE,DISPORDER,ITEMID"
            If rbtDetail.Checked Then strSql += vbCrLf + " ,ITEMNAME" + IIf(chkSubItem.Checked = True, ",SUBITEMNAME", "") + ""
            If chkCounterWisegrp.Checked Then strSql += vbCrLf + ",ITEMCTRNAME"
            If chkitemgrp.Checked Then strSql += vbCrLf + ",ITEMGROUPNAME"
            If Chktablegrpwise.Checked Then strSql += vbCrLf + ",TABLECODE"
        ...
        End If
        strSql += vbCrLf + " )AGE"
        strSql += vbCrLf + " GROUP BY RESULT,STONE,COLHEAD,DISPORDER"
        If rbtDetail.Checked Then strSql += vbCrLf + " ,ITEMID,ITEMNAME" + IIf(chkSubItem.Checked = True, ",SUBITEMNAME", "") + ""
        If chkCounterWisegrp.Checked Then strSql += vbCrLf + ",ITEMCTRNAME"
        If chkitemgrp.Checked Then strSql += vbCrLf + ",ITEMGROUPNAME"
        If Chktablegrpwise.Checked Then strSql += vbCrLf + ",TABLECODE"
        If ChkDesignerGrp.Checked Then strSql += vbCrLf + ",DESIGNER"
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                ...
                If chkWithDia.Checked = True Then
                    strSql += vbCrLf + " and DIAPcs" & i & " <> '0' and DISWt" & i & " <> '0'"
                End If
                If chkWithPre.Checked = True Then
                    strSql += vbCrLf + " and PREPcs" & i & " <> '0' and PREWt" & i & " <> '0'"
                End If
                If chkWithStone.Checked = True Then
                    strSql += vbCrLf + " and STNPcs" & i & " <> '0' and STNWt" & i & " <> '0'"
                End If
                strSql += vbCrLf + " )"
            Next
        End If
        ...
        If rbtDetail.Checked Then
            strSql += vbCrLf + " ORDER BY " + IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", "") + IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", "") + IIf(Chktablegrpwise.Checked, "TABLECODE,", "") + IIf(ChkDesignerGrp.Checked, "DESIGNER,", "") + "itemId," + IIf((chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked), "ITEMNAME,", "") + "result"
        Else
            strSql += vbCrLf + " ORDER BY " + IIf(ChkItemDesign.Checked, "ITEMNAME,", "") + IIf(ChkItemDesignGrp.Checked, "ITEMGROUPNAME,", "") + IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", "") + IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", "") + IIf(Chktablegrpwise.Checked, "TABLECODE,", "") + IIf(ChkDesignerGrp.Checked, "DESIGNER,", "") + "RESULT"
        End If
        strSql += vbCrLf + " END "
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        If chkSubItem.Checked = True And rbtDetail.Checked Then
            If chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked Then
                ''-----------------------------ITEM TOTAL--------------------------
                ...
                strSql += vbCrLf + " BEGIN "
                strSql += vbCrLf + "  INSERT INTO TEMP" & systemId & "AGE1(ITEMNAME," + IIf(chkSubItem.Checked = True, "subItemName,", "") + IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", IIf(Chktablegrpwise.Checked, "TABLECODE,", IIf(ChkDesignerGrp.Checked, "DESIGNER,", IIf(chkCatGroup.Checked, "CATEGORYGROUP,", "")))))
                If Not chkrangefrmmaster.Checked Then
                ...
                Else
                    ...
                    Next
                End If
                strSql += vbCrLf + " MAXPCS, MAXGRSWT,ITEMID,RESULT,STONE,COLHEAD,DISPORDER)"
                ...
                strSql += vbCrLf + "  MAX(ITEMID)+1,'2'RESULT,'1'STONE,'O'COLHEAD,DISPORDER"
                strSql += vbCrLf + "  FROM TEMP" & systemId & "AGE1 where RESULT = 1 "
                strSql += vbCrLf + "  GROUP BY ITEMNAME,DISPORDER"
                If chkCounterWisegrp.Checked Then strSql += vbCrLf + " ,ITEMCTRNAME"
                If chkitemgrp.Checked Then strSql += vbCrLf + " ,ITEMGROUPNAME"
                If Chktablegrpwise.Checked Then strSql += vbCrLf + " ,TABLECODE"
                If ChkDesignerGrp.Checked Then strSql += vbCrLf + " ,DESIGNER"
            ...
            End If
        End If
        If (chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked) And rbtDetail.Checked Then
            ''-----------------------------GROUP BY TOTAL--------------------------
            strSql = "IF (SELECT COUNT(*) FROM SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "AGE1')>0"
            strSql += " BEGIN "
            strSql += vbCrLf + " IF (SELECT COUNT(*) FROM TEMP" & systemId & "AGE1)>0"
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + "  INSERT INTO TEMP" & systemId & "AGE1(ITEMNAME," + IIf(chkSubItem.Checked = True, "subItemName,", "")
            strSql += vbCrLf + IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", IIf(Chktablegrpwise.Checked, "TABLECODE,", IIf(ChkDesignerGrp.Checked, "DESIGNER,", IIf(chkCatGroup.Checked, "CATEGORYGROUP,", "")))))
            If Not chkrangefrmmaster.Checked Then
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then strSql += vbCrLf + " PCS1,GRSWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithDia.Checked Then strSql += vbCrLf + " DIAPCS1,DIAWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithPre.Checked Then strSql += vbCrLf + " PREPCS1,PREWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithStone.Checked Then strSql += vbCrLf + " STNPCS1,STNWT1,"
                If txtFromDay2_NUM.Text <> "" And txtToDay2_NUM.Text <> "" Then strSql += vbCrLf + " PCS2,GRSWT2,"
                If txtFromDay2_NUM.Text <> "" And txtToDay2_NUM.Text <> "" And chkWithDia.Checked Then strSql += vbCrLf + " DIAPCS2,DIAWT2,"
                If txtFromDay2_NUM.Text <> "" And txtToDay2_NUM.Text <> "" And chkWithPre.Checked Then strSql += vbCrLf + " PREPCS2,PREWT2,"
                If txtFromDay2_NUM.Text <> "" And txtToDay2_NUM.Text <> "" And chkWithStone.Checked Then strSql += vbCrLf + " STNPCS2,STNWT2,"
                If txtFromDay3_NUM.Text <> "" And txtToDay3_NUM.Text <> "" Then strSql += vbCrLf + " PCS3,GRSWT3,"
                If txtFromDay3_NUM.Text <> "" And txtToDay3_NUM.Text <> "" And chkWithDia.Checked Then strSql += vbCrLf + " DIAPCS3,DIAWT3,"
                If txtFromDay3_NUM.Text <> "" And txtToDay3_NUM.Text <> "" And chkWithPre.Checked Then strSql += vbCrLf + " PREPCS3,PREWT3,"
                If txtFromDay3_NUM.Text <> "" And txtToDay3_NUM.Text <> "" And chkWithStone.Checked Then strSql += vbCrLf + " STNPCS3,STNWT3,"
                If txtFromDay4_NUM.Text <> "" And txtToDay4_NUM.Text <> "" Then strSql += vbCrLf + " PCS4,GRSWT4,"
                If txtFromDay4_NUM.Text <> "" And txtToDay4_NUM.Text <> "" And chkWithDia.Checked Then strSql += vbCrLf + " DIAPCS4,DIAWT4,"
                If txtFromDay4_NUM.Text <> "" And txtToDay4_NUM.Text <> "" And chkWithPre.Checked Then strSql += vbCrLf + " PREPCS4,PREWT4,"
                If txtFromDay4_NUM.Text <> "" And txtToDay4_NUM.Text <> "" And chkWithStone.Checked Then strSql += vbCrLf + " STNPCS4,STNWT4,"
            Else
                For i As Integer = 0 To dtrange.Rows.Count - 1
                    strSql += vbCrLf + "PCS" & i & ", GRSWT" & i & ", "
                Next
            ...
            End If
            strSql += vbCrLf + " ,ITEMID,RESULT,STONE,COLHEAD,DISPORDER)"
            strSql += vbCrLf + "  SELECT " & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME", IIf(chkitemgrp.Checked, "ITEMGROUPNAME", IIf(Chktablegrpwise.Checked, "TABLECODE", IIf(ChkDesignerGrp.Checked, "DESIGNER", "")))) & "+' TOTAL'," & IIf(chkSubItem.Checked = True, "'',", "") + "" & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", IIf(Chktablegrpwise.Checked, "TABLECODE,", IIf(ChkDesignerGrp.Checked, "DESIGNER,", IIf(chkCatGroup.Checked, "CATEGORYGROUP,", ""))))) & ""
            If Not chkrangefrmmaster.Checked Then
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then
                    strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PCS1,0)))),"
                    strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,2),ISNULL(GRSWT1,0)))),"
                    If chkWithDia.Checked Then
                        ...
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(DIAWT1,0)))),"
                    End If
                    If chkWithPre.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PREPCS1,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(PREWT1,0)))),"
                    End If
                    If chkWithStone.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(STNPCS1,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(STNWT1,0)))),"
                    End If
                End If
                If txtFromDay2_NUM.Text <> "" And txtToDay2_NUM.Text <> "" Then
                    strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PCS2,0)))),"
                    ...
                    If chkWithDia.Checked Then
                        ...
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(DIAWT2,0)))),"
                    End If
                    If chkWithPre.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PREPCS2,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(PREWT2,0)))),"
                    End If
                    If chkWithStone.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(STNPCS2,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(STNWT2,0)))),"
                    End If
                End If
                If txtFromDay3_NUM.Text <> "" And txtToDay3_NUM.Text <> "" Then
                    strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PCS3,0)))),"
                    ...
                    If chkWithDia.Checked Then
                        ...
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(DIAWT3,0)))),"
                    End If
                    If chkWithPre.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PREPCS3,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(PREWT3,0)))),"
                    End If
                    If chkWithStone.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(STNPCS3,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(STNWT3,0)))),"
                    End If
                End If
                If txtFromDay4_NUM.Text <> "" And txtToDay4_NUM.Text <> "" Then
                    strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PCS4,0)))),"
                    ...
                    If chkWithDia.Checked Then
                        ...
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(DIAWT4,0)))),"
                    End If
                    If chkWithPre.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PREPCS4,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(PREWT4,0)))),"
                    End If
                    If chkWithStone.Checked Then
                        strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(STNPCS4,0)))),"
                        strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(STNWT4,0)))),"
                    End If
                End If
            Else
                For i As Integer = 0 To dtrange.Rows.Count - 1
            ...
            If chkWithDia.Checked Then
                ...
                strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(MAXDIAWT,0)))),"
            End If
            If chkWithPre.Checked Then
                strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(MAXPREPCS,0)))),"
                strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(MAXPREWT,0)))),"
            End If
            If chkWithStone.Checked Then
                strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(MAXSTNPCS,0)))),"
                strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(MAXSTNWT,0)))),"
            End If
            strSql += vbCrLf + "  MAX(ITEMID)+1,'4'RESULT,'1'STONE,'G' COLHEAD,DISPORDER"
            strSql += vbCrLf + "  FROM TEMP" & systemId & "AGE1 where RESULT = 1 "
            strSql += vbCrLf + "  GROUP BY DISPORDER ," & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME", IIf(chkitemgrp.Checked, "ITEMGROUPNAME", IIf(Chktablegrpwise.Checked, "TABLECODE", IIf(ChkDesignerGrp.Checked, "DESIGNER", IIf(chkCatGroup.Checked, "CATEGORYGROUP", ""))))) & ""
            strSql += vbCrLf + "  ORDER BY " & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME", IIf(chkitemgrp.Checked, "ITEMGROUPNAME", IIf(Chktablegrpwise.Checked, "TABLECODE", IIf(ChkDesignerGrp.Checked, "DESIGNER", IIf(chkCatGroup.Checked, "CATEGORYGROUP", ""))))) & ""
            strSql += vbCrLf + " END "
            strSql += vbCrLf + "END "
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
        ...
        If ChkItemDesignGrp.Checked And rbtSummary.Checked Then
            ''-----------------------------GROUP BY TOTAL--------------------------
            strSql = "IF (SELECT COUNT(*) FROM SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "AGE1')>0"
            strSql += " BEGIN "
            strSql += vbCrLf + " IF (SELECT COUNT(*) FROM TEMP" & systemId & "AGE1)>0"
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + "  INSERT INTO TEMP" & systemId & "AGE1(ITEMGROUPNAME,DESIGNER,"
            If Not chkrangefrmmaster.Checked Then
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then strSql += vbCrLf + " PCS1,GRSWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithDia.Checked Then strSql += vbCrLf + " DIAPCS1,DIAWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithPre.Checked Then strSql += vbCrLf + " PREPCS1,PREWT1,"
            ...
            End If
            strSql += vbCrLf + " ,RESULT,STONE,COLHEAD,DISPORDER)"
            strSql += vbCrLf + "  SELECT ITEMGROUPNAME,ITEMGROUPNAME+' TOTAL',"
            If Not chkrangefrmmaster.Checked Then
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then
                    strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PCS1,0)))),"
                    strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,2),ISNULL(GRSWT1,0)))),"
            ...
            If chkWithStone.Checked Then
                ...
                strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,3),ISNULL(MAXSTNWT,0)))),"
            End If
            strSql += vbCrLf + "  '4'RESULT,'1'STONE,'G' COLHEAD,DISPORDER"
            strSql += vbCrLf + "  FROM TEMP" & systemId & "AGE1 where RESULT = 1 "
            strSql += vbCrLf + "  GROUP BY DISPORDER ,ITEMGROUPNAME"
            strSql += vbCrLf + " END "
            strSql += vbCrLf + "END "
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()

            strSql = vbCrLf + "INSERT INTO TEMP" & systemId & "AGE1(ITEMGROUPNAME,DESIGNER,RESULT,DISPORDER,STONE,COLHEAD)"
            strSql += vbCrLf + "SELECT DISTINCT ITEMGROUPNAME ,ITEMGROUPNAME,0,DISPORDER,0,'T' FROM TEMP" & systemId & "AGE1 WHERE RESULT=1"
        ...
        If ChkItemDesign.Checked And rbtSummary.Checked Then
            ''-----------------------------GROUP BY TOTAL--------------------------
            strSql = "IF (SELECT COUNT(*) FROM SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "AGE1')>0"
            strSql += " BEGIN "
            strSql += vbCrLf + " IF (SELECT COUNT(*) FROM TEMP" & systemId & "AGE1)>0"
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + "  INSERT INTO TEMP" & systemId & "AGE1(ITEMNAME,DESIGNER,"
            If Not chkrangefrmmaster.Checked Then
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then strSql += vbCrLf + " PCS1,GRSWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithDia.Checked Then strSql += vbCrLf + " DIAPCS1,DIAWT1,"
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" And chkWithPre.Checked Then strSql += vbCrLf + " PREPCS1,PREWT1,"
            ...
            End If
            strSql += vbCrLf + " ,RESULT,STONE,COLHEAD,DISPORDER)"
            strSql += vbCrLf + "  SELECT ITEMNAME,ITEMNAME+' TOTAL',"
            If Not chkrangefrmmaster.Checked Then
                If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then
                    strSql += vbCrLf + "  CONVERT(VARCHAR,SUM(CONVERT(INT,ISNULL(PCS1,0)))),"
                    strSql += vbCrLf + "  CONVERT(NUMERIC(20,3),SUM(CONVERT(NUMERIC(18,2),ISNULL(GRSWT1,0)))),"
            ...
            strSql += vbCrLf + "  '4'RESULT,'1'STONE,'G' COLHEAD,DISPORDER"
            strSql += vbCrLf + "  FROM TEMP" & systemId & "AGE1 where RESULT = 1 "
            strSql += vbCrLf + "  GROUP BY DISPORDER ,ITEMNAME"
            strSql += vbCrLf + " END "
            strSql += vbCrLf + "END "
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()

            strSql = vbCrLf + "INSERT INTO TEMP" & systemId & "AGE1(ITEMNAME,DESIGNER,RESULT,DISPORDER,STONE,COLHEAD)"
            strSql += vbCrLf + "SELECT DISTINCT ITEMNAME ,ITEMNAME,0,DISPORDER,0,'T' FROM TEMP" & systemId & "AGE1 WHERE RESULT=1"
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()
        End If
        ...
        strSql += vbCrLf + " BEGIN "
        If rbtDetail.Checked Then
            strSql += vbCrLf + "  INSERT INTO TEMP" & systemId & "AGE1(DISPORDER,ITEMNAME" + IIf(chkSubItem.Checked = True, ",subItemName", "") + IIf(chkCounterWisegrp.Checked, ",ITEMCTRNAME", "") + "" + IIf(chkitemgrp.Checked, ",ITEMGROUPNAME", "") + IIf(Chktablegrpwise.Checked, ",TABLECODE", "") + IIf(ChkDesignerGrp.Checked, ",DESIGNER", "") + IIf(chkCatGroup.Checked, ",CATEGORYGROUP", "") + ","
        Else
            strSql += vbCrLf + "  INSERT INTO TEMP" & systemId & "AGE1(DISPORDER" + IIf(chkCounterWisegrp.Checked, ",ITEMCTRNAME", "") + ""
            strSql += vbCrLf + IIf(chkitemgrp.Checked, ",ITEMGROUPNAME", "") + IIf(Chktablegrpwise.Checked, ",TABLECODE", "") + IIf(ChkItemDesignGrp.Checked, ",DESIGNER,ITEMGROUPNAME", "") + IIf(ChkItemDesign.Checked, ",DESIGNER,ITEMNAME", "")
            strSql += vbCrLf + IIf(ChkDesignerGrp.Checked, ",DESIGNER", "") + IIf(chkCatGroup.Checked, ",CATEGORYGROUP", "") + ","
        End If
        If Not chkrangefrmmaster.Checked Then
            If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                strSql += vbCrLf + "  PCS" & i & ", GRSWT" & i & ","
                If chkWithDia.Checked Or chkWithPre.Checked Or chkWithStone.Checked Then
                    strSql += vbCrLf + "  DIAPCS" & i & ", DIAWT" & i & ","
                    strSql += vbCrLf + "  PREPCS" & i & ", PREWT" & i & ","
                    strSql += vbCrLf + "  STNPCS" & i & ", STNWT" & i & ","
                End If
            Next
        End If
        ...
        If chkWithDia.Checked Or chkWithPre.Checked Or chkWithStone.Checked Then
            ...
            strSql += vbCrLf + "  MAXSTNPCS, MAXSTNWT,"
        End If
        If rbtDetail.Checked Then strSql += vbCrLf + "  ITEMID,RESULT,STONE,COLHEAD)" Else strSql += vbCrLf + "  RESULT,STONE,COLHEAD)"
        If rbtDetail.Checked Then
            strSql += vbCrLf + "  SELECT 9999,'GRAND TOTAL'" + IIf(chkSubItem.Checked, ",''", "") + IIf(chkCounterWisegrp.Checked, ",'ZZZZZZZZZZZ'", "") + "" + IIf(chkitemgrp.Checked, ",'ZZZZZZZZZZZZ'", "") + IIf(Chktablegrpwise.Checked, ",'ZZZZ'", "") + IIf(ChkDesignerGrp.Checked, ",'ZZZZ'", "") + IIf(chkCatGroup.Checked, ",'ZZZZ'", "") + ","
        Else
            If ChkItemDesignGrp.Checked Then
                strSql += vbCrLf + "  SELECT 9999,'GRAND TOTAL','ZZZZZ',"
            ElseIf ChkItemDesign.Checked Then
                strSql += vbCrLf + "  SELECT 9999,'GRAND TOTAL','ZZZZZ',"
            Else
                strSql += vbCrLf + "  SELECT 9999,'GRAND TOTAL',"
            End If
        End If
        If Not chkrangefrmmaster.Checked Then
            If txtFromDay1_NUM.Text <> "" And txtToDay1_NUM.Text <> "" Then
        ...
        End If
        If rbtDetail.Checked Then strSql += vbCrLf + "  MAX(ITEMID)+5,'5'RESULT,'1'STONE,'Z' COLHEAD" Else strSql += vbCrLf + "  '5'RESULT,'1'STONE,'Z' COLHEAD"
        strSql += vbCrLf + "  FROM TEMP" & systemId & "AGE1 where RESULT = 1 "
        ...
        If (chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked) And rbtDetail.Checked Then
            ...
            strSql += vbCrLf + " IF (SELECT COUNT(*) FROM TEMP" & systemId & "AGE1)>0"
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + " INSERT INTO TEMP" & systemId & "AGE1(DISPORDER,ITEMNAME,ITEMID,RESULT," + IIf(chkSubItem.Checked = True, "subItemName,", "") + "STONE,COLHEAD" + IIf(chkCounterWisegrp.Checked, ",ITEMCTRNAME", "") + IIf(chkitemgrp.Checked, ",ITEMGROUPNAME", "") + IIf(Chktablegrpwise.Checked, ",TABLECODE", "") + IIf(ChkDesignerGrp.Checked, ",DESIGNER", "") + IIf(chkCatGroup.Checked, ",CATEGORYGROUP", "") + ")"
            strSql += vbCrLf + " SELECT DISTINCT DISPORDER,ITEMNAME,ITEMID,"
            strSql += vbCrLf + " 0 RESULT" + IIf(chkSubItem.Checked, ",''", "") + ",STONE,'T'" + IIf(chkCounterWisegrp.Checked, ",ITEMCTRNAME", "") + IIf(chkitemgrp.Checked, ",ITEMGROUPNAME", "") + IIf(Chktablegrpwise.Checked, ",TABLECODE", "") + IIf(ChkDesignerGrp.Checked, ",DESIGNER", "") + IIf(chkCatGroup.Checked, ",CATEGORYGROUP", "") + " FROM TEMP" & systemId & "AGE1 WHERE RESULT = 1"
            strSql += vbCrLf + " END "
            strSql += vbCrLf + " END "
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
        ...
        If (chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked) And rbtDetail.Checked Then
            ...
            strSql += vbCrLf + " BEGIN "
            strSql += vbCrLf + " INSERT INTO TEMP" & systemId & "AGE1(DISPORDER,ITEMNAME,ITEMID,RESULT," + IIf(chkSubItem.Checked = True, "subItemName,", "") + "STONE,COLHEAD," & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME", IIf(chkitemgrp.Checked, "ITEMGROUPNAME", IIf(Chktablegrpwise.Checked, "TABLECODE", IIf(ChkDesignerGrp.Checked, "DESIGNER", IIf(chkCatGroup.Checked, "CATEGORYGROUP", ""))))) & ")"
            strSql += vbCrLf + " SELECT DISTINCT DISPORDER," & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME,", IIf(chkitemgrp.Checked, "ITEMGROUPNAME,", IIf(Chktablegrpwise.Checked, "TABLECODE,", IIf(ChkDesignerGrp.Checked, "DESIGNER,", IIf(chkCatGroup.Checked, "CATEGORYGROUP,", ""))))) & "0,0 RESULT" + IIf(chkSubItem.Checked, ",''", "") + ",STONE,'B'," & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME", IIf(chkitemgrp.Checked, "ITEMGROUPNAME", IIf(Chktablegrpwise.Checked, "TABLECODE", IIf(ChkDesignerGrp.Checked, "DESIGNER", IIf(chkCatGroup.Checked, "CATEGORYGROUP", ""))))) & " FROM TEMP" & systemId & "AGE1 WHERE RESULT = 4"
            strSql += vbCrLf + " END "
            strSql += vbCrLf + " END "
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
        ...
        If (chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked) And rbtDetail.Checked Then
            ...
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
            strSql = vbCrLf + " UPDATE TEMP" & systemId & "AGE1 SET TEMPGROUPPCOLUMN= " & IIf(chkCounterWisegrp.Checked, "ITEMCTRNAME", IIf(chkitemgrp.Checked, "ITEMGROUPNAME", IIf(Chktablegrpwise.Checked, "TABLECODE", IIf(ChkDesignerGrp.Checked, "DESIGNER", IIf(chkCatGroup.Checked, "CATEGORYGROUP", ""))))) & " "
        ...
        '''''''''''''''''ORDER BY FINAL ''''''''''''''''''''''''''''''''''''''
        If rbtDetail.Checked Then
            strSql += vbCrLf + " FROM TEMP" & systemId & "AGE1 ORDER BY " + IIf(chkCounterWisegrp.Checked, "DISPORDER,", "") + IIf(chkitemgrp.Checked, "DISPORDER,", "") + IIf(Chktablegrpwise.Checked, "TABLECODE,", "") + IIf(ChkDesignerGrp.Checked, "DESIGNER,", "") + IIf(chkCatGroup.Checked, "CATEGORYGROUP,", "") + " TEMPGROUPPCOLUMN "
        Else
            If ChkItemDesignGrp.Checked Then
                strSql += vbCrLf + " FROM TEMP" & systemId & "AGE1 ORDER BY DISPORDER,ITEMGROUPNAME"
            ElseIf ChkItemDesign.Checked Then
                strSql += vbCrLf + " FROM TEMP" & systemId & "AGE1 ORDER BY DISPORDER,ITEMNAME"
            Else
                strSql += vbCrLf + " FROM TEMP" & systemId & "AGE1 ORDER BY " + IIf(chkCounterWisegrp.Checked, "DISPORDER,", "") + IIf(chkitemgrp.Checked, "DISPORDER,", "") + IIf(Chktablegrpwise.Checked, "DISPORDER,", "") + IIf(ChkDesignerGrp.Checked, "DISPORDER,", "") + IIf(chkCatGroup.Checked, "DISPORDER,", "") + " TEMPGROUPPCOLUMN "
            End If
        End If
        If chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or chkCatGroup.Checked Then
            If rbtDetail.Checked Then strSql += vbCrLf + " ,ITEMID"
        ...
        If rbtDetail.Checked Then
            ...
            If Chktablegrpwise.Checked Then strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = TABLECODE+' TOTAL' WHERE RESULT  = 4 "
            If ChkDesignerGrp.Checked Then strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = DESIGNER+' TOTAL' WHERE RESULT  = 4 "
            If chkCatGroup.Checked Then strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = CATEGORYGROUP+' TOTAL' WHERE RESULT  = 4 "
        ...
        Else
            ...
            strSql += vbCrLf + "    BEGIN "
            If chkitemgrp.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = ITEMGROUPNAME WHERE RESULT =1"
            ElseIf ChkDesignerGrp.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = DESIGNER WHERE RESULT =1"
            ElseIf chkCounterWisegrp.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = ITEMCTRNAME WHERE RESULT =1"
            ElseIf Chktablegrpwise.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = TABLECODE WHERE RESULT =1"
            ElseIf chkCatGroup.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = CATEGORYGROUP WHERE RESULT =1"
            ElseIf ChkItemDesignGrp.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = DESIGNER WHERE RESULT IN(0,1,4)"
            ElseIf ChkItemDesign.Checked Then
                strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = DESIGNER WHERE RESULT IN(0,1,4)"
            End If
            strSql += vbCrLf + "        UPDATE TEMP" & systemId & "AGEWISE SET PARTICULAR = 'GRAND TOTAL' WHERE RESULT =5"
            strSql += vbCrLf + "    END "
            strSql += vbCrLf + " END "
        End If
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        ...
        Else
            For i As Integer = 0 To dtrange.Rows.Count - 1
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET PCS" & i & " = NULL WHERE PCS" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET GRSWT" & i & " = NULL WHERE GRSWT" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET DIAPCS" & i & " = NULL WHERE DIAPCS" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET DIAWT" & i & " = NULL WHERE DIAWT" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET PREPCS" & i & " = NULL WHERE PREPCS" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET PREWT" & i & " = NULL WHERE PREWT" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET STNPCS" & i & " = NULL WHERE STNPCS" & i & " = 0 "
                strSql += vbCrLf + " UPDATE TEMP" & systemId & "AGEWISE SET STNWT" & i & " = NULL WHERE STNWT" & i & " = 0 "
            Next
        End If
        ...
        End If
        If chkason.Checked Then
            lblTitle.Text = "AGEWISE SALES ANALYSIS AS ON " + dtpAsOnDate.Value.ToString("dd-MM-yyyy") & IIf(chkCmbCostcentre.Text <> "" And chkCmbCostcentre.Text <> "ALL", " :" & chkCmbCostcentre.Text, "")
        Else
            lblTitle.Text = "AGEWISE SALES ANALYSIS BETWEEN " & dtpAsOnDate.Value.ToString("dd-MM-yyyy") & "AND " & dtpTo.Value.ToString("dd-MM-yyyy") & "" & IIf(chkCmbCostcentre.Text <> "" And chkCmbCostcentre.Text <> "ALL", " :" & chkCmbCostcentre.Text, "")
        End If
        ...
        With gridView
            ...
            If chkWithStone.Checked Then
                If .Columns.Contains("SAM1") Then .Columns("SAM1").Visible = False
                If .Columns.Contains("SAM2") Then .Columns("SAM2").Visible = False
            End If
            If chkSubItem.Checked Then If .Columns.Contains("SUBITEMNAME") Then .Columns("SUBITEMNAME").Visible = False
            ...
            If chkCatGroup.Checked Then .Columns("CATEGORYGROUP").Visible = False
            If ChkItemDesignGrp.Checked Then .Columns("ITEMGROUPNAME").Visible = False : .Columns("DESIGNER").Visible = False
            If ChkItemDesign.Checked Then .Columns("ITEMNAME").Visible = False : .Columns("DESIGNER").Visible = False
            If .Columns.Contains("COLHEAD") Then .Columns("COLHEAD").Visible = False
            If .Columns.Contains("STONE") Then .Columns("STONE").Visible = False
            If .Columns.Contains("DISPORDER") Then .Columns("DISPORDER").Visible = False

            Dim i As Int16 = IIf((chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked), 3, 2)
            ...
            i = IIf((chkCounterWisegrp.Checked Or chkitemgrp.Checked Or Chktablegrpwise.Checked Or ChkDesignerGrp.Checked Or chkCatGroup.Checked), 4, 3)
            If cnthead > 2 Then
    ...
    Function funcGridHeadWidth() As Integer
        ...
        End If
    End Function
    Private Sub chkCmbItemgroup_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles chkCmbItemgroup.KeyDown
        If e.KeyCode = Keys.Enter And chkCmbItemgroup.Text <> "" And chkCmbItemgroup.Text <> "ALL" Then
            strSql = " SELECT 'ALL' ITEMNAME,1 RESULT UNION ALL "
            strSql += " SELECT ITEMNAME,2 RESULT FROM " & cnAdminDb & "..ITEMMAST "
            strSql += " WHERE ITEMGROUP IN(SELECT GROUPID FROM " & cnAdminDb & "..ITEMGROUPMAST WHERE GROUPNAME IN ('" & chkCmbItemgroup.Text.Replace(",", "','") & "'))"
    ...
    Private Sub btnView_Search_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        ...
        If chkCatGroup.Checked = False And chkCounterWisegrp.Checked = False And Chktablegrpwise.Checked = False And ChkDesignerGrp.Checked = False And chkitemgrp.Checked = False And ChkItemDesignGrp.Checked = False And ChkItemDesign.Checked = False Then
            If rbtSummary.Checked Then ChkDesignerGrp.Checked = True
        End If
        If rbtDetail.Checked And ChkItemDesignGrp.Checked Then
            MsgBox("Detailed Not Enabled for ItemGroup with Designer...", MsgBoxStyle.Information)
            Exit Sub
        End If
        If rbtDetail.Checked And ChkItemDesign.Checked Then
            MsgBox("Detailed Not Enabled for Item with Designer...", MsgBoxStyle.Information)
            Exit Sub
        End If
            ...
            If dtrange.Rows.Count > 0 Then
                For i As Integer = 0 To dtrange.Rows.Count - 1
                    If Val(dtrange.Rows(i).Item("FROMDAY").ToString) < Val(dtrange.Rows(i).Item("TODAY").ToString) Then
                        strSql += " INSERT INTO TEMP" & systemId & "AGEDIFFDAYS(FROMDAY,TODAY)VALUES(" & Val(dtrange.Rows(i).Item("FROMDAY").ToString) & "," & Val(dtrange.Rows(i).Item("TODAY").ToString) & ")"
                    End If
                Next
            End If
        End If
    ...
    Private Sub gridView_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles gridView.KeyPress
        ...
        ElseIf UCase(e.KeyChar) = "D" Then
            If rbtDetail.Checked Then DetailedView() Else MsgBox("Detail not Enabled...", MsgBoxStyle.Information)
        End If
    End Sub

    Private Sub DetailedView()
        Dim itemName As String = gridView.CurrentRow.Cells("ITEMNAME").Value.ToString
        Dim itemId As String = gridView.CurrentRow.Cells("ITEMID").Value.ToString
        ...
        If gridView.Columns.Contains("DESIGNER") = True Then
            DesignerId = Val(objGPack.GetSqlValue("SELECT DESIGNERID FROM " & cnAdminDb & "..DESIGNER WHERE DESIGNERNAME='" & Designer & "'", "DESIGNERID").ToString)
        End If
        ...
        If subItemName <> "" Then
            If MessageBox.Show("Do you want to view item detail?", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) <> Windows.Forms.DialogResult.Yes Then
                'subitem detail
                filtration += " AND SUBITEMID = (SELECT SUBITEMID FROM " & cnAdminDb & "..SUBITEMMAST WHERE SUBITEMNAME = '" & subItemName & "' and itemid = " & itemId & ")"
            End If
        End If
        'item detail
        filtration += " AND ITEMID = " & itemId
        ...
        strSql = vbCrLf + " CREATE TABLE TEMP" & systemId & "DETAGEANALYSIS"
        strSql += vbCrLf + " ("
        strSql += vbCrLf + " PARTICULAR VARCHAR(200)"
        strSql += vbCrLf + " ,TAGNO VARCHAR(12)"
        strSql += vbCrLf + " ,PCS INT,GRSWT NUMERIC(15,3),NETWT NUMERIC(15,3)"
        strSql += vbCrLf + " ,DIAPCS INT,DIAWT NUMERIC(15,3)"
        strSql += vbCrLf + " ,PREPCS INT,PREWT NUMERIC(15,3)"
        strSql += vbCrLf + " ,STNPCS INT,STNWT NUMERIC(15,3)"
        strSql += vbCrLf + " ,RECDATE VARCHAR(12),DAYS INT"
        strSql += vbCrLf + " ,COSTCENTRE VARCHAR(50),EMPNAME VARCHAR(300),COLHEAD VARCHAR(3)"
        strSql += vbCrLf + " ,SNO INT IDENTITY(1,1)"
        strSql += vbCrLf + " ,RESULT INT"
        If Designer <> "" Then
            strSql += vbCrLf + " ,DESIGNERID INT ,DESIGNER VARCHAR(50)"
        End If
        strSql += vbCrLf + " )"
        strSql += vbCrLf + " "
        strSql += vbCrLf + " IF (SELECT TOP 1 1 FROM SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "DETAGE') > 0 DROP TABLE TEMP" & systemId & "DETAGE"
        strSql += vbCrLf + " SELECT "
        ...
        strSql += vbCrLf + " ,TAGNO"
        strSql += vbCrLf + " ,ISSPCS AS PCS,ISSWT AS GRSWT"
        strSql += vbCrLf + " ,(SELECT NETWT FROM " & cnStockDb & "..ISSUE WHERE TRANDATE=T.ISSDATE AND TRANNO=T.REFNO AND ITEMID=T.ITEMID AND TAGNO=T.TAGNO AND TRANTYPE IN('SA','OD','RD'))NETWT"
        ...
        strSql += vbCrLf + " ,(SELECT SUM(STNPCS) FROM " & cnAdminDb & "..ITEMTAGSTONE WHERE TAGSNO=T.SNO AND STNITEMID IN(SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE='P')) AS PREPCS"
        strSql += vbCrLf + " ,(SELECT SUM(STNWT) FROM " & cnAdminDb & "..ITEMTAGSTONE WHERE TAGSNO=T.SNO AND STNITEMID IN(SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE='P')) AS PREWT"
        ...
        If chkActualDate.Checked Then
            strSql += vbCrLf + " ,CONVERT(vARCHAR(10),ACTUALRECDATE,103)RECDATE"
        Else
            strSql += vbCrLf + " ,CONVERT(vARCHAR(10),RECDATE,103)RECDATE"
        End If
        strSql += vbCrLf + " ,DATEDIFF(D," & IIf(chkActualDate.Checked, "ACTUALRECDATE", "RECDATE") & ",T.ISSDATE)+1 AS DAYS,TAGVAL"
        strSql += vbCrLf + " ,(SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTID=T.COSTID)COSTNAME"
        strSql += vbCrLf + " ,(SELECT EMPNAME + '[' + CONVERT(VARCHAR,EMPID) + ']' FROM " & cnAdminDb & "..EMPMASTER WHERE EMPID IN (SELECT TOP 1 EMPID FROM " & cnStockDb & "..ISSUE WHERE BATCHNO=T.BATCHNO))EMPNAME"
        strSql += vbCrLf + " INTO TEMP" & systemId & "DETAGE"
        strSql += vbCrLf + " FROM " & cnAdminDb & "..ITEMTAG AS T"
        strSql += vbCrLf + filtration
        If chkason.Checked Then
            strSql += vbCrLf + " AND ISSDATE <= '" & dtpAsOnDate.Value.Date.ToString("yyyy-MM-dd") & "'"
        Else
            strSql += vbCrLf + " AND ISSDATE BETWEEN '" & dtpAsOnDate.Value.Date.ToString("yyyy-MM-dd") & "'AND'" & dtpTo.Value.Date.ToString("yyyy-MM-dd") & "'"
        ...
        strSql += vbCrLf + " FOR SELECT FROMDAY,TODAY FROM TEMP" & systemId & "AGEDIFFDAYS"
        ...
        strSql += vbCrLf + " 	INSERT INTO TEMP" & systemId & "DETAGEANALYSIS(PARTICULAR,COLHEAD,RESULT)"
        ...
        strSql += vbCrLf + " 	INSERT INTO TEMP" & systemId & "DETAGEANALYSIS(PARTICULAR,TAGNO,PCS,GRSWT,NETWT,DIAPCS,DIAWT,PREPCS,PREWT,STNPCS,STNWT,RECDATE,DAYS,COSTCENTRE,EMPNAME,RESULT"
        If Designer <> "" Then
            strSql += vbCrLf + " ,DESIGNERID,DESIGNER"
        End If
        strSql += vbCrLf + " )"
        strSql += vbCrLf + " 	SELECT PARTICULAR,TAGNO,PCS,GRSWT,NETWT,DIAPCS,DIAWT,PREPCS,PREWT,STNPCS,STNWT,RECDATE,DAYS,COSTNAME,EMPNAME,1 RESULT"
        If Designer <> "" Then
            strSql += vbCrLf + " ," & DesignerId & " ,'" & Designer & "'"
        End If
        strSql += vbCrLf + " 	FROM TEMP" & systemId & "DETAGE "
        strSql += vbCrLf + " 	WHERE DAYS BETWEEN @FROMDAY AND @TODAY"
        strSql += vbCrLf + " 	ORDER BY TAGVAL"
        strSql += vbCrLf + " 	"
        strSql += vbCrLf + " 	/** Inserting Sub Total **/"
        strSql += vbCrLf + " 	IF (SELECT COUNT(*) FROM TEMP" & systemId & "DETAGE WHERE DAYS BETWEEN @FROMDAY AND @TODAY)>0"
        strSql += vbCrLf + " 	BEGIN"
        strSql += vbCrLf + " 		INSERT INTO TEMP" & systemId & "DETAGEANALYSIS(PARTICULAR,PCS,GRSWT,NETWT,DIAPCS,DIAWT,PREPCS,PREWT,STNPCS,STNWT,RESULT,COLHEAD)"
        strSql += vbCrLf + " 		SELECT CONVERT(VARCHAR,@FROMDAY) + ' TO ' + CONVERT(VARCHAR,@TODAY) + ' => '"
        strSql += vbCrLf + " 		+ CONVERT(VARCHAR,COUNT(*)) + ' Tags' AS PARTICULAR,SUM(PCS),SUM(GRSWT),SUM(NETWT),SUM(DIAPCS),SUM(DIAWT),SUM(PREPCS),SUM(PREWT),SUM(STNPCS),SUM(STNWT),2 RESULT,'S'COLHEAD"
        strSql += vbCrLf + " 		FROM TEMP" & systemId & "DETAGE "
        strSql += vbCrLf + " 		WHERE DAYS BETWEEN @FROMDAY AND @TODAY"
        strSql += vbCrLf + " 	END"
        strSql += vbCrLf + " "
        ...
        strSql += vbCrLf + " SELECT @MAXDAY = ISNULL((SELECT MAX(TODAY) FROM TEMP" & systemId & "AGEDIFFDAYS),0)"
        strSql += vbCrLf + " INSERT INTO TEMP" & systemId & "DETAGEANALYSIS(PARTICULAR,COLHEAD,RESULT)"
        ...
        strSql += vbCrLf + " "
        strSql += vbCrLf + " /** Inserting Sub Total **/"
        strSql += vbCrLf + " IF (SELECT COUNT(*) FROM TEMP" & systemId & "DETAGE WHERE DAYS > @MAXDAY)>0"
        strSql += vbCrLf + " BEGIN"
        strSql += vbCrLf + " 	INSERT INTO TEMP" & systemId & "DETAGEANALYSIS(PARTICULAR,PCS,GRSWT,NETWT,DIAPCS,DIAWT,PREPCS,PREWT,STNPCS,STNWT,RESULT,COLHEAD)"
        ...
        strSql += vbCrLf + " IF (SELECT COUNT(*) FROM TEMP" & systemId & "DETAGEANALYSIS WHERE RESULT = 1)>0"
        strSql += vbCrLf + " BEGIN"
        strSql += vbCrLf + " INSERT INTO TEMP" & systemId & "DETAGEANALYSIS(PARTICULAR,PCS,GRSWT,NETWT,DIAPCS,DIAWT,PREPCS,PREWT,STNPCS,STNWT,RESULT,COLHEAD)"
        strSql += vbCrLf + " SELECT 'GRAND TOTAL => ' + CONVERT(VARCHAR,COUNT(*)) + ' Tags' AS PARTICULAR"
        strSql += vbCrLf + " ,SUM(PCS),SUM(GRSWT),SUM(NETWT),SUM(DIAPCS),SUM(DIAWT),SUM(PREPCS),SUM(PREWT),SUM(STNPCS),SUM(STNWT),3 RESULT,'G'COLHEAD FROM TEMP" & systemId & "DETAGE"
        strSql += vbCrLf + " END"
        strSql += vbCrLf + " "
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        ...
        End If
        If chkason.Checked Then
            tit += " DETAILED SALES AGING ANALYSIS REPORT AS ON " & dtpAsOnDate.Text & "" + vbCrLf
        Else
            tit += " DETAILED SALES AGING ANALYSIS REPORT BETWEEN " & dtpAsOnDate.Text & " AND " + dtpTo.Text + " " + vbCrLf
        End If
    ...
    Private Sub DataGridView_SummaryFormatting(ByVal dgv As DataGridView)
        With dgv
            ...
            Next
            If .Columns.Contains("DESIGNER") Then .Columns("DESIGNER").Visible = True : .Columns("DESIGNERID").Visible = True
            If chkActualDate.Checked Then
                .Columns("RECDATE").HeaderText = "ACTUALRECDATE"
            End If
            ...
            .Columns("PREPCS").Width = 80
            .Columns("PREWT").Width = 100
    ...
    Private Sub Prop_Sets()
        Dim obj As New frmAgeWiseSalesAnalysis_Properties
        ...
        obj.p_chkCatgrp = chkCatGroup.Checked
        ...
        obj.p_rbtDetail = rbtDetail.Checked
        obj.p_rbtSummary = rbtSummary.Checked
        obj.p_chkitemDesigngrp = ChkItemDesignGrp.Checked
        GetChecked_CheckedList(chkCmbtablecode, obj.p_chkCmbtablecode)
        GetChecked_CheckedList(chkCmbDesigner, obj.p_chkCmbDesigner)
        SetSettingsObj(obj, Me.Name, GetType(frmAgeWiseSalesAnalysis_Properties))
    End Sub

    Private Sub Prop_Gets()
        Dim obj As New frmAgeWiseSalesAnalysis_Properties
        GetSettingsObj(obj, Me.Name, GetType(frmAgeWiseSalesAnalysis_Properties))
        ...
        chkCatGroup.Checked = obj.p_chkCatgrp
        ...
        rbtDetail.Checked = obj.p_rbtDetail
        rbtSummary.Checked = obj.p_rbtSummary
        ChkItemDesignGrp.Checked = obj.p_chkitemDesigngrp
    ...
    Private Function GetRange() As Boolean
        ...
        If chkCmbItem.Text <> "ALL" And chkCmbItem.Text <> "" Then
            itemid = GetSqlValue(cn, " SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMNAME IN('" & chkCmbItem.Text.Replace(",", "','") & "')")
        Else
            itemid = 0
        End If
        strSql = "SELECT FROMDAY,TODAY FROM " & cnAdminDb & "..STKREORDER WHERE FROMFLAG='D'"
        If chkCmbItem.Text <> "ALL" And chkCmbItem.Text <> "" Then
            strSql += "AND ITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMNAME IN('" & chkCmbItem.Text.Replace(",", "','") & "'))"
        Else
            strSql += "AND ITEMID IN (0)"
        End If
        strSql += "ORDER BY FROMDAY"
    ...
    Private Sub chkitemgrp_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkitemgrp.CheckedChanged
        If chkitemgrp.Checked = True Then
            ...
            chkCatGroup.Checked = False
            ChkItemDesignGrp.Checked = False
            ChkItemDesign.Checked = False
        End If
    End Sub
    ...
    Private Sub chkCounterGroup_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkCatGroup.CheckedChanged
        If chkCatGroup.Checked = True Then
            chkCounterWisegrp.Checked = False
            Chktablegrpwise.Checked = False
            ChkDesignerGrp.Checked = False
            chkitemgrp.Checked = False
    ...
    Private Sub rbtSummary_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtSummary.CheckedChanged
        If rbtSummary.Checked Then
            ChkItemDesignGrp.Enabled = True
        Else
            ChkItemDesignGrp.Enabled = False
            ChkItemDesignGrp.Checked = False
        End If
    End Sub

    Private Sub ChkItemDesignGrp_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ChkItemDesignGrp.CheckedChanged
        If ChkItemDesignGrp.Checked = True Then
            chkCounterWisegrp.Checked = False
            Chktablegrpwise.Checked = False
            ChkDesignerGrp.Checked = False
            chkCatGroup.Checked = False
    ...
    Private Sub ChkItemDesign_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkItemDesign.CheckedChanged
        If ChkItemDesign.Checked = True Then
            chkCounterWisegrp.Checked = False
            Chktablegrpwise.Checked = False
            ChkDesignerGrp.Checked = False
            chkCatGroup.Checked = False
...
Public Class frmAgeWiseSalesAnalysis_Properties
    ...
    Public Property p_chkCounterWise() As Boolean
        ...
        End Set
    End Property
    Private chkitemDesigngrp As Boolean = False
    Public Property p_chkitemDesigngrp() As Boolean
        Get
            Return chkitemDesigngrp
        End Get
        Set(ByVal value As Boolean)
            chkitemDesigngrp = value
        End Set
    End Property
    ...
    Public Property p_chkWithApp() As Boolean
        ...
        End Set
    End Property
    Private chkWithCum As Boolean = False
    Public Property p_chkWithCum() As Boolean
        Get
            Return chkWithCum
        End Get
        Set(ByVal value As Boolean)
        ...
        End Set
    End Property
    Private chkWithTr As Boolean = False
    Public Property p_chkWithTr() As Boolean
        Get
            Return chkWithTr
        End Get
        Set(ByVal value As Boolean)
    ...
    Public Property p_rbtTag() As Boolean
        ...
        End Set
    End Property
    Private rbtNTag As Boolean = False
    Public Property p_rbtNTag() As Boolean
        Get
            Return rbtNTag
        End Get
        Set(ByVal value As Boolean)
    ...
    Public Property p_rbtSummary() As Boolean
        ...
        End Set
    End Property
    Private rbtDetail As Boolean = True
    Public Property p_rbtDetail() As Boolean
        Get
            Return rbtDetail
        End Get
        Set(ByVal value As Boolean)
