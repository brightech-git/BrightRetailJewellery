...
Public Class frmWastagewiseSales
    ...
    Private Sub btnView_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        ...
        Try
            ...
            Me.Refresh()
            'Dim TEMPTABLE As String = "TEMP" + Right(Guid.NewGuid, 4)
            If rbttable.Checked Then
                strSql = vbCrLf + " IF OBJECT_ID('TEMPTABLEDB..TEMP" & systemId & "WASTAGE')IS NOT NULL DROP TABLE TEMPTABLEDB..TEMP" & systemId & "WASTAGE"
                strSql += vbCrLf + "  SELECT TABLECODE, RANGE, SUM(PCS) PCS, SUM(GRSWT) GRSWT, SUM(NETWT) NETWT"
                strSql += vbCrLf + "  ,SUM(AMOUNT) AMOUNT,SUM(TAX) TAX,SUM(DISCOUNT) DISCOUNT,SUM(NETAMT) NETAMT,BOARDRATE "
                ...
                strSql += vbCrLf + "  ,CONVERT(NUMERIC(15,2),ROUND((SUM(DISCOUNT)/BOARDRATE)/ (SUM(GRSWT)/100),2)) DISCPERC"
                strSql += vbCrLf + "  ,0 RESULT,'D' COLHEAD INTO TEMPTABLEDB..TEMP" & systemId & "WASTAGE FROM"
                strSql += vbCrLf + "  (SELECT W.TABLECODE,CONVERT(VARCHAR,W.FROMWEIGHT)+'-'+CONVERT(VARCHAR,W.TOWEIGHT) RANGE,I.PCS,I.GRSWT "
                strSql += vbCrLf + "  ,I.NETWT,ISNULL(AMOUNT,0)+ISNULL(DISCOUNT,0)+ISNULL(SC,0)+ISNULL(FIN_DISCOUNT,0)+ISNULL((SELECT SUM(TAXAMOUNT) FROM " & cnStockDb & "..TAXTRAN WHERE SNO=I.SNO),0) AMOUNT "
                strSql += vbCrLf + "  ,ISNULL(TAX,0)TAX,ISNULL(DISCOUNT,0)+ISNULL(FIN_DISCOUNT,0) DISCOUNT"
                ...
                strSql += vbCrLf + "  LEFT JOIN " & cnAdminDb & "..WMCTABLE W ON I.TABLECODE=W.TABLECODE AND I.COSTID=W.COSTID AND I." + IIf(rbtGRSWT.Checked = True, "GRSWT", "NETWT") + " BETWEEN W.FROMWEIGHT AND W.TOWEIGHT "
                strSql += vbCrLf + "  WHERE I.TRANDATE BETWEEN '" + dtpFrom.Value.ToString("yyyy-MM-dd") + "' AND '" + dtpTo.Value.ToString("yyyy-MM-dd") + "' "
                strSql += vbCrLf + "  AND I.TRANTYPE ='SA' AND ISNULL(W.TABLECODE,'')<>'' AND I.GRSWT<>0 AND I.NETWT <>0 AND ISNULL(I.CANCEL,'')=''"
                ...
                strSql += vbCrLf + "  SELECT TABLECODE, CASE WHEN ISNULL(RANGE,'')='' THEN 'OTHER RANGE' ELSE RANGE END RANGE, SUM(PCS) PCS, SUM(GRSWT) GRSWT, SUM(NETWT) NETWT"
                strSql += vbCrLf + "  ,SUM(AMOUNT) AMOUNT,SUM(TAX) TAX,SUM(DISCOUNT) DISCOUNT,SUM(NETAMT) NETAMT,BOARDRATE "
                strSql += vbCrLf + "  ,CONVERT(NUMERIC(15,2),ROUND(SUM(NETAMT)/SUM(" + IIf(rbtGRSWT.Checked = True, "GRSWT", "NETWT") + "),2)) AVGRATE "
                strSql += vbCrLf + "  ,CONVERT(NUMERIC(15,2),ROUND(SUM(NETAMT)/SUM(" + IIf(rbtGRSWT.Checked = True, "GRSWT", "NETWT") + "),2)-BOARDRATE) RATEDIFF"
                strSql += vbCrLf + "  ,CONVERT(NUMERIC(15,2),ROUND((ROUND(SUM(NETAMT)/SUM(" + IIf(rbtGRSWT.Checked = True, "GRSWT", "NETWT") + "),2)-BOARDRATE)* (100 / BOARDRATE),2)) DIFFPERC"
                strSql += vbCrLf + "  ,CONVERT(NUMERIC(15,2),ROUND((SUM(DISCOUNT)/BOARDRATE)/ (SUM(GRSWT)/100),2)) DISCPERC"
                strSql += vbCrLf + "  ,0 RESULT,'D' COLHEAD FROM"
                strSql += vbCrLf + "  (SELECT W.TABLECODE,CONVERT(VARCHAR,W.FROMWEIGHT)+'-'+CONVERT(VARCHAR,W.TOWEIGHT) RANGE,I.PCS,I.GRSWT "
                strSql += vbCrLf + "  ,I.NETWT,ISNULL(AMOUNT,0)+ISNULL(DISCOUNT,0)+ISNULL(SC,0)+ISNULL(FIN_DISCOUNT,0)+ISNULL((SELECT SUM(TAXAMOUNT) FROM " & cnStockDb & "..TAXTRAN WHERE SNO=I.SNO),0) AMOUNT "
                strSql += vbCrLf + "  ,ISNULL(TAX,0)TAX,ISNULL(DISCOUNT,0)+ISNULL(FIN_DISCOUNT,0) DISCOUNT"
                strSql += vbCrLf + "  ,ISNULL(AMOUNT,0)+ISNULL(TAX,0)+ISNULL(SC,0)+ISNULL((SELECT SUM(TAXAMOUNT) FROM " & cnStockDb & "..TAXTRAN WHERE SNO=I.SNO),0) NETAMT "
                strSql += vbCrLf + "  ,I.BOARDRATE "
                strSql += vbCrLf + "  FROM " & cnStockDb & "..ISSUE I "
                strSql += vbCrLf + "  LEFT JOIN " & cnAdminDb & "..WMCTABLE W ON W.ITEMID=I.ITEMID AND W.SUBITEMID =I.SUBITEMID   AND W.COSTID=I.COSTID " 'AND I." + IIf(rbtGRSWT.Checked = True, "GRSWT", "NETWT") + " BETWEEN W.FROMWEIGHT AND W.TOWEIGHT "
                ...
                strSql += vbCrLf + "  AND ISNULL(I.ITEMID,'')<>'' AND ISNULL(I.TABLECODE,'')='' AND I.GRSWT<>0 AND I.NETWT <>0  AND ISNULL(I.CANCEL,'')=''"
                strSql += vbCrLf + "  AND I.GRSWT NOT IN (SELECT GRSWT FROM " & cnStockDb & "..ISSUE WHERE I." + IIf(rbtGRSWT.Checked = True, "GRSWT", "NETWT") + " BETWEEN W.FROMWEIGHT AND W.TOWEIGHT  AND TRANDATE BETWEEN '" + dtpFrom.Value.ToString("yyyy-MM-dd") + "' AND '" + dtpTo.Value.ToString("yyyy-MM-dd") + "' AND TRANTYPE='SA' AND ISNULL(CANCEL,'')='' AND ISNULL(TABLECODE,'')='' ) "
                If cmbMetal.Text <> "ALL" Then strSql += vbCrLf + "  AND I.METALID = (SELECT METALID FROM " + cnAdminDb + "..METALMAST WHERE METALNAME='" + cmbMetal.Text + "')"
                If chkCmbCostCentre.Text <> "ALL" Then strSql += vbCrLf + "  AND W.COSTID IN (SELECT COSTID FROM " + cnAdminDb + "..COSTCENTRE WHERE COSTNAME IN (" + GetQryString(chkCmbCostCentre.Text, ",") + "))"
                strSql += vbCrLf + "  ) S"
                strSql += vbCrLf + "  GROUP BY TABLECODE,RANGE,BOARDRATE"
                ...
                cmd.ExecuteNonQuery()
                strSql = " INSERT INTO TEMPTABLEDB..TEMP" & systemId & "WASTAGE(PCS,GRSWT,NETWT,BOARDRATE,AVGRATE,AMOUNT,TAX,DISCOUNT,NETAMT,DIFFPERC,RESULT,COLHEAD)"
                strSql += vbCrLf + " SELECT SUM(PCS) PCS, SUM(GRSWT) GRSWT, SUM(NETWT) NETWT,SUM(BOARDRATE)/count(*) BOARDRATE, SUM(AVGRATE)/count(*) AVGRATE"
                strSql += vbCrLf + ",SUM(AMOUNT) AMOUNT,SUM(TAX) TAX,SUM(DISCOUNT) DISCOUNT,SUM(NETAMT) NETAMT,SUM(DIFFPERC)DIFFPERC"
                strSql += vbCrLf + " ,1,'T'FROM TEMPTABLEDB..TEMP" & systemId & "WASTAGE"
                cmd = New OleDbCommand(strSql, cn)
                cmd.ExecuteNonQuery()
            Else
                
                strSql = vbCrLf + " IF OBJECT_ID('TEMPTABLEDB..TEMP" & systemId & "WASTAGE')IS NOT NULL DROP TABLE TEMPTABLEDB..TEMP" & systemId & "WASTAGE"
                strSql += vbCrLf + "  SELECT '('+CONVERT(VARCHAR(50),ITEMID)+') '+ITEMNAME PARTICULAR, ITEMNAME ITEM, SUM(PCS) PCS, SUM(GRSWT) GRSWT, SUM(NETWT) NETWT"
                strSql += vbCrLf + "  ,SUM(AMOUNT) AMOUNT,WASTPER,MCGRM"
                strSql += vbCrLf + "  ,0 RESULT,CONVERT(VARCHAR(5),'D') COLHEAD INTO TEMPTABLEDB..TEMP" & systemId & "WASTAGE FROM"
                strSql += vbCrLf + "  (SELECT I.ITEMID,IM.ITEMNAME,I.PCS,I.GRSWT "
                ...
                strSql += vbCrLf + "  ,I.WASTPER AS WASTPER,I.MCGRM AS MCGRM"
                strSql += vbCrLf + "  FROM " & cnStockDb & "..ISSUE I "
                strSql += vbCrLf + "  LEFT JOIN " & cnAdminDb & "..ITEMMAST IM ON I.ITEMID=IM.ITEMID "
                strSql += vbCrLf + "  WHERE I.TRANDATE BETWEEN '" + dtpFrom.Value.ToString("yyyy-MM-dd") + "' AND '" + dtpTo.Value.ToString("yyyy-MM-dd") + "' "
                strSql += vbCrLf + "  AND I.TRANTYPE ='SA' AND I.GRSWT<>0 AND I.NETWT <>0 AND ISNULL(I.CANCEL,'')=''"
                If cmbMetal.Text <> "ALL" Then strSql += vbCrLf + "  AND I.METALID = (SELECT METALID FROM " + cnAdminDb + "..METALMAST WHERE METALNAME='" + cmbMetal.Text + "')"
                If chkCmbCostCentre.Text <> "ALL" Then strSql += vbCrLf + "  AND I.COSTID IN (SELECT COSTID FROM " + cnAdminDb + "..COSTCENTRE WHERE COSTNAME IN (" + GetQryString(chkCmbCostCentre.Text, ",") + "))"
                strSql += vbCrLf + "  ) S"
                strSql += vbCrLf + "  GROUP BY ITEMID,ITEMNAME,WASTPER,MCGRM"
                cmd = New OleDbCommand(strSql, cn)
                cmd.CommandTimeout = 1000
                cmd.ExecuteNonQuery()
                strSql = " INSERT INTO TEMPTABLEDB..TEMP" & systemId & "WASTAGE(PARTICULAR,ITEM,PCS,GRSWT,NETWT,AMOUNT,RESULT,COLHEAD)"
                strSql += vbCrLf + " SELECT 'SUB TOTAL' PARTICULAR,ITEM,SUM(PCS) PCS, SUM(GRSWT) GRSWT, SUM(NETWT) NETWT, SUM(AMOUNT) AMOUNT"
                strSql += vbCrLf + " ,1,'T' FROM TEMPTABLEDB..TEMP" & systemId & "WASTAGE GROUP BY ITEM"
                cmd = New OleDbCommand(strSql, cn)
                cmd.ExecuteNonQuery()

                strSql = " INSERT INTO TEMPTABLEDB..TEMP" & systemId & "WASTAGE(PARTICULAR,PCS,GRSWT,NETWT,AMOUNT,ITEM,RESULT,COLHEAD)"
                strSql += vbCrLf + " SELECT 'GRAND TOTAL' PARTICULAR,SUM(PCS) PCS, SUM(GRSWT) GRSWT, SUM(NETWT) NETWT, SUM(AMOUNT) AMOUNT,'ZZZZ'"
                strSql += vbCrLf + " ,2,'G' FROM TEMPTABLEDB..TEMP" & systemId & "WASTAGE WHERE RESULT=0"
                cmd = New OleDbCommand(strSql, cn)
                cmd.ExecuteNonQuery()
            End If

            strSql = "SELECT * FROM TEMPTABLEDB..TEMP" & systemId & "WASTAGE "
            strSql += " ORDER BY "
            If rbtwast.Checked Then strSql += " ITEM,"
            strSql += " RESULT"
        ...
        With gridView
            ...
            If rbttable.Checked Then
                ...
                With .Columns("DIFFPERC")
                    .HeaderText = "RATE DIFF % "
                    .SortMode = DataGridViewColumnSortMode.NotSortable
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .Width = 80
                End With
                With .Columns("DISCPERC")
                    .HeaderText = "DISC % ON RATE"
                    .SortMode = DataGridViewColumnSortMode.NotSortable
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    .Width = 80
                End With
    ...
    Private Sub Prop_Sets()
        ...
        obj.p_rbtwast = rbtwast.Checked
        obj.p_rbttable = rbttable.Checked
    ...
    Private Sub Prop_Gets()
        ...
        rbtwast.Checked = obj.p_rbtwast
        rbttable.Checked = obj.p_rbttable
    End Sub
...
Public Class frmWastagewiseSales_Properties
    ...
    Public Property p_rbtGRSWT() As Boolean
        ...
        End Set
    End Property
    Private rbtwast As Boolean = True
    Public Property p_rbtwast() As Boolean
        Get
            Return rbtwast
        End Get
        Set(ByVal value As Boolean)
        ...
        End Set
    End Property
    Private rbttable As Boolean = True
    Public Property p_rbttable() As Boolean
        Get
            Return rbttable
        End Get
        Set(ByVal value As Boolean)
