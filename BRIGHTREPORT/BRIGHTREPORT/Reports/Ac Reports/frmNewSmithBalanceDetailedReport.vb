...
Public Class frmNewSmithBalanceDetailedReport
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        'cmd = New OleDbCommand(strSql, cn)
        'cmd.ExecuteNonQuery()
        strSql = " IF (SELECT TOP 1 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPSMITHBALDET')>0 DROP TABLE TEMPTABLEDB..TEMPSMITHBALDET"
        cmd = New OleDbCommand(strSql, cn)
        cmd.ExecuteNonQuery()

        strSql = " select * into  [TEMPTABLEDB].[DBO].[TEMPSMITHBALDET] FROM  TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT='4' ORDER BY TRANDATE,TRANNO"
        strSql += vbCrLf + ""
        cmd = New OleDbCommand(strSql, cn)
        cmd.ExecuteNonQuery()

        strSql = " SELECT * FROM TEMPTABLEDB..TEMPSMITHBALDET"
        ...
        If chkCostName <> "" Then
            ...
            End If
        End If
        strSql = " IF (SELECT COUNT(*) FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPSMITHBALITITLE')> 0"
        strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMPSMITHBALITITLE"
        strSql += vbCrLf + " CREATE TABLE TEMPTABLEDB..TEMPSMITHBALITITLE([OPNCS] [varchar](100),[OPNWT] [varchar](100),[CLOCS] [varchar](100),"
        ...
        strSql = " INSERT INTO TEMPTABLEDB..TEMPSMITHBALITITLE SELECT "
        strSql += vbCrLf + " (SELECT CONVERT(VARCHAR(50),RPUREWT)"
        strSql += vbCrLf + "  FROM  TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=2) AS OPNCS,"
        strSql += vbCrLf + " (SELECT CONVERT(VARCHAR(50),RPUREWT)"
        strSql += vbCrLf + "  AS OPNWT  FROM  TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=1) AS OPNWT,"
        strSql += vbCrLf + " (SELECT CONVERT(VARCHAR(50),RPUREWT)"
        strSql += vbCrLf + "  AS CLOCS  FROM  TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=7) AS CLOCS,"
        strSql += vbCrLf + " (SELECT CONVERT(VARCHAR(50),RPUREWT)"
        strSql += vbCrLf + "  AS CLOWT  FROM  TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=6)AS CLOWT,"
        ...
        strSql += vbCrLf + " (SELECT CONVERT(VARCHAR(1000),'" & tit & "') )AS TITLE,"
        strSql += vbCrLf + " (SELECT SUM(RNETWT) FROM TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=4) AS RNTWTSUM,"
        strSql += vbCrLf + " (SELECT SUM(RPUREWT) FROM TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=4) AS RFNWTSUM,"
        strSql += vbCrLf + " (SELECT SUM(RAMOUNT) FROM TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=4) AS RAMTSUM,"
        strSql += vbCrLf + " (SELECT SUM(INETWT) FROM TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=4) AS INTWTSUM,"
        strSql += vbCrLf + " (SELECT SUM(IPUREWT) FROM TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=4) AS IFNWTSUM,"
        strSql += vbCrLf + " (SELECT SUM(IAMOUNT) FROM TEMPTABLEDB..XYTEMP" & Sysid & "SMITHBALDET WHERE RESULT=4) AS IAMTSUM"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        Dim objReport As New BrighttechReport
        Dim objRptViewer As New frmReportViewer
        objRptViewer.CrystalReportViewer1.ReportSource = objReport.ShowReport(New rptnewsmithdetailed, cnDataSource)
    ...
    Private Sub Prop_Sets()
        Dim obj As New frmNewSmithBalanceDetailedReport_Properties
        ...
        SetSettingsObj(obj, Me.Name, GetType(frmNewSmithBalanceDetailedReport_Properties))
    End Sub

    Private Sub Prop_Gets()
        Dim obj As New frmNewSmithBalanceDetailedReport_Properties
        GetSettingsObj(obj, Me.Name, GetType(frmNewSmithBalanceDetailedReport_Properties))
...
Public Class frmNewSmithBalanceDetailedReport_Properties
