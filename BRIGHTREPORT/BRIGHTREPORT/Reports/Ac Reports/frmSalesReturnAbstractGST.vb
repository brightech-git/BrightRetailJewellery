...
Public Class frmSalesReturnAbstractGST
    ...
    Function funcAbstract() As Integer
        ...
        strSql += vbCrLf + " ,ISNULL((SELECT SUM(AMOUNT) FROM " & cnStockDb & "..ACCTRAN WHERE BATCHNO = X.BATCHNO AND COMPANYID = '" & strCompanyId & "' AND PAYMODE = 'PV' AND TRANNO = X.TRANNO AND ACCODE = (SELECT STAXID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE = X.CATCODE)),0)AS VAT"
        strSql += vbCrLf + " ,SUM(AMOUNT)AS TOTAL"
        strSql += vbCrLf + " ,SUM(STNWT)STNWT"
        strSql += vbCrLf + " ,SUM(DIAWT)DIAWT"
        strSql += vbCrLf + " ,CUSTOMER"
        ...
        strSql += vbCrLf + " ,SUM(PCS)PCS,SUM(GRSWT)GRSWT,SUM(NETWT)NETWT,SUM(AMOUNT)AMOUNT"
        strSql += vbCrLf + " ,ISNULL((SELECT SUM(STNWT) FROM " & cnStockDb & "..RECEIPTSTONE WHERE ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT WHERE BATCHNO = I.BATCHNO AND COMPANYID = '" & strCompanyId & "') AND COMPANYID = '" & strCompanyId & "' AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'S')),0)STNWT"
        strSql += vbCrLf + " ,ISNULL((SELECT SUM(STNWT) FROM " & cnStockDb & "..RECEIPTSTONE WHERE ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT WHERE BATCHNO = I.BATCHNO AND COMPANYID = '" & strCompanyId & "') AND COMPANYID = '" & strCompanyId & "' AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'D')),0)DIAWT"
        ...
        strSql += vbCrLf + " )AS X"
        strSql += vbCrLf + " GROUP BY CATCODE,CATNAME,METALID,METALNAME,BATCHNO,TRANNO,MMONTHID,MMONTH,UUPDATED,TRANDATE,TRANTYPE,CUSTOMER"
        strSql += funcSepStnAc()
        ...
        ElseIf rbtBillNo.Checked = True Then
            strSql += vbCrLf + " CATCODE,CATNAME,' 'ITEMID,' 'METALID,' 'METALNAME,' 'MMONTHID,' 'MMONTH,UUPDATED,TRANDATE,'   '+TRANNO TRANNO,TRANTYPE"
        End If
        strSql += vbCrLf + " ,SUM(PCS)PCS,SUM(GRSWT)GRSWT,SUM(NETWT)NETWT,SUM(AMOUNT)AMOUNT"
        ...
        ElseIf rbtBillNo.Checked = True Then
            strSql += vbCrLf + " GROUP BY SALETYPE,CATCODE,CATNAME,TRANNO,CUSTOMER,TRANTYPE,UUPDATED,TRANDATE"
        End If
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
    End Function
    ...
    Function funcAbsSub() As Integer
        ...
        strSql += vbCrLf + " )AS Z"
        If rbtSummary.Checked = True Then
            ''strSql += vbCrLf + " GROUP BY CATCODE,METALNAME,METALID"
    ...
    Private Sub ReturnAbs()
        ...
        ElseIf cmbMetal.Text <> "" Then
            ...
            End If
        End If
        Dim Bill As String = "B"
        If rbtGstBill.Checked Then Bill = "G"
        If rbtVatBill.Checked Then Bill = "V"
        ...
        strSql += vbCrLf + " ,@WITSTN = '" & IIf(chkwithstustone.Checked, "Y", "N") & "'"  ''N
        ...
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET STNCT = NULL WHERE STNCT = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET STNGT = NULL WHERE STNGT = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET DIAWT = NULL WHERE DIAWT = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET SGST = NULL WHERE SGST = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET CGST = NULL WHERE CGST = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET IGST = NULL WHERE IGST = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET TDS = NULL WHERE TDS = 0"
        strSql += vbCrLf + "UPDATE TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA SET TCS = NULL WHERE TCS = 0"
        ...
        With gridView
            ...
            If .Columns.Contains("SC") Then .Columns("SC").Visible = False
            ...
            If chkwithstustone.Checked Then
                ...
                If .Columns.Contains("DIAAMT") Then .Columns("DIAAMT").Visible = True
                If .Columns.Contains("STNAMT") Then .Columns("STNAMT").Visible = True
            Else
                ...
                If .Columns.Contains("DIAAMT") Then .Columns("DIAAMT").Visible = False
    ...
    Function GridViewFormat() As Integer
        ...
        Next
    End Function
    Private Sub frmSalesReturnAbstractGST_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub frmSalesAbstract_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strSql += vbCrLf + " SELECT COSTNAME,CONVERT(VARCHAR,COSTID),2 RESULT FROM " & cnAdminDb & "..COSTCENTRE WHERE  ISNULL(COMPANYID,'" & strCompanyId & "')LIKE'%" & strCompanyId & "%'"
    ...
    Private Sub Prop_Sets()
        Dim obj As New frmSalesReturnAbstractGST_Properties
        ...
        SetSettingsObj(obj, Me.Name, GetType(frmSalesReturnAbstractGST_Properties))
    End Sub

    Private Sub Prop_Gets()
        Dim obj As New frmSalesReturnAbstractGST_Properties
        GetSettingsObj(obj, Me.Name, GetType(frmSalesReturnAbstractGST_Properties))
    ...
    Private Sub rbtBillNo_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtBillNo.CheckedChanged
        ChkItem.Visible = rbtBillNo.Checked
        chkwithstustone.Visible = rbtBillNo.Checked
        chkwithstustone.Checked = False
    ...
    Private Sub rbtSummary_CheckedChanged(sender As Object, e As EventArgs) Handles rbtSummary.CheckedChanged
        ...
        chkwithstustone.Visible = False
        chkwithstustone.Checked = False
        ChkItem.Checked = False
    End Sub

    Private Sub rbtMonth_CheckedChanged(sender As Object, e As EventArgs) Handles rbtMonth.CheckedChanged
        ChkItem.Visible = False
        chkwithstustone.Visible = False
        chkwithstustone.Checked = False
        ChkItem.Checked = False
...
Public Class frmSalesReturnAbstractGST_Properties
