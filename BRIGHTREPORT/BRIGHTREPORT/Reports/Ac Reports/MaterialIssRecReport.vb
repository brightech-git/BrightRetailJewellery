...
Public Class DealerSmithLedger
    ...
    Private Sub Prop_Sets()
        Dim obj As New MaterialIssRecReport_properties
        'GetChecked_CheckedList(chkStudStone, obj.p_chkStudStone)
        ...
        obj.p_chkStudStone = chkStudStone.Checked

        'obj.p_chkWithOtherIssue = chkWithOtherIssue.Checked
        'obj.p_chkWithStuddedStone = chkWithStuddedStone.Checked
        SetSettingsObj(obj, Me.Name, GetType(MaterialIssRecReport_properties))
    End Sub
    Private Sub Prop_Gets()
        Dim obj As New MaterialIssRecReport_properties
        GetSettingsObj(obj, Me.Name, GetType(MaterialIssRecReport_properties))
        'SetChecked_CheckedList(chkStudStone, obj.p_chkStudStone, "ALL")
        ...
        chkStudStone.Checked = obj.p_chkStudStone
    ...
    Private Function DetailView(ByVal CName As String) As DataTable
        Dim Cashchange As String = GetSqlValue(cn, " SELECT CTLTEXT FROM " & cnAdminDb & ".. SOFTCONTROL WHERE CTLID ='RPT_CASHPAY_DELSMITH' ")
        If Cashchange = "" Then
            Cashchange = "N"
        End If
        Strsql = vbCrLf + " Exec " + cnStockDb + "..Sp_RPT_Dealer_SmithLedgerReport '" & strCompanyId & "', '" & Format(dtpFrom.Value, "yyyy-MM-dd") & "','" & Format(dtpTo.Value, "yyyy-MM-dd") & "'"
        Strsql += vbCrLf + ",'N'"
        Strsql += vbCrLf + ",'" + CustName + "'"
        Strsql += vbCrLf + ",'" & IIf(ChkWitApproval.Checked, "Y", "N") & "'"
        Strsql += vbCrLf + ",'" & IIf(chkStudStone.Checked, "Y", "N") & "'"
        Strsql += vbCrLf + ",'" & Cashchange & "'"
...
NNEXT:
    ...
    End Sub
End Class
Public Class MaterialIssRecReport_properties

    Private chkStudStone As Boolean = False
    Public Property p_chkStudStone() As Boolean
        Get
            Return chkStudStone
