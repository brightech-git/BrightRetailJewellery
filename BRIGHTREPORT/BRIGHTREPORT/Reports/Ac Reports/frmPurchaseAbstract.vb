...
Public Class frmPurchaseAbstract
    ...
    Function funcSep(ByVal diaType As String) As String
        ...
        Qry += " ,ISNULL((SELECT AMOUNT FROM " & cnStockDb & "..ACCTRAN WHERE BATCHNO = X.BATCHNO AND PAYMODE = 'SV' AND TRANNO = X.TRANNO AND COMPANYID = '" & strCompanyId & "' AND ACCODE = (SELECT PTAXID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE = X.CATCODE)),0)AS VAT   "
        ...
        Qry += " ,SUM(STNWT)WT"
        Qry += " FROM " & cnStockDb & "..RECEIPTSTONE AS S WHERE TRANTYPE = 'PU' AND (SELECT DIASTONE FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMID = STNITEMID) = '" & diaType & "' AND COMPANYID = '" & strCompanyId & "'"
        Qry += strFtr1
        ...
        Qry += " ,ISNULL((SELECT -1*AMOUNT FROM " & cnStockDb & "..ACCTRAN WHERE BATCHNO = X.BATCHNO AND PAYMODE = 'SV' AND TRANNO = X.TRANNO AND COMPANYID = '" & strCompanyId & "' AND ACCODE = (SELECT PTAXID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE = X.CATCODE)),0)AS VAT  "
    ...
    Function funcAbstract() As Integer
        ...
        strSql += " ,ISNULL((SELECT SUM(AMOUNT) FROM " & cnStockDb & "..ACCTRAN WHERE BATCHNO = X.BATCHNO AND PAYMODE = 'SV' AND TRANNO = X.TRANNO AND COMPANYID = '" & strCompanyId & "' AND ACCODE = (SELECT PTAXID FROM " & cnAdminDb & "..CATEGORY WHERE CATCODE = X.CATCODE)),0)AS VAT"
        ...
        strSql += " ,SUM(PCS)PCS,SUM(GRSWT)GRSWT,SUM(NETWT)NETWT,SUM(AMOUNT)AMOUNT"
        strSql += " ,ISNULL((SELECT SUM(STNWT) FROM " & cnStockDb & "..RECEIPTSTONE WHERE ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT WHERE BATCHNO = I.BATCHNO AND COMPANYID = '" & strCompanyId & "') AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'S') AND COMPANYID = '" & strCompanyId & "'),0)STNWT"
        strSql += " ,ISNULL((SELECT SUM(STNWT) FROM " & cnStockDb & "..RECEIPTSTONE WHERE ISSSNO IN (SELECT SNO FROM " & cnStockDb & "..RECEIPT WHERE BATCHNO = I.BATCHNO AND COMPANYID = '" & strCompanyId & "') AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'D') AND COMPANYID = '" & strCompanyId & "'),0)DIAWT"
    ...
    Function funcFiltration() As String
        ...
        '    Qry += " AND CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY WHERE METALID = (SELECT METALID FROM " & cnAdminDb & "..METALMAST WHERE METALNAME = '" & cmbMetal.Text & "'))"
        'End If
    ...
    Private Sub PurchaseAbs()
        ...
        Else
            strSql = " SELECT PARTICULAR,TRANNO,BILLNO AS REFNO,BILLDATE AS REFDATE,PCS,GRSWT,NETWT,CUSTOMER,AMOUNT," ',ECS,"
            strSql += vbCrLf + "ED,ECESS,HECESS,TAX,TOTAL,SC,STNCT,STNGT,DIAWT,MMONTHID,MMONTHNAME,TRANDATE,"
            strSql += vbCrLf + "CATNAME,METALNAME,RESULT,COLHEAD,ADDRESS,PHONENO,"
            strSql += vbCrLf + "ADDRESS1,ADDRESS2,ADDRESS3,AREA,CITY,SNO "
            If rbtBillNo.Checked And chkVA.Checked = True Then
                strSql += vbCrLf + ",MODE,AMT,CREDIT "
            End If
            strSql += vbCrLf + " FROM TEMPTABLEDB..TEMP" & systemId & "ABSSUMMARY_SA ORDER BY SNO"
        End If
        ...
        With gridView
            If chkEd.Checked = False And rbtBillNo.Checked = True Then
                If PUTYPE.Contains("PU") Then
                    If .Columns.Contains("PURITY") Then .Columns("PURITY").Visible = True
                End If
            Else
                If .Columns.Contains("PURITY") Then .Columns("PURITY").Visible = False
            End If
            ...
            If chkEd.Checked = True Then
                .Columns("ED").Visible = Chkecs.Checked
                .Columns("ECESS").Visible = Chkecs.Checked
                .Columns("HECESS").Visible = Chkecs.Checked
                If .Columns.Contains("ECS") Then .Columns("ECS").Visible = True
                If Chkecs.Checked = True Then
            ...
            '.Columns("BILLNO").HeaderText = "REFNO"
            ' .Columns("BILLDATE").HeaderText = "REFDATE"
    ...
    Private Sub Prop_Sets()
        Dim obj As New frmPurchaseAbstract_Properties
        ...
        SetSettingsObj(obj, Me.Name, GetType(frmPurchaseAbstract_Properties))
    End Sub

    Private Sub Prop_Gets()
        Dim obj As New frmPurchaseAbstract_Properties
        GetSettingsObj(obj, Me.Name, GetType(frmPurchaseAbstract_Properties))
    ...
    Private Sub chkEd_CheckStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkEd.CheckStateChanged
        If chkEd.Checked = True Then
            Chkecs.Visible = True
        Else
...
Public Class frmPurchaseAbstract_Properties
