...
Public Class frmCatTrading
    ...
    Private Sub funcViewMulCost(ByVal strcompid As String, ByVal strCostId As String, ByVal summary As Boolean)
        ...
        If gridView.RowCount > 0 Then
            ...
            For M As Integer = 0 To DtCostid.Rows.Count - 1
                ...
                Next
            Next
            lblTitle.Text = cmbCompany.Text & "TRADING & PROFIT AND LOSS FOR THE PERIOD OF - 01/04/" & (IIf(dtpFrom.Value.Date.Month > 3, dtpFrom.Value.Date.Year, dtpFrom.Value.Date.Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/")
    ...
    Private Sub funcTradingMulCost(ByVal strcompid As String, ByVal strCostId As String, ByVal summary As Boolean)
        ...
        strSql += vbCrLf + "  FROM " & cnStockDb & "..ACCTRAN WHERE ISNULL(COMPANYID,'') = '" & strcompid & "' AND TRANDATE >= '" & cnTranFromDate & "' AND TRANDATE <= '" & dtpFrom.Value.Date.ToString("yyyy-MM-dd") & "' AND ISNULL(CANCEL,'') != 'Y'"
        strSql += strCostId
        'strSql += vbCrLf + " UNION ALL"
        ...
        If summary Then
            ...
            If Closin_stk = "N" Then 'Val(objGPack.GetSqlValue("SELECT COUNT(*) FROM " & cnStockDb & "..CLOSINGVAL WHERE 1=1 " & strCostId, , , )) > 0 Then
                strSql += vbCrLf + " UNION ALL"
            ...
            Else
                For K As Integer = 0 To DtCostid.Rows.Count - 1
                    ...
                    Qry += vbCrLf + " ,@FRMDATE = '" & Format(dtpFrom.Value, "yyyy-MM-dd") & "'"
                    Qry += vbCrLf + " ,@TODATE = '" & Format(dtpFrom.Value, "yyyy-MM-dd") & "'"
        ...
        For j As Integer = 0 To DtCostid.Rows.Count - 1
            strSql += vbCrLf + " 'TRADING - 01/04/" & (IIf(dtpFrom.Value.[Date].Month > 3, dtpFrom.Value.[Date].Year, dtpFrom.Value.[Date].Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/") & "' EXPACGROUPNAME1" & DtCostid.Rows(j).Item(0).ToString.Trim & ""
    ...
    Private Sub funcPLMulCost(ByVal strcompid As String, ByVal strCostId As String, ByVal summary As Boolean)
        ...
        strSql = vbCrLf + " SELECT DISTINCT "
        For J As Integer = 0 To DtCostid.Rows.Count - 1
            strSql += vbCrLf + " 'P & L - 01/04/" & (IIf(dtpFrom.Value.[Date].Month > 3, dtpFrom.Value.[Date].Year, dtpFrom.Value.[Date].Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/") & "' EXPACGROUPNAME1" & DtCostid.Rows(J).Item(0).ToString.Trim & ","
            strSql += vbCrLf + " ' 'EXPENSES1" & DtCostid.Rows(J).Item(0).ToString.Trim & ",' ' EXPENSES2" & DtCostid.Rows(J).Item(0).ToString.Trim & ",' 'EXPENSES" & DtCostid.Rows(J).Item(0).ToString.Trim & " "
            strSql += vbCrLf + " ,' ' INCACGROUPNAME1" & DtCostid.Rows(J).Item(0).ToString.Trim & ",' 'INCOME1" & DtCostid.Rows(J).Item(0).ToString.Trim & ",' 'INCOME2" & DtCostid.Rows(J).Item(0).ToString.Trim & ",' 'INCOME" & DtCostid.Rows(J).Item(0).ToString.Trim & ","
            If J = DtCostid.Rows.Count - 1 Then
                strSql += vbCrLf + " 'T' COLHEAD3" & DtCostid.Rows(J).Item(0).ToString.Trim & ",'T' COLHEAD4" & DtCostid.Rows(J).Item(0).ToString.Trim & ""
    ...
    Private Sub Report()
        ...
        If chkAsOnDate.Checked Then
            ...
            strSql += vbCrLf + ",@ASONDATE ='" & Format(dtpFrom.Value, "yyyy-MM-dd") & "' "
        Else
            strSql += vbCrLf + ",@CNDATE ='" & Format(dtpFrom.Value, "yyyy-MM-dd") & "'"
            strSql += vbCrLf + ",@ASONDATE ='" & Format(dtpTo.Value, "yyyy-MM-dd") & "' "
        End If
        strSql += vbCrLf + ",@TEMPTABLE='TEMPTABLEDB..TEMP" & systemId & "TRADING'"
        strSql += vbCrLf + ",@COMPANYID='" & strCompId & "'"
        strSql += vbCrLf + ",@COSTIDS='" & IIf(chkCostName = "", "ALL", chkCostName) & "'"
        strSql += vbCrLf + ",@SYSTEMID='" & systemId & "'"
        strSql += vbCrLf + ",@PL='" & IIf(ChkPandL.Checked, "Y", "N") & "'"
        strSql += vbCrLf + ",@WITHOTHERS='" & IIf(chkOthers.Checked, "Y", "N") & "'"
        ...
        If dt.Rows.Count > 0 Then
            With gridView
                ...
                If chkTrading.Checked Then
                    ...
                    .Columns("CGROUP").Visible = False
                    .Columns("WGT1").HeaderText = "WGT"
                    .Columns("WGT2").HeaderText = "WGT"
                    .Columns("EXPENDITURES").HeaderText = "DEBIT"
                    .Columns("INCOME").HeaderText = "CREDIT"
                Else
                    .Columns("SEP").Visible = False
                    .Columns("EXPACMAINCODE").Visible = False
                End If
            ...
            End With
            If chkTrading.Checked Then
                lblTitle.Text = cmbCompany.Text & vbCrLf & "TRADING ACCOUNT FOR THE PERIOD OF - 01/04/" & (IIf(dtpFrom.Value.Date.Month > 3, dtpFrom.Value.Date.Year, dtpFrom.Value.Date.Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/")
            Else
                lblTitle.Text = cmbCompany.Text & vbCrLf & "PROFIT AND LOSS FOR THE PERIOD OF - 01/04/" & (IIf(dtpFrom.Value.Date.Month > 3, dtpFrom.Value.Date.Year, dtpFrom.Value.Date.Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/")
            End If
    ...
    Private Sub funcView()
        ...
        If gridView.RowCount > 0 Then
            ...
            gridView.Columns("EXPENSES").HeaderText = "AMOUNT"
            ...
            gridView.Columns("EXPENSES1").HeaderText = "AMOUNT"
            ...
            gridView.Columns("EXPENSES2").HeaderText = "AMOUNT"
            ...
            gridView.Columns("INCOME").HeaderText = "AMOUNT"
            ...
            gridView.Columns("INCOME1").HeaderText = "AMOUNT"
            ...
            gridView.Columns("INCOME2").HeaderText = "AMOUNT"
            ...
            For i As Integer = 0 To gridView.RowCount - 1
                ...
                End If
            Next
            lblTitle.Text = cmbCompany.Text & vbCrLf & "TRADING & PROFIT AND LOSS FOR THE PERIOD OF - 01/04/" & (IIf(dtpFrom.Value.Date.Month > 3, dtpFrom.Value.Date.Year, dtpFrom.Value.Date.Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/")
            If chkCmbCostCentre.Text <> "" Then lblTitle.Text = lblTitle.Text & vbCrLf & " COST CENTRE :" & chkCmbCostCentre.Text
            gridView.Focus()
        Else
            MessageBox.Show("Message", "Records not found...", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    ...
    End Sub
    Private Sub funcTrading(ByVal strcompid As String, ByVal strCostId As String, ByVal summary As Boolean)
        strSql = "  IF EXISTS (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & userId.ToString() & "TRADE1')"
        strSql += "  DROP TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE1"
        strSql += "  CREATE TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE1"
        strSql += "  (COMPANYID VARCHAR(3),ACID VARCHAR(25),TRANMODE VARCHAR(1),AMOUNT NUMERIC(15,2),DEBIT NUMERIC(15,2),CREDIT NUMERIC(15,2)"
        strSql += "  ,TRANDATE SMALLDATETIME,PAYMODE VARCHAR(20),TRANNO VARCHAR(25),SEP VARCHAR(1))"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        strSql += " (COMPANYID,COSTID,CATCODE,CLOSING,RATE,VALUE,UPDATED,UPTIME)"
        strSql = "  INSERT INTO TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE1" & vbCrLf
        ...
        strSql += "  FROM " & cnStockDb & "..ACCTRAN WHERE ISNULL(COMPANYID,'') = '" & strcompid & "' AND TRANDATE >= '" & cnTranFromDate & "' AND TRANDATE <= '" & dtpFrom.Value.Date.ToString("yyyy-MM-dd") & "' AND ISNULL(CANCEL,'') != 'Y'" & vbCrLf
        strSql += strCostId
        'strSql += " UNION ALL" & vbCrLf
        'strSql += " (COMPANYID,COSTID,CATCODE,CLOSING,RATE,VALUE,UPDATED,UPTIME)"
        'strSql += " SELECT" & vbCrLf
        ...
        strSql = "  IF EXISTS (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & userId.ToString() & "TRADE2')" & vbCrLf
        strSql += "  DROP TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2" & vbCrLf
        If Summary Then
            strSql += "  SELECT " & vbCrLf
            ...
            strSql += "  INTO TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2" & vbCrLf
            strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE1 AS T INNER JOIN " & cnAdminDb & "..ACHEAD AS A" & vbCrLf
            ...
            strSql += "  AND (SG.GRPLEDGER IN ('T') OR G.GRPLEDGER IN ('T')) " & vbCrLf
            strSql += "  GROUP BY G.ACGRPCODE,SG.ACGRPNAME,G.ACMAINCODE,SG.GRPLEDGER,SG.GRPTYPE"
            If Closin_stk = "N" Then  'Val(objGPack.GetSqlValue("SELECT COUNT(*) FROM " & cnStockDb & "..CLOSINGVAL WHERE 1=1 " & strCostId, , , )) > 0 Then
                strSql += " UNION ALL"
                strSql += " SELECT 'CLSTK' ACID,'0' ACGRPCODE,'CLOSING STOCK' ACGRPNAME,'0' ACMAINCODE"
                strSql += " ,'T' GRPLEDGER,CASE WHEN SUM(VALUE) > 0 THEN 'I' ELSE 'E' END GRPTYPE"
                strSql += " ,CASE WHEN SUM(VALUE) < 0 THEN SUM(VALUE)*(-1) END DEBIT"
                strSql += " ,CASE WHEN SUM(VALUE) > 0 THEN SUM(VALUE) END CREDIT"
                strSql += " ,'CLOSING STOCK' ACNAME,'' TRANDATE"
                strSql += " ,CASE WHEN SUM(VALUE) > 0 THEN 'D' ELSE 'C' END TRANMODE,SUM(VALUE) AMOUNT"
                strSql += " ,'' PAYMODE,'' TRANNO,'CLOSING STOCK' ACMAINGRPNAME"
                strSql += " FROM " & cnStockDb & "..CLOSINGVAL WHERE ISNULL(COMPANYID,'') = '" & strCompId & "'" & vbCrLf
                strSql += strCostId
                strSql += " HAVING SUM(VALUE) <> 0"
            Else
                Dim Qry As String = ""
                'ProgressBarShow()
                If Closin_stk = "W" Then
                ...
                strSql += " UNION ALL"
                strSql += " SELECT 	'CLSTK' ACID,'0' ACGRPCODE,'CLOSING STOCK' ACGRPNAME,'0' ACMAINCODE ,'T' GRPLEDGER,"
                strSql += " CASE WHEN SUM(CAMOUNT) > 0 THEN 'I' ELSE 'E' END GRPTYPE ,CASE WHEN SUM(CAMOUNT) < 0 THEN SUM(CAMOUNT)*(-1) END DEBIT,"
                strSql += " CASE WHEN SUM(CAMOUNT) > 0 THEN SUM(CAMOUNT) END CREDIT ,'CLOSING STOCK' ACNAME,'' TRANDATE "
                strSql += " ,CASE WHEN SUM(CAMOUNT) > 0 THEN 'D' ELSE 'C' END TRANMODE"
                strSql += " ,SUM(CAMOUNT) AMOUNT ,'' PAYMODE,'' TRANNO,'CLOSING STOCK' ACMAINGRPNAME "
                strSql += " FROM TEMPTABLEDB..TEMPCSTKVALUE4" & SYSTEMID & " A "
                strSql += " WHERE A.RESULT=1"
                strSql += " HAVING SUM(CAMOUNT) <> 0"
            End If
        Else
            strSql += "  SELECT " & vbCrLf
            strSql += "   A.ACCODE ACID,G.ACGRPCODE,SG.ACGRPNAME ACGRPNAME,G.ACMAINCODE,SG.GRPLEDGER,SG.GRPTYPE" & vbCrLf
            'strSql += "   A.ACCODE ACID,G.ACGRPCODE,ACGRPNAME ACGRPNAME,ACMAINCODE,G.GRPLEDGER,G.GRPTYPE" & vbCrLf
            ...
            strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE1 AS T INNER JOIN " & cnAdminDb & "..ACHEAD AS A" & vbCrLf
            strSql += "  ON T.ACID = A.ACCODE INNER JOIN " & cnAdminDb & "..ACGROUP AS G ON G.ACGRPCODE = A.ACGRPCODE" & vbCrLf
            strSql += "  left JOIN " & cnAdminDb & "..ACGROUP AS SG ON SG.ACGRPCODE = A.ACGRPCODE"
            strSql += "  WHERE (SG.GRPLEDGER IN ('T') OR G.GRPLEDGER IN ('T'))" & vbCrLf
            If Closin_stk = "N" Then 'Val(objGPack.GetSqlValue("SELECT COUNT(*) FROM " & cnStockDb & "..CLOSINGVAL WHERE 1=1 " & strCostId, , , )) > 0 Then
                strSql += " UNION ALL"
                strSql += " SELECT 	C.CLSCODE ACID,H.ACGRPCODE,G.acgrpname ACGRPNAME,'0' ACMAINCODE 	,'T' GRPLEDGER,"
                strSql += " CASE WHEN SUM(VALUE) > 0 THEN 'I' ELSE 'E' END GRPTYPE 	,CASE WHEN SUM(VALUE) < 0 THEN SUM(VALUE)*(-1) END DEBIT 	,"
                strSql += " CASE WHEN SUM(VALUE) > 0 THEN SUM(VALUE) END CREDIT 	,H.ACNAME  ACNAME,'' TRANDATE 	,CASE WHEN SUM(VALUE) > 0 THEN 'D' ELSE 'C' END TRANMODE,SUM(VALUE) AMOUNT 	,'' PAYMODE,'' TRANNO,'CLOSING STOCK' ACMAINGRPNAME "
                strSql += " FROM " & cnStockDb & "..CLOSINGVAL A LEFT JOIN " & cnAdminDb & "..CATEGORY C ON A.CATCODE = C.CATCODE LEFT JOIN " & cnAdminDb & "..ACHEAD H ON C.CLSCODE = H.ACCODE "
                strSql += " LEFT JOIN " & cnAdminDb & "..acgroup G ON H.ACGRPCODE = G.ACGRPCODE"
                strSql += " WHERE ISNULL(A.COMPANYID,'') = '" & strCompId & "'" & vbCrLf
                strSql += strCostId + " GROUP BY H.ACGRPCODE,C.CLSCODE,H.ACNAME,G.ACGRPNAME"
                strSql += " HAVING SUM(VALUE) <> 0"
            Else
                'ProgressBarShow()
                Dim Qry As String = ""
                ...
                strSql += " SELECT 	C.CLSCODE ACID,H.ACGRPCODE,G.ACGRPNAME ACGRPNAME,'0' ACMAINCODE ,'T' GRPLEDGER,"
                strSql += " CASE WHEN SUM(CAMOUNT) > 0 THEN 'I' ELSE 'E' END GRPTYPE ,CASE WHEN SUM(CAMOUNT) < 0 THEN SUM(CAMOUNT)*(-1) END DEBIT 	,"
                strSql += " CASE WHEN SUM(CAMOUNT) > 0 THEN SUM(CAMOUNT) END CREDIT ,H.ACNAME ACNAME,'' TRANDATE ,CASE WHEN SUM(CAMOUNT) > 0 THEN 'D' ELSE 'C' END TRANMODE,SUM(CAMOUNT) AMOUNT ,'' PAYMODE,'' TRANNO,'CLOSING STOCK' ACMAINGRPNAME "
                strSql += " FROM TEMPTABLEDB..TEMPCSTKVALUE4" & SYSTEMID & " A "
                strSql += " LEFT JOIN " & cnAdminDb & "..CATEGORY C ON A.CATEGORY = C.CATNAME "
                strSql += " LEFT JOIN " & cnAdminDb & "..ACHEAD H ON C.CLSCODE = H.ACCODE "
                strSql += " LEFT JOIN " & cnAdminDb & "..ACGROUP G ON H.ACGRPCODE = G.ACGRPCODE"
                strSql += " WHERE A.RESULT=1"
                strSql += " GROUP BY H.ACGRPCODE,C.CLSCODE,H.ACNAME,G.ACGRPNAME"
                strSql += " HAVING SUM(CAMOUNT) <> 0"
        ...
        strSql = "  IF EXISTS (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & userId.ToString() & "TRADE3')" & vbCrLf
        strSql += "  DROP TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE3" & vbCrLf
        strSql += "  CREATE TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE3" & vbCrLf
        ...
        strSql += "  )" & vbCrLf
        strSql += "  " & vbCrLf
        strSql += "  IF EXISTS (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & userId.ToString() & "TRADE4')" & vbCrLf
        strSql += "  DROP TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE4" & vbCrLf
        strSql += "  CREATE TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE4" & vbCrLf
        strSql += "  (EXPACGROUPNAME1 VARCHAR(250),EXPENSES1 VARCHAR(50),EXPENSES2 VARCHAR(50),EXPENSES VARCHAR(50)" & vbCrLf
        strSql += "  ,INCACGROUPNAME1 VARCHAR(250),INCOME1 VARCHAR(50),INCOME2 VARCHAR(50),INCOME VARCHAR(50)" & vbCrLf
        strSql += "  ,RESULT INT,SEP INT,EXPACMAINCODE VARCHAR(50),INCACMAINCODE VARCHAR(50),EXPACGROUP VARCHAR(250),INCACGROUP VARCHAR(250)" & vbCrLf
        strSql += "  ,EXPACID VARCHAR(50),EXPACNAME VARCHAR(250),EXPACGROUPNAME VARCHAR(250),EXPACMAINGRPNAME VARCHAR(250)" & vbCrLf
        ...
        strSql = "  INSERT INTO TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE3" & vbCrLf
        strSql += "  (EXPACGROUPNAME1,EXPENSES1,EXPENSES2,EXPENSES,INCACGROUPNAME1,INCOME1,INCOME2,INCOME,RESULT,SEP,EXPACMAINCODE,INCACMAINCODE,EXPACGROUP,INCACGROUP,EXPACID" & vbCrLf
        strSql += "  ,EXPACNAME,EXPACGROUPNAME,EXPACMAINGRPNAME,INCACID,INCACNAME,INCACGROUPNAME,INCACMAINGRPNAME,COLHEAD)" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT ACMAINGRPNAME EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES1,NULL EXPENSES2,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'1' RESULT,'1' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,'' EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'T1' COLHEAD" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E'" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '  ' + ACGRPNAME EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'2' RESULT,'2' SEP" & vbCrLf
        ...
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'T2' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E' AND ISNULL(ACMAINCODE,'') != ISNULL(ACGRPCODE,'')" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT CASE WHEN ACMAINGRPNAME = ACGRPNAME THEN '  ' ELSE '    ' END + ACNAME EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME != ACGRPNAME THEN CONVERT(VARCHAR,ISNULL(SUM(DEBIT),0) - ISNULL(SUM(CREDIT),0)) END EXPENSES2" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME = ACGRPNAME THEN CONVERT(VARCHAR,ISNULL(SUM(DEBIT),0) - ISNULL(SUM(CREDIT),0)) END EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'3' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,ACGRPCODE EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,ACID EXPACID,ACNAME EXPACNAME,ACGRPNAME EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E'" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACGRPNAME,ACMAINCODE,ACGRPCODE,ACID,ACNAME" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME != ACGRPNAME THEN '-------------------' END EXPENSES2" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME  = ACGRPNAME THEN '-------------------' END EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'4' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,ACGRPCODE EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,ACGRPNAME EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'S2' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E'" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACGRPNAME,ACMAINCODE,ACGRPCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT ' ' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES1,CONVERT(VARCHAR,ISNULL(SUM(DEBIT),0) - ISNULL(SUM(CREDIT),0)) EXPENSES2,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'5' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,ACGRPCODE EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,ACGRPNAME EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'S2' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E' AND ISNULL(ACMAINCODE,'') != ISNULL(ACGRPCODE,'')" & vbCrLf
        ...
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '  ' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES1,'-------------------' EXPENSES2,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'6' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,ACGRPCODE EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,ACGRPNAME EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'S1' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E' AND ISNULL(ACMAINCODE,'') != ISNULL(ACGRPCODE,'')" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACGRPNAME,ACMAINCODE,ACGRPCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,CONVERT(VARCHAR,ISNULL(SUM(DEBIT),0) - ISNULL(SUM(CREDIT),0)) EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME" & vbCrLf
        strSql += "  ,'7' RESULT,'3' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,'' EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'G1' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E'" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACMAINCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,'-------------------' EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME" & vbCrLf
        strSql += "  ,'8' RESULT,'4' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,'' EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,ACMAINGRPNAME EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,'' INCACMAINGRPNAME,'G1' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'E'" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACMAINCODE" & vbCrLf
        strSql += "  ORDER BY EXPACMAINGRPNAME,SEP,EXPACGROUPNAME,RESULT,EXPACNAME" & vbCrLf
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = "  INSERT INTO TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE4" & vbCrLf
        strSql += "  (EXPACGROUPNAME1,EXPENSES1,EXPENSES2,EXPENSES,INCACGROUPNAME1,INCOME1,INCOME2,INCOME,RESULT,SEP,EXPACMAINCODE,INCACMAINCODE,EXPACGROUP,INCACGROUP,EXPACID" & vbCrLf
        strSql += "  ,EXPACNAME,EXPACGROUPNAME,EXPACMAINGRPNAME,INCACID,INCACNAME,INCACGROUPNAME,INCACMAINGRPNAME,COLHEAD)" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT NULL EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES1,NULL EXPENSES2,NULL EXPENSES" & vbCrLf
        strSql += "  ,ACMAINGRPNAME INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,NULL INCOME,'1' RESULT,'1' SEP" & vbCrLf
        strSql += "  ,'' EXPACMAINCODE,ACMAINCODE INCACMAINCODE,'' EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,'' INCACID,''INCACNAME,''INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'T1' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'I'" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT NULL EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,NULL EXPENSES" & vbCrLf
        ...
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,ACGRPNAME INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'T2' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'I' AND ISNULL(ACMAINCODE,'') != ISNULL(ACGRPCODE,'')" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME = ACGRPNAME THEN '  ' ELSE '    ' END + ACNAME INCACGROUPNAME1" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME != ACGRPNAME THEN CONVERT(VARCHAR,ISNULL(SUM(CREDIT),0) - ISNULL(SUM(DEBIT),0)) END INCOME2" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME  = ACGRPNAME THEN CONVERT(VARCHAR,ISNULL(SUM(CREDIT),0) - ISNULL(SUM(DEBIT),0)) END INCOME1,NULL INCOME,'3' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,'' EXPACMAINCODE,ACMAINCODE INCACMAINCODE,'' EXPACGROUP,ACGRPCODE INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        ...
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,'' EXPENSES2,NULL EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME != ACGRPNAME THEN '-------------------' END INCOME2" & vbCrLf
        strSql += "  ,CASE WHEN ACMAINGRPNAME  = ACGRPNAME THEN '-------------------' END INCOME1,NULL INCOME,'4' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,'' EXPACMAINCODE,ACMAINCODE INCACMAINCODE,'' EXPACGROUP,ACGRPCODE INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,ACGRPNAME INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'S2' COLHEAD" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'I'" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACGRPNAME,ACMAINCODE,ACGRPCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        ...
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,CONVERT(VARCHAR,ISNULL(SUM(CREDIT),0) - ISNULL(SUM(DEBIT),0)) INCOME1,NULL INCOME,'5' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,'' EXPACMAINCODE,ACMAINCODE INCACMAINCODE,'' EXPACGROUP,ACGRPCODE INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,ACGRPNAME INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'S1' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'I' AND ISNULL(ACMAINCODE,'') != ISNULL(ACGRPCODE,'')" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACGRPNAME,ACMAINCODE,ACGRPCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  DISTINCT '  ' EXPACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL EXPENSES2,'' EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,'-------------------' INCOME1,NULL INCOME,'6' RESULT,'2' SEP" & vbCrLf
        strSql += "  ,ACMAINCODE EXPACMAINCODE,'' INCACMAINCODE,ACGRPCODE EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME ,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,ACGRPNAME INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'S2' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'I' AND ISNULL(ACMAINCODE,'') != ISNULL(ACGRPCODE,'')" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACGRPNAME,ACMAINCODE,ACGRPCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        ...
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,NULL EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,CONVERT(VARCHAR,ISNULL(SUM(CREDIT),0) - ISNULL(SUM(DEBIT),0)) INCOME" & vbCrLf
        strSql += "  ,'7' RESULT,'3' SEP" & vbCrLf
        strSql += "  ,'' EXPACMAINCODE,ACMAINCODE INCACMAINCODE,'' EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'G1' COLHEAD" & vbTab + Environment.NewLine
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE2 WHERE ISNULL(GRPTYPE,'') = 'I'" & vbCrLf
        strSql += "  GROUP BY ACMAINGRPNAME,ACMAINCODE" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        ...
        strSql += "  ,NULL EXPENSES2,NULL EXPENSES1,'' EXPENSES" & vbCrLf
        strSql += "  ,NULL INCACGROUPNAME1" & vbCrLf
        strSql += "  ,NULL INCOME2,NULL INCOME1,'-------------------' INCOME" & vbCrLf
        strSql += "  ,'8' RESULT,'4' SEP" & vbCrLf
        strSql += "  ,'' EXPACMAINCODE,ACMAINCODE INCACMAINCODE,'' EXPACGROUP,'' INCACGROUP" & vbCrLf
        strSql += "  ,'' EXPACID,'' EXPACNAME,'' EXPACGROUPNAME,'' EXPACMAINGRPNAME" & vbCrLf
        strSql += "  ,''INCACID,''INCACNAME,''INCACGROUPNAME,ACMAINGRPNAME INCACMAINGRPNAME,'G1' COLHEAD" & vbTab + Environment.NewLine
        ...
        strSql = "  IF EXISTS(SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & userId.ToString() & "TRADE5')" & vbCrLf
        strSql += "  DROP TABLE TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  T3.EXPACGROUPNAME1,T3.EXPENSES1,T3.EXPENSES2,T3.EXPENSES,T4.INCACGROUPNAME1,T4.INCOME1,T4.INCOME2,T4.INCOME" & vbCrLf
        ...
        strSql += "  INTO TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE3 AS T3 FULL JOIN TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE4 AS T4 " & vbCrLf
        strSql += "  ON T3.SNO = T4.SNO" & vbCrLf
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = " SELECT " & vbCrLf
        strSql += " DISTINCT 'TRADING - 01/04/" & (IIf(dtpFrom.Value.[Date].Month > 3, dtpFrom.Value.[Date].Year, dtpFrom.Value.[Date].Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/") & "' EXPACGROUPNAME1,' 'EXPENSES1,' ' EXPENSES2,' 'EXPENSES " & vbCrLf
        strSql += " ,' ' INCACGROUPNAME1,' 'INCOME1,' 'INCOME2,' 'INCOME,'T' COLHEAD3,'T' COLHEAD4" & vbCrLf
        strSql += " FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        strSql += " UNION ALL" & vbCrLf
        strSql += "  SELECT " & vbCrLf
        strSql += "  EXPACGROUPNAME1" & vbCrLf
        strSql += "     ,CASE WHEN RESULT3 NOT IN (4,6,8) AND ISNULL(EXPENSES1,'') != '' THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(EXPENSES1,'') = '' THEN '0' ELSE EXPENSES1 END),'D','Y') ELSE EXPENSES1 END  EXPENSES1" & vbCrLf
        strSql += "     ,CASE WHEN RESULT3 NOT IN (4,6,8) AND ISNULL(EXPENSES2,'') != '' THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(EXPENSES2,'') = '' THEN '0' ELSE EXPENSES2 END),'D','Y') ELSE EXPENSES2 END EXPENSES2" & vbCrLf
        strSql += "     ,CASE WHEN RESULT3 NOT IN (4,6,8) AND ISNULL(EXPENSES,'') != '' THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(EXPENSES,'') = '' THEN '0' ELSE EXPENSES END),'D','Y') ELSE EXPENSES END EXPENSES " & vbCrLf
        strSql += "  ,INCACGROUPNAME1" & vbCrLf
        strSql += "     ,CASE WHEN RESULT4 NOT IN (4,6,8) AND ISNULL(INCOME1,'') != '' THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(INCOME1,'') = '' THEN '0' ELSE INCOME1 END),'D','Y') ELSE INCOME1 END  INCOME1" & vbCrLf
        strSql += "     ,CASE WHEN RESULT4 NOT IN (4,6,8) AND ISNULL(INCOME2,'') != '' THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(INCOME2,'') = '' THEN '0' ELSE INCOME2 END),'D','Y') ELSE INCOME2 END INCOME2" & vbCrLf
        strSql += "     ,CASE WHEN RESULT4 NOT IN (4,6,8) AND ISNULL(INCOME,'') != '' THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(INCOME,'') = '' THEN '0' ELSE INCOME END),'D','Y') ELSE INCOME END INCOME " & vbCrLf
        strSql += "     ,COLHEAD3,COLHEAD4" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT" & vbCrLf
        strSql += "    '' EXPACGROUPNAME1 ,NULL EXPENSES1,NULL EXPENSES2,CONVERT(VARCHAR," & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') AND ISNULL(EXPENSES,'') = '' THEN CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(EXPENSES,'') = '' THEN '0' ELSE EXPENSES END) ELSE '0' END)),'D','Y')) EXPENSES" & vbCrLf
        strSql += "  ,'' INCACGROUPNAME1 ,NULL INCOME1,NULL INCOME2,CONVERT(VARCHAR," & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') AND ISNULL(INCOME,'') = '' THEN CONVERT(NUMERIC(15,2),CASE WHEN ISNULL(INCOME,'') = '' THEN '0' ELSE INCOME END) ELSE '0' END)),'D','Y')) INCOME,'G1' COLHEAD3,'G1' COLHEAD4" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT" & vbCrLf
        strSql += "   DISTINCT '' EXPACGROUPNAME1 ,NULL EXPENSES1,NULL EXPENSES2,'-------------------' EXPENSES" & vbCrLf
        strSql += "  ,'' INCACGROUPNAME1 ,NULL INCOME1,NULL INCOME2,'-------------------' INCOME,'G1' COLHEAD3,'G1' COLHEAD4" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        strSql += "  UNION ALL" & vbCrLf
        strSql += "  SELECT" & vbCrLf
        strSql += "    CASE WHEN SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') THEN EXPENSES ELSE '0' END)) - SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') THEN INCOME ELSE '0' END)) > 0 THEN 'GROSS LOSS' END EXPACGROUPNAME1 ,NULL EXPENSES1,NULL EXPENSES2" & vbCrLf
        strSql += "  ,CONVERT(VARCHAR,CASE WHEN SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') THEN EXPENSES ELSE '0' END)) - SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') THEN INCOME ELSE '0' END)) > 0 THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') THEN EXPENSES ELSE '0' END)) - SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') THEN INCOME ELSE '0' END)),'D','Y') END) EXPENSES" & vbCrLf
        strSql += "  ,CASE WHEN SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') THEN INCOME ELSE '0' END)) - SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') THEN EXPENSES ELSE '0' END)) > 0 THEN 'GROSS PROFIT' END INCACGROUPNAME1 ,NULL INCOME1,NULL INCOME2" & vbCrLf
        strSql += "  ,CONVERT(VARCHAR,CASE WHEN SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') THEN INCOME ELSE '0' END)) - SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') THEN EXPENSES ELSE '0' END)) > 0 THEN " & cnStockDb & ".[DBO].FUNC_RUPEES_WITHCOMMA(SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT4 IN ('0','7') THEN INCOME ELSE '0' END)) - SUM(CONVERT(NUMERIC(15,2),CASE WHEN RESULT3 IN ('0','7') THEN EXPENSES ELSE '0' END)),'D','Y') END) INCOME,'G' COLHEAD3,'G' COLHEAD4" & vbCrLf
        strSql += "  FROM TEMPTABLEDB..TEMP" & systemId & userId.ToString() & "TRADE5" & vbCrLf
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dt)
        If dt.Rows.Count <= 2 Then
    ...
    Private Sub funcPL(ByVal strcompid As String, ByVal strCostId As String, ByVal summary As Boolean)
        ...
        strSql += "    - ISNULL((CASE WHEN GRPTYPE = 'I' THEN ISNULL(CREDIT,0) - ISNULL(DEBIT,0) END),0)),0) - ISNULL((SELECT SUM(VALUE) FROM " & cnStockDb & "..CLOSINGVAL WHERE COMPANYID = '" & strcompid & "'" & strCostId & "),0) AMOUNT" & vbCrLf
        ...
        strSql = " SELECT " & vbCrLf
        strSql += " DISTINCT 'P & L - 01/04/" & (IIf(dtpFrom.Value.[Date].Month > 3, dtpFrom.Value.[Date].Year, dtpFrom.Value.[Date].Year - 1)).ToString() & " - " & dtpFrom.Text.Replace("-", "/") & "' EXPACGROUPNAME1,' 'EXPENSES1,' ' EXPENSES2,' 'EXPENSES " & vbCrLf
        strSql += " ,' ' INCACGROUPNAME1,' 'INCOME1,' 'INCOME2,' 'INCOME,'T' COLHEAD3,'T' COLHEAD4" & vbCrLf
    ...
    Private Sub btnExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        If gridView.RowCount > 0 Then
            BrightPosting.GExport.Post(Me.Name, "", lblTitle.Text, gridView, BrightPosting.GExport.GExportType.Export)
        End If
    End Sub
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        If gridView.RowCount > 0 Then
            BrightPosting.GExport.Post(Me.Name, "", lblTitle.Text, gridView, BrightPosting.GExport.GExportType.Print)
        End If
    End Sub
    Private Sub Prop_Sets()
        Dim obj As New frmCatTrading_Properties
        obj.p_cmbCompany = cmbCompany.Text
        obj.p_chkTrade = chkTrading.Checked
        obj.p_chkPandL = ChkPandL.Checked
        SetSettingsObj(obj, Me.Name, GetType(frmCatTrading_Properties))
    End Sub
    Private Sub Prop_Gets()
        Dim obj As New frmCatTrading_Properties
        GetSettingsObj(obj, Me.Name, GetType(frmCatTrading_Properties))
        cmbCompany.Text = obj.p_cmbCompany
        chkTrading.Checked = obj.p_chkTrade
        ChkPandL.Checked = obj.p_chkPandL
    End Sub

    Private Sub ChkPandL_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ChkPandL.CheckedChanged
        chkTrading.Checked = Not ChkPandL.Checked
    End Sub

    Private Sub chkTrading_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkTrading.CheckedChanged
        ChkPandL.Checked = Not chkTrading.Checked
        If chkTrading.Checked Then
...
Public Class frmCatTrading_Properties
    ...
    Public Property p_chkCmbCostCentre() As List(Of String)
        ...
        End Set
    End Property
    Private chkPandL As Boolean = False
    Public Property p_chkPandL() As Boolean
        Get
            Return chkPandL
        End Get
        ...
        End Set
    End Property
    Private chkTrade As Boolean = False
    Public Property p_chkTrade() As Boolean
        Get
            Return chkTrade
        End Get
        Set(ByVal value As Boolean)
