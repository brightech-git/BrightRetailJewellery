...
Imports System.IO
Public Class frmSalesWiseDetail
    ...
    Private Sub SalesAbs()
        Try
            ...
            gridView.DataSource = Nothing
            Me.Refresh()
            strSql = " EXEC " & cnStockDb & "..SP_RPT_SALESABSTRACT_SALESWISEDETAIL"
            ...
            strSql = " SELECT MAX(TRANNO) TRANNO FROM " & cnStockDb & "..ISSUE WHERE TRANTYPE IN ('SA') AND TRANDATE BETWEEN '" & dtpFrom.Value.ToString("yyyy-MM-dd") & "' AND '" & dtpTo.Value.ToString("yyyy-MM-dd") & "' "
            ...
            strSql = " SELECT MIN(TRANNO) TRANNO FROM " & cnStockDb & "..ISSUE WHERE TRANTYPE IN ('SA') AND TRANDATE BETWEEN '" & dtpFrom.Value.ToString("yyyy-MM-dd") & "' AND '" & dtpTo.Value.ToString("yyyy-MM-dd") & "' "
            strSql += vbCrLf + " AND ISNULL(CANCEL,'')='' AND COMPANYID='" & cnCompanyId & "' AND COSTID='" & cnCostId & "' "
            If (Trim(objGPack.GetSqlValue(strSql, , ""))) <> "" Then
                lblStartBillNo.Text = objGPack.GetSqlValue(strSql, , "").ToString
            End If

            strSql = " SELECT COUNT(*) TRANNO FROM " & cnStockDb & "..ISSUE WHERE TRANTYPE IN ('SA') AND TRANDATE BETWEEN '" & dtpFrom.Value.ToString("yyyy-MM-dd") & "' AND '" & dtpTo.Value.ToString("yyyy-MM-dd") & "' "
            strSql += vbCrLf + " AND ISNULL(CANCEL,'')='' AND COMPANYID='" & cnCompanyId & "' AND COSTID='" & cnCostId & "' "
            If (Trim(objGPack.GetSqlValue(strSql, , ""))) <> "" Then
            ...
            With gridView
                .Columns("BILLNO").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .Columns("BILLDATE").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                ...
                .Columns("CREDITCARD").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .Columns("COMM").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .Columns("CATNAME").Visible = False
                .Columns("COLHEAD").Visible = False
                .Columns("RESULT").Visible = False
                .Columns("BILLNO").Width = 170
                .Columns("BILLNO").HeaderText = "PARTICULARS"
                .Columns("CREDIT").HeaderText = "CREDIT"
                .Columns("CREDITCARD").HeaderText = "C.CARD"
                .Columns("BILLDATE").Visible = chkWithBillDate.Checked
            End With

            For i As Integer = 0 To gridView.Rows.Count - 1
                Select Case gridView.Rows(i).Cells("COLHEAD").Value
                    Case "H"
                        gridView.Rows(i).Cells("BILLNO").Style.BackColor = Color.LightBlue
                        gridView.Rows(i).Cells("BILLNO").Style.Alignment = DataGridViewContentAlignment.MiddleLeft
                        gridView.Rows(i).Cells("BILLNO").Value = gridView.Rows(i).Cells("CATNAME").Value.ToString
                    ...
                    Case "T"
                        gridView.Rows(i).Cells("BILLNO").Value = "GRAND TOTAL"
                        gridView.Rows(i).Cells("BILLNO").Style.Alignment = DataGridViewContentAlignment.MiddleLeft
                        ...
                        gridView.Rows(i).DefaultCellStyle.Font = New Font("VERDANA", 8, FontStyle.Bold)
                        gridView.Rows(i).Visible = chkWithGTot.Checked
                    Case "S"
                        gridView.Rows(i).Cells("BILLNO").Value = "SUB TOTAL"
                        gridView.Rows(i).Cells("BILLNO").Style.Alignment = DataGridViewContentAlignment.MiddleLeft
                        gridView.Rows(i).DefaultCellStyle.BackColor = Color.LightGreen
                        gridView.Rows(i).DefaultCellStyle.ForeColor = Color.DarkGreen
                        gridView.Rows(i).DefaultCellStyle.Font = New Font("VERDANA", 8, FontStyle.Bold)
                    Case Else
                        If gridView.Rows(i).Cells("WEIGHT").Value.ToString = "CANCEL" Then
                            gridView.Rows(i).Cells("WEIGHT").Style.Alignment = DataGridViewContentAlignment.MiddleLeft
                        End If
                End Select
            Next

            'GridViewHeaderStyle()
            lblTitle.Text = "SALES WISE DETAIL REPORT FROM " & dtpFrom.Value.ToString("dd/MM/yyyy") & " TO " & dtpTo.Value.ToString("dd/MM/yyyy") & ""
