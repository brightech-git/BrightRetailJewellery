...
Public Class frmTransactionDetailed
    ...
    Dim rpt_format1 As Boolean = IIf(GetAdmindbSoftValue("RPT_TRANDET", "N") = "Y", True, False)
    Dim Final_Discount_Disp As Boolean = IIf(GetAdmindbSoftValue("FINALDISCOUNT_DISPLAY", "N") = "Y", True, False)
    ...
    Function funcAbstract(ByVal TempTable As String, ByVal TranTable As String, ByVal StoneTable As String, ByVal TrType As String, ByVal TaxType As String) As Integer
        ...
        strSql += " ,ISNULL((SELECT SUM(AMOUNT) FROM " & cnStockDb & "..ACCTRAN "
        strSql += " WHERE BATCHNO = T.BATCHNO AND PAYMODE = 'GV' AND COMPANYID IN (" & SelectedCompany & ")),0)AS GIFTVOUCHER"
        strSql += " ,ISNULL((SELECT SUM(AMOUNT) FROM " & cnStockDb & "..ACCTRAN "
    ...
    Function funcInsertPayment() As Integer
        ...
        strSql += " ,CASE WHEN PAYMODE = 'GV' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END GVCARD "
        ...
        strSql += " ,CASE WHEN PAYMODE = 'CH' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END CHEQUE"
        strSql += " ,CASE WHEN PAYMODE = 'SS' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END CHITCARD"
        strSql += " ,CASE WHEN PAYMODE = 'GV' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END CHITCARD"
        strSql += " ,CASE WHEN PAYMODE = 'RO' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END ROUND"
        strSql += " ,CASE WHEN PAYMODE = 'DI' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END DISCOUNT"
        strSql += " ,CASE WHEN PAYMODE = 'HC' THEN SUM(CASE WHEN TRANMODE = 'C' THEN AMOUNT ELSE -1*AMOUNT END) ELSE 0 END HANDLING"
        strSql += " ,TRANNO"
        ...
        strSql += " 0 CHEQUE,0 CHITCARD,0 GVCARD,0 DISCOUNT,0 HANDLING,'4'GGROUP,'2'RESULT,'T'"
        strSql += " END "
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        ...
        strSql += " 0 CHEQUE,0 CHITCARD,0 GVCARD,0 DISCOUNT,0 HANDLING,'4'GGROUP,'1'RESULT,' '"
        strSql += " END "
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        ...
        strSql += " INSERT INTO TEMP" & systemId & "TRANDET"
        strSql += " (CATCODE,CATNAME,PCS,GRSWT,NETWT,AMOUNT,VAT,TOTAL,GROUPNAME,TRANNO,CUSTOMER,CASH,"
        strSql += " ROUND,CREDITCARD,CHEQUE,CHITCARD,GVCARD,DISCOUNT,HANDLING,GGROUP,RESULT,COLHEAD)"
        strSql += " SELECT CATCODE,"
        strSql += " (CASE WHEN RESULT=2 THEN CATNAME ELSE TRANNO END) PARTICULAR, "
        strSql += " PCS, GRSWT, NETWT, AMOUNT, VAT, TOTAL, GROUPNAME, TRANNO, CUSTOMER, "
        strSql += " CASH,ROUND,CREDITCARD,CHEQUE,CHITCARD,GVCARD,DISCOUNT,HANDLING,GGROUP,RESULT,COLHEAD"
        strSql += " FROM TEMP" & systemId & "PAYTRAN "
        strSql += " ORDER BY COLHEAD DESC ,CATNAME,RESULT  "
        strSql += " END "
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
    ...
    Private Sub Report()
        Try
            ...
            strSql = vbCrLf + " IF OBJECT_ID('TEMPTABLEDB..TEMPTRANDET_PAYMENT','U') IS NOT NULL DROP TABLE TEMPTABLEDB..TEMPTRANDET_PAYMENT"
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()

            Dim GstFlag As Boolean = funcGstView(dtpFrom.Value)
            If ChkWithoutGrp.Checked Then
                strSql = vbCrLf + " EXEC " & cnStockDb & "..RPT_TRANSACTIONDETAILED_WITHOUTGRP"
                strSql += vbCrLf + " @DBNAME = '" & cnAdminDb & "',"
            Else
                strSql = vbCrLf + " EXEC " & cnStockDb & "..SP_RPT_TRANSACTIONDETAILED"
            End If
            ...
            strSql += vbCrLf + " ,@WITHOTHERS = '" & IIf(chkWithOthers.Checked, "Y", "N") & "'"
            strSql += vbCrLf + " ,@WITHLINKEDPURINFO = '" & IIf(chkLinkPurInfo.Checked, "Y", "N") & "'"
            strSql += vbCrLf + " ,@CASHOPENING = '" & IIf(chkWithCashOpen.Checked = True, "Y", "N") & "'"
            strSql += vbCrLf + " ,@CASHTOBDET = '" & IIf(chkctbwdet.Checked = True, "Y", "N") & "'"
            '290613 [SA] TOTAL Visible based on Check box
            strSql += vbCrLf + " ,@TOTALVIEW = '" & IIf(ChkTotal.Checked = True, "Y", "N") & "'"
            strSql += vbCrLf + " ,@WITHMISCISS = '" & IIf(ChkMiscIssue.Checked = True, "Y", "N") & "'"
            strSql += vbCrLf + " ,@WITHCANCEL = '" & IIf(ChkCancel.Checked = True, "Y", "N") & "'"
            strSql += vbCrLf + " ,@WITHGVDETAIL = '" & IIf(chkGvdetails.Checked = True, "Y", "N") & "'"
            If ChkWithoutGrp.Checked = False Then
                strSql += vbCrLf + " ,@STNSEPPOST = '" & IIf(chkStnSepPost.Checked = True, "Y", "N") & "'"
                strSql += vbCrLf + " ,@STNGRM = '" & IIf(rbtGrm.Checked = True, "Y", "N") & "'"
                strSql += vbCrLf + " ,@DATESUMMARY = '" & IIf(chkTranDateSum.Checked = True, "Y", "N") & "'"
                strSql += vbCrLf + " ,@GST = '" & IIf(funcGstView(dtpFrom.Value) = True, "Y", "N") & "'"
                strSql += vbCrLf + " ,@WITHADDR = '" & IIf(ChkAddr.Checked = True, "Y", "N") & "'"
                strSql += vbCrLf + " ,@CHITDB = '" & ChitDb & "'"
                If ChitDb.ToString = "" Then
                    strSql += vbCrLf + " ,@CHITBREAKUP = 'N'"
                Else
                    strSql += vbCrLf + " ,@CHITBREAKUP = '" & IIf(chkchitBreakup.Checked = True, "Y", "N") & "'"
                End If
                strSql += vbCrLf + " ,@WITHORDCREDITSALES = '" & IIf(chksalesordcredit.Checked = True, "Y", "N") & "'"
                If ChitDb.ToString = "" Then
                    strSql += vbCrLf + " ,@SCHADJ = 'N'"
                Else
                    strSql += vbCrLf + " ,@SCHADJ = '" & IIf(ChkSchemeAdjust.Checked = True, "Y", "N") & "'"
                End If
            End If
            If ChkWithoutGrp.Checked = False Then
                strSql += vbCrLf + " ,@WITHBILLPREFIX = '" & IIf(chkBillPrefix.Checked = True, "Y", "N") & "'"
            End If
            ...
            If SCHEME_TO_ADVANCE_NAME_CHANGE_CASH <> "" Then
                strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPTRANDET SET PARTICULAR='" & SCHEME_TO_ADVANCE_NAME_CHANGE_CASH & "' + ' TOTAL [CT]' WHERE PARTICULAR='CHIT TOTAL [CT]'"
                cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                cmd.ExecuteNonQuery()
            End If
            If SCHEME_TO_ADVANCE_NAME_CHANGE <> "" Then
                strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPTRANDET SET PARTICULAR='" & SCHEME_TO_ADVANCE_NAME_CHANGE & "' WHERE PARTICULAR='CHIT DETAILS'"
                cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                cmd.ExecuteNonQuery()
            End If
            If ChkWithoutGrp.Checked Then
                strSql = "SELECT CONVERT(INT,NULL)SNO,* FROM TEMPTABLEDB..TEMPTRANDET ORDER BY KEYNO"
            Else
                strSql = "SELECT * FROM TEMPTABLEDB..TEMPTRANDET ORDER BY KEYNO"
            End If
            dt = New DataTable
            da = New OleDbDataAdapter(strSql, cn)
            da.Fill(dt)
            ...
            End If
            Dim title As String
            title = "TRANSACTION DETAILS FROM " & dtpFrom.Text & " TO " & dtpTo.Text & ""
            If SelectedCostCentre <> "" Then title += " COSTCENTRE [" + SelectedCostCentre + "]"
            If chkLstCashCounter.CheckedItems.Count = chkLstCashCounter.Items.Count Then
                title += " CASHCOUNTER [ALL]"
            Else
            ...
            If ChkWithoutGrp.Checked Then
                For I As Integer = 0 To gridView.RowCount - 1
                    If gridView.Rows(I).Cells("TYPE").Value.ToString = "R" Then Continue For
                    If gridView.Rows(I).Cells("TYPE").Value.ToString = "P" Then Continue For
                    If gridView.Rows(I).Cells("CANCEL").Value.ToString = "Y" Then
                        gridView.Rows(I).DefaultCellStyle.ForeColor = Color.Red
                        gridView.Rows(I).Cells("TRANTYPE").Value = "CANCEL"
                    End If
                    Dim Colhead As String = gridView.Rows(I).Cells("COLHEAD").Value.ToString
                    If Colhead <> "" Then
                        No = 1
                    Else
                        gridView.Rows(I).Cells("SNO").Value = No
                        No += 1
                    End If
                Next
                gridView.Columns("CANCEL").Visible = False
                gridView.Columns("TOT").HeaderText = "TOTAL"
                gridView.Columns("SRETURN").HeaderText = "RETURN"
            End If
            ...
            With gridView
                ...
                .Columns("REMARK").Width = 120
                .Columns("CASH").Width = 100
                .Columns("ROUND OFF").Width = 100
                .Columns("CREDIT CARD").Width = 100
                .Columns("DISCOUNT").Width = 100
                .Columns("HANDLING").Width = 100
                .Columns("ACCODE").Width = 70
                .Columns("ACCODE").Visible = chkWithAccode.Checked
                ...
                End If
                If .Columns.Contains("BILLPREFIX") Then .Columns("BILLPREFIX").Visible = False
                If Final_Discount_Disp = True Then
                    If .Columns.Contains("FIN_DISCOUNT") Then .Columns("FIN_DISCOUNT").Visible = True
                Else
                    If .Columns.Contains("FIN_DISCOUNT") Then .Columns("FIN_DISCOUNT").Visible = False
                End If
                ...
                If ChkAddr.Checked = False Then
                    ...
                    If .Columns.Contains("DOORNO") Then .Columns("DOORNO").Visible = False
                    If .Columns.Contains("ADDRESS1") Then .Columns("ADDRESS1").Visible = False
                    If .Columns.Contains("ADDRESS2") Then .Columns("ADDRESS2").Visible = False
                    If .Columns.Contains("ADDRESS3") Then .Columns("ADDRESS3").Visible = False
                    If .Columns.Contains("AREA") Then .Columns("AREA").Visible = False
                    If .Columns.Contains("CITY") Then .Columns("CITY").Visible = False
                    If .Columns.Contains("STATE") Then .Columns("STATE").Visible = False
                ...
                End If
                If rpt_format1 = True Then .Columns("ADVANCE ADJ").Visible = False
            ...
            End With
            If SPECIFICFORMAT.ToString = "1" Then
                For Each gr As DataGridViewRow In gridView.Rows
                    If gr.Cells("COLHEAD").Value.ToString = "S" Then
                        gridView.Rows.RemoveAt(gr.Index)
                    End If
                Next
                For Each gr As DataGridViewRow In gridView.Rows
                    If gr.Cells("COLHEAD").Value.ToString = "S1" And gr.Cells("GROUP0").Value.ToString = "SALES DETAILS" Then
                        gridView.Rows.RemoveAt(gr.Index)
                    End If
                Next
                'For Each gr As DataGridViewRow In gridView.Rows
                '    If gr.Cells("COLHEAD").Value.ToString = "S2" Then
                '        gridView.Rows.RemoveAt(gr.Index)
                '    End If
                'Next
            End If
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        chkStnSepPost.Checked = False
        rbtGrm.Visible = False
        rbtNormal.Visible = False
        rbtGrm.Checked = True
        ' rbtGrossWt.Checked = True
        ' chkWithPcs.Checked = False
        ' chkWithOthers.Checked = True
        Prop_Gets()
        dtpFrom.Select()
    End Sub
    ...
    Private Sub Prop_Sets()
        Dim obj As New frmTransactionDetailed_properties
        ...
        obj.p_chkWithAccode = chkWithAccode.Checked
        obj.p_chkWithOthers = chkWithOthers.Checked
        obj.p_chkLinkPurInfo = chkLinkPurInfo.Checked
        obj.p_chkWithCashOpen = chkWithCashOpen.Checked
        obj.p_chkCancel = ChkCancel.Checked
        obj.p_chkGvdetails = chkGvdetails.Checked
        obj.p_chkMiscIssue = ChkMiscIssue.Checked
        obj.p_chkctbwdet = chkctbwdet.Checked
        obj.p_chkTotal = ChkTotal.Checked
        obj.p_ChkWithoutGrp = ChkWithoutGrp.Checked
        obj.p_chkBillPrefix = chkBillPrefix.Checked
        obj.p_ChkWithAddr = ChkAddr.Checked
        obj.p_chkStnSepPost = chkStnSepPost.Checked
        obj.p_rbtGrm = rbtGrm.Checked
        obj.p_rbtNormal = rbtNormal.Checked
        SetSettingsObj(obj, Me.Name, GetType(frmTransactionDetailed_properties), Save)
    End Sub
    Private Sub Prop_Gets()
        Dim obj As New frmTransactionDetailed_properties
        GetSettingsObj(obj, Me.Name, GetType(frmTransactionDetailed_properties), IIf(Authorize = False, True, False))
        ...
        chkWithAccode.Checked = obj.p_chkWithAccode
        chkWithOthers.Checked = obj.p_chkWithOthers
        chkLinkPurInfo.Checked = obj.p_chkLinkPurInfo
        chkWithCashOpen.Checked = obj.p_chkWithCashOpen
        ChkCancel.Checked = obj.p_chkCancel
        chkGvdetails.Checked = obj.p_chkGvdetails
        ChkMiscIssue.Checked = obj.p_chkMiscIssue
        chkctbwdet.Checked = obj.p_chkctbwdet
        ChkTotal.Checked = obj.p_chkTotal
        ChkWithoutGrp.Checked = obj.p_ChkWithoutGrp
        chkBillPrefix.Checked = obj.p_chkBillPrefix
        ChkAddr.Checked = obj.p_ChkWithAddr
        chkStnSepPost.Checked = obj.p_chkStnSepPost
        rbtGrm.Checked = obj.p_rbtGrm
        rbtNormal.Checked = obj.p_rbtNormal
    End Sub

    Private Sub chkWithOthers_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkWithOthers.CheckedChanged
        chkWithCashOpen.Checked = chkWithOthers.Checked
        chkWithCashOpen.Enabled = chkWithOthers.Checked
    End Sub
    ...
    Private Sub ChkWithoutGrp_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkWithoutGrp.CheckedChanged

    End Sub

    Private Sub chkStnSepPost_CheckedChanged(sender As Object, e As EventArgs) Handles chkStnSepPost.CheckedChanged
        If chkStnSepPost.Checked Then
...
Public Class frmTransactionDetailed_properties
    ...
    Public Property p_chkWithPcs() As Boolean
        ...
        End Set
    End Property
    Private chkWithAccode As Boolean = False
    Public Property p_chkWithAccode() As Boolean
        Get
            Return chkWithAccode
        End Get
        Set(ByVal value As Boolean)
            chkWithAccode = value
        End Set
    End Property
    Private chkWithOthers As Boolean = True
    Public Property p_chkWithOthers() As Boolean
        Get
            Return chkWithOthers
        End Get
        Set(ByVal value As Boolean)
            chkWithOthers = value
        End Set
    ...
    Public Property p_chkWithCashOpen() As Boolean
        ...
        End Set
    End Property
    Private chkLinkPurInfo As Boolean = True
    Public Property p_chkLinkPurInfo() As Boolean
        Get
            Return chkLinkPurInfo
        End Get
        Set(ByVal value As Boolean)
            chkLinkPurInfo = value
        End Set
    End Property
    Private chkMiscIssue As Boolean = False
    Public Property p_chkMiscIssue() As Boolean
        ...
        End Set
    End Property
    Private chkCancel As Boolean = False
    Public Property p_chkCancel() As Boolean
        Get
            Return chkCancel
        End Get
        Set(ByVal value As Boolean)
    ...
    Public Property p_chkTotal() As Boolean
        ...
        End Set
    End Property
    Private chkctbwdet As Boolean = False
    Public Property p_chkctbwdet() As Boolean
        Get
            Return chkctbwdet
        End Get
        Set(ByVal value As Boolean)
        ...
        End Set
    End Property
    Private chkGvdetails As Boolean = False
    Public Property p_chkGvdetails() As Boolean
        Get
            Return chkGvdetails
        End Get
        Set(ByVal value As Boolean)
        ...
        End Set
    End Property
    Private ChkWithoutGrp As Boolean = False
    Public Property p_ChkWithoutGrp() As Boolean
        Get
            Return ChkWithoutGrp
        End Get
        Set(ByVal value As Boolean)
            ChkWithoutGrp = value
        End Set
    ...
    Public Property p_ChkWithAddr() As Boolean
        ...
        End Set
    End Property
    Private chkStnSepPost As Boolean = False
    Public Property p_chkStnSepPost() As Boolean
        Get
            Return chkStnSepPost
        End Get
        Set(ByVal value As Boolean)
            chkStnSepPost = value
        End Set
    End Property
    Private rbtGrm As Boolean = False
    Public Property p_rbtGrm() As Boolean
        Get
            Return rbtGrm
        End Get
        Set(ByVal value As Boolean)
