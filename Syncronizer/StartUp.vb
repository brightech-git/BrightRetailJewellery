            ...
            If InsMode = InsertMode.Receive Then
                If Row!TAGIMAGE.ToString <> "" Then
                    ...
                    ElseIf Not IO.Directory.Exists(ImageDirPath) And Row!IMAGE_CTRLID.ToString.Contains(":") Then
                        Return ""
                    End If
                    If File.Exists(ImageDirPath & Row!TAGIMAGENAME) Then
                        File.Copy(ImageDirPath & Row!TAGIMAGENAME, ImageDirPath & DateAndTime.Now.ToString("ddMMyyyyhhmmss") & "_" & Row!TAGIMAGENAME)
                        File.SetAttributes(ImageDirPath & Row!TAGIMAGENAME, FileAttribute.Normal)
                        File.Delete(ImageDirPath & Row!TAGIMAGENAME)
                    End If
...
CounterStarts:
        ...
        StrSql = vbCrLf + "  IF OBJECT_ID('TEMPTABLEDB.." & _WebDbTblPrefix & "_SYNCTABLE') IS NULL"
        StrSql += vbCrLf + "  BEGIN"
        StrSql += vbCrLf + "  	CREATE TABLE TEMPTABLEDB.." & _WebDbTblPrefix & "_SYNCTABLE("
        StrSql += vbCrLf + "  		[UID] [int] IDENTITY(1,1) NOT NULL,"
        StrSql += vbCrLf + "  		[FROMID] [varchar](2) NULL,"
        StrSql += vbCrLf + "  		[TOID] [varchar](2) NULL,"
        StrSql += vbCrLf + "  		[CONTENT] [image] NULL,"
        StrSql += vbCrLf + "  		[UPDFILE] [varchar](30) NULL"
        StrSql += vbCrLf + "  	) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]"
        StrSql += vbCrLf + "  END"
        _Cmd = New OleDbCommand(StrSql, _CnAdmin)
        _Cmd.ExecuteNonQuery()
        ...
        Try
            ...
            While 1 = 1
                ...
                StrSql = " UPDATE " & _WebDbTblPrefix & "_SYNCTABLE SET FROMID=FROMID WHERE UPDFILE = '" & Row.Item("UPDFILE").ToString & "' "
                ...
                If DsReceive.Tables(0).Rows.Count > 200 Then
                    Throw New Exception("Count 200 Above")
                End If
                If _Sync_Via_R Then
                    R_NewCompId = GMethods.GetSqlValue(_CnAdmin, "SELECT COMPID FROM " & _Admindb & "..SYNCCOSTCENTRE WHERE COSTID = '" & R_CostId & "'", , , _Tran)
                Else
                    R_NewCompId = GMethods.GetSqlValue(_CnAdmin, "SELECT COMPID FROM " & IIf(_VbDbPrefix = "", _CompanyId, _VbDbPrefix) & "SAVINGS..SYNCCOSTCENTRE WHERE COSTID = '" & R_CostId & "'", , , _Tran)
                End If
                ...
                R_ImageDir = Nothing
                If _Sync_Via_R Then
                    R_OldCompId = GMethods.GetSqlValue(_CnAdmin, "SELECT COMPID FROM " & _Admindb & "..SYNCCOSTCENTRE WHERE COSTID = '" & Row.Item("FROMID").ToString & "'", , , _Tran)
                Else
                    R_OldCompId = GMethods.GetSqlValue(_CnAdmin, "SELECT COMPID FROM " & IIf(_VbDbPrefix = "", _CompanyId, _VbDbPrefix) & "SAVINGS..SYNCCOSTCENTRE WHERE COSTID = '" & Row.Item("FROMID").ToString & "'", , , _Tran)
                End If
        ...
        Catch ex As Exception
            ...
            'If ex.Message.Contains("Please make appropriate path") Then
                SetStatus(ex.Message.ToString)
                _ObjStatus.AddStatus(ex.Message.ToString)
            ''Else
            '    SetStatus("ConStr")
