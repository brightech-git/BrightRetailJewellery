...
Public Class frmBillDueAuthorize
    ...
    Public Sub New(ByVal totAmt As Double, ByVal discAmt As Double)
        ' This call is required by the Windows Form Designer.
        InitializeComponent()
        objGPack.Validator_Object(Me)
        Me.DiscAmt = discAmt
    ...
    Private Sub btnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOk.Click
        ...
        If txtPassword.Text = "" Then
            ...
            Exit Sub
        End If
        strSql = "SELECT PASSWORD,DUEPER,DUEAMT FROM " & cnAdminDb & "..DISCAUTHORIZE WHERE EMPID = " & Val(txtEmpId_NUM.Text) & ""
        strSql += " AND PASSWORD = '" & objGPack.Encrypt(txtPassword.Text) & "'"
        Dim dt As New DataTable
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            If CheckAuthrorize(Val(dt.Rows(0).Item("DUEPER").ToString), Val(dt.Rows(0).Item("DUEAMT").ToString)) = False Then
    ...
    Private Function CheckAuthrorize(ByVal alDiscPer As Double, ByVal alDiscAmt As Double) As Boolean
        ...
        If alDiscPer <> 0 And alDiscAmt <> 0 Then
            If DiscAmt > alDiscAmt Then
                MsgBox("This User Authorize Rs " & alDiscAmt & " Only")
                Return True
            End If
            If discPer > alDiscPer Then
                MsgBox("This User Authorize Rs " & Math.Round(TotAmt * (alDiscPer / 100), 2) & " Only")
                Return True
            End If
