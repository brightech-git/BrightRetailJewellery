...
Public Class frmPurifyrec
    ...
    Public Sub New()
        ...
        GridView.Columns("ICATEGORY").Visible = False '.MinimumWidth = 150
        ...
        GridView.Columns("SAMPLEWT").Visible = False '.DefaultCellStyle.Format = "0.000"
        GridView.Columns("SCRAPWT").Visible = False '.DefaultCellStyle.Format = "0.000"
...
GenBillNo:
            If flagupdate = True And flagupdateBatchno <> "" And flagupdateTranno <> "" Then
                ...
                strSql += vbCrLf + " AND TRANTYPE = 'RMP' "
                ...
                strSql += " AND RECISS = 'R' AND TRANTYPE = 'PR'"
                cmd = New OleDbCommand(strSql, cn, tran)
                cmd.ExecuteNonQuery()
    ...
    Public Sub InsertIssDetails(ByVal index As Integer)
        With GridView.Rows(index)
            ...
            Dim Metalid As String = objGPack.GetSqlValue("SELECT METALID FROM " & cnAdminDb & "..CATEGORY WHERE CATNAME = '" & .Cells("RCATEGORY").Value.ToString & "'", , , tran).ToString
            Dim catCode As String = objGPack.GetSqlValue("SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY WHERE CATNAME = '" & .Cells("RCATEGORY").Value.ToString & "'", , , tran).ToString
    ...
    Private Sub cmbSmith_MAN_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbSmith_MAN.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            ...
            'strSql += vbCrLf + " AND ACCODE = (SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & cmbSmith_MAN.Text & "')"
            'strSql += vbCrLf + " AND ISNULL(TRANFLAG,'') <> 'S'"
            'strSql += vbCrLf + " AND ISNULL(TRANTYPE,'') = 'RMP'"
            ...
            If flagupdate = False Then
                ...
                strSql = vbCrLf + " SELECT BAGNO,METAL,SUM(CASE WHEN SEP = 'R' THEN GRSWT ELSE -1*GRSWT END) AS GRSWT,SUM(CASE WHEN SEP = 'R' THEN NETWT ELSE -1*NETWT END) AS NETWT"
                strSql += vbCrLf + " FROM"
                strSql += vbCrLf + " ("
                strSql += vbCrLf + " SELECT 'R' SEP,BAGNO"
                strSql += " ,M.METALNAME METAL "
                ...
                strSql += " GROUP BY R.BAGNO,METALNAME"
                strSql += " UNION ALL"
                ...
                strSql += " WHERE ISNULL(BAGNO,'') <> ''" + vbCrLf
                strSql += " AND ACCODE = (SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & cmbSmith_MAN.Text & "')"
                strSql += " AND ISNULL(CANCEL,'') = ''  AND R.TRANTYPE = 'PR'"
                strSql += " AND ISNULL(COMPANYID,'') = '" & strCompanyId & "'"
                strSql += " GROUP BY R.BAGNO,METALNAME"
                strSql += " )X"
                strSql += " WHERE GRSWT <> 0"
                strSql += " GROUP BY X.BAGNO,X.METAL" + vbCrLf
                strSql += vbCrLf + " having SUM(CASE WHEN SEP = 'R' THEN GRSWT ELSE -1*GRSWT END) > 0 or SUM(CASE WHEN SEP = 'R' THEN NETWT ELSE -1*NETWT END) > 0 "
                da = New OleDbDataAdapter(strSql, cn)
    ...
    Private Sub cmbReceivedCat_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbReceivedCat.GotFocus
    ...
    Private Sub CalcRate()
        ...
        Dim rate As Double = buRate * ((txtPurity_PER.Text) / 100)
    ...
    Private Sub gridViewPendingBagNo_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridViewPendingBagNo.CellContentClick

    End Sub

    Private Sub txtPurity_PER_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtPurity_PER.KeyDown
        If e.KeyCode = Keys.Enter Then
            PureWt_Calc()
        End If
    End Sub

    Private Sub txtPurity_PER_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtPurity_PER.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            PureWt_Calc()
        End If
    End Sub

    Private Sub txtPurity_PER_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtPurity_PER.TextChanged
        PureWt_Calc()
    End Sub

    Private Sub PureWt_Calc()
        Dim wt As Decimal = Val(txtReceivedWt_WET.Text)
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        ...
        If gridViewOpen.Rows.Count > 0 Then
            BrightPosting.GExport.Post(Me.Name, strCompanyName, "Purification Receipt", gridViewOpen, BrightPosting.GExport.GExportType.Export)
        End If
    End Sub
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        If Not BrighttechPack.Methods.GetRights(_DtUserRights, Me.Name, BrighttechPack.Methods.RightMode.Excel) Then Exit Sub
        If gridViewOpen.Rows.Count > 0 Then
            BrightPosting.GExport.Post(Me.Name, strCompanyName, "Purification Receipt", gridViewOpen, BrightPosting.GExport.GExportType.Print)
        End If
    End Sub
    Private Function filterQry(ByVal _date As String, ByVal _batchno As String) As String
        Dim Qry As String = ""
        Qry = ""
        Qry += vbCrLf + " I.TRANDATE = '" & _date & "' AND I.TRANTYPE = 'RMP' "
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            If dt.Rows.Count > 0 Then
                If Val(dt.Rows(0).Item("CNT").ToString) = 1 Then
                    ...
                    cmbReceivedCat.Text = dt.Rows(0).Item("CATEGORYNAME").ToString
