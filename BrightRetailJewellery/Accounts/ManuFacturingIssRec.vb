...
Imports System.Linq
Public Class ManuFacturingIssRec
    ...
    Public objMIRTran As New ManuFacturingIssRecTran
    Dim objTds As New frmAccEd
    Dim defProcVAlue As String = Nothing
    Public objOrderInfo As ManuFacturingIssRec_ORDERINFO = Nothing
    ...
    Public Oordno, Mordno As String
    ...
    Dim Order_WithOutGrswt As Boolean = False
    Dim MI_ALLOW_MULTI_ISSUE As Boolean = IIf(GetAdmindbSoftValue("MI_ALLOW_MULTI_ISSUE", "N") = "Y", True, False)



    Public Sub New(ByVal oMaterial As Material, ByVal oTransactionType As String, ByVal oTranDate As Date, ByVal Accode As String, Optional ByVal SelectedSno As String = "", Optional ByVal CostId As String = "", Optional ByVal OrdNo As String = "", Optional ByVal Stax As Decimal = 0, Optional ByVal _Gst As Boolean = True)
        ...
        With dgvOrder
            ...
            .Columns("UNIT").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        ...
        If UCase(oTransactionType) <> "PURCHASE RETURN" And UCase(oTransactionType) <> "PURCHASE" And UCase(oTransactionType) <> "PURCHASE[APPROVAL]" And UCase(oTransactionType) <> "INTERNAL TRANSFER" And UCase(oTransactionType) <> "APPROVAL RECEIPT" Then
            ...
            lblOthVat.Text = "Tds"
            txtOVatPer_AMT.Text = IIf(TdsPer <> 0, Format(TdsPer, "0.00"), "")
            ...
            If RateCalcAmt = True And UCase(oTransactionType) = "PURCHASE" Then
                oRateLock = False
            Else
                oRateLock = True
            End If
        ...
        Else
            ...
            If GstFlag Then
                ...
                lblOthVat.Text = "GST"
                txtOVatPer_AMT.Text = IIf(PTaxPer <> 0, Format(PTaxPer, "0.00"), "")
        ...
        End If
        AddHandler cmbOProcess.SelectedIndexChanged, AddressOf CmbProcess_Selection
        AddHandler cmbMProcess.SelectedIndexChanged, AddressOf CmbProcess_Selection
        AddHandler cmbSProcess.SelectedIndexChanged, AddressOf CmbProcess_Selection
        'AddHandler ObjMinValue.txtMinWastage_Per.KeyPress, AddressOf ObjMinValues_txtMinWastage_Per_KeyPress
        'CmbProcess_SelectionChanged
    End Sub
    ...
    Private Function CheckJobNo(ByVal txtJobNo As TextBox) As Boolean
        ...
        If _JobNoEnable = False Then
            ...
            'StrSql += " WHERE ORNO = '" & GetCostId(BillCostId) & "" & txtJobNo.Text & "'"
            ...
            If objGPack.GetSqlValue(StrSql) <> "" Then
                Return True
            End If
            If STOCKVALIDATION Then Return True
    ...
    Private Sub LoadProcess(ByVal Cmb As ComboBox)
        StrSql = vbCrLf + " SELECT ORDSTATE_NAME,ORDSTATE_ID FROM " & cnAdminDb & "..ORDERSTATUS ORDER BY DISPORDER, ORDSTATE_NAME"
        ...
        StrSql = "SELECT ISNULL(SMITH,'') FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & Cmb.Text & "' AND ISNULL(SMITH,'') = 'Y'"
        Order_WithOutGrswt = IIf(GetSqlValue(cn, StrSql) = "Y", True, False)
    End Sub
    'Private funct CmbProcess_SelectionChanged()
    '    StrSql = "SELECT ISNULL(SMITH,'') FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & Cmb.Text & "' AND ISNULL(SMITH,'') = 'Y'"
    '    Order_WithOutGrswt = IIf(GetSqlValue(cn, StrSql) = "Y", True, False)
    'End Sub
    Private Function CmbProcess_Selection(ByVal sender As Object, ByVal e As EventArgs)
        Dim Value As String = DirectCast(sender, ComboBox).Text
        StrSql = "SELECT ISNULL(SMITH,'') FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & Value & "' AND ISNULL(SMITH,'') = 'Y'"
        Order_WithOutGrswt = IIf(GetSqlValue(cn, StrSql) = "Y", True, False)
        If Order_WithOutGrswt = True Then
            If rbtOrnament.Checked Then txtOGrsWt_WET.Text = "" : txtONetWt_WET.Text = "" : txtOPureWt_WET.Text = ""
            If rbtMetal.Checked Then txtMGrsWt_WET.Text = "" : txtMNetWt_WET.Text = "" : txtMPureWt_WET.Text = ""
            If rbtStone.Checked Then txtSGrsWt_WET.Text = ""
    ...
    Private Sub btnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOk.Click
        ...
        If rbtOrnament.Checked Then
            ...
            If txtOOrdNo.Text <> "" Then
                If _JobNoEnable = False Then
                    If Mid(txtOOrdNo.Text, 1, 1) = "O" Or Mid(txtOOrdNo.Text, 1, 1) = "R" Then
                ...
                End If
            End If
            If Val(txtOGrsWt_WET.Text) = 0 And Val(txtOPcs_NUM.Text) = 0 And Order_WithOutGrswt = False Then
                MsgBox("Grs. Weight should not empty", MsgBoxStyle.Information)
                txtOGrsWt_WET.Select()
                Exit Sub
            End If
            If MRMI_ALLOWZERONETWT = False And Val(txtONetWt_WET.Text) = 0 And Val(txtOPcs_NUM.Text) = 0 And Order_WithOutGrswt = False Then
        ...
        ElseIf rbtMetal.Checked Then
            ...
            If txtMOrdNo.Text <> "" Then
                ...
                End If
            End If
            If Val(txtMGrsWt_WET.Text) = 0 And Val(txtMPcs_NUM.Text) = 0 And Order_WithOutGrswt = False Then
                MsgBox("Grs. Weight should not empty", MsgBoxStyle.Information)
        ...
        ElseIf rbtStone.Checked Then
            If oEditRowIndex <> -1 Then
                If txtSOrdNo.Text <> "" Then
                    ...
                    End If
                End If
                If Val(txtSGrsWt_WET.Text) = 0 And Val(txtSPcs_NUM.Text) = 0 And Order_WithOutGrswt = False Then
    ...
    Private Sub ManuFacturingIssRec_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub ManuFacturingIssRec_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
SKIPDEFPROCESS:
    ...
    , rbtOthers.CheckedChanged
        ...
        If txtOOrdNo.Text <> "" Then ORDERNO = txtOOrdNo.Text
        If txtMOrdNo.Text <> "" Then ORDERNO = txtMOrdNo.Text
        If txtSOrdNo.Text <> "" Then ORDERNO = txtSOrdNo.Text
        ClearAll()
        ...
        If rbtOrnament.Checked = True Then
            If oMaterial = Material.Issue Then
                ...
                txtTagNo.Select()
            End If
            If ORDERNO <> "" Then txtOOrdNo.Text = ORDERNO
        Else
            txtTagNo.Enabled = False
        End If
        If oMaterial = Material.Issue Then
        ...
        If rbtStone.Checked = True Then
            ...
            End If
            If ORDERNO <> "" Then txtSOrdNo.Text = ORDERNO
        ElseIf rbtMetal.Checked = True Then
            If ORDERNO <> "" Then txtMOrdNo.Text = ORDERNO
        ...
        If UCase(oTransactionType) = "PURCHASE RETURN" Or
            ...
            If GstFlag Then
                ...
                If Not dr Is Nothing Then
                    ...
                    End If
                End If
                txtOVatPer_AMT.Text = IIf(PTaxPer <> 0, Format(PTaxPer, "0.000"), "")
...
#Region "Ornament Methods & Events"
    ...
    Public Sub cmbOItem_SelectedValueChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbOItem.SelectedValueChanged
        If rbtOrnament.Checked Then If txtTagNo.Text = "" And cmbOItem.Text = "" Then Exit Sub
        If rbtOrnament.Checked Then LoadItemDetail(cmbOItem)
    End Sub
    ...
    Private Sub txttdsPer_AMT_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtOVatPer_AMT.KeyPress, txtMVatPer_PER.KeyPress, txtSVatPer_PER.KeyPress
    ...
    Private Sub txtOVatPer_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOVatPer_AMT.TextChanged
        If Val(txtOVatPer_AMT.Text) = 0 Then
            txtOVat_AMT.Clear()
            Exit Sub
        End If
        CalcOVatTds()
    ...
    Private Sub txtOVat_AMT_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtOVat_AMT.KeyDown
        If Val(txtOVatPer_AMT.Text) <> 0 And Not PURVATEDITACC Then e.Handled = True
    End Sub
    ...
    Private Sub CalcOVatTds()
        ...
        vatTds = (Val(txtOGrsAmt_AMT.Text) + Val(txtOED_AMT.Text)) * (Val(txtOVatPer_AMT.Text) / 100)
    ...
    , cmbOGrsNet.SelectedIndexChanged
        If rbtMetal.Checked Then
            StrSql = "SELECT ISNULL(SMITH,'') FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & cmbMProcess.Text & "' AND ISNULL(SMITH,'') = 'Y'"
            Order_WithOutGrswt = IIf(GetSqlValue(cn, StrSql) = "Y", True, False)
            If Order_WithOutGrswt = True Then
                txtMGrsWt_WET.Text = "" : txtMNetWt_WET.Text = "" : txtMPureWt_WET.Text = ""
                objStone.dtGridStone.Clear()
                objStone.dtGridStone.AcceptChanges()
            End If
        ElseIf rbtOrnament.Checked Then
            StrSql = "SELECT ISNULL(SMITH,'') FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & cmbOProcess.Text & "' AND ISNULL(SMITH,'') = 'Y'"
            Order_WithOutGrswt = IIf(GetSqlValue(cn, StrSql) = "Y", True, False)
            If Order_WithOutGrswt = True Then
                txtOGrsWt_WET.Text = "" : txtONetWt_WET.Text = "" : txtOPureWt_WET.Text = ""
                objStone.dtGridStone.Clear()
                objStone.dtGridStone.AcceptChanges()
            End If
        ElseIf rbtStone.Checked Then
            StrSql = "SELECT ISNULL(SMITH,'') FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & cmbSProcess.Text & "' AND ISNULL(SMITH,'') = 'Y'"
            Order_WithOutGrswt = IIf(GetSqlValue(cn, StrSql) = "Y", True, False)
            If Order_WithOutGrswt = True Then
                txtSGrsWt_WET.Text = ""
                objStone.dtGridStone.Clear()
    ...
    Private Sub OVat_GotFocus(ByVal sender As Object, ByVal e As EventArgs) Handles txtOVatPer_AMT.GotFocus
...
cmbSItem.SelectedValueChanged
    ...
    Public Sub cmbSMetal_SelectedValueChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbSMetal.SelectedValueChanged
        ...
        If Not STOCKVALIDATION Then loadJobDetails()
    End Sub
...
#End Region
    ...
    Function Get_OrirTableInfo(TempOrSno As String, EntOrder As String)
        ...
        If MIMR_allowClosedOrder Then
            ...
            StrSql += vbCrLf + " O.PCS--ISNULL((SELECT SUM(isnull(PCS,0)) FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND  ORSTATUS = 'R' AND ISNULL(CANCEL,'')='' AND ENTRYORDER in  (" & EntOrder & ") AND ORDSTATE_ID<>54),0) PCS"
            StrSql += vbCrLf + " ,O.GRSWT--ISNULL((SELECT SUM(isnull(GRSWT,0)) FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = 'R'  AND ISNULL(CANCEL,'')='' AND ENTRYORDER in  (" & EntOrder & ") AND ORDSTATE_ID<>54),0) GRSWT "
            StrSql += vbCrLf + " ,O.NETWT--ISNULL((SELECT SUM(isnull(NETWT,0)) FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = 'R'  AND ISNULL(CANCEL,'')='' AND ENTRYORDER in  (" & EntOrder & ") AND ORDSTATE_ID<>54),0) NETWT"
            StrSql += vbCrLf + " ,O.SNO,O.ORNO "
            StrSql += vbCrLf + " ,ISNULL((SELECT TOP 1 ORDSTATE_ID FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = '" & IIf(oMaterial = Material.Issue, "R", "I") & "'  AND ISNULL(CANCEL,'')='' AND ENTRYORDER IN  (" & EntOrder & ") AND ORDSTATE_ID<>54 ORDER BY ENTRYORDER DESC),0) ORDSTATE_ID"
        ...
        Else
            If ORDER_MULTI_MIMR = False Then
                ...
                StrSql += vbCrLf + " WHERE O.SNO IN (" & TempOrSno & ")"
            Else
                StrSql = vbCrLf + " SELECT 1 FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO IN (" & TempOrSno & ") AND ISNULL(CANCEL,'') = ''"
                StrSql += vbCrLf + " AND ORDSTATE_ID NOT IN (SELECT ORDSTATE_ID FROM " & cnAdminDb & "..ORDERSTATUS WHERE ISNULL(SMITH,'') = 'Y')"
                If oMaterial = Material.Issue And MI_ALLOW_MULTI_ISSUE Then
                    StrSql += vbCrLf + " AND ORSTATUS = 'R' AND CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY WHERE METALID NOT IN ('T','D'))"
                End If
                If Val(objGPack.GetSqlValue(StrSql, "").ToString) = 0 Then
                    ...
                    If TempOrSno <> "" Then
                        StrSql += vbCrLf + " WHERE O.SNO IN (" & TempOrSno & ")"
                    Else
                        StrSql += vbCrLf + " WHERE SUBSTRING(O.ORNO,6,20) = '" & objOrderInfo.OrderNo & "' AND COMPANYID = '" & strCompanyId & "' AND COSTID = '" & BillCostId & "'"
                    End If
                Else
                    StrSql = "SELECT M.METALNAME     AS METAL,ISNULL(CA.CATNAME,C.CATNAME) CATEGORY,IT.ITEMNAME ITEM,SIT.SUBITEMNAME SUBITEM,"
                    StrSql += vbCrLf + " ISNULL((SELECT TOP 1 PCS FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND  ORSTATUS = '" & IIf(oMaterial = Material.Issue, "R", "I") & "' AND ISNULL(CANCEL,'')='' AND ENTRYORDER in  (" & EntOrder & ") AND ORDSTATE_ID<>54 ORDER BY ENTRYORDER DESC),0) PCS"
                    StrSql += vbCrLf + " ,ISNULL((SELECT TOP 1 GRSWT FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = '" & IIf(oMaterial = Material.Issue, "R", "I") & "'  AND ISNULL(CANCEL,'')='' AND ENTRYORDER in  (" & EntOrder & ") AND ORDSTATE_ID<>54 ORDER BY ENTRYORDER DESC),0) GRSWT "
                    StrSql += vbCrLf + " ,ISNULL((SELECT TOP 1 NETWT FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = '" & IIf(oMaterial = Material.Issue, "R", "I") & "'  AND ISNULL(CANCEL,'')='' AND ENTRYORDER in  (" & EntOrder & ") AND ORDSTATE_ID<>54 ORDER BY ENTRYORDER DESC),0) NETWT"
                    StrSql += vbCrLf + " ,ISNULL((SELECT TOP 1 SNO FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = '" & IIf(oMaterial = Material.Issue, "R", "I") & "'  AND ISNULL(CANCEL,'')='' AND ORDSTATE_ID<>54 ORDER BY ENTRYORDER DESC),0) ORIRSNO"
                    StrSql += vbCrLf + " ,O.SNO,O.ORNO "
                    StrSql += vbCrLf + " ,ISNULL((SELECT TOP 1 ORDSTATE_ID FROM " & cnAdminDb & "..ORIRDETAIL WHERE ORSNO = O.SNO AND ORSTATUS = '" & IIf(oMaterial = Material.Issue, "R", "I") & "'  AND ISNULL(CANCEL,'')='' AND ENTRYORDER IN  (" & EntOrder & ") AND ORDSTATE_ID<>54 ORDER BY ENTRYORDER DESC),0) ORDSTATE_ID"
                    StrSql += vbCrLf + " FROM " & cnAdminDb & "..ORMAST O "
            ...
            If _JobNoEnable = False Then
                ...
                If CType(sender, Control).Text <> "" Then
                    If CType(sender, Control).Text.StartsWith("O") Or CType(sender, Control).Text.StartsWith("R") Then
                        'If Mid(CType(sender, Control).Text, 4, 1) = "O" Or Mid(CType(sender, Control).Text, 4, 1) = "R" Then
                        'If objOrderInfo Is Nothing Then
                        Dim Type As String
                        If rbtMetal.Checked Then Type = "M"
                        If rbtOrnament.Checked Then Type = "O"
                        If rbtStone.Checked Then Type = "S"
                        objOrderInfo = New ManuFacturingIssRec_ORDERINFO(CType(sender, Control).Text, BillCostId, IIf(oMaterial = Material.Issue, True, False), IIf(ORDER_MULTI_MIMR, Maccode, ""), Type)
                        ...
                        For cnt As Integer = 0 To objOrderInfo.DgvOrder.Rows.Count - 1
                            If CType(objOrderInfo.DgvOrder.Rows(cnt).Cells("CHECK").Value, Boolean) = True Then
                                ...
                                EntOrder += "'" & objOrderInfo.DgvOrder.Rows(cnt).Cells("ENTRYORDER").Value.ToString & "',"
                                Pcs += Val(objOrderInfo.DgvOrder.Rows(cnt).Cells("PCS").Value.ToString)
                                Grswt += Val(objOrderInfo.DgvOrder.Rows(cnt).Cells("GRSWT").Value.ToString)
                                NetWt += Val(objOrderInfo.DgvOrder.Rows(cnt).Cells("NETWT").Value.ToString)
                                IsMultilple = Val(objOrderInfo.DgvOrder.Rows(cnt).Cells("ISMULTIPLE").Value.ToString)
                                'ORIRSNO += objOrderInfo.DgvOrder.Rows(cnt).Cells("ORIRSNO").Value.ToString & ","
                            End If
                        Next

                        If ORIRSNO Is Nothing Then ORIRSNO = ""
                        If TempOrSno.Contains(",") = True Then
                            TempOrSno = TempOrSno.Substring(0, TempOrSno.Length - 1)
                            EntOrder = EntOrder.Substring(0, EntOrder.Length - 1)
                            ' ORIRSNO = ORIRSNO.Substring(0, ORIRSNO.Length - 1)
                        End If
                        If dgvOrder.Rows.Count > 0 Then
                            For cnt As Integer = 0 To dgvOrder.Rows.Count - 1
                                If dgvOrder.Rows(cnt).Cells("ORSNO").Value.ToString = Replace(TempOrSno, "'", "") Then MsgBox("Order Already Loaded") : Exit Sub
                            Next
                        End If
                        If IsMultilple = 1 Then
                            StrSql = "SELECT TOP 1 SNO FROM  " & cnAdminDb & "..ORIRDETAIL O  WHERE ENTRYORDER IN ( " & EntOrder & ") "
                            ORIRSNO = GetSqlValue(cn, StrSql)
                            StrSql = "SELECT TOP 1 OS.ORDSTATE_NAME FROM  " & cnAdminDb & "..ORIRDETAIL O INNER JOIN " & cnAdminDb & "..ORDERSTATUS OS ON  OS.ORDSTATE_ID = O.ORDSTATE_ID WHERE ENTRYORDER IN ( " & EntOrder & ")  AND ISNULL(SMITH,'') = 'Y'"
                            OrderStatus = GetSqlValue(cn, StrSql)
                        End If
                        If (IsMultilple = 0) Then
                            DtOrder = Get_OrirTableInfo(TempOrSno, EntOrder)
                        Else
                            DtOrder = objOrderInfo.DgvOrder.DataSource()
                        End If
                        DtOrder.AcceptChanges()
                            ...
                            If DtOrder.Rows.Count > 0 Then
                                ...
                                If txtMOrdNo.Focus <> True Then
                                    ...
                                    cmbOAcPostCategory.Text = DtOrder.Rows(0).Item("CATEGORY").ToString : cmbOAcPostCategory.Enabled = False
                                    cmbOIssuedCategory.Text = DtOrder.Rows(0).Item("CATEGORY").ToString : cmbOIssuedCategory.Enabled = False
...
duplicate2:
                                ...
                                Else
                                    'cmbOProcess.Text = "RECEIVED FROM SMITH"
                                    'StrSql = "SELECT TOP 1 ORDSTATE_NAME FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_ID = '" & DtOrder.Rows(0).Item("ORDSTATE_ID").ToString & "' AND ISNULL(SMITH,'') = 'Y'"
                                    'If GetSqlValue(cn, StrSql) = "" Then
                                    '    cmbOProcess.Text = "RECEIVED FROM SMITH"
                                    'Else
                                    '    cmbOProcess.Text = GetSqlValue(cn, StrSql)
                                    'End If
                                    If OrderStatus = "" Then
                                        cmbOProcess.Text = "RECEIVED FROM SMITH"
                                    Else
                                        cmbOProcess.Text = OrderStatus
                                    End If
                                End If
...
showjobs:
    ...
    '                If CType(sender, Control).Text <> "" Then
    '                    If objOrderInfo Is Nothing Then
    '                        objOrderInfo = New ManuFacturingIssRec_ORDERINFO(CType(sender, Control).Text, BillCostId, IIf(oMaterial = Material.Issue, True, False), IIf(ORDER_MULTI_MIMR, Maccode, ""))
    '                        'objOrderInfo.BillCostId = BillCostId
    '                    End If
    '                    If objOrderInfo.ShowDialog() <> Windows.Forms.DialogResult.OK Then
    '                        Exit Sub
        ...
        If e.KeyChar = Chr(Keys.Enter) Then
            ...
            If _JobNoEnable = False Then
                If Mid(txtSOrdNo.Text, 1, 1).ToUpper = "R" Or Mid(txtSOrdNo.Text, 1, 1).ToUpper = "O" Then
                    ...
                    If CType(sender, Control).Text <> "" Then
                        If objOrderInfo Is Nothing Then
                            objOrderInfo = New ManuFacturingIssRec_ORDERINFO(CType(sender, Control).Text, BillCostId, IIf(oMaterial = Material.Issue, True, False), IIf(ORDER_MULTI_MIMR, Maccode, ""), "S")
                            'objOrderInfo.BillCostId = BillCostId
                        End If
                        If objOrderInfo.ShowDialog() <> Windows.Forms.DialogResult.OK Then
                            Exit Sub
                        End If
                        Dim TempOrSno As String = ""
                        Dim TempOrIrSno As String = ""
                        For cnt As Integer = 0 To objOrderInfo.DgvOrder.Rows.Count - 1
                            If CType(objOrderInfo.DgvOrder.Rows(cnt).Cells("CHECK").Value, Boolean) = True Then
                                TempOrSno += "'" & objOrderInfo.DgvOrder.Rows(cnt).Cells("SNO").Value.ToString & "',"
                                If objOrderInfo.DgvOrder.Columns.Contains("orirsno") Then
                                    TempOrIrSno += "'" & objOrderInfo.DgvOrder.Rows(cnt).Cells("ORIRSNO").Value.ToString & "',"
                                End If
                            End If
                        Next
                        If TempOrSno.Contains(",") = True Then
                            TempOrSno = TempOrSno.Substring(0, TempOrSno.Length - 1)
                            If TempOrIrSno.Contains(",") = True Then TempOrIrSno = TempOrIrSno.Substring(0, TempOrIrSno.Length - 1)
                        End If
                            LoadMetalDetails(cmbSMetal)
                        LoadCategoryDetails(cmbSCategory)
                        LoadItemDetail(cmbSItem)
                        ...
                        Dim NAMES = From row In dtGridStuddedStone
                                    Select row.Field(Of String)("REFSNO") Distinct
                        ...
                        If rbtStone.Checked Then
                            ...
                            StrSql += vbCrLf + " AND ORDSTATE_ID NOT IN (SELECT ORDSTATE_ID FROM " & cnAdminDb & "..ORDERSTATUS WHERE ISNULL(SMITH,'') = 'Y')"
                            If oMaterial = Material.Issue And MI_ALLOW_MULTI_ISSUE Then
                                StrSql += vbCrLf + " AND ORSTATUS = 'R' AND CATCODE IN (SELECT CATCODE FROM " & cnAdminDb & "..CATEGORY WHERE METALID IN ('T','D'))"
                            End If
                            If Val(objGPack.GetSqlValue(StrSql, "").ToString) = 0 Then

                                StrSql = vbCrLf + "SELECT M.METALNAME AS METAL,ISNULL(CA.CATNAME,C.CATNAME) CATEGORY,IT.ITEMNAME ITEM,SIT.SUBITEMNAME SUBITEM,"
                                StrSql += vbCrLf + " OS.STNPCS-ISNULL((SELECT SUM(isnull(STNPCS,0)) FROM " & cnAdminDb & "..ORIRDETAILSTONE  WHERE ORSNO = ORD.SNO  "
                                StrSql += vbCrLf + " AND ISNULL(CANCEL,'')=''),0) PCS,OS.STNWT -ISNULL((SELECT SUM(isnull(STNWT,0)) "
                                StrSql += vbCrLf + " FROM " & cnAdminDb & "..ORIRDETAILSTONE OS WHERE ORSNO = ORD.SNO   AND ISNULL(CANCEL,'')=''),0) WEIGHT "
                                StrSql += vbCrLf + " , OS.STNUNIT,ORD.ORDSTATE_ID"
                                StrSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),100) PURITY,O.SNO,O.ORNO,OS.CUTID, OS.COLORID,OS.CLARITYID,OS.SHAPEID ,OS.SETTYPEID,OS.HEIGHT,OS.WIDTH,''SEIVE,OS.SNO ORIRSNO, ROW_NUMBER()OVER(ORDER BY IT.ITEMNAME,SIT.SUBITEMNAME)ROWID  FROM " & cnAdminDb & "..ORSTONE  OS  "
                                StrSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..ORMAST AS O ON OS.ORSNO = O.SNO AND ISNULL(O.CANCEL,'') = ''"
                                StrSql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..ORIRDETAIL AS ORD ON ORD.ORSNO = O.SNO AND ORD.ORSTATUS= 'R' AND ISNULL(ORD.CANCEL,'') = ''"
                                ...
                                StrSql += vbCrLf + " AND OS.SNO NOT IN ('" & ORDSNO & "') "
                                StrSql += vbCrLf + " AND ISNULL(O.CANCEL,'') = '' AND ISNULL(ORD.CANCEL,'') = '' "
                            Else
                                StrSql = vbCrLf + "SELECT"
                                StrSql += vbCrLf + "M.METALNAME AS METAL,ISNULL(CA.CATNAME,C.CATNAME) CATEGORY,IT.ITEMNAME ITEM,SIT.SUBITEMNAME SUBITEM,"
                                StrSql += vbCrLf + " OS.PCS-ISNULL((SELECT SUM(ISNULL(STNPCS,0)) FROM " & cnAdminDb & "..ORIRDETAILSTONE  WHERE ORSNO = ORD.SNO  "
                                StrSql += vbCrLf + " AND ISNULL(CANCEL,'')=''),0) PCS,OS.GRSWT WEIGHT "
                                StrSql += vbCrLf + " ,CASE WHEN OS.STONEUNIT = 'C' THEN 'CARET' ELSE 'GRM' END STNUNIT "
                                StrSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),100) PURITY,O.SNO,O.ORNO,OS.CUTID, OS.COLORID,OS.CLARITYID,OS.SHAPEID "
                                StrSql += vbCrLf + " ,ORD.ORDSTATE_ID,OS.SETTYPEID,OS.HEIGHT,OS.WIDTH,SEIVE,ORD.SNO ORIRSNO, ROW_NUMBER()OVER(ORDER BY IT.ITEMNAME,SIT.SUBITEMNAME)ROWID  "
                                If oMaterial = Material.Issue Then
                                    StrSql += vbCrLf + "  FROM " & cnStockDb & "..RECEIPT OS"
                                Else
                                    StrSql += vbCrLf + "  FROM " & cnStockDb & "..ISSUE OS"
                                End If
                                StrSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..ORIRDETAIL AS ORD ON OS.BATCHNO  = ORD.BATCHNO AND OS.SNO = ORD.TRANSNO"
                                StrSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..ORMAST AS O ON ORD.ORSNO = O.SNO"
                                StrSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..ITEMMAST IT ON OS.ITEMID=IT.ITEMID "
                                StrSql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..SUBITEMMAST SIT ON OS.SUBITEMID = SIT.SUBITEMID "
                                StrSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..METALMAST M ON M.METALID = IT.METALID  "
                                StrSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY C ON C.CATCODE= IT.CATCODE  "
                                StrSql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..OUTSTANDING OU ON OU.BATCHNO= O.BATCHNO AND OU.RUNNO=O.ORNO  "
                                StrSql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..CATEGORY CA ON CA.CATCODE= OU.CATCODE  "
                                If TempOrSno <> "" Then
                                    StrSql += vbCrLf + " WHERE O.SNO IN (" & TempOrSno & ")"
                                    If TempOrIrSno <> "" Then StrSql += vbCrLf + " AND ORD.SNO IN (" & TempOrIrSno & ")"
                                Else
                                    StrSql += vbCrLf + " WHERE SUBSTRING(O.ORNO,6,20) = '" & objOrderInfo.OrderNo & "' AND COMPANYID = '" & strCompanyId & "' AND COSTID = '" & BillCostId & "'"
                                End If
                                StrSql += vbCrLf + " AND ORD.SNO NOT IN ('" & ORDSNO & "') "
                                StrSql += vbCrLf + " AND ISNULL(OS.CANCEL,'') = '' AND ISNULL(ORD.CANCEL,'') = '' "
                            End If
                            Da = New OleDbDataAdapter(StrSql, cn)
                            Da.Fill(DtOrder)
                            DtOrder.AcceptChanges()
                            If DtOrder.Rows.Count > 0 Then
                                ReturnValue = BrighttechPack.SearchDialog.Show("Searching CateGory", StrSql, cn, 3, 20)
                                For cnt As Integer = 0 To DtOrder.Rows.Count - 1
                                    If ReturnValue = "" Then Exit For
                                    If DtOrder.Rows(cnt).Item("ROWID") = ReturnValue Then
                                        RowIndex = cnt
                                    End If
                                Next

                                cmbSCategory.Text = DtOrder.Rows(RowIndex).Item("CATEGORY").ToString : cmbSCategory.Enabled = False
                                'cmbSAcPostCategory.Items.Add(DtOrder.Rows(RowIndex).Item("CATEGORY").ToString)
                                cmbSAcPostCategory.Text = DtOrder.Rows(RowIndex).Item("CATEGORY").ToString : cmbSAcPostCategory.Enabled = False
                                cmbSIssuedCategory.Text = DtOrder.Rows(RowIndex).Item("CATEGORY").ToString : cmbSIssuedCategory.Enabled = False
                                cmbSMetal.Text = DtOrder.Rows(RowIndex).Item("METAL").ToString : cmbSMetal.Enabled = False
                                If oMaterial = Material.Receipt Then
                                    cmbSItem.Enabled = False
                                    cmbSSubItem.Enabled = False
                                Else
                                    ...
                                    cmbSSubItem.Enabled = True
                                End If
                                cmbSItem.Text = DtOrder.Rows(RowIndex).Item("ITEM").ToString
                                cmbSSubItem.Text = DtOrder.Rows(RowIndex).Item("SUBITEM").ToString
                                CmbSPurity.Text = DtOrder.Rows(RowIndex).Item("PURITY").ToString
                                txtSPcs_NUM.Text = DtOrder.Rows(RowIndex).Item("PCS").ToString
                                txtSGrsWt_WET.Text = DtOrder.Rows(RowIndex).Item("WEIGHT").ToString
                                ORSNO = DtOrder.Rows(RowIndex).Item("SNO").ToString
                                ORNO = DtOrder.Rows(RowIndex).Item("ORNO").ToString
                                ColorId = DtOrder.Rows(RowIndex).Item("COLORID").ToString
                                CutId = DtOrder.Rows(RowIndex).Item("CUTID").ToString
                                ClarityId = DtOrder.Rows(RowIndex).Item("CLARITYID").ToString
                                ShapeId = DtOrder.Rows(RowIndex).Item("SHAPEID").ToString
                                SetTypeId = DtOrder.Rows(RowIndex).Item("SETTYPEID").ToString
                                StnHeight = DtOrder.Rows(RowIndex).Item("HEIGHT").ToString
                                StnWidth = DtOrder.Rows(RowIndex).Item("WIDTH").ToString
                                ORIRSNO = DtOrder.Rows(RowIndex).Item("ORIRSNO").ToString
                            Else
                                MsgBox("There Is No Stone Details Found For Selection") : Exit Sub
                            End If
                            ...
                            Else
                                If DtOrder.Rows.Count > 0 Then
                                    StrSql = "SELECT TOP 1 ORDSTATE_NAME FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_ID = '" & DtOrder.Rows(RowIndex).Item("ORDSTATE_ID").ToString & "' AND ISNULL(SMITH,'') = 'Y'"
                                End If
                                If GetSqlValue(cn, StrSql) = "" Then
                                    cmbOProcess.Text = "RECEIVED FROM SMITH"
                                    cmbMProcess.Text = "RECEIVED FROM SMITH"
                                ...
                                Else
                                    cmbOProcess.Text = GetSqlValue(cn, StrSql)
                                    cmbMProcess.Text = GetSqlValue(cn, StrSql)
                                    cmbSProcess.Text = GetSqlValue(cn, StrSql)
                                End If
                            ...
                            '                            Dim mDr As DataRow
                            '                            mDr = MdtOdDet.NewRow
                            '                            mDr("ORNO") = DtOrder.Rows(RowIndex)("ORNO").ToString
                            '                            mDr("ORSNO") = DtOrder.Rows(RowIndex)("SNO").ToString
                            '                            mDr("PCS") = DtOrder.Rows(RowIndex)("PCS").ToString
                            '                            ''mDr("GRSWT") = IIf(DtOrder.Rows(i)("STNUNIT").ToString = "C", Val(DtOrder.Rows(i)("WEIGHT").ToString) / 5, Val(DtOrder.Rows(i)("WEIGHT").ToString))
                            '                            ''mDr("NETWT") = IIf(DtOrder.Rows(i)("STNUNIT").ToString = "C", Val(DtOrder.Rows(i)("WEIGHT").ToString) / 5, Val(DtOrder.Rows(i)("WEIGHT").ToString))
                            '                            mDr("GRSWT") = Val(DtOrder.Rows(RowIndex)("WEIGHT").ToString)
                            '                            mDr("NETWT") = Val(DtOrder.Rows(RowIndex)("WEIGHT").ToString)
                            '                            MdtOdDet.Rows.Add(mDr)
                            '                            MdtOdDet.AcceptChanges()
                            '                            'Next
                            'againskip:
    ...
    Private Sub txtOAmount_AMT_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOAmount_AMT.LostFocus
        ...
        Dim vatAmt As Decimal = Val(txtOVat_AMT.Text)
        'If oTransactionType = "PURCHASE RETURN" Then vatAmt = vatAmt * -1
        grsAmt = Val(txtOAmount_AMT.Text) - IIf(lblOVat.Text.ToUpper = "TDS", -1 * vatAmt, vatAmt) - Val(txtOED_AMT.Text)
        If lblOVat.Text.ToUpper = "TDS" Then
    ...
    Private Sub txtMAmount_AMT_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMAmount_AMT.LostFocus
        ...
        Dim vatAmt As Decimal = Val(txtMVat_AMT.Text)
        'If oTransactionType = "PURCHASE RETURN" Then vatAmt = vatAmt * -1
        grsAmt = Val(txtMAmount_AMT.Text) - IIf(lblMVat.Text.ToUpper = "TDS", -1 * vatAmt, vatAmt)
        If lblMVat.Text.ToUpper = "TDS" Then
    ...
    Private Sub txtSAmount_AMT_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSAmount_AMT.LostFocus
        ...
        Dim vatAmt As Decimal = Val(txtSVat_AMT.Text)
        'If oTransactionType = "PURCHASE RETURN" Then vatAmt = vatAmt * -1
        grsAmt = Val(txtSAmount_AMT.Text) - IIf(lblSVat.Text.ToUpper = "TDS", -1 * vatAmt, vatAmt)
        If lblSVat.Text.ToUpper = "TDS" Then
    ...
    Private Sub Getdealerwmc(Optional ByVal wt As Double = 0)
        ...
        If editflag = False Then
            ...
            If txtOOrdNo.Text = "" Then txtOTouchAMT.Text = ""
    ...
    Private Sub txtSRemark2_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtSRemark2.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            ...
            If txtStRowIndex.Text = "" Then
                Dim mDr As DataRow
                mDr = MdtOdDet.NewRow
                mDr("ORNO") = ORNO.ToString
                mDr("ORSNO") = ORSNO.ToString
                mDr("PCS") = Val(txtSPcs_NUM.Text)
                ''mDr("GRSWT") = IIf(DtOrder.Rows(i)("STNUNIT").ToString = "C", Val(DtOrder.Rows(i)("WEIGHT").ToString) / 5, Val(DtOrder.Rows(i)("WEIGHT").ToString))
                ''mDr("NETWT") = IIf(DtOrder.Rows(i)("STNUNIT").ToString = "C", Val(DtOrder.Rows(i)("WEIGHT").ToString) / 5, Val(DtOrder.Rows(i)("WEIGHT").ToString))
                mDr("GRSWT") = Val(txtSGrsWt_WET.Text)
                mDr("NETWT") = Val(txtSGrsWt_WET.Text)
                MdtOdDet.Rows.Add(mDr)
                MdtOdDet.AcceptChanges()
            End If
            dgvOrder.DataSource = MdtOdDet
...
showjobs:
    ...
    Private Sub txtOGrsWt_WET_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOGrsWt_WET.Leave, txtMGrsWt_WET.Leave, txtSGrsWt_WET.Leave
        If Order_WithOutGrswt = True Then Exit Sub
        If STOCKVALIDATION And oMaterial = Material.Issue And txtTagNo.Text.Trim = "" Then
            If CheckOrdRep() Then Exit Sub
    ...
    Private Sub txtTagNo_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtTagNo.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            StrSql += vbCrLf + "  T.NETWT,T.LESSWT,T.GRSNET,PT.PURTOUCH "
            ...
            StrSql += vbCrLf + "  LEFT JOIN " & cnAdminDb & "..PURITEMTAG AS PT ON PT.TAGNO=T.TAGNO"
            ...
            StrSql += vbCrLf + "  AND ISNULL(T.APPROVAL,'')='" & cnCostId & "'"
            Cmd = New OleDbCommand(StrSql, cn)
            Da = New OleDbDataAdapter(Cmd)
            ...
            If dtTag.Rows.Count > 0 Then
                ...
                ElseIf rbtOrnament.Checked Then
                    ...
                    If oTransactionType = "PURCHASE RETURN" Then
                        StrSql += vbCrLf + "  ,T.PURRATE AS RATE ,T.PURAMT AS AMT"
                        StrSql += vbCrLf + "  FROM " & cnAdminDb & "..PURITEMTAGSTONE AS T "
                    Else
                    ...
                    If oTransactionType = "PURCHASE RETURN" Then
                        StrSql += vbCrLf + "  AND TAGSNO IN(SELECT TAGSNO FROM " & cnAdminDb & "..ITEMTAGSTONE "
                        StrSql += vbCrLf + "  WHERE TAGNO = '" & txtTagNo.Text.ToString.Trim & "' "
    ...
    Private Sub txtOSgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOSgst_AMT.TextChanged
        If Val(txtOSgst_AMT.Text) = 0 Then
            txtOSgst_AMT.Clear()
            Exit Sub
        End If
        CalcOGST()
    ...
    Private Sub txtOCgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOCgst_AMT.TextChanged
        If Val(txtOCgst_AMT.Text) = 0 Then
            txtOCgst_AMT.Clear()
            Exit Sub
        End If
        CalcOGST()
    ...
    Private Sub txtOIgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOIgst_AMT.TextChanged
        If Val(txtOIgst_AMT.Text) = 0 Then
            txtOIgst_AMT.Clear()
            Exit Sub
        End If
        CalcOGST()
    ...
    Private Sub txtMSgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMSgst_AMT.TextChanged
        If Val(txtMSgst_AMT.Text) = 0 Then
            txtMSgst_AMT.Clear()
            Exit Sub
        End If
        CalcMGST()
    ...
    Private Sub txtMCgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMCgst_AMT.TextChanged
        If Val(txtMCgst_AMT.Text) = 0 Then
            txtMCgst_AMT.Clear()
            Exit Sub
        End If
        CalcMGST()
    ...
    Private Sub txtMIgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMIgst_AMT.TextChanged
        If Val(txtMIgst_AMT.Text) = 0 Then
            txtMIgst_AMT.Clear()
            Exit Sub
        End If
        CalcMGST()
    ...
    Private Sub txtSIgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSIgst_WET.TextChanged
        If Val(txtSIgst_WET.Text) = 0 Then
            txtSIgst_WET.Clear()
            Exit Sub
        End If
        CalcSGST()
    ...
    Private Sub txtSCgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSCgst_WET.TextChanged
        If Val(txtSCgst_WET.Text) = 0 Then
            txtSCgst_WET.Clear()
            Exit Sub
        End If
        CalcSGST()
    ...
    Private Sub txtSSgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSSgst_WET.TextChanged
        If Val(txtSSgst_WET.Text) = 0 Then
            txtSSgst_WET.Clear()
            Exit Sub
        End If
        CalcSGST()
    ...
    Private Sub txtOthSgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOthSgst_AMT.TextChanged
        If Val(txtOthSgst_AMT.Text) = 0 Then
    ...
    Private Sub txtMGrsWt_WET_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txtMGrsWt_WET.KeyDown
        If e.KeyCode = Keys.Enter Then
            If ORADDITIONALDETAIL = True Then
                ShowOrderAdditionalDetails()
            End If
    ...
    Private Sub GridStuddStone_KeyPress(sender As Object, e As KeyPressEventArgs) Handles GridStuddStone.KeyPress
        Dim rowIndex As Integer = Nothing
        If GridStuddStone.Rows.Count > 0 Then
            rowIndex = GridStuddStone.CurrentRow.Index
            dtGridStuddedStone.Rows.Remove(dtGridStuddedStone.Rows(rowIndex))
            dtGridStuddedStone.AcceptChanges()
        End If
    End Sub
    ...
    Private Sub txtOthCgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOthCgst_AMT.TextChanged
        If Val(txtOthCgst_AMT.Text) = 0 Then
    ...
    Private Sub txtOthIgst_AMT_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtOthIgst_AMT.TextChanged
        If Val(txtOthIgst_AMT.Text) = 0 Then
