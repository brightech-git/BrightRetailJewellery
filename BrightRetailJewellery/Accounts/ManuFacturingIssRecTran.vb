...
Public Class ManuFacturingIssRecTran
    ...
    Private ObjMaterialDia As ManuFacturingIssRec
    ...
    Public xOordno, xMordno As String
    Public xOProcessname, xMProcessname As String
    ...
    Private Sub Initializer()
        ...
        With DtTran
            ...
            .Columns.Add("VAT", GetType(Decimal))
            ...
            .Columns.Add("METISSREC", GetType(ManuFacturingIssRec))
            ...
            .Columns.Add("REFSNO", GetType(String))
    ...
    Private Sub LoadTransactionType()
        ...
        For Each Ro As DataRow In DtTran.Rows
            If Ro.RowState <> DataRowState.Deleted Then
                ...
                'grsWt += IIf(Ro.Item("UNIT").ToString = "CARAT", Val(Ro.Item("GRSWT").ToString) / 5, Val(Ro.Item("GRSWT").ToString))
                'netWt += IIf(Ro.Item("UNIT").ToString = "CARAT", Val(Ro.Item("NETWT").ToString) / 5, Val(Ro.Item("NETWT").ToString))
                'pureWt += IIf(Ro.Item("UNIT").ToString = "CARAT", Val(Ro.Item("PUREWT").ToString) / 5, Val(Ro.Item("PUREWT").ToString))
    ...
    Private Sub ManuFacturingIssRecTran_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub ManuFacturingIssRecTran_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        End If
        If MRMI_ORDERNO And ORDER_MULTI_MIMR Then
            lblOrdNo.Visible = True
            txtOrdNo.Visible = True
        End If
        ...
        End If
        GstFlag = GST
        PicPath = GetAdmindbSoftValue("OR_PICPATH")
        defaultPic = GetAdmindbSoftValue("TAGCATALOGPATH")
    ...
    Private Sub LoadEditValues()
        ...
        StrSql = vbCrLf + "  SELECT SUM(AMOUNT) "
        ...
        StrSql += vbCrLf + "  ,DISCOUNT"
        StrSql += vbCrLf + "  ,(SELECT GROUPNAME FROM " & cnAdminDb & "..STONEGROUP WHERE GROUPID = I.STNGRPID)STNGRPNAME"
        If OMaterialType = MaterialType.Issue Then
            StrSql += vbCrLf + "  FROM " & cnStockDb & "..ISSUE AS I"
        Else
            StrSql += vbCrLf + "  FROM " & cnStockDb & "..RECEIPT AS I"
        ...
        For Each Ro In dtTranInfo.Rows
            ...
            ObjMaterialDia = New ManuFacturingIssRec(OMaterialType, cmbTransactionType.Text, dtpTrandate.Value, _Accode)
            ...
            With ObjMaterialDia
                ...
                If Ro.Item("FLAG").ToString = "O" Then
                    ...
                    .cmbOGrsNet.Text = Ro.Item("GRSNET").ToString
                    ...
                    .txtOVatPer_AMT.Text = Ro.Item("VATPER").ToString
    ...
    Private Sub btnNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        tabGroup.SelectedTab = TabGridView
        lblRunno.Text = ""
        lblRunno.Visible = True
        picOrder.Image = Nothing
        MdtOdDet.Clear()
        LoadTransactionType()
    ...
    Private Sub OriDetail(ByVal ObjMaterialDia As ManuFacturingIssRec, ByVal _Index As Integer)
        With ObjMaterialDia
            'ORIRDETAIL 
            If ObjMaterialDia.dgvOrder.Rows.Count > 0 Then
                ...
                For k As Integer = 0 To ObjMaterialDia.dgvOrder.Rows.Count - 1
                    ...
                    ElseIf .rbtStone.Checked Then
                        mdr("GRSWT") = Val(ObjMaterialDia.dgvOrder.Rows(k).Cells("GRSWT").Value.ToString)
                        mdr("NETWT") = Val(ObjMaterialDia.dgvOrder.Rows(k).Cells("NETWT").Value.ToString)
                    End If
                    mdr("ROWINDEX") = _Index
    ...
    Private Sub LoadTransaction(ByVal ObjMaterialDia As ManuFacturingIssRec)
        With ObjMaterialDia
            ...
            If .rbtOrnament.Checked Then
                ...
                DtTran.Rows(index).Item("GROSSAMT") = IIf(Val(.txtOGrsAmt_AMT.Text) <> 0, Val(.txtOGrsAmt_AMT.Text), DBNull.Value)
                DtTran.Rows(index).Item("VATPER") = IIf(Val(.txtOVatPer_AMT.Text) <> 0, Val(.txtOVatPer_AMT.Text), DBNull.Value)
                ...
                DtTran.Rows(index).Item("REFSNO") = .ORIRSNO.ToString
            ...
            ElseIf .rbtStone.Checked Then
                If (EditBatchno <> Nothing Or .oEditRowIndex <> -1) Then
                    ...
                    'DtTran.Rows(index).Item("REFSNO") = IIf(Val(.txtODisc.Text) <> 0, Val(.txtODisc.Text), DBNull.Value)
                    DtTran.Rows(index).Item("REFSNO") = .ORIRSNO.ToString
                    DgvTran.AutoResizeRow(index)
                Else
                    If .GridStuddStone.Rows.Count > 0 Then
                        ...
                        For i As Integer = 0 To .GridStuddStone.Rows.Count - 1
                            ...
                            DtTran.Rows(index).Item("REFSNO") = .GridStuddStone.Rows(i).Cells("REFSNO").Value.ToString
                            DgvTran.AutoResizeRow(index)
                            index = index + 1
                        Next
                    End If
    ...
    Private Sub cmbTransactionType_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbTransactionType.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If GstFlag And (UCase(cmbTransactionType.Text) = "PURCHASE RETURN" Or _
                         UCase(cmbTransactionType.Text) = "PURCHASE" Or _
                         UCase(cmbTransactionType.Text) = "PURCHASE[APPROVAL]" Or _
                         UCase(cmbTransactionType.Text) = "INTERNAL TRANSFER" Or _
            ...
            If cmbTransactionType.Text = "PURCHASE[APPROVAL]" Then
                ...
                End If
            End If
            ObjMaterialDia = New ManuFacturingIssRec(OMaterialType, cmbTransactionType.Text, dtpTrandate.Value, _Accode, OrderSnos, CostCenterId, txtOrdNo.Text, SRVTPER, GstFlag)
            ObjMaterialDia.Text = "MATERIAL " & cmbTransactionType.Text
            ...
            If ObjMaterialDia.ShowDialog = Windows.Forms.DialogResult.OK Then
                ...
                If ObjMaterialDia.rbtOrnament.Checked Then
                    ...
                    If xOpurityper = Nothing Then xOpurityper = Val(ObjMaterialDia.CmbOPurity.Text)
                ElseIf ObjMaterialDia.rbtMetal.Checked Then
                    ...
                    If xMpurityper = Nothing Then xMpurityper = Val(ObjMaterialDia.CmbMPurity.Text)
                End If
            End If
        ElseIf e.KeyCode = Keys.Escape Then
    ...
    Private Sub DgvTran_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles DgvTran.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If DgvRow.Cells("TRANTYPE").Value.ToString <> "" Then
                Dim obj As ManuFacturingIssRec
                obj = CType(DgvRow.Cells("METISSREC").Value, ManuFacturingIssRec)
                obj.editflag = True
...
GenBillNo:
            ...
            If AccPost Then
                ...
                InsertIntoAccTran(TranNo, TranMode, _
...
GETRUNNO:
            ...
            If IO.File.Exists(Application.StartupPath & "\BillPrint.exe") Then
                ...
                Else
                    ...
                    LSet("TYPE", 15) & IIf(OMaterialType = MaterialType.Receipt, ":SMR", ":SMI") & ";" & _
        ...
        For Each roCategory As DataRow In dtCategory.Rows
            ...
            For Each roMath As DataRow In matchTable.Rows
                ...
                If cmbTransactionType.Text <> "INTERNAL TRANSFER" Then
                    ...
                    For Each stRow As DataRow In CType(roMath.Item("METISSREC"), ManuFacturingIssRec).objStone.dtGridStone.Rows
                    ...
                    For Each stRow As DataRow In CType(roMath.Item("METISSREC"), ManuFacturingIssRec).objMisc.dtGridMisc.Rows
                    ...
                    Next
                    For Each stRow As DataRow In CType(roMath.Item("METISSREC"), ManuFacturingIssRec).ObjAlloy.dtGridAlloy.Rows
            ...
            If OMaterialType = MaterialType.Receipt Then
                ...
                If amt <> 0 Then
                    ...
                    ElseIf cmbTransactionType.Text = "PURCHASE" Or cmbTransactionType.Text = "PURCHASE[APPROVAL]" Then
                        ...
                        InsertIntoAccTran(tNo, IIf(tranMode = "D", "C", "D"),
                        _Accode,
                        amt + vatAmt + stnVat + TempVat + EDAmt, pcs, grsWt, netWT, payMode, accode, txtBillNo.Text, dtpBillDate_OWN.Value.ToString("yyyy-MM-dd"))
                        ...
                        If (vatAmt <> 0) Then
                            If GstFlag Then
                                If InterStateBill = False Then
                                    ...
                                    StrSql += " ,'" & dtpBillDate_OWN.Value.ToString("yyyy-MM-dd") & "'" 'TRANDATE
                                    StrSql += " ,'" & payMode & "'"
            ...
            Else
                ...
                If amt <> 0 Then
                    ...
                    ElseIf cmbTransactionType.Text = "PURCHASE RETURN" Then
                        ...
                        InsertIntoAccTran(tNo, IIf(tranMode = "D", "C", "D"),
                       _Accode,
                       amt + vatAmt + stnVat, pcs, grsWt, netWT, payMode, accode, txtBillNo.Text, dtpBillDate_OWN.Value.ToString("yyyy-MM-dd"))
                    Else
                        TdsRemark = "TDS Rs " & Format(vatAmt, "0.00") & " @" & Format(TdsPer, "0.00") & "% for " & Format(amt, "0.00")
                        tranMode = "C"
                        payMode = "TI"
...
Optional ByVal Remark2 As String = Nothing _
    ...
    Private Sub Save_WithOutJobNo(ByVal index As Integer)
        With DgvTran.Rows(index)
            Dim Obj As ManuFacturingIssRec
            Obj = CType(.Cells("METISSREC").Value, ManuFacturingIssRec)
            ...
            Select Case Type
                Case "O" 'Ornament
                    ...
                    End If
                    If TdsPer = Nothing Then TdsPer = Val(Obj.txtOVatPer_AMT.Text)
                Case "M" 'Metal
            ...
            If OMaterialType = MaterialType.Receipt And .Cells("JOBNO").Value.ToString <> "" And Mid(.Cells("JOBNO").Value.ToString, 1, 1) = "R" Then
                ...
                StrSql += " ," & TranNo & "" 'TRANNO
                StrSql += " ,'" & dtpTrandate.Value.ToString("yyyy-MM-dd") & "'" 'TRANDATE
                StrSql += " ,'" & IIf(OMaterialType = MaterialType.Issue, "I", "R") & "" & Mid(.Cells("TRANTYPE").Value.ToString, 1, 2) & "'" 'TRANTYPE
            ...
            Else
                ...
                If cmbTransactionType.Text = "PURCHASE[APPROVAL]" Then
                    ...
                    If Not drr Is Nothing Then
                        ...
                        RecPurewt = funcCalcOPureWt(Mid(.Cells("GRSNET").Value.ToString, 1, 1), _
            ...
            dtOrIrStone = CType(DgvTran.Rows(index).Cells("METISSREC").Value, ManuFacturingIssRec).objStone.dtGridStone.Copy
            Dim Dpcs, Spcs, Dwt, Swt As Decimal
            ''Stone
            Dpcs = 0 : Spcs = 0 : Dwt = 0 : Swt = 0
            For Each stRow As DataRow In CType(.Cells("METISSREC").Value, ManuFacturingIssRec).objStone.dtGridStone.Rows
            ...
            dtemp = CType(.Cells("METISSREC").Value, ManuFacturingIssRec).objAddtionalDetails.dtOrAdditionalDetails
            Dim Trantype As String
            For Each dr As DataRow In CType(.Cells("METISSREC").Value, ManuFacturingIssRec).objAddtionalDetails.dtOrAdditionalDetails.Rows
            ...
            Next
            ''Misc
            For Each miscRow As DataRow In CType(.Cells("METISSREC").Value, ManuFacturingIssRec).objMisc.dtGridMisc.Rows
                InsertMiscDetails(issSno, TranNo, miscRow)
            Next
            ''ALLOY
            If CType(.Cells("METISSREC").Value, ManuFacturingIssRec).ObjAlloy.dtGridAlloy.Rows.Count > 0 Then
                For Each stRow As DataRow In CType(.Cells("METISSREC").Value, ManuFacturingIssRec).ObjAlloy.dtGridAlloy.Rows
            ...
            '        Dim R() As DataRow = CType(CType(.Cells("METISSREC").Value, ManuFacturingIssRec).objOrderInfo.DgvOrder.DataSource, DataTable).Select("CHECK = 'TRUE'")
            ...
            '        If OMaterialType = MaterialType.Receipt Then
            '            For Each DgvRow As DataGridViewRow In CType(.Cells("METISSREC").Value, ManuFacturingIssRec).objOrderInfo.DgvOrder.Rows
            ...
            If _JobNoEnable = False Then
                ...
                dttt = dgvOrderDet.DataSource
                If dgvOrderDet.Rows.Count > 0 Then
                    Dim DtOrIrDetail As New DataTable
                    DtOrIrDetail = BrighttechPack.GetTableStructure(cnAdminDb, "ORIRDETAIL", cn, tran)
                    Dim EntryOrder As Integer = objGPack.GetSqlValue("SELECT ISNULL(MAX(ENTRYORDER),0) FROM " & cnAdminDb & "..ORIRDETAIL", , , tran)
                    Dim RowOrder As DataRow = Nothing
                    Dim DgvRow As DataGridViewRow = dgvOrderDet.Rows(index)
                    If OMaterialType = MaterialType.Issue Then
                        ...
                        Else
                            ...
                            RowOrder("TRANNO") = TranNo
                            RowOrder("TRANDATE") = dtpTrandate.Value.ToString("yyyy-MM-dd")
                            RowOrder("ENTRYDATETIME") = dtpTrandate.Value.ToString("yyyy-MM-dd") & " " & Now.ToShortTimeString()
                            RowOrder("DESIGNERID") = Val(objGPack.GetSqlValue(" SELECT DESIGNERID FROM " & cnAdminDb & "..DESIGNER WHERE ISNULL(ACCODE,'') = '" & _Accode & "'", , , tran))
                            'RowOrder("PCS") = IIf(SelectedOrderCount = 1, Val(.Cells("PCS").Value.ToString), Val(DgvRow.Cells("PCS").Value.ToString))
                            'RowOrder("GRSWT") = IIf(SelectedOrderCount = 1, Val(.Cells("GRSWT").Value.ToString), Val(DgvRow.Cells("GRSWT").Value.ToString))
                            'RowOrder("NETWT") = IIf(SelectedOrderCount = 1, Val(.Cells("NETWT").Value.ToString), Val(DgvRow.Cells("NETWT").Value.ToString))
                            ...
                            RowOrder("PROID") = DBNull.Value
                            'RowOrder("ORDSTATE_ID") = objGPack.GetSqlValue("SELECT ORDSTATE_ID FROM " & cnAdminDb & "..ORDERSTATUS WHERE ORDSTATE_NAME = '" & .Cells("ORDSTATE_NAME").Value.ToString & "'", , , tran)
                            RowOrder("ORDSTATE_ID") = OrdStateId 'IIf(OMaterialType = MaterialType.Issue, 2, 3)
                            RowOrder("ENTRYORDER") = EntryOrder
                            RowOrder("CATCODE") = catCode
                            RowOrder("ACCODE") = _Accode
                            RowOrder("TRANSNO") = issSno
                            RowOrder("REFSNO") = .Cells("REFSNO").Value.ToString
                            DtOrIrDetail.Rows.Add(RowOrder)
                        End If
                    Else
                        RowOrder = DtOrIrDetail.NewRow
            ...
            End If
            If OMaterialType = MaterialType.Receipt Then
                StrSql = "UPDATE " & cnAdminDb & "..ORIRDETAIL SET REFSNO = '" & issSno & "' WHERE SNO = '" & .Cells("REFSNO").Value.ToString & "'"
            ...
            If Lotautopost And OMaterialType = MaterialType.Receipt _
                ...
                If ACC_LOTAUTOPOST_TYPE <> "" Then
                    If Type <> "T" And ACC_LOTAUTOPOST_TYPE.Contains("R" & Mid(cmbTransactionType.Text, 1, 2)) Then
                        Call LOTISSUES(TranNo, issSno, _Accode, Itemid, subItemid, index, Val(.Cells("PCS").Value.ToString), Val(.Cells("Grswt").Value.ToString), Val(.Cells("Netwt").Value.ToString), Val(.Cells("TOUCH").Value.ToString), Val(.Cells("Wastage").Value.ToString), Val(.Cells("Mc").Value.ToString), Isedit, lotDiaWt, _StkType)
                    End If
                Else
                    Call LOTISSUES(TranNo, issSno, _Accode, Itemid, subItemid, index, Val(.Cells("PCS").Value.ToString), Val(.Cells("Grswt").Value.ToString), Val(.Cells("Netwt").Value.ToString), Val(.Cells("TOUCH").Value.ToString), Val(.Cells("Wastage").Value.ToString), Val(.Cells("Mc").Value.ToString), Isedit, lotDiaWt, _StkType)
                End If
    ...
    , ByVal Mcharge As Decimal, ByVal Isediting As Boolean, ByVal DiaWt As Decimal, ByVal StkType As String)
        ...
        If Isediting = True And MREDIT_WITHLOT Then
            ...
            lotNo = objGPack.GetSqlValue("SELECT LOTNO from " & cnAdminDb & "..ITEMLOT WHERE SNO = '" & lotSno & "'", , , tran)
            Cgrswt = objGPack.GetSqlValue("SELECT SUM(CGRSWT) CGRSWT FROM " & cnAdminDb & "..ITEMLOT WHERE LOTNO = '" & lotNo & "'", , , tran)
            Cpcs = objGPack.GetSqlValue("SELECT SUM(CPCS)CPCS FROM " & cnAdminDb & "..ITEMLOT WHERE LOTNO = '" & lotNo & "'", , , tran)
...
GENLOTNO:
    ...
    Private Sub Save_JobNo(ByVal index As Integer)
        With DgvTran.Rows(index)
            ...
            For Each stRow As DataRow In CType(.Cells("METISSREC").Value, ManuFacturingIssRec).MaterialStoneDia.dtGridStone.Rows
                If OMaterialType = MaterialType.Issue Then
                    issSno = GetNewSno(IIf(_AccAudit, TranSnoType.TISSUECODE, TranSnoType.ISSUECODE), tran)
                Else
                    issSno = GetNewSno(IIf(_AccAudit, TranSnoType.TRECEIPTCODE, TranSnoType.RECEIPTCODE), tran)
...
, ByVal TNO As Integer, ByVal stRow As DataRow, ByVal TotAlloy As Decimal)
    ...
    Private Sub cmbTransactionType_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbTransactionType.Enter
        ...
        If OMaterialType = MaterialType.Receipt Then
            If Not CheckBckDaysEntry(userId, "", dtpTrandate.Value, "MANUFACTURING RECEIPT") Then
                dtpTrandate.Focus()
                Exit Sub
            Else
                Exit Sub
            End If
        Else
            If Not CheckBckDaysEntry(userId, "", dtpTrandate.Value, "MANUFACTURING ISSUE") Then
                dtpTrandate.Focus()
                Exit Sub
            Else
                Exit Sub
    ...
    Private Sub cmbAcName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbAcName.SelectedIndexChanged
        ...
        If dtAdd.Rows.Count > 0 Then
            For i As Integer = 0 To dtAdd.Rows.Count - 1
                If i = 0 Then lblAddress.Text = dtAdd.Rows(i).Item("ADDRESS").ToString
    ...
    Private Sub txtOrdNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtOrdNo.KeyDown
        If e.KeyCode = Keys.Insert Then
            StrSql = vbCrLf + " SELECT *,ROW_NUMBER() OVER(ORDER BY CONVERT(INT,SUBSTRING(ORDERNO,2,7))) OSNO FROM ( "
            StrSql += vbCrLf + " SELECT ORNO,SUBSTRING(OM.ORNO,6,20)ORDERNO,OM.STYLENO"
            StrSql += vbCrLf + " ,IM.ITEMNAME AS ITEM,SM.SUBITEMNAME AS SUBITEM"
            ...
            StrSql += vbCrLf + " AND COSTID = '" & GetSqlValue(cn, "SELECT TOP 1 COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME = '" & cmbCostCentre.Text & "'") & "'"
            ...
            StrSql += vbCrLf + " AND STATUS NOT IN('READY FOR DELIVERY') ORDER BY OSNO "
            ...
            If dtStk.Rows.Count > 0 Then
                ...
                If Not StkRow Is Nothing Then
                    txtOrdNo.Text = StkRow.Item("ORDERNO").ToString
                    lblRunno.Text = StkRow.Item("ORNO").ToString : lblRunno.Visible = True
                End If
            Else
    ...
    Function CheckOrder() As Boolean
        StrSql = "SELECT ORNO FROM " & cnAdminDb & "..ORMAST WHERE SUBSTRING(ORNO,6,20) = '" & txtOrdNo.Text & "' AND ISNULL(CANCEL,'') = ''"
        ...
        End If
    End Function
    Function GetProcessTime(ByVal Accode As String) As String
        Dim DTtemp As New DataTable
        Dim ProcessTime As String = ""
        StrSql = vbCrLf + " WITH PROCESSTIME AS( "
        StrSql += vbCrLf + " SELECT CONVERT(INT,DATEDIFF(SS  ,ENTRYDATETIME,GETDATE())) DIFFTIME,ENTRYDATETIME    FROM " & cnAdminDb & "..ORIRDETAIL "
        StrSql += vbCrLf + " WHERE ISNULL(ENTRYDATETIME,'') <> '' AND SUBSTRING(ORNO,6,20) = '" & txtOrdNo.Text & "'  "
        StrSql += vbCrLf + " And acCode = '" & Accode & "' AND ENTRYORDER = (SELECT MAX(ENTRYORDER) FROM   " & cnAdminDb & "..ORIRDETAIL  WHERE SUBSTRING(ORNO,6,20) = '" & txtOrdNo.Text & "' AND ISNULL(CANCEL,'') = '') ) "
        StrSql += vbCrLf + "SELECT      FLOOR ( DIFFTIME / 31207680 ) AS YEARS "
        StrSql += vbCrLf + ",   FLOOR( (DIFFTIME / 2600640 ) - FLOOR ( DIFFTIME / 31207680  )  * 12 ) As MONTHS "
        StrSql += vbCrLf + ",   FLOOR( (DIFFTIME / 604800 ) - FLOOR ( DIFFTIME / 2600640 )  * 4.3 ) AS WEEKS "
        StrSql += vbCrLf + ",   FLOOR( (DIFFTIME / 86400 ) - FLOOR( DIFFTIME / 604800 ) * 7 ) AS DAYS "
        StrSql += vbCrLf + ",   FLOOR( ( DIFFTIME / 3600 ) - FLOOR( DIFFTIME / 86400 ) * 24 ) AS HOURS "
        StrSql += vbCrLf + ",   FLOOR( ( DIFFTIME / 60 )   - FLOOR( DIFFTIME / 3600 ) * 60 ) AS MINUTES "
        StrSql += vbCrLf + ",   DIFFTIME - FLOOR( DIFFTIME / 60 ) * 60 AS SECONDS FROM PROCESSTIME "
        Da = New OleDbDataAdapter(StrSql, cn)
        DTtemp = New DataTable
        Da.Fill(DTtemp)
        If DTtemp.Rows.Count > 0 Then
            If Val(DTtemp.Rows(0).Item("YEARS").ToString) <> 0 Then
                ProcessTime = ProcessTime & Val(DTtemp.Rows(0).Item("YEARS").ToString) & " YEARS "
            End If
            If Val(DTtemp.Rows(0).Item("MONTHS").ToString) <> 0 Then
                ProcessTime = ProcessTime & Val(DTtemp.Rows(0).Item("MONTHS").ToString) & " MONTHS "
            End If
            If Val(DTtemp.Rows(0).Item("DAYS").ToString) <> 0 Then
                ProcessTime = ProcessTime & Val(DTtemp.Rows(0).Item("DAYS").ToString) & " DAYS "
            End If
            If Val(DTtemp.Rows(0).Item("HOURS").ToString) <> 0 Then
                ProcessTime = ProcessTime & Val(DTtemp.Rows(0).Item("HOURS").ToString) & " HOURS "
            End If
            If Val(DTtemp.Rows(0).Item("MINUTES").ToString) <> 0 Then
                ProcessTime = ProcessTime & Val(DTtemp.Rows(0).Item("MINUTES").ToString) & " MINUTES "
            End If
            If Val(DTtemp.Rows(0).Item("SECONDS").ToString) <> 0 Then
                ProcessTime = ProcessTime & Val(DTtemp.Rows(0).Item("SECONDS").ToString) & " SECONDS "
            End If
            ProcessTime += " LATER"
            lblProcessDateTime.Text = ProcessTime : lblProcessDateTime.Visible = True : lblProcessDateTime.ForeColor = Color.Red
        Else
            lblProcessDateTime.Text = "" : lblProcessDateTime.Visible = False
        End If
    ...
    Private Sub txtOrdNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtOrdNo.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            If txtOrdNo.Text <> "" Then
                ...
                Dim PictFile As String = Nothing
                If CheckOrder() = False Then MsgBox("Invalid Order No") : Exit Sub
                If MRMI_ORDERNO And ORDER_MULTI_MIMR Then
                    StrSql = vbCrLf + "SELECT TOP 1 I.ACCODE,O.ORDSTATE_ID,I.TRANDATE,O.ORNO  FROM " & cnStockDb & IIf(OMaterialType.Issue, "..RECEIPT", "..ISSUE") & " AS I"
                    StrSql += vbCrLf + "INNER JOIN " & cnAdminDb & "..ORIRDETAIL AS O ON I.BATCHNO=O.BATCHNO AND ISNULL(I.CANCEL,'') <> 'Y' AND ISNULL(O.CANCEL,'') <> 'Y'   "
                    StrSql += vbCrLf + "WHERE SUBSTRING(O.ORNO,6,20) = '" & txtOrdNo.Text & "' "
                    StrSql += vbCrLf + " AND O.COMPANYID = '" & strCompanyId & "'"
                    If cmbCostCentre.Text <> "" Then
                        StrSql += vbCrLf + " AND O.COSTID = '" & CostCenterId & "'"
                    End If
                    StrSql += vbCrLf + "ORDER BY O.ENTRYORDER DESC "
                    Acc = objGPack.GetSqlValue(StrSql, "ACCODE", "").ToString

                    StrSql = "SELECT PICTFILE,ORNO FROM " & cnAdminDb & "..ORMAST O WHERE SUBSTRING(ORNO, 6, 20) = '" & txtOrdNo.Text & "' "
                    StrSql += vbCrLf + " AND O.COMPANYID = '" & strCompanyId & "'"
                    If cmbCostCentre.Text <> "" Then
                        StrSql += vbCrLf + " AND O.COSTID = '" & CostCenterId & "'"
                    End If

                    lblRunno.Text = objGPack.GetSqlValue(StrSql, "ORNO", "").ToString : lblRunno.Visible = True
                    Runno = objGPack.GetSqlValue(StrSql, "ORNO", "").ToString
                    PictFile = objGPack.GetSqlValue(StrSql, "PICTFILE", "").ToString
                    StrSql = vbCrLf + "SELECT TOP 1 I.ORDSTATE_ID,I.TRANDATE,(SELECT USERNAME FROM " & cnAdminDb & "..USERMASTER WHERE USERID = I.USERID) USERNAME  FROM " & cnStockDb & "..ISSUE AS I"
                    StrSql += vbCrLf + "INNER JOIN " & cnAdminDb & "..ORIRDETAIL AS O ON I.BATCHNO=O.BATCHNO AND ISNULL(I.CANCEL,'') <> 'Y' AND ISNULL(O.CANCEL,'') <> 'Y'   "
                    StrSql += vbCrLf + "WHERE SUBSTRING(O.ORNO,6,20) = '" & txtOrdNo.Text & "' "
                    StrSql += vbCrLf + " AND O.COMPANYID = '" & strCompanyId & "'"
                    If cmbCostCentre.Text <> "" Then
                    ...
                    Da.Fill(DTtemp)
                    If DTtemp.Rows.Count > 0 Then
                        If DTtemp.Rows(0).Item("ORDSTATE_ID").ToString = "4" Then
                            Dim DeliveryDate As Date = DTtemp.Rows(0).Item("TRANDATE").ToString
                            MsgBox("This Order Is Ready To Delivery" + vbCrLf + " As On " & DeliveryDate.Date.ToString("dd-MM-yyyy") & " By " & DTtemp.Rows(0).Item("USERNAME").ToString)
                            Exit Sub
                        End If
                    End If
                Else
                    ...
                    If cmbCostCentre.Text <> "" Then
                        StrSql += vbCrLf + " AND COSTID = '" & CostCenterId & "'"
                    End If
                    Acc = objGPack.GetSqlValue(StrSql, "ACCODE", "").ToString
                End If
                ...
                '''Process Time
                GetProcessTime(Acc)
                If PicPath <> "" Then ImagePath = PicPath
                If ImagePath = "" And defaultPic <> "" Then ImagePath = defaultPic
                If ImagePath <> "" And System.IO.Directory.Exists(ImagePath) Then
                    ImagePath += PictFile
                    If System.IO.File.Exists(ImagePath) Then
                        Dim OrderImage As Image = Nothing
                        OrderImage = Image.FromFile(ImagePath)
                        AutoImageSizer(ImagePath, picOrder, PictureBoxSizeMode.StretchImage)
                        picOrder.Image = Image.FromFile(ImagePath)
                        picOrder.Visible = True
                        tabGroup.SelectedTab = TabPicture
                    Else
                        AutoImageSizer(ImagePath, picOrder, PictureBoxSizeMode.StretchImage)
                        picOrder.Image = My.Resources.no_photo
                        picOrder.Visible = True
                        tabGroup.SelectedTab = TabPicture
                    End If
                Else
                ...
                If cmbAcName.Text = "" Then cmbAcName.Select() : Exit Sub
                If MRMI_ORDERNO And ORDER_MULTI_MIMR And OMaterialType = MaterialType.Issue Then cmbAcName.Select() : Exit Sub
                ObjMaterialDia = New ManuFacturingIssRec(OMaterialType, cmbTransactionType.Text, dtpTrandate.Value, Acc, , CostCenterId, txtOrdNo.Text)
                ObjMaterialDia.Text = "MATERIAL " & cmbTransactionType.Text
