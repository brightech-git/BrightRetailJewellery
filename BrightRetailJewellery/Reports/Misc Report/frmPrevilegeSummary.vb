...
Public Class frmPrevilegeSummary
    ...
    Private Sub PRIVTRAN()
        ...
        strSql = "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET OPPOINTS = NULL WHERE OPPOINTS = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET CURPOINTS = NULL WHERE CURPOINTS = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET REDEEMPOINTS = NULL WHERE REDEEMPOINTS = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET CLOSINGPOINTS = NULL WHERE CLOSINGPOINTS = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET OPPOINTSVALUE = NULL WHERE OPPOINTSVALUE = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET CURPOINTSVALUE = NULL WHERE CURPOINTSVALUE = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET REDEEMPOINTSVALUE = NULL WHERE REDEEMPOINTSVALUE = 0"
        strSql += vbCrLf + "UPDATE MASTER..TEMP" & systemId & "PREVILEGESUMMARY SET CLOSINGPOINTSVALUE = NULL WHERE CLOSINGPOINTSVALUE = 0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = " SELECT * FROM MASTER..TEMP" & systemId & "PREVILEGESUMMARY ORDER BY SNO"
        ...
        Dim TITLE As String
        TITLE += lblcustname.Text & "  PREVILEGE SUMMARY FROM " & dtpFrom.Text & " TO " & dtpTo.Text & ""
    ...
    Private Sub TXTPREVILEGEID_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)

    End Sub


    Private Sub chkwithpoints_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
