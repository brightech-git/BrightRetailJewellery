...
Public Class frmTrading
    ...
    Dim TRADING_PROFITLOSS_FORMAT1 As Boolean = IIf(GetAdmindbSoftValue("TRADING_PROFITLOSS_FORMAT1", "N", ) = "Y", True, False)
    ...
    Private Sub btnView_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView.Click
        ...
        Else
            gridView.DataSource = Nothing
            If TRADING_PROFITLOSS_FORMAT1 Then
                rbtCls_Stock_Man.Checked = False
                funcViewNew()
            End If
            funcView()
    ...
    Private Sub funcViewNew()
        ...
        strSql = vbCrLf + " IF (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPCATSTOCK')>0"
        strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMPCATSTOCK"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        ...
        strSql += vbCrLf + " SELECT @FRMDATE = '" + cnTranFromDate.ToString("yyyy-MM-dd") + "'"
        strSql += vbCrLf + " SELECT @TODATE = '" + dtpDate.Value.Date.ToString("yyyy-MM-dd") + "'"
        strSql += vbCrLf + " "
        strSql += vbCrLf + " DECLARE @OFRMDATE SMALLDATETIME "
        strSql += vbCrLf + " DECLARE @OTODATE SMALLDATETIME "
        strSql += vbCrLf + " SELECT @OFRMDATE = '" + cnTranFromDate.ToString("yyyy-MM-dd") + "'"
        strSql += vbCrLf + " SELECT @OTODATE = '" + dtpDate.Value.Date.ToString("yyyy-MM-dd") + "'"
        strSql += vbCrLf + " "
        strSql += vbCrLf + " SELECT CONVERT(VARCHAR(100),SEP)SEP,X.CATCODE,C.CATNAME,GRSWT"
        strSql += vbCrLf + " ,(CASE WHEN ISNULL(RATE,0)=0 THEN (SELECT TOP 1 SRATE FROM " & cnAdminDb & "..RATEMAST WHERE RDATE=X.TRANDATE AND METALID=C.METALID AND PURITY IN (SELECT TOP 1 PURITY FROM " & cnAdminDb & "..PURITYMAST AS P WHERE C.PURITYID=C.PURITYID)) END)RRATE"
        strSql += vbCrLf + " ,C.METALID,(SELECT TOP 1 PURITY FROM " & cnAdminDb & "..PURITYMAST AS P WHERE C.PURITYID=C.PURITYID)PURITY"
        strSql += vbCrLf + " ,RATE,BOARDRATE"
        strSql += vbCrLf + " ,AMOUNT,NETWT,PUREWT,TRANDATE,TRANNO,YYEAR,MMONTHID,CONVERT(VARCHAR(100),'')RESULT,CONVERT(NUMERIC(15,2),0)ADJGRSWT"
        strSql += vbCrLf + " INTO TEMPTABLEDB..TEMPCATSTOCK"
        strSql += vbCrLf + " FROM ("
        strSql += vbCrLf + " SELECT "
        strSql += vbCrLf + " CASE WHEN ISNULL(M.STUDDED,'')='S' THEN '2STUD' ELSE '1LOOSE' END FRM,'ISS_OPENING'SEP,I.CATCODE"
        strSql += vbCrLf + " ,SUM(I.GRSWT)GRSWT,RATE,AMOUNT,BOARDRATE"
        ...
        strSql += vbCrLf + " AND I.TRANTYPE <> 'WOT' AND I.TRANTYPE <> 'MI'"
        strSql += vbCrLf + " AND I.COMPANYID IN (SELECT COMPANYID FROM " & cnAdminDb & "..COMPANY WHERE COMPANYNAME IN ('" + strCompanyName + "'))"
        ...
        strSql += vbCrLf + " GROUP BY I.CATCODE,TRANDATE,TRANNO,TRANSTATUS,M.STUDDED,RATE,AMOUNT,BOARDRATE"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT "
        strSql += vbCrLf + " '1LOOSE'FRM,'ISS_OPENING'SEP,I.CATCODE"
        strSql += vbCrLf + " ,SUM(GRSWT)GRSWT,RATE,AMOUNT,BOARDRATE"
        ...
        strSql += vbCrLf + " AND I.TRANTYPE <> 'WOT' AND I.TRANTYPE <> 'MI'"
        strSql += vbCrLf + " AND COMPANYID IN (SELECT COMPANYID FROM " & cnAdminDb & "..COMPANY WHERE COMPANYNAME IN ('" + strCompanyName + "'))  )"
        strSql += vbCrLf + " AND I.TRANTYPE <> 'WOT' AND I.TRANTYPE <> 'MI'"
        strSql += vbCrLf + " AND I.COMPANYID IN (SELECT COMPANYID FROM " & cnAdminDb & "..COMPANY WHERE COMPANYNAME IN ('" + strCompanyName + "'))"
        If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
            strSql += " AND COSTID IN"
            strSql += "(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
        End If
        strSql += vbCrLf + " GROUP BY I.CATCODE,TRANDATE,TRANNO,RATE,AMOUNT,BOARDRATE"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT "
        strSql += vbCrLf + " CASE WHEN ISNULL(M.STUDDED,'')='S' THEN '2STUD' ELSE '1LOOSE' END FRM,'REC_OPENING'SEP,R.CATCODE"
        strSql += vbCrLf + " ,SUM(R.GRSWT)GRSWT,RATE,AMOUNT,BOARDRATE"
        ...
        strSql += vbCrLf + " AND R.COMPANYID IN (SELECT COMPANYID FROM " & cnAdminDb & "..COMPANY WHERE COMPANYNAME IN ('" + strCompanyName + "'))"
        If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
            strSql += " AND COSTID IN"
            strSql += "(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
        End If
        strSql += vbCrLf + " GROUP BY R.CATCODE,TRANDATE,TRANNO,TRANSTATUS,M.STUDDED,RATE,AMOUNT,BOARDRATE"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT '1LOOSE'FRM,'REC_OPENING'SEP,CATCODE ,GRSWT,RATE,AMOUNT,BOARDRATE,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
        strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
        strSql += vbCrLf + " FROM ("
        strSql += vbCrLf + " SELECT  "
        strSql += vbCrLf + " ISNULL(I.CATCODE,'') CATCODE "
        strSql += vbCrLf + " ,SUM(GRSWT)GRSWT,RATE,AMOUNT,RATE BOARDRATE"
        strSql += vbCrLf + " ,0 AS ALLOY"
        strSql += vbCrLf + " ,SUM(NETWT)NETWT"
        strSql += vbCrLf + " ,SUM(PUREWT)PUREWT"
        ...
        strSql += vbCrLf + " WHERE TRANDATE BETWEEN @OFRMDATE AND @OTODATE AND ISNULL(CANCEL,'') = ''"
        ...
        If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
            ...
            strSql += "(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
        End If
        strSql += vbCrLf + " GROUP BY I.CATCODE,TRANDATE,TRANNO,TRANSTATUS,RATE,AMOUNT"
        strSql += vbCrLf + " ) X"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT '1LOOSE'FRM,'ISS_OPENING'SEP,CATCODE ,GRSWT,RATE,AMOUNT,RATE BOARDRATE ,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
        strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
        strSql += vbCrLf + " FROM ("
        strSql += vbCrLf + " SELECT  "
        strSql += vbCrLf + " ISNULL(I.CATCODE,'') CATCODE "
        strSql += vbCrLf + " ,SUM(GRSWT)GRSWT,RATE,AMOUNT"
        ...
        If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
            ...
            strSql += "(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
        End If
        strSql += vbCrLf + " GROUP BY CATCODE,TRANDATE,TRANNO,TRANSTATUS,RATE,AMOUNT"
        strSql += vbCrLf + " ) X"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT "
        strSql += vbCrLf + " '1LOOSE'FRM,'REC_OPENING'SEP,R.CATCODE"
        strSql += vbCrLf + " ,SUM(R.GRSWT)GRSWT,RATE,AMOUNT,RATE BOARDRATE"
        strSql += vbCrLf + " , 0 ALLOY"
        strSql += vbCrLf + " ,SUM(NETWT)NETWT"
        strSql += vbCrLf + " ,SUM(PUREWT)PUREWT"
        strSql += vbCrLf + " ,@FRMDATE TRANDATE,-1 TRANNO,' 'TRANSTATUS"
        ...
        If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
            ...
            strSql += "(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
        End If
        strSql += vbCrLf + " GROUP BY R.CATCODE,RATE,AMOUNT"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT "
        ...
        strSql += vbCrLf + " GROUP BY R.CATCODE,RATE,AMOUNT"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT CASE WHEN FRM='' THEN '1LOOSE' ELSE FRM END AS FRM,'ISSUE'SEP,CATCODE ,GRSWT,RATE,AMOUNT,BOARDRATE,ALLOY ,NETWT,PUREWT,TRANDATE,TRANNO,TRANSTATUS "
        strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
        strSql += vbCrLf + " FROM ("
        ...
        If chkCmbCostCentre.Text <> "ALL" And chkCmbCostCentre.Text <> "" Then
            ...
            strSql += "(SELECT COSTID FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTNAME IN (" & GetQryString(chkCmbCostCentre.Text) & "))"
        End If
        strSql += vbCrLf + " GROUP BY I.CATCODE,I.TRANDATE,I.TRANNO,RATE,AMOUNT,BOARDRATE"
        strSql += vbCrLf + " UNION ALL"
        ...
        strSql += vbCrLf + " ) X"
        strSql += vbCrLf + " UNION ALL"
        strSql += vbCrLf + " SELECT CASE WHEN FRM='' THEN '1LOOSE' ELSE FRM END AS FRM,'RECEIPT'SEP,CATCODE ,GRSWT,RATE,AMOUNT,BOARDRATE,ALLOY ,NETWT,PUREWT ,TRANDATE,TRANNO,TRANSTATUS "
        strSql += vbCrLf + " ,YYEAR,MMONTHID,MMONTH "
        strSql += vbCrLf + " FROM ("
        ...
        strSql += vbCrLf + " ) X"
        strSql += vbCrLf + " ) X"
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS C ON C.CATCODE=X.CATCODE"
        strSql += vbCrLf + " /*INNER JOIN " & cnAdminDb & "..PURITYMAST AS P ON C.PURITYID=C.PURITYID*/"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPCATSTOCK SET RATE=RRATE WHERE ISNULL(RATE,0)=0 AND ISNULL(RRATE,0)<>0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPCATSTOCK SET RATE=BOARDRATE WHERE ISNULL(RATE,0)=0 AND ISNULL(BOARDRATE,0)<>0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPCATSTOCK SET RATE=CONVERT(NUMERIC(15,2),AMOUNT/GRSWT) WHERE ISNULL(RATE,0)=0 AND ISNULL(AMOUNT,0)<>0 AND ISNULL(GRSWT,0)<>0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " UPDATE A"
        strSql += vbCrLf + " SET RATE=(CASE WHEN ISNULL(RATE,0)=0 THEN (SELECT TOP 1 SRATE FROM " & cnAdminDb & "..RATEMAST WHERE RDATE=A.TRANDATE AND METALID=C.METALID AND PURITY=91.6) END)"
        strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCATSTOCK AS A "
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS C ON C.CATCODE=A.CATCODE"
        strSql += vbCrLf + " WHERE ISNULL(RATE,0)=0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " UPDATE A"
        strSql += vbCrLf + " SET RATE=(CASE WHEN ISNULL(RATE,0)=0 THEN (SELECT TOP 1 SRATE FROM " & cnAdminDb & "..RATEMAST WHERE RDATE<A.TRANDATE AND METALID=C.METALID AND PURITY=91.6 ORDER BY RDATE DESC) END)"
        strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCATSTOCK AS A "
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS C ON C.CATCODE=A.CATCODE"
        strSql += vbCrLf + " WHERE ISNULL(RATE,0)=0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        ...
        strSql = vbCrLf + " UPDATE A"
        strSql += vbCrLf + " SET RATE=(CASE WHEN ISNULL(RATE,0)=0 THEN (SELECT TOP 1 SRATE FROM " & cnAdminDb & "..RATEMAST WHERE RDATE<A.TRANDATE AND METALID=C.METALID AND PURITY=91.6 AND ISNULL(SRATE,0)<>0) END)"
        strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCATSTOCK AS A "
        strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS C ON C.CATCODE=A.CATCODE"
        strSql += vbCrLf + " WHERE ISNULL(RATE,0)=0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        ...
        strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPCATSTOCK SET AMOUNT=CONVERT(NUMERIC(15,2),GRSWT*RATE) WHERE ISNULL(AMOUNT,0)=0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " UPDATE TEMPTABLEDB..TEMPCATSTOCK SET AMOUNT=CONVERT(NUMERIC(15,2),AMOUNT) WHERE ISNULL(RATE,0)=0 AND ISNULL(GRSWT,0)=0"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()


        Dim dtTran As New DataTable
        strSql = " SELECT * FROM TEMPTABLEDB..TEMPCATSTOCK WHERE 1<>1"
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dtTran)

        Dim dtCat As New DataTable
        strSql = " SELECT DISTINCT CATNAME FROM TEMPTABLEDB..TEMPCATSTOCK "
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dtCat)
        For Each dr As DataRow In dtCat.Rows
            strSql = "SELECT * FROM TEMPTABLEDB..TEMPCATSTOCK AS A WHERE SEP LIKE 'REC%' AND CATNAME='" + dr("CATNAME").ToString + "' ORDER BY TRANDATE,TRANNO"
            Dim dtRec As New DataTable
            da = New OleDbDataAdapter(strSql, cn)
            da.Fill(dtRec)

            strSql = "SELECT * FROM TEMPTABLEDB..TEMPCATSTOCK AS A WHERE SEP LIKE 'ISS%' AND CATNAME='" + dr("CATNAME").ToString + "' ORDER BY TRANDATE,TRANNO"
            Dim dtIss As New DataTable
            da = New OleDbDataAdapter(strSql, cn)
            da.Fill(dtIss)

            For Each drr As DataRow In dtRec.Rows
                Dim RecWt As Double = Val(drr("GRSWT").ToString)
                Dim AdjRecWt As Double = RecWt
nxtRec:

                'For Each dri As DataRow In dtIss.Select("RESULT=''")
                If dtIss.Select("GRSWT>ADJGRSWT").Length > 0 Then
                    For Each dri As DataRow In dtIss.Select("GRSWT>ADJGRSWT")
nxtIss:
                        If AdjRecWt <= 0 Then
                            Exit For
                        End If
                        Dim IssWt As Double = Math.Round(Val(dri("GRSWT").ToString) - Val(dri("ADJGRSWT").ToString), 3)
                        Dim drn As DataRow
                        drn = dtTran.NewRow
                        drn("CATCODE") = drr("CATCODE")
                        drn("CATNAME") = drr("CATNAME")
                        drn("RATE") = drr("RATE")
                        If AdjRecWt > IssWt Then
                            drn("GRSWT") = IssWt
                            AdjRecWt -= IssWt
                            dri("RESULT") = "CLOSED"
                            drn("RESULT") = "CLOSED"
                            dri("ADJGRSWT") = dri("GRSWT")
                            dtIss.AcceptChanges()
                        Else
                            drn("GRSWT") = AdjRecWt
                            dri("ADJGRSWT") = Math.Round(Val(dri("ADJGRSWT").ToString) + AdjRecWt, 3)
                            dtIss.AcceptChanges()
                            AdjRecWt = 0
                            drr("RESULT") = "CLOSED"
                            drn("RESULT") = "CLOSED"
        ...
        strSql = vbCrLf + " IF (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPCATSTOCKFINAL')>0"
        strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMPCATSTOCKFINAL"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = " SELECT * INTO TEMPTABLEDB..TEMPCATSTOCKFINAL FROM TEMPTABLEDB..TEMPCATSTOCK WHERE 1<>1"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        ...
        Try
            ...
            cn_Sql = New SqlConnection("Data Source=" & ConInfo.lServerName & ";Initial Catalog=TEMPTABLEDB;User ID=" & IIf(ConInfo.lDbUserId <> "", ConInfo.lDbUserId, "SA") & ";Password=" & BrighttechPack.Decrypt(ConInfo.lDbPwd) & "")
            cn_Sql.Open()

            'Dim sqlBK As New SqlBulkCopy(cn_Sql.ConnectionString, SqlBulkCopyOptions.Default)
            Dim sqlBK As New SqlBulkCopy(cn_Sql.ConnectionString, SqlBulkCopyOptions.KeepNulls)
            sqlBK.DestinationTableName = "TEMPCATSTOCKFINAL"
            sqlBK.WriteToServer(dtTran)
        ...
        strSql = vbCrLf + " IF (SELECT 1 FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPCLSSTOCK')>0"
        strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMPCLSSTOCK"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = vbCrLf + " SELECT CATCODE,CATNAME,CONVERT(NUMERIC(15,3),SUM(CASE WHEN ISNULL(SEP,'') LIKE 'REC%' THEN ISNULL(GRSWT,0) ELSE -1*ISNULL(GRSWT,0) END))GRSWT"
        strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),AVG(RATE))AVGRATE,CONVERT(NUMERIC(15,2),(SELECT AVG(RATE)RATE FROM TEMPTABLEDB..TEMPCATSTOCKFINAL WHERE ISNULL(RESULT,'')='' AND CATNAME=A.CATNAME))WTRATE"
        strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),NULL)VALUE,'" + strCompanyId + "' COMPANYID INTO TEMPTABLEDB..TEMPCLSSTOCK FROM TEMPTABLEDB..TEMPCATSTOCK AS A GROUP BY CATCODE,CATNAME ORDER BY CATNAME"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = " UPDATE TEMPTABLEDB..TEMPCLSSTOCK SET VALUE=CONVERT(NUMERIC(15,2),ISNULL(GRSWT,0)*ISNULL(WTRATE,0))"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
...
TTTT:
            ...
            If summary Then
                ...
                ElseIf TRADING_PROFITLOSS_FORMAT1 Then
                    strSql += vbCrLf + " UNION ALL"
                    strSql += vbCrLf + " SELECT 'CLSTK' ACID,'0' ACGRPCODE,'CLOSING STOCK' ACGRPNAME,'0' ACMAINCODE"
                    strSql += vbCrLf + " ,'T' GRPLEDGER,CASE WHEN SUM(VALUE) > 0 THEN 'I' ELSE 'E' END GRPTYPE"
                    strSql += vbCrLf + " ,CASE WHEN SUM(VALUE) < 0 THEN SUM(VALUE)*(-1) END DEBIT"
                    ...
                    strSql += vbCrLf + " ,CASE WHEN SUM(VALUE) > 0 THEN 'D' ELSE 'C' END TRANMODE,SUM(VALUE) AMOUNT"
                    strSql += vbCrLf + " ,'' PAYMODE,'' TRANNO,'CLOSING STOCK' ACMAINGRPNAME"
                    strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCLSSTOCK WHERE ISNULL(COMPANYID,'') = '" & strcompid & "'"
                    strSql += vbCrLf + " HAVING SUM(VALUE) <> 0"
                Else
                    Dim Qry As String
            ...
            Else
                ...
                ElseIf TRADING_PROFITLOSS_FORMAT1 Then
                    ...
                    strSql += vbCrLf + " CASE WHEN SUM(VALUE) > 0 THEN 'I' ELSE 'E' END GRPTYPE 	,CASE WHEN SUM(VALUE) < 0 THEN SUM(VALUE)*(-1) END DEBIT 	,"
                    strSql += vbCrLf + " CASE WHEN SUM(VALUE) > 0 THEN SUM(VALUE) END CREDIT 	,H.ACNAME  ACNAME,'' TRANDATE 	,CASE WHEN SUM(VALUE) > 0 THEN 'D' ELSE 'C' END TRANMODE,SUM(VALUE) AMOUNT 	,'' PAYMODE,'' TRANNO,'CLOSING STOCK' ACMAINGRPNAME "
                    strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCLSSTOCK A LEFT JOIN " & cnAdminDb & "..CATEGORY C ON A.CATCODE = C.CATCODE LEFT JOIN " & cnAdminDb & "..ACHEAD H ON C.CLSCODE = H.ACCODE "
                    strSql += vbCrLf + " LEFT JOIN " & cnAdminDb & "..acgroup G ON H.ACGRPCODE = G.ACGRPCODE"
                    strSql += vbCrLf + " WHERE ISNULL(A.COMPANYID,'') = '" & strcompid & "'"
...
TTTT:
    ...
    Private Sub Prop_Sets()
        ...
        SetSettingsObj(obj, Me.Name, GetType(frmTrading_Properties))
    End Sub
    Private Sub Prop_Gets()
        Dim obj As New frmTrading_Properties
        GetSettingsObj(obj, Me.Name, GetType(frmTrading_Properties))
    ...
    Private Sub gridView_KeyPress(sender As Object, e As KeyPressEventArgs) Handles gridView.KeyPress
        If e.KeyChar.ToString.ToUpper = Chr(Keys.D) Then
            strSql = vbCrLf + " SELECT PARTICULAR"
            strSql += vbCrLf + " ,(CASE WHEN ISNULL(GRSWT,0)=0 THEN NULL ELSE GRSWT END)GRSWT"
            strSql += vbCrLf + " ,(CASE WHEN ISNULL(AVGRATE,0)=0 THEN NULL ELSE AVGRATE END)AVGRATE"
            strSql += vbCrLf + " ,(CASE WHEN ISNULL(WTRATE,0)=0 THEN NULL ELSE WTRATE END)WTRATE"
            strSql += vbCrLf + " ,(CASE WHEN ISNULL(VALUE,0)=0 THEN NULL ELSE VALUE END)VALUE"
            strSql += vbCrLf + " ,RESULT "
            strSql += vbCrLf + " FROM ("
            strSql += vbCrLf + " SELECT DISTINCT CONVERT(VARCHAR(150),C.METALNAME)PARTICULAR"
            strSql += vbCrLf + " ,CONVERT(VARCHAR(150),C.METALNAME)METALNAME"
            strSql += vbCrLf + " ,CONVERT(VARCHAR(150),NULL)CATNAME"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),NULL)GRSWT"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),NULL)AVGRATE"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),NULL)WTRATE"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),NULL)VALUE,0 AS RESULT"
            strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCLSSTOCK AS A"
            strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS B ON A.CATCODE=B.CATCODE"
            strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..METALMAST AS C ON B.METALID=C.METALID"
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT "
            strSql += vbCrLf + " DISTINCT CONVERT(VARCHAR(150),'		'+A.CATNAME)PARTICULAR"
            strSql += vbCrLf + " ,C.METALNAME,A.CATNAME,A.GRSWT,A.AVGRATE,A.WTRATE,A.VALUE,1 AS RESULT"
            strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCLSSTOCK AS A"
            strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS B ON A.CATCODE=B.CATCODE"
            strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..METALMAST AS C ON B.METALID=C.METALID"
            strSql += vbCrLf + " UNION ALL"
            strSql += vbCrLf + " SELECT "
            strSql += vbCrLf + " CONVERT(VARCHAR(150),ISNULL(C.METALNAME,'') +' TOTAL')PARTICULAR"
            strSql += vbCrLf + " ,C.METALNAME,CONVERT(VARCHAR(150),'ZZZZZZZ')CATNAME"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),SUM(A.GRSWT))GRSWT"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),AVG(A.AVGRATE))AVGRATE"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),AVG(A.WTRATE))WTRATE"
            strSql += vbCrLf + " ,CONVERT(NUMERIC(15,2),SUM(A.VALUE))VALUE"
            strSql += vbCrLf + " ,2 AS RESULT"
            strSql += vbCrLf + " FROM TEMPTABLEDB..TEMPCLSSTOCK AS A"
            strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..CATEGORY AS B ON A.CATCODE=B.CATCODE"
            strSql += vbCrLf + " INNER JOIN " & cnAdminDb & "..METALMAST AS C ON B.METALID=C.METALID"
            strSql += vbCrLf + " GROUP BY C.METALNAME"
            strSql += vbCrLf + " )X"
            strSql += vbCrLf + " ORDER BY METALNAME,CATNAME"
            ...
            objGridShower.Text = "Trading"
            Dim tit As String = "Closing Stock"
            ...
            For Each dgvRow As DataGridViewRow In objGridShower.gridView.Rows
                If dgvRow.Cells("RESULT").Value.ToString = "0" Then
                    ...
                    dgvRow.DefaultCellStyle.ForeColor = Color.MediumBlue
...
Public Class frmTrading_Properties
