...
Public Class GroupLedgerNew
    ...
    Dim dtAccNames As DataTable = Nothing
    ...
    Private Sub GroupLedger_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        cmbAcName.Location = chklstAcname.Location
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Dim chkMltAcName As String = Nothing
        ...
        If chklstAcname.Text <> "ALL" And chklstAcname.Text <> "" Then
            chkMltAcName = GetQryString(chklstAcname.Text)
            chkMltAcName = chkMltAcName.Replace("'", "")
        Else
            chkMltAcName = "ALL"
        End If
        ...
        If chkMultiSelect.Checked = True Then
            StrSql += vbCrLf + " ,@ACCNAME = '" & IIf(chkMltAcName = "", "ALL", chkMltAcName) & "'"
        Else
        ...
        If RptType = "A" Or RptType = "L" Or RptType = "W" Then
            ...
            StrSql = "SELECT COMPANYNAME,ADDRESS1,ADDRESS2,ADDRESS3,AREACODE,CASE WHEN ISNULL(PHONE,'') <> '' THEN 'PH : ' + PHONE ELSE '' END PHONE,CASE WHEN ISNULL(EMAIL,'') <> '' THEN 'EMAIL : ' + EMAIL ELSE '' END EMAIL,PANNO,LOCALTAXNO AS TINNO,CSTNO "
            ...
            Dim s As String = ""
            Dim objCformletterNew As New CformletterNew
            Dim objCformlettersNew As New CformlettersNew
            Dim objCformletterNew2 As New CformletterNew2

            If Nathellaform = True And Remark2 <> "Y" Then
                objCformletterNew.SetParameterValue("Rptprmcompany", objrepcompany1)
                objCformletterNew.SetParameterValue("rptprmaddress1", objrepaddress1)
                objCformletterNew.SetParameterValue("repprmaddress2", objrepmail1)
                objCformletterNew.SetParameterValue("rptpandet", objrptpandet)
                objCformletterNew.SetParameterValue("rptsmithbal", objrepsmithbal)
                If chkgpletterwithled.Checked = False Then
                    objCformletterNew.GroupHeaderSection6.SectionFormat.EnableSuppress = True
                    objCformletterNew.GroupHeaderSection5.SectionFormat.EnableNewPageAfter = False
                Else
                    objCformletterNew.GroupHeaderSection6.SectionFormat.EnableSuppress = False
                    objCformletterNew.GroupHeaderSection5.SectionFormat.EnableNewPageAfter = True
                End If
            ElseIf Nathellaform = True And Remark2 = "Y" Then
                objCformletterNew2.SetParameterValue("Rptprmcompany", objrepcompany1)
                objCformletterNew2.SetParameterValue("rptprmaddress1", objrepaddress1)
                objCformletterNew2.SetParameterValue("repprmaddress2", objrepmail1)
                objCformletterNew2.SetParameterValue("rptpandet", objrptpandet)
                objCformletterNew2.SetParameterValue("rptsmithbal", objrepsmithbal)
                objCformletterNew2.SetParameterValue("FromDate", objFromDate)
                objCformletterNew2.SetParameterValue("ToDate", objToDate)
                objCformletterNew2.SetParameterValue("TINDETAILS", objTinDeatils)
                If chkgpletterwithled.Checked = False Then
                    objCformletterNew2.GroupHeaderSection6.SectionFormat.EnableSuppress = True
                    objCformletterNew2.GroupHeaderSection5.SectionFormat.EnableNewPageAfter = False
                Else
                    objCformletterNew2.GroupHeaderSection6.SectionFormat.EnableSuppress = False
                    objCformletterNew2.GroupHeaderSection5.SectionFormat.EnableNewPageAfter = True
                End If
            Else
                objCformlettersNew.SetParameterValue("Rptprmcompany", objrepcompany1)
                objCformlettersNew.SetParameterValue("rptprmaddress1", objrepaddress1)
                objCformlettersNew.SetParameterValue("repprmaddress2", objrepmail1)
                objCformlettersNew.SetParameterValue("rptpandet", objrptpandet)
                objCformlettersNew.SetParameterValue("rptsmithbal", objrepsmithbal)
                If chkgpletterwithled.Checked = False Then
                    objCformlettersNew.GroupHeaderSection6.SectionFormat.EnableSuppress = True
                    objCformlettersNew.GroupHeaderSection5.SectionFormat.EnableNewPageAfter = False
                Else
                    objCformlettersNew.GroupHeaderSection6.SectionFormat.EnableSuppress = False
                    objCformlettersNew.GroupHeaderSection5.SectionFormat.EnableNewPageAfter = True
                End If
            End If

            StrSql = "IF OBJECT_ID('MASTER..TEMPSIGNATURE')IS NOT NULL DROP TABLE MASTER..TEMPSIGNATURE "
            StrSql += vbCrLf + " CREATE TABLE MASTER..TEMPSIGNATURE(SIGN IMAGE)"
            Cmd = New OleDbCommand(StrSql, cn) : Cmd.CommandTimeout = 1000
            Cmd.ExecuteNonQuery()
            ...
            If Nathellaform = True And Remark2 <> "Y" Then
                objRptViewer.CrystalReportViewer1.ReportSource = objReport.ShowReport(objCformletterNew, cnDataSource, cnStockDb)
            ElseIf Nathellaform = True And Remark2 = "Y" Then
                objRptViewer.CrystalReportViewer1.ReportSource = objReport.ShowReport(objCformletterNew2, cnDataSource, cnStockDb)
            Else
                objRptViewer.CrystalReportViewer1.ReportSource = objReport.ShowReport(objCformlettersNew, cnDataSource, cnStockDb)
            End If
            ...
            If PdfMail And chklstAcname.Items.Count = 1 Then
                If MsgBox("Do you want to send mail? ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    ...
                    Try
                        ...
                        If Nathellaform = True Then
                            CrExportOptions = objCformletterNew.ExportOptions
                        Else
                            CrExportOptions = objCformlettersNew.ExportOptions
                        End If
                        ...
                        End With
                        If Nathellaform = True Then
                            objCformletterNew.Export()
                        Else
                            objCformlettersNew.Export()
                        End If
    ...
    Private Sub accupload()
        ...
        Chkresgrpname = ""
        'If chklstAcname.Items.Count > 0 Then Exit Sub
        ...
        If chklstgroup.CheckedItems.Count > 0 Then
            ...
            If Trim(Chkresgrpname) <> "" Then
                ...
                StrSql = StrSql & " SELECT ACNAME,2 RESULT FROM " & cnAdminDb & "..ACHEAD WHERE ACGRPCODE IN ("
                StrSql = StrSql & " SELECT ACGRPCODE FROM " & cnAdminDb & "..ACGROUP "
                StrSql = StrSql & " WHERE ACGRPNAME IN ('" & Replace(Chkresgrpname, ",", "','") & "'))"
                StrSql = StrSql & " ORDER BY RESULT,ACNAME"
        ...
        'If chklstgroup.Text <> "ALL" And chklstgroup.Text <> "" Then
        '    StrSql += " AND ACGRPCODE = (SELECT ACGRPCODE FROM " & cnAdminDb & "..ACGROUP WHERE ACGRPNAME = '" & chklstgroup.Text & "')"
        'End If
    ...
    Private Sub Prop_Sets()
        ...
        SetSettingsObj(obj, Me.Name, GetType(GroupLedgerNew_Properties))
    End Sub
    ...
    Private Sub Prop_Gets()
        ...
        GetSettingsObj(obj, Me.Name, GetType(GroupLedgerNew_Properties))
        ...
        SetChecked_CheckedList(chklstAcname, obj.p_chklstAcname, Nothing)
        'cmbAcName.Text = obj.p_chklstAcname
    ...
    Private Sub chkacgrpSelectAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkacgrpSelectAll.CheckedChanged
        If chkacgrpSelectAll.Checked = True Then
        ...
        Else
            ...
            StrSql = " SELECT 'ALL' ACNAME,1 RESULT"
            StrSql = StrSql & " UNION ALL"
            StrSql = StrSql & " SELECT ACNAME,2 RESULT FROM " & cnAdminDb & "..ACHEAD where 1<>1"
            StrSql = StrSql & " ORDER BY RESULT,ACNAME"
            'FillCheckedListBox(StrSql, chklstAcname)
            'btnNew_Click(Me, New EventArgs)
            objGPack.FillCombo(StrSql, cmbAcName, , False)
    ...
    Private Sub chkaccSelectAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'SetChecked_CheckedList(chkCmbAcName, chkaccSelectAll.Checked)
    End Sub
    ...
    Private Sub cmbAcName_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbAcName.SelectedIndexChanged
        'StrSql = " SELECT ACNAME FROM " & cnAdminDb & "..ACHEAD WHERE ISNULL(ACTIVE,'Y') <> 'H' "
        'StrSql += " AND MACCODE = (SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & cmbAcName.Text & "')"
        'StrSql += " ORDER BY ACNAME"
    ...
    Private Sub chkMultiSelect_CheckStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkMultiSelect.CheckStateChanged
        If chkMultiSelect.Checked Then
            chklstAcname.Visible = True
            txtAcname.Visible = False
            cmbAcName.Visible = False
            DgvSearch.Visible = False
        Else
            chklstAcname.Visible = False
            txtAcname.Visible = True
...
Public Class GroupLedgerNew_Properties
