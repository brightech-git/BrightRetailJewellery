...
Imports System.IO
Public Class frmTagedItemsPendingStockView
    ...
    Private Sub DetailedReport()
        ...
        strSql += vbCrLf + "     DROP TABLE TEMPTABLEDB..TEMPITEMTAGTITLE"
        strSql += vbCrLf + " SELECT 'TAG STOCK VIEW AS ON " & dtpAsOnDate.Text & "' AS TITLE INTO TEMPTABLEDB..TEMPITEMTAGTITLE "
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = " IF (SELECT COUNT(*) FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPITEMTAGSTOCK')> 0"
        strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMPITEMTAGSTOCK"
        ...
        strSql += vbCrLf + " INTO TEMPTABLEDB..TEMPITEMTAGSTOCK"
        ...
        If Include.Contains("C") Then
            strSql = " IF (SELECT COUNT(*) FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPITEMTAGSTOCK1')> 0"
            strSql += vbCrLf + " DROP TABLE TEMPTABLEDB..TEMPITEMTAGSTOCK1"
            strSql += vbCrLf + " DECLARE @DEFPATH VARCHAR(200)"
            strSql += vbCrLf + " SELECT @DEFPATH = '" & defaultPic & "'"
            strSql += vbCrLf + " SELECT SNO"
            strSql += vbCrLf + " ,CASE WHEN SUBITEMID <> 0 THEN (SELECT SUBITEMNAME FROM " & cnAdminDb & "..SUBITEMMAST WHERE SUBITEMID = T.SUBITEMID)"
            ...
            strSql += vbCrLf + " ,(SELECT CASE WHEN ISNULL(SEAL,'') <> '' THEN SEAL ELSE DESIGNERNAME END FROM " & cnAdminDb & "..DESIGNER WHERE DESIGNERID = T.DESIGNERID)AS DESIGNER"
            strSql += vbCrLf + " ,PCTFILE AS PCTFILE,NARRATION,TAGVAL,STYLENO"
            strSql += vbCrLf + " INTO TEMPTABLEDB..TEMPITEMTAGSTOCK1"
            ...
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
            strSql = " INSERT INTO TEMPTABLEDB..TEMPITEMTAGSTOCK"
            strSql += vbCrLf + " SELECT * FROM TEMPITEMTAGSTOCK1"
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
        End If

        strSql = " ALTER TABLE TEMPTABLEDB..TEMPITEMTAGSTOCK ADD TAGIMAGE IMAGE"
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()

        strSql = " IF (SELECT COUNT(*) FROM TEMPTABLEDB..SYSOBJECTS WHERE NAME = 'TEMPITEMTAGSTONESTOCK')> 0"
        strSql += vbCrLf + "    DROP TABLE TEMPTABLEDB..TEMPITEMTAGSTONESTOCK"
        ...
        strSql += vbCrLf + " INTO TEMPTABLEDB..TEMPITEMTAGSTONESTOCK"
        ...
        strSql = " SELECT SNO,PCTFILE FROM TEMPTABLEDB..TEMPITEMTAGSTOCK"
        ...
        For Each ro As DataRow In dtImage.Rows
            ...
            If IO.File.Exists(fileDestPath) Then
                ...
                If IO.Directory.Exists(Finfo.Directory.FullName) Then
                    ...
                    fileStr.Close()
                    strSql = " UPDATE TEMPTABLEDB..TEMPITEMTAGSTOCK SET TAGIMAGE = ? WHERE SNO = '" & ro!SNO.ToString & "'"
                    cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
    ...
    Function funcFillStoneDetails() As Integer
        ...
        strSql += vbCrLf + " FROM " & cnAdminDb & "..PITEMTAGSTONE S" + vbCrLf
    ...
    Function funcAddNewRow(ByVal maxDesc As String, ByVal maxVal As String, ByVal minDesc As String, ByVal minVal As String, ByVal otherDesc1 As String, ByVal otherVal1 As String, ByVal otherDesc2 As String, ByVal otherVal2 As String) As Integer
        ...
        tempDt.Rows.Add(ro)
    End Function
    Function funcAddNewRowt(ByVal otherDesc1 As String, ByVal otherVal1 As String) As Integer
        Dim ro As DataRow = Nothing
        ro = tempDt.NewRow
        ro("OtherDesc1") = otherDesc1
        ro("OtherVal1") = otherVal1
    ...
    Function funcFillGrandDetails() As Integer
        ...
        strSql += vbCrLf + " INTO TEMPTABLEDB..TEMP" & systemId & "GRANDTOTTAGSTOCKVIEW FROM " & cnAdminDb & "..PITEMTAG T"
        strSql += funcFiltrationString()
        cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
        cmd.ExecuteNonQuery()
        If Include.Contains("C") Then
    ...
    Function funcFiltrationString(Optional ByVal cTag As Boolean = False) As String
        ...
        If txtLotNo_NUM.Text <> "" Or txtLotNoto_NUM.Text <> "" Then
            ...
            fromlotno = GetSqlValue(cn, "SELECT SNO FROM " & cnAdminDb & "..ITEMLOT WHERE LOTNO = '" & txtLotNo_NUM.Text & "'")
            tolotno = GetSqlValue(cn, "SELECT SNO FROM " & cnAdminDb & "..ITEMLOT WHERE LOTNO = '" & txtLotNoto_NUM.Text & "'")
            a = GetSqlValue(cn, "SELECT  TOP 1 TAGVAL FROM " & cnAdminDb & "..PITEMTAG WHERE LOTSNO = '" & fromlotno & "' ORDER BY TAGVAL asc ")
            b = GetSqlValue(cn, "SELECT TOP 1  TAGVAL FROM " & cnAdminDb & "..PITEMTAG WHERE LOTSNO = '" & tolotno & "' ORDER BY TAGVAL DESC ")
            str += "  AND ( TAGVAL BETWEEN '" & a & "' AND '" & b & "' )"
            'str += " and (LOTSNO BETWEEN'" & fromlotno & "' AND '" & tolotno & "' )"
        End If
        If txtFromWt_WET.Text <> "" Or txtToWt_WET.Text <> "" Then
            str += " and (T.GrsWt between '" & Val(txtFromWt_WET.Text) & "' and '" & Val(txtToWt_WET.Text) & "')"
        End If
        ...
        If Studded = True Then
            If chkCmbStoneName.Items.Count > 0 Or chkCmbStSubItemName.Items.Count > 0 Then
                'If chkCmbStoneName.GetItemChecked(0) = False Then
                If (chkCmbStoneName.Text <> "ALL" And chkCmbStoneName.Text <> "") Or (chkCmbStSubItemName.Text <> "ALL" And chkCmbStSubItemName.Text <> "") Then
                    str += " AND ISNULL(SNO,'') IN (SELECT TAGSNO FROM " & cnAdminDb & "..PITEMTAGSTONE "
                    If chkCmbStoneName.Items.Count > 0 Then
                        If chkCmbStoneName.GetItemChecked(0) = False Then
                            str += " WHERE STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE ISNULL(ITEMNAME,'') IN (" & GetQryString(chkCmbStoneName.Text, ",") & "))"
                        End If
    ...
    Private Sub btnView_Search_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        ...
        Try
            ...
            strSql += vbCrLf + " ,RECDATE,TRANSFERDATE,TAGNO,CASE WHEN PCS <> 0 THEN PCS ELSE NULL END PCS,CASE WHEN GRSWT <> 0 THEN GRSWT ELSE NULL END GRSWT,CASE WHEN LESSWT <> 0 THEN LESSWT ELSE NULL END LESSWT,"
            ...
            strSql += vbCrLf + " (SELECT ISNULL(SUM(ISNULL(STNPCS,0)),0) SPCS FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE "
            ...
            'Stone
            strSql += vbCrLf + " (SELECT SUM(CASE WHEN S.STONEUNIT = 'G' THEN S.STNWT ELSE 0 END) FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE "
            ...
            strSql += vbCrLf + " (SELECT SUM(CASE WHEN S.STONEUNIT = 'C' THEN S.STNWT ELSE 0 END) FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE "
            ...
            strSql += vbCrLf + " (SELECT SUM(S.STNAMT) FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE "
            ...
            'Diamond
            strSql += vbCrLf + " (SELECT ISNULL(SUM(ISNULL(S.STNPCS,0)),0) DPCS FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE "
            ...
            strSql += vbCrLf + " FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE S.ITEMID = T.ITEMID AND S.TAGNO = T.TAGNO AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'D'))) DIAWT "

            strSql += vbCrLf + " ,(SELECT ISNULL(SUM(STNAMT),0) DAMT FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE  S.ITEMID = T.ITEMID "
            strSql += vbCrLf + " AND S.TAGNO = T.TAGNO AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'D'))DIAAMT ,"
            'Presious
            strSql += vbCrLf + " (SELECT ISNULL(SUM(ISNULL(STNPCS,0)),0) PPCS FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE "
            ...
            strSql += vbCrLf + " FROM " & cnAdminDb & "..PITEMTAGSTONE S WHERE S.ITEMID = T.ITEMID AND S.TAGNO = T.TAGNO AND STNITEMID IN (SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE = 'P'))) PREWT,"
            ...
            strSql += vbCrLf + " ,(SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE C WHERE C.COSTID=T.COSTID) AS COSTCENTRE"
            strSql += vbCrLf + " ,(SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE C WHERE C.COSTID=T.TCOSTID) AS TCOSTCENTRE"
            strSql += vbCrLf + " INTO TEMPTABLEDB..TEMP" & systemId & "TAGSTOCKVIEW "
            strSql += " FROM " & cnAdminDb & "..PITEMTAG AS T"
            strSql += funcFiltrationString()
            cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
            cmd.ExecuteNonQuery()
            ...
            ElseIf CmbGroupBy.Text = "SUBITEM" Then
                ...
                If filter <> "" Then
                    strSql += "," & filter
                End If
            ElseIf CmbGroupBy.Text = "COSTCENTRE" Then
                ...
                strSql = "INSERT INTO TEMPTABLEDB..TEMP" & systemId & "TAGSTOCKVIEWFINAL(PARTICULAR,PCS, GRSWT, NETWT, SALVALUE,"
                strSql += "DIAPCS, DIAWT,PREPCS,PREWT,WAST,MC,"
                strSql += "COSTCENTRE, RESULT, COLHEAD)"
                strSql += "SELECT 'SUB TOTAL', SUM(PCS), SUM(GRSWT), SUM(NETWT), SUM(SALVALUE), "
                strSql += "SUM(DIAPCS),SUM(DIAWT),SUM(PREPCS),SUM(PREWT),SUM(WAST),SUM(MC),"
                strSql += "COSTCENTRE, '2', 'S' FROM TEMPTABLEDB..TEMP" & systemId & "TAGSTOCKVIEWFINAL  WHERE RESULT = '1'"
                ...
                strSql = "INSERT INTO TEMPTABLEDB..TEMP" & systemId & "TAGSTOCKVIEWFINAL(PARTICULAR,TAGNO,RESULT,COLHEAD,COSTCENTRE)"
                strSql += "SELECT 'TAG NO', COUNT(TAGNO), '9999', 'G','ZZZZ' FROM TEMPTABLEDB..TEMP" & systemId & "TAGSTOCKVIEWFINAL"
                cmd = New OleDbCommand(strSql, cn) : cmd.CommandTimeout = 1000
                cmd.ExecuteNonQuery()
            ...
            With gridTotalView
                ...
                '.Columns("TCOSTCENTRE").Visible = False
                ...
                With .Columns("SIZENAME")
                    ...
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
                End With
                With .Columns("TCOSTCENTRE")
                    .HeaderText = "TRANSFER COSTCENTRE"
                    .Width = 100
                    .DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
                End With
    ...
    Private Sub gridTotalView_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridTotalView.RowEnter
        ...
        strSql += vbCrLf + " (SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTID = T.TCOSTID)TCOSTCENTRE" + vbCrLf
        strSql += vbCrLf + " FROM " & cnAdminDb & "..PITEMTAG AS T" + vbCrLf
        ...
        With dt.Rows(0)
            ...
            "Weight Unit", .Item("WeightUnit").ToString, "CostCentre", .Item("CostCentre").ToString)
            funcAddNewRow("Wastage ", .Item("MaxWast").ToString, "Min Wastage ", .Item("MinWast").ToString, "Fine Rate", .Item("FineRate").ToString, "Tr.CostCentre", .Item("TCOSTCENTRE").ToString)
            ...
            ' funcAddNewRowt("Transfer Costcentre", .Item("transfercostcentre").ToString)
        End With
        gridFullDetails.DataSource = tempDt
    ...
    Private Sub gridTotalView_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles gridTotalView.KeyPress
        ...
        '        qry = " WHERE TAGSNO IN (SELECT SNO FROM " & cnAdminDb & "..PITEMTAG AS T "
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        CmbGroupBy.Items.Add("COSTCENTRE")
...
Public Class frmTagedItemsPendingStockView_Properties
