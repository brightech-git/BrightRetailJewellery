...
Public Class frmDesignerLotEntry
...
GENLOTNO:
            ...
            For cnt As Integer = 0 To itemDt.Rows.Count - 1
                With itemDt.Rows(cnt)
                    ...
                    strSql += " UPTIME,SYSTEMID,APPVER,TABLECODE,FROMITEMID,OPENTIME,RANGESNO,STKTYPE,WASTDISCPER,HALLMARK,PONUMBER)VALUES("
    ...
    Private Sub frmLotEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'strSql = "select DISTINCT PONUMBER from " & cnAdminDb & "..PURCHASEORDER where TRANFLAG = 1"
        strSql = ";with cte1 as("
        strSql += vbCrLf + "select PONUMBER,isnull(sum(PO_PIECES),0)PO_PIECES from " & cnAdminDb & "..PURCHASEORDER a"
        strSql += vbCrLf + "where TRANFLAG = 1"
        strSql += vbCrLf + "group by PONUMBER"
        strSql += vbCrLf + "),"
        strSql += vbCrLf + "cte2 as("
        strSql += vbCrLf + "select PONUMBER,isnull(sum(PCS),0)PCS from " & cnAdminDb & "..ITEMLOT where ISNULL(ponumber,'') <> ''"
        strSql += vbCrLf + "group by PONUMBER"
        strSql += vbCrLf + ")"
        strSql += vbCrLf + "select cte1.PONUMBER from cte1 "
        strSql += vbCrLf + "left join cte2 on cte1.PONUMBER = cte2.PONUMBER "
        strSql += vbCrLf + "where (isnull(cte1.PO_PIECES,0) - isnull(cte2.PCS,0)) > 0"
        objGPack.FillCombo(strSql, cmbPonum, True, False)
    ...
    Private Sub txtItemCode_Num_Man_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtItemCode_Num_Man.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then 'Enter Key
            ...
            With dt.Rows(0)
                ...
                If .Item("SUBITEM").ToString = "Y" Then cmbSubItemName_Man.Enabled = False
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        ...
        If gridViewTotal.Rows.Count > 0 Then
            ...
            End With
        End If
        btnAdd.Enabled = False : btnSave.Focus()
    End Sub
    ...
    Private Sub cmbSupplier_Man_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbSupplier_Man_OWN.GotFocus, cmbPonum.GotFocus
    ...
    Private Sub cmbSupplier_Man_OWN_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbSupplier_Man_OWN.LostFocus, cmbPonum.LostFocus
    ...
    Private Sub btnTransfer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTransfer.Click
        ...
        End Try
    End Sub
    Private Sub cmbPonum_KeyDown(sender As Object, e As KeyEventArgs) Handles cmbPonum.KeyDown
        'Try
        '    If e.KeyCode = Keys.Enter Then
        '        If cmbPonum.Text <> "" Then
        '            strSql = $"select a.ITEMID,b.ITEMNAME,RIGHT(PARTICULAR, CHARINDEX('-', REVERSE(PARTICULAR)) - 1) AS SUBITEM, PO_PIECES,PONUMBER from {cnAdminDb}..PURCHASEORDER a"
        '            strSql &= vbCrLf & $"join {cnAdminDb}..ITEMMAST b on b.ITEMID = a.ITEMID "
        '            strSql &= vbCrLf & $"where PONUMBER = '{cmbPonum.Text}'"
        '            Dim dt As New DataTable
        '            dt = GetSqlTable(strSql, cn)
        '            Dim ofrmPurchaseOrderDetail As New frmPurchaseOrderDetail(dt, True)
        '            If ofrmPurchaseOrderDetail.ShowDialog() = Windows.Forms.DialogResult.OK Then
        '                txtItemCode_Num_Man.Text = dt.Rows(0)("ITEMID").ToString()
        '                txtItemName.Text = dt.Rows(0)("ITEMNAME").ToString()
        '                txtPiece_Num_Man.Text = dt.Rows(0)("PO_PIECES").ToString()
        '                PO_Pieces = dt.Rows(0)("PO_PIECES").ToString()
    ...
    Private Sub txtPiece_Num_Man_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles txtPiece_Num_Man.Validating
        Try
            If Val(txtPiece_Num_Man.Text) > PO_Pieces Then
                MessageBox.Show($"Pieces should be less than or equal to {PO_Pieces}")
                txtPiece_Num_Man.Focus()
                Exit Sub
            End If
        Catch ex As Exception
    ...
    Private Sub cmbPonum_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles cmbPonum.Validating
        Try
            If cmbPonum.Text <> "" Then
                'strSql = $"select a.ITEMID,b.ITEMNAME,SUM(PO_PIECES) PO_PIECES,PONUMBER from {cnAdminDb}..PURCHASEORDER a"
                'strSql &= vbCrLf & $"join {cnAdminDb}..ITEMMAST b on b.ITEMID = a.ITEMID "
                'strSql &= vbCrLf & $"where PONUMBER = '{cmbPonum.Text}'"
                'strSql &= vbCrLf & $"Group By a.ITEMID,b.ITEMNAME,PONUMBER"
                strSql = $";with cte1 as("
                strSql += vbCrLf + $"select ITEMID,PONUMBER,isnull(sum(PO_PIECES),0)PO_PIECES from {cnAdminDb}..PURCHASEORDER a"
                strSql += vbCrLf + $"where TRANFLAG = 1 and PONUMBER = '{cmbPonum.Text}'"
                strSql += vbCrLf + $"group by PONUMBER,ITEMID"
                strSql += vbCrLf + $"),"
                strSql += vbCrLf + $"cte2 as("
                strSql += vbCrLf + $"select PONUMBER,isnull(sum(PCS),0)PCS from {cnAdminDb}..ITEMLOT where ISNULL(ponumber,'') = '{cmbPonum.Text}'"
                strSql += vbCrLf + $"group by PONUMBER"
                strSql += vbCrLf + $")"
                strSql += vbCrLf + $"select cte1.ITEMID,cte1.PONUMBER,(cte1.PO_PIECES - isnull(cte2.PCS,0))PO_PIECES,b.ITEMNAME from cte1"
                strSql += vbCrLf + $"left join cte2 on cte1.PONUMBER = cte2.PONUMBER "
                strSql += vbCrLf + $"join {cnAdminDb}..ITEMMAST b on b.ITEMID = cte1.ITEMID"
                strSql += vbCrLf + $"where (isnull(cte1.PO_PIECES,0) - isnull(cte2.PCS,0)) > 0 "
                Dim dt As New DataTable
                dt = GetSqlTable(strSql, cn)
                If dt.Rows.Count > 0 Then
                    ...
                    txtItemName.Text = dt.Rows(0)("ITEMNAME").ToString()
                    PO_Pieces = dt.Rows(0)("PO_PIECES").ToString()
                    txtPiece_Num_Man.Text = PO_Pieces
                    lblPo_Pen_Piece.Text = PO_Pieces
                    PoNumber = dt.Rows(0)("PONUMBER").ToString()
                    SendKeys.Send("{TAB}")
                Else
                    Exit Sub
