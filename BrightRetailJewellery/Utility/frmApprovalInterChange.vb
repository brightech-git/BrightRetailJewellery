...
Public Class frmApprovalInterChange
    ...
    Dim objMiscRemark As New frmBillRemark
    Public Sub New() '
        ...
        With dtGridView
            ...
            .Columns.Add("IREMARK", GetType(String))
            .Columns.Add("RREMARK", GetType(String))
        End With
        ...
        With gridView
            ...
            .Columns("IREMARK").Visible = False
            .Columns("RREMARK").Visible = False
        End With
    ...
    Private Sub frmApprovalIssUpdate_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        objGPack.FillCombo(strSql, CmbNewParty, False, False)
    ...
    Private Sub btnView_Search_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView_Search.Click
        Try
            ...
            strSql += vbCrLf + " ,PNAME,ITEM,PSALESMAN,CONVERT (VARCHAR(200), '')AS IMAGEPATH,CONVERT(VARBINARY(8000),'') AS TMPIMG,1 RESULT,CONVERT(VARCHAR(4),NULL)COLHEAD,DUEDATE,ISSNO,BATCHNO,CONVERT(VARCHAR(50),NULL)IREMARK,CONVERT(VARCHAR(50),NULL)RREMARK"
            ...
            strSql += vbCrLf + " ,(SELECT TOP 1 ISNULL(CONVERT(VARCHAR(12),DUEDATE,103),'') FROM " & cnAdminDb & "..CUSTOMERINFO WHERE BATCHNO = I.BATCHNO) DUEDATE,I.SNO AS ISSNO,BATCHNO"
            ...
            gridView.Columns("IREMARK").Visible = False
            gridView.Columns("RREMARK").Visible = False
    ...
    Private Sub gridView_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles gridView.GotFocus
        ...
        lblStatus.Text = "Press U to Update / Space-Bar to mark"
    End Sub
    ...
    Private Sub gridView_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles gridView.KeyPress
        ...
        ElseIf UCase(e.KeyChar) = Chr(Keys.Space) Then
            If CmbNewParty.Text.ToString = "" Then
                MsgBox("Please select New Party Name.", MsgBoxStyle.Information)
                Exit Sub
            End If
            If gridView.CurrentRow.Cells("RESULT").Value.ToString <> "1" Then Exit Sub
            If gridView.Rows(gridView.CurrentRow.Index).Cells("NEW_PARTY").Value.ToString <> "" Then
                gridView.Rows(gridView.CurrentRow.Index).Cells("NEW_PARTY").Value = ""
                gridView.CurrentRow.DefaultCellStyle.BackColor = Color.White
            Else
                gridView.Rows(gridView.CurrentRow.Index).Cells("NEW_PARTY").Value = CmbNewParty.Text.ToString
                gridView.CurrentRow.DefaultCellStyle.BackColor = Color.LightGreen
            End If
        End If
    End Sub
    ...
    Private Sub cmbUptPartyname_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbUptPartyname.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            ...
            objMiscRemark.Text = "Entry Remark"
            ...
            If Not objMiscRemark Is Nothing Then
                gridView.CurrentRow.Cells("RREMARK").Value = objMiscRemark.cmbRemark1_OWN.Text
                gridView.CurrentRow.Cells("IREMARK").Value = objMiscRemark.cmbRemark2_OWN.Text
            Else
                gridView.CurrentRow.Cells("RREMARK").Value = ""
                gridView.CurrentRow.Cells("IREMARK").Value = ""
            End If
                ...
                For K As Integer = 0 To dtrunno.Rows.Count - 1
                    ...
                    For i As Integer = 0 To dtGridView.Rows.Count - 1
                        ...
                        If newtranno = True Then
                            PSNO = objGPack.GetSqlValue("SELECT TOP 1 PSNO FROM " & cnAdminDb & "..CUSTOMERINFO WHERE BATCHNO = '" & dtGridView.Rows(i).Item("BATCHNO").ToString & "'", , "", tran)
                            TRANNO = GetBillNoValue("GEN-APPRECBILLNO", tran)
                            BATCHNO = GetNewBatchno(cnCostId, BILLDATE, tran)
                            newtranno = False
                        End If
                        insertintoissrec(dtGridView.Rows(i).Item("ISSNO").ToString, BILLDATE, "AR", TRANNO, BATCHNO, dtGridView.Rows(i).Item("PRUNNO").ToString, PSNO, dtGridView.Rows(i).Item("RREMARK").ToString)
                    Next
                Next
                ...
                For J As Integer = 0 To dtparty.Rows.Count - 1
                    ...
                    For i As Integer = 0 To dtGridView.Rows.Count - 1
                        ...
                        If newtranno = True Then
                            ...
                            newtranno = False
                        End If
                        insertintoissrec(dtGridView.Rows(i).Item("ISSNO").ToString, BILLDATE, "AI", TRANNO, BATCHNO, RUNNO, PSNO, "", dtGridView.Rows(i).Item("IREMARK").ToString)
                    Next
                Next
                tran.Commit()
                tran = Nothing
    ...
    , ByVal tranno As Integer, ByVal BatchNo As String, ByVal runno As String, ByVal PSNO As String _
    , Optional ByVal RREMARK As String = "", Optional ByVal IREMARK As String = "")
        ...
        If trantype = "AR" Then
            strSql += " ,'" & RREMARK & "'" 'REMARK1
        Else
            strSql += " ,'" & IREMARK & "'" 'REMARK1
        End If
    ...
    Private Sub CmbNewParty_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles CmbNewParty.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            objMiscRemark = New frmBillRemark
            If objMiscRemark.Visible Then Exit Sub
            objMiscRemark.Text = "Entry Remark"
            ...
            For i As Integer = 0 To gridView.Rows.Count - 1
                gridView.Rows(i).Cells("NEW_PARTY").Value = cmbUptPartyname.Text
                If Not objMiscRemark Is Nothing Then
                    gridView.Rows(i).Cells("RREMARK").Value = objMiscRemark.cmbRemark1_OWN.Text
                    gridView.Rows(i).Cells("IREMARK").Value = objMiscRemark.cmbRemark2_OWN.Text
                Else
                    gridView.Rows(i).Cells("RREMARK").Value = ""
                    gridView.Rows(i).Cells("IREMARK").Value = ""
                End If
            Next
            btnUpdate.Focus()
