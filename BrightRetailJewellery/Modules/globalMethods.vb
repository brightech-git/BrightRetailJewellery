...
Module globalMethods
    ...
    Public Sub PrintStockTransferNew(ByVal RefNo As String, ByVal RefDate As Date, ByVal Type As String)
        If RefNo = "" Then Exit Sub
        Dim HoName As String = ""
        strSql = "SELECT COSTNAME FROM " & cnAdminDb & "..COSTCENTRE WHERE COSTID IN (SELECT COSTID FROM " & cnAdminDb & "..SYNCCOSTCENTRE WHERE ISNULL(MAIN,'')='Y')"
        HoName = objGPack.GetSqlValue(strSql).ToString()
        ...
        If Detail Then
            If countergroup Then
                ...
                strSql += vbCrLf + "  INNER JOIN " & cnAdminDb & "..ITEMMAST AS IM ON IM.ITEMID = T.ITEMID"
                strSql += vbCrLf + "  LEFT JOIN " & cnAdminDb & "..SUBITEMMAST AS SIM ON SIM.ITEMID = T.ITEMID AND SIM.SUBITEMID = T.SUBITEMID"
                strSql += vbCrLf + "  WHERE T.REFNO = '" & RefNo & "' AND T.REFDATE = '" & RefDate.ToString("yyyy-MM-dd") & "'"
                strSql += vbCrLf + " UNION ALL"
                strSql += vbCrLf + "  SELECT CTR.ITEMCTRNAME,IM.ITEMNAME,/*IM.ITEMNAME+' '+*/SIM.SUBITEMNAME  AS ITEM,TAGNO"
                strSql += vbCrLf + "  ,CASE WHEN T.PCS<>0 THEN T.PCS/T.PCS END NOTAG"
        ...
        Else
            ...
            If countergroup Then
                ...
                strSql = "  SELECT SNO,ITEM,PCS,GRSWT,NETWT,DIAPCS,DIAWT,RESULT FROM " & TempFile & " ORDER BY ITEMCTRNAME,RESULT,CONVERT(INT,SNO)*1 "
                da = New OleDbDataAdapter(strSql, cn)
                da.Fill(vDtTranInfo1)
                totStr = "RESULT=1"
            Else
                ...
                cmd = New OleDbCommand(strSql, cn) : cmd.ExecuteNonQuery()
                strSql = "  SELECT SNO,ITEM,PCS,GRSWT,NETWT,DIAPCS,DIAWT FROM " & TempFile & " ORDER BY CONVERT(INT,SNO)*1 "
                da = New OleDbDataAdapter(strSql, cn)
                da.Fill(vDtTranInfo1)
            End If
            vDtTranInfo1.Columns("SNO").Caption = 60
            ...
            Ro = vDtTranINfo2.NewRow
            Ro.Item("ITEM") = "TOTAL"
            Ro.Item("PCS") = IIf(Val(vDtTranInfo1.Compute("SUM(PCS)", totStr).ToString) = 0, DBNull.Value, Val(vDtTranInfo1.Compute("SUM(PCS)", totStr).ToString))
            Ro.Item("GRSWT") = IIf(Val(vDtTranInfo1.Compute("SUM(GRSWT)", totStr).ToString) = 0, DBNull.Value, Format(Val(vDtTranInfo1.Compute("SUM(GRSWT)", totStr).ToString), "#0.000"))
            Ro.Item("NETWT") = IIf(Val(vDtTranInfo1.Compute("SUM(NETWT)", totStr).ToString) = 0, DBNull.Value, Format(Val(vDtTranInfo1.Compute("SUM(NETWT)", totStr).ToString), "#0.000"))
            Ro.Item("DIAPCS") = IIf(Val(vDtTranInfo1.Compute("SUM(DIAPCS)", totStr).ToString) = 0, DBNull.Value, Val(vDtTranInfo1.Compute("SUM(DIAPCS)", totStr).ToString))
            Ro.Item("DIAWT") = IIf(Val(vDtTranInfo1.Compute("SUM(DIAWT)", totStr).ToString) = 0, DBNull.Value, Format(Val(vDtTranInfo1.Compute("SUM(DIAWT)", totStr).ToString), "#0.000"))
            vDtTranINfo2.pTableBackColor = Color.LightGoldenrodYellow
            vDtTranINfo2.Rows.Add(Ro)
            vDtTranINfo2.pRowHeight = 21
            vDtTranINfo2.pColHeaderVisible = False
        ...
        Dim DtFootHead1 As New BrightPosting.GDatatable
        Dim vDtTranFooter1 As New BrightPosting.GDatatable
        Dim DtFootHead2 As New BrightPosting.GDatatable
        Dim vDtTranFooter2 As New BrightPosting.GDatatable

        DtFootHead1.pTableContentAlignment = StringAlignment.Center
        DtFootHead1.Columns.Add("DES1", GetType(String))
        DtFootHead1.Rows.Add()
        Ro = DtFootHead1.NewRow
        Ro.Item("DES1") = cnCostName
        DtFootHead1.Rows.Add(Ro)
        DtFootHead1.pRowHeight = 21
        DtFootHead1.pColHeaderVisible = False
        DtFootHead1.pCellBorder = False
        DtFootHead1.pContentFont = New Font("verdana", 10, FontStyle.Bold)
        DtFootHead1.pTableContentAlignment = StringAlignment.Center


        DtFootHead2.pTableContentAlignment = StringAlignment.Center
        DtFootHead2.Columns.Add("DES1", GetType(String))
        DtFootHead2.Rows.Add()
        Ro = DtFootHead2.NewRow
        Ro.Item("DES1") = tCostName
        DtFootHead2.Rows.Add(Ro)
        DtFootHead2.pRowHeight = 21
        DtFootHead2.pColHeaderVisible = False
        DtFootHead2.pCellBorder = False
        DtFootHead2.pContentFont = New Font("verdana", 10, FontStyle.Bold)
        DtFootHead2.pTableContentAlignment = StringAlignment.Center
        Ro = DtFootHead2.NewRow
        DtFootHead2.Rows.Add(Ro)
        ...
        strSql = "SELECT 1 FROM " & cnAdminDb & "..SYNCCOSTCENTRE WHERE COSTID='" & cnCostId & "' AND ISNULL(MAIN,'')='Y'"
        If Val(objGPack.GetSqlValue(strSql).ToString) = 0 Then
            IsBranch = True
        End If

        If IsBranch Then
            vDtTranFooter1.Columns.Add("DES4", GetType(String))
        End If


        vDtTranFooter2.pTableContentAlignment = StringAlignment.Near
        vDtTranFooter2.Columns.Add("DES1", GetType(String))
        vDtTranFooter2.Columns.Add("DES2", GetType(String))
        vDtTranFooter2.Columns.Add("DES3", GetType(String))
        If Not (tCostName = HoName) Then
            vDtTranFooter2.Columns.Add("DES4", GetType(String))
        End If
        ...
        If IsBranch Then
            Ro.Item("DES1") = "TRANSFERED BY"
            Ro.Item("DES2") = "COUNTER INCHRG."
            Ro.Item("DES3") = "SRM SIGN"
            Ro.Item("DES4") = "TRANSIT BY"
        Else
            Ro.Item("DES1") = "TRANSFERED BY"
            Ro.Item("DES2") = "VERIFIED BY"
            Ro.Item("DES3") = "TRANSIT BY"
        End If
        vDtTranFooter1.Rows.Add(Ro)
        vDtTranFooter1.pRowHeight = 180
        vDtTranFooter1.pColHeaderVisible = False
        vDtTranFooter1.pCellBorder = False
        vDtTranFooter1.pContentFont = New Font("verdana", 9, FontStyle.Regular)
        vDtTranFooter1.pTableContentAlignment = StringAlignment.Center
        Ro = vDtTranFooter1.NewRow
        vDtTranFooter1.Rows.Add(Ro)
        Ro = vDtTranFooter1.NewRow
        ...
        Ro = vDtTranFooter2.NewRow
        If cnCostName <> HoName And tCostName <> HoName Then
            Ro.Item("DES1") = "RECEIVED BY"
            'Ro.Item("DES2") = "INDIVIDUAL PRODUCT" + Environment.NewLine + " WT CHECKED BY"
            Ro.Item("DES2") = "WT CHECKED BY"
            Ro.Item("DES3") = "COUNTER INCHRG."
            Ro.Item("DES4") = "SRM SIGN"
        ElseIf Not (tCostName = HoName) Then
            Ro.Item("DES1") = "RECEIVED BY"
            Ro.Item("DES2") = "COUNTER INCHRG."
            Ro.Item("DES3") = "WT CHECKED BY"
            Ro.Item("DES4") = "SRM SIGN"
        Else
            Ro.Item("DES1") = "RECEIVED BY"
            Ro.Item("DES2") = "WT CHECKED BY"
            Ro.Item("DES3") = "MANAGER SIGN"
        End If

        vDtTranFooter2.Rows.Add(Ro)
        vDtTranFooter2.pRowHeight = 60
        vDtTranFooter2.pColHeaderVisible = False
        vDtTranFooter2.pCellBorder = False
        vDtTranFooter2.pContentFont = New Font("verdana", 9, FontStyle.Regular)
        vDtTranFooter2.pTableContentAlignment = StringAlignment.Center
        Ro = vDtTranFooter2.NewRow
        vDtTranFooter2.Rows.Add(Ro)
        Ro = vDtTranFooter2.NewRow
        vDtTranFooter2.Rows.Add(Ro)
        ...
        lstSource.Add(DtFootHead1)
        lstSource.Add(vDtTranFooter1)
        lstSource.Add(DtFootHead2)
        lstSource.Add(vDtTranFooter2)
        Dim obj As New BrightPosting.GListPrinter(lstSource)
        obj.Print()
    End Sub


    Public Sub PrintStockTransfer(ByVal RefNo As String, ByVal RefDate As Date, ByVal Type As String)
        If RefNo = "" Then Exit Sub
        ...
        Else
            ...
            If countergroup Then
                ...
                strSql = "  SELECT SNO,ITEM,PCS,GRSWT,NETWT,DIAPCS,DIAWT,RESULT FROM " & TempFile & " ORDER BY ITEMCTRNAME,RESULT "
                da = New OleDbDataAdapter(strSql, cn)
                da.Fill(vDtTranInfo1)
                totStr = "RESULT=1"
            Else
        ...
        vDtTranFooter1.Columns.Add("DES3", GetType(String))
        strSql = "SELECT 1 FROM " & cnAdminDb & "..SYNCCOSTCENTRE WHERE COSTID='" & cnCostId & "' AND ISNULL(MAIN,'')='Y'"
        If Val(objGPack.GetSqlValue(strSql).ToString) = 0 And STKTRAN_PRINTNEW Then
            vDtTranFooter1.Columns.Add("DES4", GetType(String))
        End If
        vDtTranFooter1.Rows.Add()
        Ro = vDtTranFooter1.NewRow
    ...
    Public Sub PrintStockTransfer_DETSUM(ByVal RefNo As String, ByVal RefDate As Date, ByVal Type As String)
        If RefNo = "" Then Exit Sub
        ...
        Dim vDtTranINfo2 As New BrightPosting.GDatatable
        Dim vDtTranINfo3 As New BrightPosting.GDatatable
        Dim vDtTranSumInfo1 As New BrightPosting.GDatatable
        Dim vDtTranSumINfo2 As New BrightPosting.GDatatable
        Dim vDtTranSumINfo3 As New BrightPosting.GDatatable
        Dim vDtTranFoot As New BrightPosting.GDatatable
        Dim vDtTranFoot2 As New BrightPosting.GDatatable
        Dim vDtTranFoot3 As New BrightPosting.GDatatable
        ''DETAIL
        ...
        Else
            ...
            strSql += vbCrLf + "  INNER JOIN " & cnAdminDb & "..ITEMMAST AS IM ON IM.ITEMID = T.ITEMID"
            strSql += vbCrLf + "  WHERE T.RECISS = 'I' AND T.REFNO = '" & RefNo & "' AND T.REFDATE = '" & RefDate.ToString("yyyy-MM-dd") & "'"
            strSql += vbCrLf + "  )X ORDER BY ITEM,TAGNO "
            da = New OleDbDataAdapter(strSql, cn)
            da.Fill(vDtTranInfo1)
            vDtTranInfo1.Columns("ITEM").Caption = 150
            vDtTranInfo1.Columns("TAGNO").Caption = 70
        ...
        Else
            ...
            strSql += vbCrLf + "  WHERE T.REFNO = '" & RefNo & "' AND T.REFDATE = '" & RefDate.ToString("yyyy-MM-dd") & "'"
            strSql += vbCrLf + "  )X "
            strSql += vbCrLf + "  GROUP BY ITEM ORDER BY ITEM"
            cmd = New OleDbCommand(strSql, cn)
            cmd.ExecuteNonQuery()
            strSql = " ALTER TABLE " & TempFile & " ADD SNO INT IDENTITY NOT NULL "
            cmd = New OleDbCommand(strSql, cn) : cmd.ExecuteNonQuery()
            ...
            da.Fill(vDtTranSumInfo1)
        End If
        vDtTranSumInfo1.Columns("SNO").Caption = 60
        vDtTranSumInfo1.Columns("ITEM").Caption = 200
        vDtTranSumInfo1.Columns("PCS").Caption = 60
        vDtTranSumInfo1.Columns("GRSWT").Caption = 80
        vDtTranSumInfo1.Columns("NETWT").Caption = 80
        vDtTranSumInfo1.Columns("DIAPCS").Caption = 60
        vDtTranSumInfo1.Columns("DIAWT").Caption = 80
        vDtTranSumInfo1.pRowHeight = 21
        vDtTranSumInfo1.pColHeaderVisible = True
        vDtTranSumInfo1.pCellBorder = True
        vDtTranSumInfo1.pCellBorderColor = Pens.LightGray
        vDtTranSumInfo1.pContentFont = New Font("verdana", 8, FontStyle.Regular)

        vDtTranSumINfo2 = vDtTranSumInfo1.Clone
        Ro = vDtTranSumINfo2.NewRow
        Ro.Item("ITEM") = "TOTAL"
        Ro.Item("PCS") = IIf(Val(vDtTranSumInfo1.Compute("SUM(PCS)", totStr).ToString) = 0, DBNull.Value, Val(vDtTranSumInfo1.Compute("SUM(PCS)", totStr).ToString))
        Ro.Item("GRSWT") = IIf(Val(vDtTranSumInfo1.Compute("SUM(GRSWT)", totStr).ToString) = 0, DBNull.Value, Format(Val(vDtTranSumInfo1.Compute("SUM(GRSWT)", totStr).ToString), "#0.000"))
        Ro.Item("NETWT") = IIf(Val(vDtTranSumInfo1.Compute("SUM(NETWT)", totStr).ToString) = 0, DBNull.Value, Format(Val(vDtTranSumInfo1.Compute("SUM(NETWT)", totStr).ToString), "#0.000"))
        Ro.Item("DIAPCS") = IIf(Val(vDtTranSumInfo1.Compute("SUM(DIAPCS)", totStr).ToString) = 0, DBNull.Value, Val(vDtTranSumInfo1.Compute("SUM(DIAPCS)", totStr).ToString))
        Ro.Item("DIAWT") = IIf(Val(vDtTranSumInfo1.Compute("SUM(DIAWT)", totStr).ToString) = 0, DBNull.Value, Format(Val(vDtTranSumInfo1.Compute("SUM(DIAWT)", totStr).ToString), "#0.000"))
        vDtTranSumINfo2.pTableBackColor = Color.LightGoldenrodYellow
        vDtTranSumINfo2.Rows.Add(Ro)
        vDtTranSumINfo2.pRowHeight = 21
        vDtTranSumINfo2.pColHeaderVisible = False
        vDtTranSumINfo2.pCellBorder = True
        vDtTranSumINfo2.pCellBorderColor = Pens.LightGray
        vDtTranSumINfo2.pContentFont = New Font("verdana", 8, FontStyle.Bold)
        ...
        Dim vDtTransumhead As New BrightPosting.GDatatable
        vDtTransumhead.Columns.Add("DES1", GetType(String))
        Ro = vDtTransumhead.NewRow
        Ro.Item("DES1") = "SUMMARY"
        vDtTransumhead.Rows.Add(Ro)
        vDtTransumhead.pRowHeight = 21
        vDtTransumhead.pColHeaderVisible = False
        vDtTransumhead.pCellBorder = False
        vDtTransumhead.pContentFont = New Font("verdana", 10, FontStyle.Bold)
        vDtTransumhead.pTableContentAlignment = StringAlignment.Center
        Dim lstSource As New List(Of Object)
        lstSource.Clear()
        lstSource.Add(vDtTranTitle1)
        lstSource.Add(vDtTranTitle2)
        ...
        lstSource.Add(vDtTranINfo3)
        lstSource.Add("")
        lstSource.Add(vDtTransumhead)
        lstSource.Add(vDtTranSumInfo1)
        lstSource.Add(vDtTranSumINfo2)
        lstSource.Add(vDtTranSumINfo3)
        If countergroup Then lstSource.Add(vDtTranFoot) : lstSource.Add(vDtTranFoot2)
        lstSource.Add(vDtTranFooter1)
        Dim obj As New BrightPosting.GListPrinter(lstSource)
        obj.Print()
    ...
    Public Function GetuserPwd(ByVal Optid As Integer, ByVal xCostId As String, Optional ByVal xUserid As Integer = Nothing, Optional ByVal xCtrid As Integer = 0) As Integer
        ...
        If xCostId <> "" Then Sqlqry += " AND COSTID = '" & xCostId & "'"
        If xUserid <> 0 Then Sqlqry += " AND PWDUSERID = " & xUserid
        Sqlqry += " AND PWDEXPIRY=0"
        Sqlqry += " AND PWDDATE='" & GetEntryDate(GetServerDate) & "'"
        Sqlqry += " UNION ALL"
        Sqlqry += " SELECT PWDID FROM " & cnAdminDb & "..PWDMASTER WHERE PWDOPTIONID =" & Optid & " AND PWDSTATUS NOT IN('C','E') "
        If xCostId <> "" Then Sqlqry += " AND COSTID = '" & xCostId & "'"
        If xUserid <> 0 Then Sqlqry += " AND PWDUSERID = " & xUserid
        Sqlqry += " AND PWDEXPIRY <> 0"
        Sqlqry += " AND DATEADD(DAY,CONVERT(INT,PWDEXPIRY),PWDDATE)>='" & GetEntryDate(GetServerDate) & "'"
    ...
    Public Enum TranSnoType
        ...
        ORADTRAN = 73
        ITEMTAGHALLMARKCODE = 74
        ESTISSHALLMARKCODE = 75
        ISSHALLMARKCODE = 76
        ESTRECEIPTHALLMARKCODE = 77
        RECEIPTHALLMARKCODE = 78
        SCHEMEADJTRANCODE = 79
    End Enum
...
GetNewBillNo:
        ...
        strSql = " SELECT 'CHECK' FROM " & cnAdminDb & "..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "ESTBILLNO'"
        If objGPack.GetSqlValue(strSql, , , tran).Length > 0 Then 'TEMPBILLNO EXIST
            strSql = " SELECT 'CHECK' FROM " & cnAdminDb & "..TEMP" & systemId & "ESTBILLNO WHERE CTLID = '" & BillControlId & "'"
            If objGPack.GetSqlValue(strSql, , , tran).Length > 0 Then 'BILLNO ALREADY GENERATED
                strSql = "SELECT CONVERT(INT,CTLTEXT) FROM " & cnAdminDb & "..TEMP" & systemId & "ESTBILLNO WHERE CTLID = '" & BillControlId & "'"
...
GenerateNewBillNo:
            ...
            If cmd.ExecuteNonQuery() = 0 Then
                ...
                GoTo GetNewBillNo
            End If
            strSql = " IF (SELECT COUNT(*) FROM " & cnAdminDb & "..SYSOBJECTS WHERE NAME = 'TEMP" & systemId & "ESTNO')>0"
            strSql += " 	BEGIN"
            strSql += " 	INSERT INTO " & cnAdminDb & "..TEMP" & systemId & "ESTNO(CTLID,CTLTEXT)SELECT '" & BillControlId & "' CTLID,'" & NewBillNo + 1 & "'CTLTEXT"
            strSql += " 	END"
            strSql += " ELSE"
            strSql += " 	BEGIN "
            strSql += " 	CREATE TABLE " & cnAdminDb & "..TEMP" & systemId & "ESTNO(SNO INT IDENTITY(1,1),CTLID VARCHAR(50),CTLTEXT VARCHAR(200))"
            strSql += " 	INSERT INTO " & cnAdminDb & "..TEMP" & systemId & "ESTNO(CTLID,CTLTEXT)SELECT '" & BillControlId & "' CTLID,'" & NewBillNo + 1 & "'CTLTEXT"
            strSql += " 	END"
            cmd = New OleDbCommand(strSql, cn, tran)
            cmd.ExecuteNonQuery()
...
GenerateNewBillNo:
            ...
            If CENTR_DB_BR Then strSql += " AND ISNULL(COSTID,'') = '" & strBCostid & "'"
            NewBillNo = Val(objGPack.GetSqlValue(strSql, , , tran))
    ...
    Public Function GetServerTime(Optional ByVal tran As OleDbTransaction = Nothing) As Date
        ...
        Else
            Return CType(objGPack.GetSqlValue("SELECT CONVERT(SMALLDATETIME,CONVERT(VARCHAR(12),'" & Date.Now.ToLongTimeString & "',108))", , , tran), Date)
        ...
        Select Case UCase(CType(sender, ToolStripMenuItem).Tag)
            ...
            Case Else
                ...
                If USERWISECOMPANY Then
                    Dim UserCompanyid As String = ""
                    strSql = " SELECT USERCOMPANYID FROM " & cnAdminDb & "..USERMASTER WHERE USERID  = " & userId
                    UserCompanyid = GetSqlValue(cn, strSql)
                    UserCompanyid = Replace(UserCompanyid, ",", "','")
                    strSql = " SELECT COMPANYID,COMPANYNAME FROM " & cnAdminDb & "..COMPANY C "
                    strSql += " WHERE ISNULL(SHORTKEY,'') = '" & CType(sender, ToolStripMenuItem).Tag & "'"
                    strSql += " AND COMPANYID IN ( '" & UserCompanyid & "')"
                Else
...
NextIteration:
    ...
    Public Function GetActualEntryDate() As Date
        ...
        If Dt.Rows.Count > 0 Then
            If Dt.Rows(0).Item(0).ToString <> "" Then
                If EntryDAte < CType(Dt.Rows(0).Item(0), Date) Then
  ...
  , Optional ByVal _SyncDb As String = Nothing) As Boolean
    ...
    Public Function ExecQuery(ByVal mode As SyncMode _
           ...
           , Optional ByVal ChitSyncDb As String = ""
               ) As Boolean
        ...
        If mode = SyncMode.Master Then
            ...
            If ChitSyncDb.ToString <> "" Then
                'strSql = "SELECT COSTID,STATEID FROM " & ChitSyncDb & "..SYNCCOSTCENTRE WHERE COSTID <> '" & cnCostId & "' AND ISNULL(ACTIVE,'Y')<>'N'"
                strSql = "SELECT COSTID,(SELECT STATEID FROM " & ChitSyncDb & "..SYNCCOSTCENTRE WHERE COSTID=A.COSTID)STATEID FROM " & ChitSyncDb & "..COSTCENTRE AS A WHERE COSTID <> '" & cnCostId & "' AND ISNULL(ACTIVE,'Y')<>'N'"
            Else
            ...
            For Each ro As DataRow In dtCostId.Rows
                ...
                Exec(qry.Replace("'", "''"), cn, ro!COSTID.ToString, imagePath, tran, ImagePathCtrlId, SyncDb)
            Next
            ...
            If GetAdmindbSoftValue("SYNC-STOCK", "N", tran) = "Y" Then
                ...
                For Each ro As DataRow In dtCostId.Rows
                    ...
                    If ro!COSTID.ToString = toId Then
                        Exec(qry.Replace("'", "''"), cn, ro!COSTID.ToString, imagePath, tran, ImagePathCtrlId, SyncDb)
                    Else
                        Exec(OriginalQry.Replace("'", "''"), cn, ro!COSTID.ToString, imagePath, tran, ImagePathCtrlId, SyncDb)
                    End If
                Next
            Else
                If toId <> Nothing Then
                    If toId = cnCostId Then Return True
                    Exec(qry.Replace("'", "''"), cn, toId, imagePath, tran, ImagePathCtrlId, SyncDb)
                End If
            End If
        ElseIf mode = SyncMode.Transaction Then
            If toId <> Nothing And toAdvId Is Nothing Then
                ...
                Exec(qry.Replace("'", "''"), cn, syncTo, imagePath, tran, ImagePathCtrlId, SyncDb)
            Else
                If toAdvId <> Nothing Then
                    Exec(qry.Replace("'", "''"), cn, toAdvId, imagePath, tran, ImagePathCtrlId, SyncDb)
                End If
            End If
        End If
        Return True
                ...
                For Each dCol As DataColumn In Row.Table.Columns
                    ...
                    ElseIf dCol.DataType.Name = "DateTime" Then
                        ...
                        ElseIf Row.Item(dCol.ColumnName).ToString = "1/1/0001" Then
                            _Values = _Values & "," + "NULL" + ""
                        Else
                            _Values = _Values & ",'" & Microsoft.VisualBasic.Format(Row.Item(dCol.ColumnName), "yyyy/MM/dd HH:mm") & "'"
                        End If
    ...
    Public Function CheckBckDays(ByVal Userid As Integer, ByVal Menuname As String, ByVal dtdate As Date, Optional MenuText As String = "") As Boolean
        ...
        If dtdate < mserverdate Then
            ...
            If Userid = 0 Or (Menuname = "" And MenuText.ToString = "") Then Return True
            Dim ro() As DataRow
            If MenuText <> "" Then
                ro = _DtUserRights.Select("MENUTEXT LIKE '" & MenuText & "%'")
            Else
                ro = _DtUserRights.Select("ACCESSID LIKE '" & Menuname & "%'")
            End If
    ...
    Public Function CheckBckDaysEntry(ByVal Userid As Integer, ByVal Menuname As String, ByVal dtdate As Date, Optional MenuText As String = "") As Boolean
        Dim mserverdate As Date = GetServerDate()
        If dtdate < mserverdate Then
            If Userid = 999 Then Return True
            Dim mdifdays As Integer = Math.Abs(DateDiff(DateInterval.Day, mserverdate, dtdate))
    ...
    Public Function GetNewBatchno(ByVal CostId As String, ByVal BillDate As Date, ByVal Transact As OleDbTransaction, Optional ByVal Isestimate As Boolean = False) As String
        ...
        'Return objGPack.GetSqlValue(StrSql, , , Transact) 'comment on 11-08-2018
    End Function
    ...
    Public Function GetNewSno_Chit(ByVal CtlId As TranSnoType, ByVal Transact As OleDbTransaction, ByVal ProcDbName As String, Optional ByVal ProcName As String = "GET_SNO_SCHEME", Optional ByVal CompId As String = Nothing) As String
        If CompId = Nothing Then CompId = strCompanyId
        Dim StrSql As String = Nothing
        StrSql = " DECLARE @RETSNOVALUE VARCHAR(15)"
        StrSql += " EXEC " & ProcDbName & ".." & ProcName
        StrSql += " @COSTID = '" & cnCostId & "'"
        StrSql += " ,@CTLID = '" & CtlId.ToString & "'"
        StrSql += " ,@CHECKDB = '" & ProcDbName & "'"
        StrSql += " ,@CHECKTABLENAME = '" & CtlId.ToString.ToUpper.Replace("CODE", "") & "'"
...
#End Region
    ...
    Public Sub SetGlobalVariables()
        ...
        _IsCostCentre = IIf(GetAdmindbSoftValuefromDt(dtSoftKeys, "COSTCENTRE", "N") = "Y", True, False)
        ...
        GEN_SKUFILE = IIf(GetAdmindbSoftValuefromDt(dtSoftKeys, "GEN_SKUFILE", "N") = "Y", True, False)
        SKUFILEPATH = GetAdmindbSoftValuefromDt(dtSoftKeys, "SKUFILEPATH", "")
        RATE_BRANCHWISE = IIf(GetAdmindbSoftValuefromDt(dtSoftKeys, "RATE_BRANCHWISE", "N") = "Y", True, False)
        ISDISPLAY_92 = IIf(GetAdmindbSoftValuefromDt(dtSoftKeys, "ISDISPLAY_92", "N") = "Y", True, False)
        GstDate = GetAdmindbSoftValuefromDt(dtSoftKeys, "GSTDATE", "")
        Dim BillDate As Date = GetEntryDate(GetServerDate())
        If BillDate >= GstDate And GetAdmindbSoftValuefromDt(dtSoftKeys, "GST", "N") = "Y" Then
            GST = True
        End If
        SRCESS = IIf(GetAdmindbSoftValuefromDt(dtSoftKeys, "SR_CESS", "N") = "Y", True, False)
    End Sub
    Public Function GetSqlValue(ByVal Cn As OleDb.OleDbConnection, ByVal Qry As String) As Object
        ...
        Return Obj
    End Function
    Public Function GetSqlValue(ByVal Qry As String, ByVal Cn As OleDb.OleDbConnection, ByVal tran As OleDbTransaction) As String
        Dim Obj As String = ""
        Dim Da As OleDb.OleDbDataAdapter
        Dim DtTemp As New DataTable
    ...
    Public Function GetBillDate(ByRef BillDate As Date) As Boolean
        If GetAdmindbSoftValue("GLOBALDATE", "N").ToUpper = "Y" Then
            ...
            Dim chkdbName1 As String = objGPack.GetSqlValue("SELECT DBNAME FROM " & cnAdminDb & "..DBMASTER WHERE '" & Format(RetBilldate.Date, "yyyy-MM-dd") & "' BETWEEN STARTDATE AND ENDDATE").ToString
            Dim chkdbName2 As String = objGPack.GetSqlValue("SELECT DBNAME FROM " & cnAdminDb & "..DBMASTER WHERE '" & Format(BillDate, "yyyy-MM-dd") & "' BETWEEN STARTDATE AND ENDDATE").ToString
            If chkdbName1 <> chkdbName2 Then
                BillDate = BillDate
            Else
    ...
    Public Function usrpwdokonly(ByVal PWDCTRL As String, ByVal IS_USERLEVELPWD As Boolean) As Boolean
        ...
        If IS_USERLEVELPWD Then
            ...
            Dim Optionid As Integer = Val("" & objGPack.GetSqlValue(Sqlqry, , , tran))
            Sqlqry = "SELECT PWDTYPE FROM " & cnAdminDb & "..PRJPWDOPTION WHERE OPTIONNAME ='" & PWDCTRL & "' AND ACTIVE = 'Y'"
        ...
        Return False
    End Function
    Public Function usrpwdok(ByVal PWDCTRL As String, ByVal IS_USERLEVELPWD As Boolean, Optional ByVal msgstr As String = Nothing) As Boolean
        If IS_USERLEVELPWD Then
            Dim pwdid As Integer = 0 : Dim pwdpass As Boolean = False : Dim strmsg As String = msgstr
            Dim Sqlqry As String = "Select Optionid from " & cnAdminDb & "..PRJPWDOPTION where OPTIONNAME ='" & PWDCTRL & "' AND active = 'Y'"
            Dim Optionid As Integer = Val("" & objGPack.GetSqlValue(Sqlqry, , , tran))
            Sqlqry = "SELECT PWDTYPE FROM " & cnAdminDb & "..PRJPWDOPTION WHERE OPTIONNAME ='" & PWDCTRL & "' AND ACTIVE = 'Y'"
            Dim PwdType As String = objGPack.GetSqlValue(Sqlqry, , , tran)
            If Optionid = 0 Then pwdpass = True
            If Optionid <> 0 And Not _IsAdmin Then
                pwdid = GetuserPwd(Optionid, cnCostId, userId)
                If pwdid <> 0 Or PwdType = "S" Then
                    Dim objUpwd As New frmUserPassword(pwdid, Optionid,, Nothing, strmsg)
                    If objUpwd.ShowDialog <> Windows.Forms.DialogResult.OK Then pwdpass = False Else pwdpass = True
                Else
    ...
    Public Function usrpwdid(ByVal PWDCTRL As String, ByVal IS_USERLEVELPWD As Boolean, Optional ByVal Fin_disc As String = Nothing, Optional ByVal Fin_disc_per As String = Nothing) As Integer
        If IS_USERLEVELPWD Then
            Dim pwdid As Integer = 0 : Dim pwdpass As Boolean = False
            Dim Sqlqry As String = "Select Optionid from " & cnAdminDb & "..PRJPWDOPTION where OPTIONNAME ='" & PWDCTRL & "' AND active = 'Y'"
            Dim Optionid As Integer = Val("" & objGPack.GetSqlValue(Sqlqry, , , tran))
            ...
            If userId <> 999 And Optionid <> 0 And Not _IsAdmin Then
                pwdid = GetuserPwd(Optionid, cnCostId, userId)
                If pwdid <> 0 Or PwdType = "S" Then
                    Dim objUpwd As New frmUserPassword(pwdid, Optionid, True,,, Fin_disc, Fin_disc_per)
                    If objUpwd.ShowDialog <> Windows.Forms.DialogResult.OK Then
                        pwdid = 0
                    Else
                        Return objUpwd.userpwdid
                    End If
                End If
            Else
    ...
    Public Function funcOTPaccess(ByVal StrfrmName As String, ByVal StroptName As String, ByVal CostId As String, ByVal userId As Integer)
        Dim Optionid As Integer = 0
        Dim mfrmname As String = StrfrmName & "~OWN~AUT"
        If ChkDbClosed(cnStockDb) = True Then MsgBox("Transaction for this finincial year Closed." & vbCrLf & "Not able to Cancel.", MsgBoxStyle.Information) : Exit Function
        Dim pwdid As Integer = 0 : Dim pwdpass As Boolean = False
        Dim Sqlqry As String = "SELECT OPTIONID FROM " & cnAdminDb & "..PRJPWDOPTION WHERE OPTIONNAME ='" & StroptName & "' AND ACTIVE = 'Y'"
        Optionid = Val("" & objGPack.GetSqlValue(Sqlqry, , , tran))
        Sqlqry = "SELECT PWDTYPE FROM " & cnAdminDb & "..PRJPWDOPTION WHERE OPTIONNAME ='" & StroptName & "' AND ACTIVE = 'Y'"
        Dim PwdType As String = objGPack.GetSqlValue(Sqlqry, , , tran)
        ...
        If Optionid <> 0 Then
            ...
            If pwdid <> 0 Or PwdType = "S" Then
                Dim objUpwd As New frmUserPassword(pwdid, Optionid)
                If objUpwd.ShowDialog <> Windows.Forms.DialogResult.OK Then MsgBox("Access Denied.") : Exit Function Else pwdpass = True
            End If
        Else
            pwdpass = False
        End If
    ...
    Public Sub GenerateSkuFile(ByVal _Cn As OleDbConnection, ByVal _Tran As OleDbTransaction, ByVal BatchNo As String, Optional ByVal TAGNOS As String = "", Optional ByVal REFNO As String = "")
        If GEN_SKUFILE And SKUFILEPATH <> "" Then
            If BatchNo <> "" Then
                strSql = "SELECT TAGNO FROM " & cnAdminDb & "..ITEMTAG "
                strSql += " WHERE TAGNO IN("
                strSql += " SELECT TAGNO FROM " & cnStockDb & "..ISSUE WHERE BATCHNO='" & BatchNo & "'"
                strSql += " UNION"
                strSql += " SELECT TAGNO FROM " & cnStockDb & "..RECEIPT WHERE BATCHNO='" & BatchNo & "'"
                strSql += " )"
                strSql += " ORDER BY TAGNO"
            ElseIf TAGNOS <> "" Then
                TAGNOS = Mid(TAGNOS, 1, Len(TAGNOS) - 1)
                strSql = "SELECT TAGNO FROM " & cnAdminDb & "..ITEMTAG "
                strSql += " WHERE TAGNO IN( " & TAGNOS & " )"
                strSql += " ORDER BY TAGNO"
            ElseIf REFNO <> "" Then
                strSql = "SELECT TAGNO FROM " & cnAdminDb & "..ITEMTAG "
            ...
            da = New OleDbDataAdapter(cmd)
            da.Fill(dtSku)
            Dim FILEPATH() As String = SKUFILEPATH.Split("$")
            Dim SkuPath As String = ""
            If FILEPATH.Length > 1 Then
                SkuPath = FILEPATH(0)
                If IO.Directory.Exists(SkuPath) = False Then
                    SkuPath = FILEPATH(1)
                End If
            ElseIf FILEPATH.Length = 1 Then
                SkuPath = FILEPATH(0)
            End If
            If dtSku.Rows.Count > 0 And IO.Directory.Exists(SkuPath) Then
                Dim fileName As String = "\" & Today.Date.Now.ToString("yyyyMMddHHmmss") & ".Ini"
                Dim write As IO.StreamWriter
                write = IO.File.CreateText(SkuPath & fileName)
                For cnt As Integer = 0 To dtSku.Rows.Count - 1
                    Dim Sku As String = ""
                    Dim PreSku As String = ""
                    Dim Content As String = ""
                    With dtSku.Rows(cnt)
                        Sku = Mid(.Item("TAGNO").ToString, 1, 7)
                        Dim Costname As String = ""
                        Dim SizeName As String = ""
                        Dim DiaGrade As String = ""
                        If Sku <> PreSku Then
                            PreSku = Sku
                            strSql = " SELECT DISTINCT (SELECT SIZENAME FROM " & cnAdminDb & "..ITEMSIZE WHERE SIZEID=T.SIZEID)SIZE "
                            strSql += " FROM " & cnAdminDb & "..ITEMTAG T"
                            strSql += " WHERE TAGNO='" & .Item("TAGNO").ToString & "'"
                            Dim dtsize As New DataTable
                            cmd = New OleDbCommand(strSql, _Cn, _Tran)
                            da = New OleDbDataAdapter(cmd)
                            da.Fill(dtsize)
                            If dtsize.Rows.Count > 0 Then
                                SizeName = dtsize.Rows(0).Item("SIZE").ToString
                            End If
                            strSql = " SELECT DISTINCT COSTID,"
                            strSql += " (SELECT TOP 1 CLARITYNAME FROM " & cnAdminDb & "..STNCLARITY WHERE CLARITYID=T.CLARITYID)CLARITY "
                            strSql += " ,(SELECT TOP 1 COLORNAME FROM " & cnAdminDb & "..STNCOLOR WHERE COLORID=T.COLORID)COLOR "
                            strSql += " ,(SELECT SIZENAME FROM " & cnAdminDb & "..ITEMSIZE WHERE SIZEID="
                            strSql += " (SELECT TOP 1 SIZEID FROM " & cnAdminDb & "..ITEMTAG WHERE SNO=T.TAGSNO))SIZE "
                            strSql += " FROM " & cnAdminDb & "..ITEMTAGSTONE T"
                            strSql += " WHERE SUBSTRING(TAGNO,1,7)='" & Sku & "'"
                            strSql += " AND STNITEMID IN(SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE DIASTONE='D')"
                            Dim dtGrade As New DataTable
                            cmd = New OleDbCommand(strSql, _Cn, _Tran)
                            da = New OleDbDataAdapter(cmd)
                            da.Fill(dtGrade)
                            Dim Grade As New List(Of String)
                            If dtGrade.Rows.Count > 0 Then
                                If Mid(.Item("TAGNO").ToString, 8, 1) = "A" Then
                                    DiaGrade = "VVS-EF"
                                ElseIf Mid(.Item("TAGNO").ToString, 8, 1) = "B" Then
                                    DiaGrade = "SI-GH"
                                Else
                                    DiaGrade = dtGrade.Rows(0).Item("CLARITY").ToString & "-" & dtGrade.Rows(0).Item("COLOR").ToString
                                End If
                            End If
                            strSql = vbCrLf + "SELECT CC.COSTNAME ,COUNT(IT.COSTID)NO,CC.COSTID FROM  "
                            strSql += vbCrLf + "" & cnAdminDb & "..COSTCENTRE CC "
                            strSql += vbCrLf + "LEFT JOIN " & cnAdminDb & "..ITEMTAG IT ON CC.COSTID =IT.COSTID "
                            strSql += vbCrLf + "AND SUBSTRING(TAGNO,1,8)='" & Mid(.Item("TAGNO").ToString, 1, 8) & "' AND ISSDATE IS NULL "
                            strSql += vbCrLf + "AND ISNULL(APPROVAL,'')=''"
                            If SizeName <> "" Then
                                strSql += vbCrLf + " AND IT.SIZEID IN(SELECT SIZEID FROM " & cnAdminDb & "..ITEMSIZE WHERE SIZENAME='" & SizeName & "')"
                            End If
                            strSql += vbCrLf + "GROUP BY CC.COSTNAME,CC.COSTID"
                            Dim dtCost As New DataTable
                            cmd = New OleDbCommand(strSql, _Cn, _Tran)
                            da = New OleDbDataAdapter(cmd)
                            da.Fill(dtCost)
                            Dim Cnt1 As Integer = dtCost.Compute("SUM(NO)", Nothing)
                            If dtCost.Rows.Count > 0 Then
                                For JJ As Integer = 0 To dtCost.Rows.Count - 1
                                    With dtCost.Rows(JJ)
                                        Costname += .Item("COSTNAME").ToString & "=" & .Item("NO").ToString
                                        If JJ <> dtCost.Rows.Count - 1 Then
                                            Costname += ";"
                                        End If
                                    End With
                                Next
                            End If
                        End If
                        Content = Sku & "," & DiaGrade & "," & SizeName & "," & Costname
    ...
    Function funcGenPwd1(ByVal optionId As Integer, ByVal MobileNo As String, ByVal Msg As String, Optional ByVal _rep As Boolean = False _
                         , Optional ByVal fin_dis_val As String = Nothing, Optional ByVal fin_dis_per As String = Nothing, Optional ByVal BillNo As String = "")
        ...
        Dim empy As Int32 = 0
        anew = Mid(anew, 1, 4)
        Dim billdate As Date = GetServerDate()
        If _rep Then
            Msg = Msg.Replace("<OTP>", anew)
            Msg = Msg.Replace("<COMPANY>", strCompanyName)
            Msg = Msg.Replace("<COSTCENTRE>", cnCostName)
            Msg = Msg.Replace("<DISCAMT>", fin_dis_val)
            Msg = Msg.Replace("<DISCPER>", fin_dis_per)
            Msg = Msg.Replace("<BILLNO>", BillNo.ToString)
            Msg = Msg.Replace("<BILLDATE>", billdate.ToString("dd-MM-yyyy"))
            Msg = Msg.Replace("<TIME>", GetServerTime())
            Msg = Msg.Replace("<USERNAME>", GetSqlValue(cn, "SELECT TOP 1 USERNAME FROM " & cnAdminDb & "..USERMASTER WHERE USERID=" + userId.ToString + ""))
        End If
        ...
        Else
            ...
            ExecQuery(SyncMode.Stock, strSql, cn, tran, cnCostId)
        End If
        Dim mobilesms As String() = MobileNo.Split(",")
        For i As Integer = 0 To mobilesms.Length - 1
            SmsSend(Msg, mobilesms(i).ToString)
        Next
        Return pwdid
    End Function

    Function funcGenPwd(ByVal optionId As Integer, ByVal MobileNo As String, ByVal Msg As String, Optional ByVal _rep As Boolean = False)
        'If SMSURL = "" Then MsgBox("URL not found..") : Exit Function
        If MobileNo = "" Then MsgBox("MobileNo not Updated...") : Exit Function
        Dim _r As Random = New Random
        Dim pwdid As Int32
        strSql = "SELECT ISNULL(MAX(PWDID),0) FROM " & cnAdminDb & "..PWDMASTER"
        ...
        If MobileNo.Contains(",") Then
            ...
            For i As Integer = 0 To Mob.Length - 1
                ...
                ExecQuery(SyncMode.Stock, strSql, cn, tran, cnCostId)
                If _rep Then
                    SENDURLSMS(Mob(i), Msg)
                Else
                    SENDURLSMS(Mob(i), anew & Msg)
                End If
            Next
        Else
            strSql = " UPDATE " & cnAdminDb & "..PWDMASTER SET PWDSTATUS='E',PWDCLSDATE= '" + GetServerDate() + "',PWDCLSTIME='" & GetServerTime() + "'"
            ...
            ExecQuery(SyncMode.Stock, strSql, cn, tran, cnCostId)
            If _rep Then
                SENDURLSMS(MobileNo, Msg)
            Else
                SENDURLSMS(MobileNo, anew & Msg)
            End If
        End If
    End Function

    Public Function SENDURLSMS(ByVal mobno As String, ByVal msg As String) As Int16
        If SMSURL = "" Then
            If msg <> "" And mobno.Length = 10 Then
                strSql = "SELECT COUNT(*)CNT FROM SYSDATABASES WHERE NAME='AKSHAYASMSDB'"
                If objGPack.GetSqlValue(strSql, "CNT", 0) > 0 Then
                    strSql = "INSERT INTO AKSHAYASMSDB..SMSDATA(MOBILENO,MESSAGES,STATUS,EXPIRYDATE,UPDATED)"
                    strSql += " VALUES"
                    strSql += " ("
                    strSql += " '" & mobno.Trim & "',N'" & msg & "','N'"
                    strSql += " ,'" & Today.Date.ToString("yyyy-MM-dd") & "','" & Today.Date.ToString("yyyy-MM-dd") & "'"
                    strSql += " ) "
                    cmd = New OleDbCommand(strSql, cn, tran)
                    cmd.ExecuteNonQuery()
        ...
        address = address.Replace("<SMSMSG>", msg)
        address = address.Replace("<SMSTO>", mobno)
    ...
    End Function
    Public Function CALLURL(ByVal address As String) As Int16
        Try
    ...
    Public Function FuncprintBarcode_Multi(ByVal ItemId As String, ByVal Tagno As String, ByVal PrintFormat As Double, ByVal METALID As String)
        Try
    ...
    Public Function funcMatureDate(ByVal Grp As String, ByVal RegNo As Integer) As Date
        Try
            strSql = " SELECT * FROM " & cnChitCompanyid & "SH0708..SCHEMETRAN WHERE "
            strSql += " GROUPCODE = '" & Grp & "' AND REGNO = '" & RegNo & "' "
            strSql += " AND ISNULL(CANCEL,'') = '' AND ISNULL(CHEQUERETDATE,'') = ''"
            strSql += " ORDER BY INSTALLMENT"
            Dim dt, dtIns As New DataTable
            Dim MsNo As Long
            Dim Date1 As DateTime
            Dim DateF As DateTime
            Dim DateT As DateTime
            Dim LastInsDate As DateTime
            Dim FirstInsDate As DateTime
            Dim DelayDate As DateTime
            Dim DelayIns As Integer
            Dim MulInsDate As DateTime
            Dim MulIns As Integer
            Dim maxDays As Integer = 0
            MsNo = 1
            da = New OleDbDataAdapter(strSql, cn)
            da.Fill(dt)
            If dt.Rows.Count > 0 Then
                Dim dtDelay As New DataTable
                Dim dtTemp As New DataTable
                dtDelay.Columns.Add("INSNO")
                dtDelay.Columns.Add("RECDATE")
                dtDelay.Columns.Add("DEFAMONTH")
                dtDelay.Columns.Add("PAYBYDATE")
                MulIns = 0
                Dim ro As DataRow
                Dim totDays As Long
                strSql = " SELECT ISNULL(INSTALMENT,0)NOOFINS,M.JOINDATE FROM " & cnChitCompanyid & "SAVINGS..SCHEME S"
                strSql += " INNER JOIN " & cnChitCompanyid & "SAVINGS..SCHEMEMAST AS M ON M.SCHEMEID=S.SCHEMEID"
                strSql += " WHERE  M.GROUPCODE  = '" & Grp & "' AND M.REGNO = '" & RegNo & "' "
                da = New OleDbDataAdapter(strSql, cn)
                dtIns = New DataTable
                da.Fill(dtIns)
                Dim NoofIns As Integer = 0
                Dim JoinDate As Date
                If dtIns.Rows.Count > 0 Then
                    NoofIns = Val(dtIns.Rows(0).Item("NOOFINS").ToString)
                    JoinDate = dtIns.Rows(0).Item("JOINDATE")
                End If
                For i As Integer = 0 To dt.Rows.Count - 1
                    With dt.Rows(i)
                        ro = dtDelay.NewRow
                        ro!INSNO = .Item("INSTALLMENT").ToString
                        Dim TempD1 As Date = .Item("RDATE").ToString
                        If maxDays < TempD1.Day Then
                            maxDays = TempD1.Day
                        End If
                        ro!RECDATE = Format(TempD1, "dd/MM/yyyy")
                        If MsNo > 1 Then
                            LastInsDate = dt.Rows(i - 1).Item("RDATE").ToString
                            If Month(LastInsDate) = Month(.Item("RDATE").ToString) And Year(LastInsDate) = Year(.Item("RDATE").ToString) Then
                                MulIns = MulIns + 1
                                dtTemp = Nothing
                                Dim TempDate As Date
                                TempDate = .Item("RDATE").ToString
                                strSql = "SELECT DATEADD(month," & MulIns & ",'" & Format(TempDate, "yyyy-MM-dd") & "') as MONDIFF"
                                MulInsDate = objGPack.GetSqlValue(strSql, "MONDIFF", "", )
                            Else
                                MulInsDate = LastInsDate
                            End If
                            dtTemp = Nothing
                            dtTemp = New DataTable
                            Dim TempDate1 As Date
                            TempDate1 = .Item("RDATE").ToString
                            strSql = " SELECT "
                            strSql += " DATEDIFF(month,'" & Format(MulInsDate, "yyyy-MM-dd") & "','" & Format(TempDate1, "yyyy-MM-dd") & "') AS MONDIFF,"
                            strSql += " DATEADD(month," & MsNo - 1 & ",'" & Format(FirstInsDate, "yyyy-MM-dd") & "') AS ACTUALMONTH,"
                            strSql += " DATEADD(month," & MsNo - DelayIns & ",'" & Format(DelayDate, "yyyy-MM-dd") & "') AS ACTUALMONTHTO"
                            da = New OleDbDataAdapter(strSql, cn)
                            da.Fill(dtTemp)
                            Dim ActualMonthTo As Date
                            Dim ActualMonth As Date
                            Dim MonthDiff As Integer = 0
                            If dtTemp.Rows.Count > 0 Then
                                MonthDiff = Val(dtTemp.Rows(0).Item("MONDIFF").ToString)
                                ActualMonth = dtTemp.Rows(0).Item("ACTUALMONTH").ToString
                                ActualMonthTo = dtTemp.Rows(0).Item("ACTUALMONTHTO").ToString
                            End If
                            If MonthDiff > 1 Then
                                Dim TempDate As String = "01/" & Month(ActualMonthTo) & "/" & Year(ActualMonthTo)
                                DateT = Date.Parse(TempDate, ukCultureInfo.DateTimeFormat)
                                DateT = DateT.AddDays(-1)
                                If .Item("RDATE").ToString > DateT Then
                                    DelayDate = .Item("RDATE").ToString
                                    DelayIns = MsNo - 1
                                    dtTemp = Nothing
                                    dtTemp = New DataTable
                                    Dim TempDate3 As Date
                                    TempDate3 = .Item("RDATE").ToString
                                    strSql = "SELECT DATEDIFF(month,'" & Format(DateT, "yyyy-MM-dd") & "','" & Format(TempDate3, "yyyy-MM-dd") & "') as MONDIFF"
                                    da = New OleDbDataAdapter(strSql, cn)
                                    da.Fill(dtTemp)
                                    If dtTemp.Rows.Count > 0 Then
                                        ro!DEFAMONTH = Val(dtTemp.Rows(0).Item("MONDIFF").ToString)
                                    End If
                                    ro!PAYBYDATE = Format(DateT, "dd/MM/yyyy")
                                    totDays = totDays + Val(dtTemp.Rows(0).Item("MONDIFF").ToString)
                                End If
                                MulIns = 0
                            End If
                            DateF = .Item("RDATE").ToString
                            Dim TempFirst As String
                            Dim daysInMonth As Integer = 0
                            daysInMonth = System.DateTime.DaysInMonth(FirstInsDate.Year, FirstInsDate.Month)
                            If DateF.Day > daysInMonth Then
                                TempFirst = daysInMonth & "/" & FirstInsDate.Month & "/" & FirstInsDate.Year
                            Else
                                TempFirst = DateF.Day & "/" & FirstInsDate.Month & "/" & FirstInsDate.Year
                            End If

                            FirstInsDate = Date.Parse(TempFirst, ukCultureInfo.DateTimeFormat)
                        Else
                            FirstInsDate = .Item("RDATE").ToString
                            DelayDate = FirstInsDate
                        End If
                        MsNo = MsNo + 1
                        Date1 = .Item("RDATE").ToString
                    End With
                    dtDelay.Rows.Add(ro)
                Next
                MsNo = MsNo - 1

                strSql = "SELECT DATEADD(month," & NoofIns & ",'" & Format(JoinDate, "yyyy-MM-dd") & "') AS MDATE"
                ro = dtDelay.NewRow
                ro!INSNO = ""
                ro!RECDATE = "Maturity"
                Dim TempMat As Date = objGPack.GetSqlValue(strSql, "MDATE", "", ).ToString
                ro!DEFAMONTH = Format(TempMat, "dd/MM/yyyy")
                ro!PAYBYDATE = ""
                dtDelay.Rows.Add(ro)
                ro = dtDelay.NewRow
                ro!INSNO = ""
                ro!RECDATE = "No.Of.Defaults"
                ro!DEFAMONTH = totDays
                ro!PAYBYDATE = ""
                dtDelay.Rows.Add(ro)
                Dim TempMDate As String
                If FirstInsDate.MaxValue.Day < maxDays Then maxDays = FirstInsDate.MaxValue.Day
                Dim daysInMonth1 As Integer = 0
                daysInMonth1 = System.DateTime.DaysInMonth(FirstInsDate.Year, FirstInsDate.Month)
                If maxDays > daysInMonth1 Then
                    TempMDate = daysInMonth1 & "/" & FirstInsDate.Month & "/" & FirstInsDate.Year
                Else
                    TempMDate = maxDays & "/" & FirstInsDate.Month & "/" & FirstInsDate.Year
                End If

                FirstInsDate = Date.Parse(TempMDate, ukCultureInfo.DateTimeFormat)
                strSql = "SELECT DATEADD(month," & NoofIns + totDays & ",'" & Format(FirstInsDate, "yyyy-MM-dd") & "') AS MDATE"
                Date1 = objGPack.GetSqlValue(strSql, "MDATE", "", )
                ro = dtDelay.NewRow
                ro!INSNO = ""
                ro!RECDATE = "Actual Maturity"
                ro!DEFAMONTH = Format(Date1, "dd/MM/yyyy")
                ro!PAYBYDATE = ""
                Return Date1
            End If
        Catch ex As Exception
    ...
    Public Function textInvoiceCharacterNotallowed(sender As Object, e As KeyPressEventArgs) As Boolean
        Select Case e.KeyChar
            Case " ", "", "[", "]", "\", "*", """", "<", ">", "|", ":", "(", ")", "+", "''", "?", ";", "!", "@", "#", "$", "^", "&", "_", "=", "~", "`", "{", "}", "%", "."  ',"-","/"
                e.Handled = True
                Return True
        ...
        Return False
    End Function
    Public Sub AutoResizeNewMethod(ByVal grid1 As DataGridView)
    ...
    Public Function GetBillNoValueGstRegister(ByVal BillControlId As String, ByVal tran As OleDbTransaction) As Integer
        Dim isfirst As Boolean = True
GetNewBillNo:
        Dim NewBillNo As Integer = Nothing
        If False Then 'TEMPBILLNO EXIST
        Else 'TEMPBILLNO NOT EXIST
GenerateNewBillNo:
            strSql = "SELECT CONVERT(INT,CTLTEXT) FROM " & cnStockDb & "..BILLCONTROL WHERE CTLID = '" & BillControlId & "' AND COMPANYID = '" & strCompanyId & "'"
            If strBCostid <> Nothing And isfirst Then strSql += " AND ISNULL(COSTID,'') = '" & strBCostid & "'"
