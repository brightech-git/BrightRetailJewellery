...
Public Class frmAgewisestockmaster
    ...
    Function funcCallGrid() As Integer
        ...
        strSql += " FROM " & cnAdminDb & "..STKREORDER AS SR WHERE FROMFLAG='D'"
        ...
        gridView.Columns("TODAY").HeaderText = "TO DAY"
        gridView.Columns("TODAY").Width = 70
        gridView.Columns("TODAY").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        gridView.Columns("FROMDAY").HeaderText = "FROM DAY"
        gridView.Columns("FROMDAY").Width = 70
        gridView.Columns("FROMDAY").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
    ...
    Function funcAdd() As Integer
        ...
        strSql += " TODAY,Userid,"
        strSql += "FROMDAY,DESIGNID,"
        strSql += " Updated,Uptime,sizeid,FROMFLAG)Values ("
        strSql += " '" & Sno & "'"
    ...
    Function funcUpdate() As Integer
        ...
        strSql += " ,SubItemId= " & SubItemId & ""
        strSql += " ,COSTID='" & COSTID & "'"
        strSql += " ,FROMDAY=" & Val(txtMinPcs.Text) & ""
        strSql += " ,TODAY=" & Val(txtMaxPiece.Text) & ",DESIGNID=" & Designid
        strSql += " ,Userid=" & userId & ""
        ...
        strSql += " where Sno = '" & Sno & "' AND FROMFLAG='D'"
        Try
            ExecQuery(SyncMode.Master, strSql, cn)
    ...
    Function funcGetDetails(ByVal temp As String) As String
        ...
        strSql += " FROMDAY"
        strSql += ",TODAY,Designid"
        strSql += " from " & cnAdminDb & "..StkReorder as SR"
        strSql += " Where Sno = '" & temp & "' AND FROMFLAG='D'"
        ...
        With dt.Rows(0)
            ...
            txtMaxPiece.Text = .Item("TODAY").ToString
            txtMinPcs.Text = .Item("FROMDAY").ToString
            Me.cmbDesignerName.Text = .Item("Designername").ToString
            If cmbSubItem_Man.Text <> "" Then cmbSubItem_Man.Enabled = True Else cmbSubItem_Man.Enabled = False
        End With
        Sno = temp
    End Function
    ...
    Private Sub cmbMetal_Man_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbMetal_Man.SelectedIndexChanged
        If flagSave = True Then
            Exit Sub
        End If
        strSql = " SELECT 'ALL' ITEMNAME,0 RESULT UNION ALL "
        strSql += " Select ITEMNAME ,1 RESULT from " & cnAdminDb & "..ItemMast"
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Else
            If cmbCostSearch_OWN.Text <> "" Or cmbItemSearch_OWN.Text <> "" Or cmbSubItemSearch_OWN.Text <> "" Then
                strSql += " where C.CostName='" & cmbCostSearch_OWN.Text & "'"
                strSql += " and I.ItemName='" & cmbItemSearch_OWN.Text & "'"
                strSql += " and S.SubItemName='" & cmbSubItemSearch_OWN.Text & "'"
                strSql += " and SR.FROMFLAG='D'"
            End If
        End If
        funcOpenGrid(strSql, gridView)
        With gridView
            ...
            .Columns("FROMDAY").HeaderText = "FROM DAY"
            .Columns("TODAY").HeaderText = "TO DAY"
        End With
        gridView.Select()
        gridView.Columns("FROMDAY").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        gridView.Columns("TODAY").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
