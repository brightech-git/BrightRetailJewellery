...
Public Class frmWeightPieceWiseStockReOrder
    ...
    Function funcCallGrid() As Integer
        ...
        strSql += " ,(Select DesignerName from " & cnAdminDb & "..DESIGNER Where DesignerId=SR.DesignId) as DESIGNERNAME,ISNULL(RANGECAPTION,'') RANGECAPTION"
        ...
        gridView.Columns("MAXPIECE").HeaderText = "MAX PIECE"
        gridView.Columns("MAXPIECE").Width = 70
        gridView.Columns("MAXPIECE").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
    ...
    Function funcLoadDesignerName(ByVal combo As ComboBox) As Integer
        strSql = " SELECT DesignerName FROM ( "
        strSql += " Select DesignerName, 1 result from " & cnAdminDb & "..Designer"
        strSql += " UNION ALL"
        strSql += " select 'ALL' DesignerName, 0 result "
        strSql += " ) X order by result,DesignerName"
    ...
    Function funcSave() As Integer
        ...
        If Val(txtWtTo.Text) <= Val(txtWtFrom_Own.Text) Then
            MsgBox("from and to weight mismatch", MsgBoxStyle.Information)
            txtWtFrom_Own.Focus()
            txtWtFrom_Own.SelectAll()
            Exit Function
        End If
        If Val(txtMaxPcs.Text) <= Val(txtMinPcs.Text) Then
            MsgBox("max pieces and min pieces mismatch", MsgBoxStyle.Information)
            txtMinPcs.Focus()
            txtMinPcs.SelectAll()
            Exit Function
        End If
        If Val(txtMaxWeight.Text) <= Val(txtMinWeight.Text) Then
            MsgBox("max and min weight mismatch")
            txtMinWeight.Focus()
            txtMinWeight.Select()
            Exit Function
        End If
    ...
    Function funcAdd() As Integer
        ...
        strSql = vbCrLf + " SELECT COUNT(*) CNT FROM " & cnAdminDb & "..STKREORDER "
        strSql += vbCrLf + " WHERE ITEMID = " & ItemId & " AND SUBITEMID = " & SubItemId & " "
        strSql += vbCrLf + " AND FROMWEIGHT = '" & Val(txtWtFrom_Own.Text) & "' AND TOWEIGHT = '" & Val(txtWtTo.Text) & "' "
        strSql += vbCrLf + " AND RANGEMODE = 'B'"
        If DesignId > 0 Then
            strSql += vbCrLf + " AND DESIGNID = " & DesignId & ""
        End If
        If GetSqlValue(cn, strSql) > 0 Then
            MsgBox("Already Available this range", MsgBoxStyle.Information)
            txtWtFrom_Own.Focus()
            Exit Function
        End If
        ...
        strSql = vbCrLf + " SELECT COUNT(*) CNT FROM " & cnAdminDb & "..STKREORDER "
        strSql += vbCrLf + " WHERE ITEMID = " & ItemId & " AND SUBITEMID = " & SubItemId & " "
        strSql += vbCrLf + " AND " & Val(txtWtFrom_Own.Text) & " BETWEEN FROMWEIGHT AND TOWEIGHT "
        strSql += vbCrLf + " AND RANGEMODE = 'B'"
        If DesignId > 0 Then
            strSql += vbCrLf + " AND DESIGNID = " & DesignId & ""
        End If
        If GetSqlValue(cn, strSql) > 0 Then
            MsgBox("Already Available from range", MsgBoxStyle.Information)
            txtWtFrom_Own.Focus()
            Exit Function
        End If
        ...
        strSql = vbCrLf + " SELECT COUNT(*) CNT FROM " & cnAdminDb & "..STKREORDER "
        strSql += vbCrLf + " WHERE ITEMID = " & ItemId & " AND SUBITEMID = " & SubItemId & " "
        strSql += vbCrLf + " AND " & Val(txtWtTo.Text) & " BETWEEN FROMWEIGHT AND TOWEIGHT "
        strSql += vbCrLf + " AND RANGEMODE = 'B'"
        If DesignId > 0 Then
            strSql += vbCrLf + " AND DESIGNID = " & DesignId & ""
        End If
        If GetSqlValue(cn, strSql) > 0 Then
            MsgBox("Already Available TO range", MsgBoxStyle.Information)
            txtWtTo.Focus()
            Exit Function
        End If
        ...
        strSql += " Updated,Uptime,sizeid"
        strSql += " ,fromweight,toweight,rangemode,weight,MAXPIECE,minweight, maxweight"
        strSql += " ,RANGECAPTION "
        strSql += " )"
        strSql += " Values ("
        strSql += " '" & Sno & "'"
        ...
        strSql += " ," & Val("" & txtMinPcs.Text) & "" 'min piece
        strSql += " ," & DesignId & " " 'design
        ...
        strSql += " ,'B' " 'rangemode
        strSql += " ,'0' " 'weight
        strSql += " ,'" & Val(txtMaxPcs.Text) & "' " 'max piecs
        strSql += " ,'" & Val(txtMinWeight.Text) & "' " 'min weight
        strSql += " ,'" & Val(txtMaxWeight.Text) & "' " 'max weight
        strSql += " ,'" & txtRangeCaption.Text & "' " 'RANGECAPTION
        strSql += " )"
        Try
    ...
    Function funcUpdate() As Integer
        ...
        strSql += " ,COSTID='" & COSTID & "'"
        strSql += " ,PIECE= 0 "
        strSql += " ,MINPIECE=" & Val(txtMinPcs.Text) & ""
        ...
        strSql += " ,WEIGHT = '0'"
        strSql += " ,MAXPIECE = '" & Val(txtMaxPcs.Text) & "'"
        strSql += " ,MINWEIGHT = '" & Val(txtMinWeight.Text) & "' "
        strSql += " ,MAXWEIGHT = '" & Val(txtMaxWeight.Text) & "'"
        strSql += " ,RANGECAPTION = '" & txtRangeCaption.Text & "'"
    ...
    Function funcGetDetails(ByVal temp As String) As String
        ...
        'strSql += " ,weight"
        'strSql += " ,Piece"
        strSql += " ,minpiece,maxpiece"
        strSql += " ,minweight,maxweight,ISNULL(RANGECAPTION,'') RANGECAPTION"
        ...
        With dt.Rows(0)
            ...
            txtMaxPcs.Text = .Item("Maxpiece").ToString
            ...
            txtMinWeight.Text = .Item("minweight").ToString
            txtMaxWeight.Text = .Item("maxweight").ToString
            txtRangeCaption.Text = .Item("RANGECAPTION").ToString
        End With
            Sno = temp
    End Function
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        If UCase(cmbSubItemSearch_OWN.Text) = "ALL" Then
            If cmbCostSearch_OWN.Text = "ALL" And cmbItemSearch_OWN.Text = "ALL" And cmbSubItemSearch_OWN.Text = "ALL" And cmbCostSearch_OWN.Text = "" And cmbItemSearch_OWN.Text = "" And cmbSubItemSearch_OWN.Text = "" Then
                funcOpenGrid(strSql, gridView)
                With gridView
            ...
            ElseIf cmbCostSearch_OWN.Text <> "" Or cmbCostSearch_OWN.Text <> "ALL" Or cmbItemSearch_OWN.Text <> "ALL" Or cmbSubItemSearch_OWN.Text <> "" Then
                strSql += " WHERE"
                If cmbCostSearch_OWN.Text <> "" And cmbCostSearch_OWN.Text <> "ALL" Then
                    strSql += " C.CostName='" & cmbCostSearch_OWN.Text & "' and "
                End If
                If cmbItemSearch_OWN.Text <> "" And cmbItemSearch_OWN.Text <> "ALL" Then
                    strSql += " I.ItemName='" & cmbItemSearch_OWN.Text & "'  and "
                End If
                strSql += " FROMDAY IS NULL"
        ...
        Else
            If cmbCostSearch_OWN.Text <> "" Or cmbItemSearch_OWN.Text <> "" Or cmbSubItemSearch_OWN.Text <> "" Then
                ...
                If cmbItemSearch_OWN.Text <> "" And cmbItemSearch_OWN.Text <> "ALL" Then
                    strSql += " I.ItemName='" & cmbItemSearch_OWN.Text & "'  and "
                End If
                If cmbSubItemSearch_OWN.Text <> "" And cmbSubItemSearch_OWN.Text <> "ALL" Then
                    strSql += "S.SubItemName='" & cmbSubItemSearch_OWN.Text & "' AND "
                End If
                strSql += " FROMDAY IS NULL"
                funcOpenGrid(strSql, gridView)
                With gridView
    ...
    Private Sub txtWtFrom_Own_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtWtFrom_Own.GotFocus
        If cmbItem_Man.Text <> "" And Val(txtWtFrom_Own.Text) = 0 And flagSave = False Then
            strSql = " SELECT ITEMID FROM " & cnAdminDb & "..ITEMMAST WHERE ITEMNAME = '" & cmbItem_Man.Text.Trim & "' "
            Dim itemid As Integer = Val(GetSqlValue(cn, strSql).ToString)
            Dim subitemid As Integer = 0
            If itemid > 0 And cmbSubItem_Man.Text <> "" Then
                strSql = " SELECT SUBITEMID FROM " & cnAdminDb & "..SUBITEMMAST WHERE ITEMID = '" & itemid & "' "
                strSql += " AND SUBITEMNAME = '" & cmbSubItem_Man.Text.Trim & "' "
                subitemid = Val(GetSqlValue(cn, strSql).ToString)
            End If
            If cmbDesignerName.Text.Trim <> "" And cmbDesignerName.Text.Trim <> "ALL" Then
                Dim designid As Integer = 0
                strSql = " SELECT DESIGNERID from " & cnAdminDb & "..DESIGNER where designername = '" & cmbDesignerName.Text & "'"
                designid = Val(GetSqlValue(cn, strSql).ToString)
                strSql = " SELECT ISNULL(MAX(TOWEIGHT),0)+0.001 TOWEIGHT FROM " & cnAdminDb & "..STKREORDER "
                strSql += "  WHERE ITEMID = '" & itemid & "'   "
                strSql += "  AND SUBITEMID = '" & subitemid & "' "
                If designid > 0 Then
                    strSql += "  AND DESIGNID = '" & designid & "'"
                End If
                txtWtFrom_Own.Text = Val(GetSqlValue(cn, strSql).ToString)
            End If
            If cmbDesignerName.Text.Trim = "" Or cmbDesignerName.Text.Trim = "ALL" Then
                strSql = " SELECT ISNULL(MAX(TOWEIGHT),0)+0.001 TOWEIGHT FROM " & cnAdminDb & "..STKREORDER "
                strSql += "  WHERE ITEMID = '" & itemid & "'   "
                strSql += "  AND SUBITEMID = '" & subitemid & "' "
                strSql += " AND (ISNULL(DESIGNID,'') = '0' OR ISNULL(DESIGNID,'') = '')"
                txtWtFrom_Own.Text = Val(GetSqlValue(cn, strSql).ToString)
            End If
        End If
    End Sub

    Private Sub txtWeight_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles _
    txtWtFrom_Own.Leave, txtWtTo.Leave, txtMinWeight.Leave, txtMaxWeight.Leave
        Dim currTextBox As TextBox = sender
        If currTextBox.Equals(txtWtFrom_Own) Then
            txtWtFrom_Own.Text = Format(Val(txtWtFrom_Own.Text), "0.000")
        ElseIf currTextBox.Equals(txtWtTo) Then
            txtWtTo.Text = Format(Val(txtWtTo.Text), "0.000")
        ElseIf currTextBox.Equals(txtMaxWeight) Then
            txtMaxWeight.Text = Format(Val(txtMaxWeight.Text), "0.000")
        ElseIf currTextBox.Equals(txtMinWeight) Then
            txtMinWeight.Text = Format(Val(txtMinWeight.Text), "0.000")
        End If
    End Sub

    Private Sub frmWeightPieceWiseStockReOrder_Load(sender As Object, e As EventArgs) Handles MyBase.Load
