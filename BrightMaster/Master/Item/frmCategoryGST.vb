...
Public Class frmCategoryGST
    ...
    End Sub
    Private Sub ShowMiscDia()
        If objMc.Visible Then Exit Sub
        objMc.BackColor = Me.BackColor
        objMc.StartPosition = FormStartPosition.CenterScreen
        objMc.txtS_SGSTTax_Per.Select()
        objMc.ShowDialog()
    End Sub
    Function funcAcNameGeneration() As Integer
        ...
        If flagSave = False Then
            ...
            If dt.Rows.Count > 0 Then
                ...
                cmbCess_OWN.Items.Add("SALES " + catName + " CESS A/C")
                cmbCess_OWN.Text = "SALES " + catName + " CESS A/C"
    ...
    Function funcAcNameGenerationMis() As Integer
        ...
        If flagSave Then
            ...
            If dt.Rows.Count > 0 Then
                ...
                If cmbCess_OWN.Text = "" Then
                    cmbCess_OWN.Items.Clear()
                    cmbCess_OWN.Items.Add("SALES " + catName + " CESS A/C")
                    cmbCess_OWN.Text = "SALES " + catName + " CESS A/C"
                End If
    ...
    Function funcClear()
        ...
        txtS_SGSTTax_WET.Text = ""
        txtS_CGSTTax_WET.Text = ""
        txtS_IGSTTax_WET.Text = ""
        txtCessTax_WET.Text = ""
        txtP_SGSTTax_WET.Text = ""
        txtP_CGSTTax_WET.Text = ""
        txtP_IGSTTax_WET.Text = ""
        cmbPASCharge_OWN.Items.Clear()
    ...
    Function funcNew()
        ...
        strSql = " SELECT ISNULL(MAX(CATCODE),0)+1 CATCODE FROM " & cnAdminDb & "..CATEGORY WHERE ISNUMERIC(CATCODE) = 1 "
        dt.Clear()
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
    ...
    Function funcSave() As Integer
        If Not BrighttechPack.Methods.GetRights(_DtUserRights, "frmCategory", BrighttechPack.Methods.RightMode.Save) Then Exit Function
        ...
        End If
        If cmbS_CGST_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbS_CGST_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbS_CGST_OWN.Select()
            Exit Function
        End If
        If cmbS_SGST_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbS_SGST_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbS_SGST_OWN.Select()
            Exit Function
        End If
        If cmbS_IGST_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbS_IGST_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbS_IGST_OWN.Select()
            Exit Function
        End If

        If cmbCess_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbCess_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbCess_OWN.Select()
            Exit Function
        End If
        ...
        If cmbP_CGST_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbS_CGST_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbP_CGST_OWN.Select()
            Exit Function
        End If
        If cmbP_SGST_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbS_SGST_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbP_SGST_OWN.Select()
            Exit Function
        End If
        If cmbP_IGST_OWN.Text = "" Then
            MsgBox(objGPack.GetNextLable(Me, cmbS_IGST_OWN, False).Text + E0001, MsgBoxStyle.Information)
            cmbP_IGST_OWN.Select()
            Exit Function
        End If
    ...
    Function funcOpen()
        ...
        dt.Clear()
        strSql = " SELECT"
        strSql += " CATCODE,CATNAME,SHORTNAME,HSN,DISPLAYORDER,"
    ...
    Function funcAdd()
        ...
        Try
            ...
            Dim MC_SGST As String = Nothing
            Dim MC_CGST As String = Nothing
            Dim MC_IGST As String = Nothing
            Dim CESS As String = Nothing
            ...
            If objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & objMc.cmbS_SGST_OWN.Text & "'", , , tran) <> "" Then
                MC_SGST = objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & objMc.cmbS_SGST_OWN.Text & "'", , , tran)
            Else
                MC_SGST = "C" + CatCode.Remove(0, 1) & "14"
            End If

            If objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & objMc.cmbS_CGST_OWN.Text & "'", , , tran) <> "" Then
                MC_CGST = objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & objMc.cmbS_CGST_OWN.Text & "'", , , tran)
            Else
                MC_CGST = "C" + CatCode.Remove(0, 1) & "15"
            End If

            If objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & objMc.cmbS_IGST_OWN.Text & "'", , , tran) <> "" Then
                MC_IGST = objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & objMc.cmbS_IGST_OWN.Text & "'", , , tran)
            Else
                MC_IGST = "C" + CatCode.Remove(0, 1) & "16"
            End If


            If objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & cmbCess_OWN.Text & "'", , , tran) <> "" Then
                CESS = objGPack.GetSqlValue("SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD WHERE ACNAME = '" & cmbCess_OWN.Text & "'", , , tran)
            Else
                CESS = "C" + CatCode.Remove(0, 1) & "17"
            End If
            ...
            strSql += " ,BILLDISPLAY,ACCTDISPLAY"
            strSql += " ,SALESID,PURCHASEID,SRETURNID,SPRETURNID"
            strSql += " ,OPNCODE"
            ...
            strSql += " ,CESSID,CESSTAX"
            strSql += " ,P_SGSTID,P_SGSTTAX"
            strSql += " ,P_CGSTID,P_CGSTTAX"
            strSql += " ,P_IGSTID,P_IGSTTAX"
            If chkMkAcc.Checked Then
                strSql += " ,MC_SGSTID,MC_SGSTTAX"
                strSql += " ,MC_CGSTID,MC_CGSTTAX"
                strSql += " ,MC_IGSTID,MC_IGSTTAX"
            End If
            strSql += " ,HSN"
            strSql += " ) VALUES ("
            ...
            strSql += " ," & Val(txtS_SGSTTax_WET.Text) & "" ''S_SGSTTAX
            strSql += " ,'" & saId_CGST & "'" ''S_CGST
            strSql += " ," & Val(txtS_CGSTTax_WET.Text) & "" ''S_CGSTTAX
            strSql += " ,'" & saId_IGST & "'" ''S_CGST
            strSql += " ," & Val(txtS_IGSTTax_WET.Text) & "" ''S_IGSTTAX

            strSql += " ,'" & CESS & "'" ''S_CGST
            strSql += " ," & Val(txtCessTax_WET.Text) & "" ''S_IGSTTAX

            strSql += " ,'" & puId_SGST & "'" ''P_CGST
            strSql += " ," & Val(txtP_SGSTTax_WET.Text) & "" ''P_SGSTTAX
            strSql += " ,'" & puId_CGST & "'" ''P_CGST
            strSql += " ," & Val(txtP_CGSTTax_WET.Text) & "" ''P_CGSTTAX
            strSql += " ,'" & puId_IGST & "'" ''P_CGST
            strSql += " ," & Val(txtP_IGSTTax_WET.Text) & "" ''P_IGSTTAX
            If chkMkAcc.Checked Then
                strSql += " ,'" & MC_SGST & "'" ''MC_SGST
                strSql += " ," & Val(objMc.txtS_SGSTTax_Per.Text) & "" ''MC_SGSTTAX
                strSql += " ,'" & MC_CGST & "'" ''MC_CGST
                strSql += " ," & Val(objMc.txtS_CGSTTax_Per.Text) & "" ''MC_CGSTTAX
                strSql += " ,'" & MC_IGST & "'" ''MC_IGST
                strSql += " ," & Val(objMc.txtS_IGSTTax_Per.Text) & "" ''MC_IGSTTAX
            End If
            ...
            ''SalesId_SGST
            InsertIntoAcHead(CESS, cmbCess_OWN.Text, 3, "Y")
    ...
    Function funcUpdate() As Integer
        ...
        Try
            ...
            ''Find SA_CESS
            strSql = " SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD"
            strSql += vbCrLf + "  WHERE ACNAME = '" & cmbCess_OWN.Text & "'"
            CESS = objGPack.GetSqlValue(strSql, , Nothing, tran)
            If CESS = Nothing And cmbCess_OWN.Text <> "" Then
                CESS = "C" + CatCode.Remove(0, 1) & "17"
                InsertIntoAcHead(CESS, cmbCess_OWN.Text, 3)
            End If
            strSql = " SELECT ACCODE FROM " & cnAdminDb & "..ACHEAD"
            strSql += vbCrLf + "  WHERE ACNAME = '" & cmbCess_OWN.Text & "'"
            CESS = objGPack.GetSqlValue(strSql, , , tran)
            ...
            strSql += vbCrLf + "  ,S_SGSTTAX = '" & Val(txtS_SGSTTax_WET.Text) & "'"
            strSql += vbCrLf + "  ,S_CGSTID = '" & saId_CGST & "'"
            strSql += vbCrLf + "  ,S_CGSTTAX = '" & Val(txtS_CGSTTax_WET.Text) & "'"
            strSql += vbCrLf + "  ,S_IGSTID = '" & saId_IGST & "'"
            strSql += vbCrLf + "  ,S_IGSTTAX = '" & Val(txtS_IGSTTax_WET.Text) & "'"
            strSql += vbCrLf + "  ,CESSID = '" & CESS & "'"
            strSql += vbCrLf + "  ,CESSTAX = '" & Val(txtCessTax_WET.Text) & "'"
            strSql += vbCrLf + "  ,P_SGSTID = '" & puId_SGST & "'"
            strSql += vbCrLf + "  ,P_SGSTTAX = '" & Val(txtP_SGSTTax_WET.Text) & "'"
            strSql += vbCrLf + "  ,P_CGSTID = '" & puId_CGST & "'"
            strSql += vbCrLf + "  ,P_CGSTTAX = '" & Val(txtP_CGSTTax_WET.Text) & "'"
            strSql += vbCrLf + "  ,P_IGSTID = '" & puId_IGST & "'"
            strSql += vbCrLf + "  ,P_IGSTTAX = '" & Val(txtP_IGSTTax_WET.Text) & "'"
            If chkMkAcc.Checked Then
                strSql += " ,MC_SGSTID='" & MC_SGST & "'" ''MC_SGST
                strSql += " ,MC_SGSTTAX=" & Val(objMc.txtS_SGSTTax_Per.Text) & "" ''MC_SGSTTAX
                strSql += " ,MC_CGSTID='" & MC_CGST & "'" ''MC_CGST
                strSql += " ,MC_CGSTTAX=" & Val(objMc.txtS_CGSTTax_Per.Text) & "" ''MC_CGSTTAX
                strSql += " ,MC_IGSTID='" & MC_IGST & "'" ''MC_IGST
                strSql += " ,MC_IGSTTAX=" & Val(objMc.txtS_IGSTTax_Per.Text) & "" ''MC_IGSTTAX
            End If
    ...
    Function funcGetDetails(ByVal tempCatCode As String)
        ...
        flagSave = True
        strSql = " SELECT"
        strSql += vbCrLf + "  CATCODE,CATNAME,SHORTNAME,HSN,DISPLAYORDER,"
        ...
        strSql += vbCrLf + "  ,(SELECT ACNAME FROM " & cnAdminDb & "..ACHEAD WHERE ACCODE = C.CESSID)AS CESS"
        ...
        strSql += vbCrLf + "  ,S_SGSTTAX, S_CGSTTAX, S_IGSTTAX,CESSTAX, P_SGSTTAX, P_CGSTTAX, P_IGSTTAX"
        strSql += vbCrLf + "  ,MC_SGSTTAX, MC_CGSTTAX, MC_IGSTTAX"
        strSql += vbCrLf + "  ,(SELECT ACNAME FROM " & cnAdminDb & "..ACHEAD WHERE ACCODE = C.MC_SGSTID)AS MC_SGST"
        strSql += vbCrLf + "  ,(SELECT ACNAME FROM " & cnAdminDb & "..ACHEAD WHERE ACCODE = C.MC_CGSTID)AS MC_CGST"
        strSql += vbCrLf + "  ,(SELECT ACNAME FROM " & cnAdminDb & "..ACHEAD WHERE ACCODE = C.MC_IGSTID)AS MC_IGST"
        ...
        With dt.Rows(0)
            ...
            cmbCess_OWN.Text = .Item("CESS").ToString
            ...
            txtS_SGSTTax_WET.Text = .Item("S_SGSTTAX").ToString
            txtS_CGSTTax_WET.Text = .Item("S_CGSTTAX").ToString
            txtS_IGSTTax_WET.Text = .Item("S_IGSTTAX").ToString

            txtCessTax_WET.Text = .Item("CESSTAX").ToString

            txtP_SGSTTax_WET.Text = .Item("P_SGSTTAX").ToString
            txtP_CGSTTax_WET.Text = .Item("P_CGSTTAX").ToString
            txtP_IGSTTax_WET.Text = .Item("P_IGSTTAX").ToString
            objMc.cmbS_SGST_OWN.Text = .Item("MC_SGST").ToString
            objMc.cmbS_CGST_OWN.Text = .Item("MC_CGST").ToString
            objMc.cmbS_IGST_OWN.Text = .Item("MC_IGST").ToString
            objMc.txtS_SGSTTax_Per.Text = .Item("MC_SGSTTAX").ToString
            objMc.txtS_CGSTTax_Per.Text = .Item("MC_CGSTTAX").ToString
            objMc.txtS_IGSTTax_Per.Text = .Item("MC_IGSTTAX").ToString
    ...
    Function funcLoadAcHeadNames()
        ...
        If dt.Rows.Count > 0 Then
            ...
            objGPack.FillCombonew(dt, cmbCess_OWN, , False)

            objGPack.FillCombonew(dt, objMc.cmbS_CGST_OWN, , False)
            objGPack.FillCombonew(dt, objMc.cmbS_SGST_OWN, , False)
            objGPack.FillCombonew(dt, objMc.cmbS_IGST_OWN, , False)
        End If
    ...
    Function funcLoadTaxMode()
        ...
        If dt.Rows.Count > 0 Then
            ...
            Next
            cmbTaxMode.Text = dt.Rows(0).Item("TaxName").ToString
        End If
        Return 0
    End Function
    ...
    Private Sub btnOpen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOpen.Click
        If Not BrighttechPack.Methods.GetRights(_DtUserRights, "frmCategory", BrighttechPack.Methods.RightMode.View) Then Exit Sub
        funcOpen()
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If Not gridView.RowCount > 0 Then
            Exit Sub
        End If
        If Not BrighttechPack.Methods.GetRights(_DtUserRights, "frmCategory", BrighttechPack.Methods.RightMode.Delete) Then Exit Sub
    ...
    Private Sub chkMkAcc_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkMkAcc.CheckedChanged
        If chkMkAcc.Checked Then
            ShowMiscDia()
        End If
    End Sub
