...
Public Class frmDisigner
    ...
    Function funcCallGrid() As Integer
        ...
        dt.Clear()
        strSql = " SELECT DE.DESIGNERID,"
        strSql += " DE.DESIGNERNAME ""DESIGNER NAME"",DE.SEAL,"
        ...
        strSql += " FROM " & cnAdminDb & "..DESIGNER DE"
        ...
        gridView.Columns("DESIGNER NAME").Width = 300
    ...
    Function funcAdd()
        ...
        Try
            ...
            strSql = " SELECT ISNULL(MAX(DESIGNERID),0)+1 AS DESIGNERID FROM "
            strSql += " " & cnAdminDb & "..DESIGNER"
            designerId = Val(objGPack.GetSqlValue(strSql, , , tran))

            strSql = " INSERT INTO " & cnAdminDb & "..DESIGNER"
            strSql += " ("
            strSql += " DESIGNERID,"
            strSql += " DESIGNERNAME,SEAL,"
            strSql += " ACTIVE,"
            strSql += " USERID,"
            strSql += " UPDATED,"
            ...
            strSql += " )VALUES ("
            strSql += " " & designerId & ""
            strSql += " ,'" & txtDesignerName__Man.Text & "'"
            strSql += " ,'" & txtSeal__Man.Text & "'"
    ...
    Function funcUpdate() As Integer
        ...
        Try
            tran = cn.BeginTransaction()
            strSql = "UPDATE " & cnAdminDb & "..DESIGNER SET"
            strSql += " DESIGNERNAME = '" & txtDesignerName__Man.Text & "'"
            strSql += " ,SEAL = '" & txtSeal__Man.Text & "'"
    ...
    Function funcGetDetails(ByVal tempId As Integer)
        Dim dt As New DataTable
        dt.Clear()
        strSql = " select DesignerName,SEAL,"
        strSql += " case when Active = 'Y' then 'YES' else 'NO' end as Active"
        strSql += " ,(SELECT ACNAME FROM " & cnAdminDb & "..ACHEAD WHERE ACCODE = DE.ACCODE)AS ACNAME"
        strSql += " from " & cnAdminDb & "..Designer de where DesignerId = " & tempId & ""
        da = New OleDbDataAdapter(strSql, cn)
        da.Fill(dt)
        ...
        End If
        With dt.Rows(0)
            txtDesignerName__Man.Text = .Item("DesignerName").ToString
            txtSeal__Man.Text = .Item("SEAL").ToString
    ...
    Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Dim delKey As String = gridView.Rows(gridView.CurrentRow.Index).Cells("DESIGNERID").Value.ToString
        chkQry += " SELECT TOP 1 DESIGNERID  FROM " & cnAdminDb & "..WMCTABLE"
        chkQry += " WHERE DESIGNERID = '" & delKey & "'"
        chkQry += " UNION"
        chkQry += " SELECT TOP 1 DESIGNERID  FROM " & cnAdminDb & "..ITEMLOT"
        chkQry += " WHERE DESIGNERID = '" & delKey & "'"
        chkQry += " UNION"
        chkQry += " SELECT TOP 1 DESIGNERID  FROM " & cnAdminDb & "..ITEMTAG"
        chkQry += " WHERE DESIGNERID = '" & delKey & "'"
        chkQry += " UNION"
        chkQry += " SELECT TOP 1 DESIGNERID  FROM " & cnAdminDb & "..ITEMNONTAG"
        chkQry += " WHERE DESIGNERID = '" & delKey & "'"
        chkQry += " UNION"
        chkQry += " SELECT TOP 1 DESIGNERID  FROM " & cnAdminDb & "..ORIRDETAIL"
        chkQry += " WHERE DESIGNERID = '" & delKey & "'"
        DeleteItem(SyncMode.Master, chkQry, "DELETE FROM " & cnAdminDb & "..DESIGNER WHERE DESIGNERID = '" & delKey & "'")
        funcCallGrid()
    End Sub

    Private Sub txtDesignerName__Man_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtDesignerName__Man.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            If objGPack.DupChecker(txtDesignerName__Man, "SELECT 1 FROM " & cnAdminDb & "..DESIGNER WHERE DESIGNERNAME = '" & txtDesignerName__Man.Text & "' AND DESIGNERID <> '" & designerId & "'") Then
                Exit Sub
            Else
                SendKeys.Send("{TAB}")
            End If
    ...
    Private Sub txtSeal__Man_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSeal__Man.GotFocus
    ...
    Private Function CheckAcNameAvail() As Boolean
        ...
        If Accode = "" Then
            ...
            Return True
        End If
        strSql = " SELECT 'Y' FROM " & cnAdminDb & "..DESIGNER AS DE WHERE ACCODE = '" & Accode & "'"
        strSql += " AND DESIGNERID <> " & designerId & ""
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        If gridView.Rows.Count > 0 Then
            BrightPosting.GExport.Post(Me.Name, strCompanyName, "HEAD OF DESIGNER", gridView, BrightPosting.GExport.GExportType.Export)
        End If
    End Sub
End Class